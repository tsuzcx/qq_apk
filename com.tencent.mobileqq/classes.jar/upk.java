  ...
  implements Runnable
{
  ...
  public upk(LineLayer paramLineLayer, int paramInt1, int paramInt2, Bitmap paramBitmap, uqb paramuqb)
  {
    this.jdField_a_of_type_JavaLangString = (AppConstants.bM + "temp" + File.separator);
    QLog.d("SaveTempFileJob", 2, "SaveTempFileJob begin:");
    this.jdField_a_of_type_JavaLangRefWeakReference = new WeakReference(paramuqb);
    if (paramBitmap == null) {
      return;
    }
  ...
  {
    new Handler(Looper.getMainLooper()).post(new upl(this, paramString));
  }
