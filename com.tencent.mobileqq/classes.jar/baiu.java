...
public class baiu
{
  ...
  static
  {
    ...
    jdField_b_of_type_JavaLangString = bdeo.c.pattern() + "|" + bdeo.d.pattern() + "|" + "[a-zA-Z0-9\\+\\.\\_\\%\\-\\+]{1,256}\\@[a-zA-Z0-9][a-zA-Z0-9\\-]{0,64}(\\.[a-zA-Z0-9][a-zA-Z0-9\\-]{0,25})+" + "|" + "(?=.*,)(?=.*#)(\\+*[0-9]+[\\-,]*)?(\\([0-9]+\\)[\\-,]*)?([0-9][0-9\\-,][0-9\\-,#]+#)" + "|" + "[\\d]{5,15}" + "|" + jdField_a_of_type_JavaLangString;
  }
  
  static List<bahv> a(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (bdeo.jdField_b_of_type_JavaUtilRegexPattern.matcher(paramString).find())
      {
        ...
        while (localMatcher1.find())
        {
          ...
          if (bdeo.c.matcher(str).find()) {
            localArrayList.add(new bahv(str, i, j));
          } else if (bdeo.d.matcher(str).find()) {
            localArrayList.add(new bahv(str, i, j));
