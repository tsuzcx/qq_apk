...
public class sxs
  implements slx<szf<qqstory_service.ReqBatchStoryPollData>, tbd>
{
  ...
  static
  {
    jdField_a_of_type_JavaLangString = skt.a("StorySvc.batch_poll_data");
  }
  
  public sxs(List<String> paramList)
  {
  ...
  public static void a(@NonNull List<String> paramList)
  {
    ...
    while (i < j)
    {
      new sxs(paramList.subList(i, Math.min(i + 5, j))).a();
      i += 5;
    }
  }
  ...
  public void a(@NonNull szf<qqstory_service.ReqBatchStoryPollData> paramszf, @Nullable tbd paramtbd, @NonNull ErrorMessage paramErrorMessage)
  {
    if ((paramErrorMessage.isFail()) || (paramtbd == null))
    {
      urk.c("Q.qqstory.pollData.GetVidPollInfoHandler", "Error onCmdRespond:%s", paramErrorMessage);
      return;
    }
    paramszf = new qqstory_service.RspBatchStoryPollData();
    Object localObject;
    ...
    for (;;)
    {
      try
      {
        ...
        paramtbd = (sqd)sqg.a(5);
        paramErrorMessage = new sxt();
        i = 0;
        if (i >= paramszf.poll_info_list.size()) {
          break;
        }
        localObject = (qqstory_struct.VidPollInfo)paramszf.poll_info_list.get(i);
        localStoryVideoItem = paramtbd.a(((qqstory_struct.VidPollInfo)localObject).vid.get().toStringUtf8());
      ...
      {
        urk.c("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond Request parse Error!", paramszf);
        return;
      }
      ...
      paramtbd.a(localStoryVideoItem);
      label271:
      paramErrorMessage.a.add(localObject);
      i += 1;
    }
    int i = 0;
    while (i < paramszf.rate_info_list.size())
    {
      localObject = (qqstory_struct.VidRateInfo)paramszf.rate_info_list.get(i);
      urk.b("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond, VidRateInfo:[vid=%s, selfRateResult=%s, totalRateCount=%s, totalRateScore=%s]", ((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8(), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).self_rate_result.get()), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_count.get()), Long.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_score.get()));
      localStoryVideoItem = paramtbd.a(((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8());
      if (localStoryVideoItem != null)
      {
