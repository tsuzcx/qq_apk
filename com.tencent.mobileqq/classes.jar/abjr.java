...
{
  private Map<String, AdIPCManager.Handler> a = new HashMap();
  ...
  public boolean register(String paramString, AdIPCManager.Handler paramHandler)
  {
    ...
    for (;;)
    {
      abrl.b("GdtIPCAdapter", String.format("register action:%s result:%b", new Object[] { paramString, Boolean.valueOf(bool) }));
      return bool;
  ...
  public AdIPCManager.Result send(Context paramContext, AdIPCManager.Params paramParams)
  {
    ...
    for (String str = paramParams.getToProcessName();; str = null)
    {
      ...
      abrl.d("GdtIPCAdapter", String.format("send error actionad:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
      return null;
      localObject1 = null;
      break;
    }
    label113:
    abrl.b("GdtIPCAdapter", String.format("send action:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
    ...
    abrl.d("GdtIPCAdapter", String.format("send error action:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
    return null;
  }
  ...
  public void send(Context paramContext, AdIPCManager.Params paramParams, WeakReference<AdIPCManager.Callback> paramWeakReference)
  {
    ...
    if (paramParams != null)
    {
      ...
      abrl.d("GdtIPCAdapter", String.format("send error action:%s from:%s to:%s", new Object[] { str1, paramContext, str2 }));
    }
    label97:
    do
    {
      ...
      str1 = null;
      break;
      abrl.b("GdtIPCAdapter", String.format("send action:%s from:%s to:%s", new Object[] { str1, paramContext, str2 }));
    ...
    {
      QIPCClientHelper.getInstance().getClient().callServer("gdt_ipc_async_module_client_to_server", str1, paramParams.bundle, new abjs(this, paramWeakReference, paramParams));
      return;
    }
    QIPCClientHelper.getInstance().getClient().callServer("gdt_ipc_async_module_client_to_server", str1, paramParams.bundle, new abjt(this, paramWeakReference, paramParams));
  }
}
