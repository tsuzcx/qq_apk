...
public class wax
{
  ...
  public static void a(Context paramContext, SpannableStringBuilder paramSpannableStringBuilder)
  {
    try
    {
      ...
      int i = 0;
      while (i < wbh.b.length)
      {
        for (int j = str.indexOf(wbh.b[i]); j >= 0; j = str.indexOf(wbh.b[i], j + wbh.b[i].length())) {
          paramSpannableStringBuilder.setSpan(new VerticalCenterImageSpan(a(i, f, paramContext, null), 0), j, wbh.b[i].length() + j, 33);
        }
        i += 1;
      }
      return;
