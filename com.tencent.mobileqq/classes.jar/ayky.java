...
public class ayky
{
  ...
  static
  {
    ...
    jdField_b_of_type_JavaLangString = bbft.c.pattern() + "|" + bbft.d.pattern() + "|" + "[a-zA-Z0-9\\+\\.\\_\\%\\-\\+]{1,256}\\@[a-zA-Z0-9][a-zA-Z0-9\\-]{0,64}(\\.[a-zA-Z0-9][a-zA-Z0-9\\-]{0,25})+" + "|" + "(?=.*,)(?=.*#)(\\+*[0-9]+[\\-,]*)?(\\([0-9]+\\)[\\-,]*)?([0-9][0-9\\-,][0-9\\-,]+#)" + "|" + "[\\d]{5,15}" + "|" + jdField_a_of_type_JavaLangString;
  }
  
  static List<ayjz> a(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (bbft.jdField_b_of_type_JavaUtilRegexPattern.matcher(paramString).find())
      {
        ...
        while (localMatcher1.find())
        {
          ...
          if (bbft.c.matcher(str).find()) {
            localArrayList.add(new ayjz(str, i, j));
          } else if (bbft.d.matcher(str).find()) {
            localArrayList.add(new ayjz(str, i, j));
