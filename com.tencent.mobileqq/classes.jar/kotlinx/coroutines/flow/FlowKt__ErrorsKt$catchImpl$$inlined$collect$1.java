package kotlinx.coroutines.flow;

import kotlin.Metadata;
import kotlin.ResultKt;
import kotlin.Unit;
import kotlin.coroutines.Continuation;
import kotlin.coroutines.intrinsics.IntrinsicsKt;
import kotlin.jvm.internal.Ref.ObjectRef;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

@Metadata(bv={1, 0, 3}, d1={""}, d2={"kotlinx/coroutines/flow/FlowKt__CollectKt$collect$3", "Lkotlinx/coroutines/flow/FlowCollector;", "emit", "", "value", "(Ljava/lang/Object;Lkotlin/coroutines/Continuation;)Ljava/lang/Object;", "kotlinx-coroutines-core"}, k=1, mv={1, 1, 16})
public final class FlowKt__ErrorsKt$catchImpl$$inlined$collect$1
  implements FlowCollector<T>
{
  public FlowKt__ErrorsKt$catchImpl$$inlined$collect$1(FlowCollector paramFlowCollector, Ref.ObjectRef paramObjectRef) {}
  
  @Nullable
  public Object emit(Object paramObject, @NotNull Continuation paramContinuation)
  {
    if ((paramContinuation instanceof FlowKt__ErrorsKt.catchImpl..inlined.collect.1.1))
    {
      localObject1 = (FlowKt__ErrorsKt.catchImpl..inlined.collect.1.1)paramContinuation;
      if ((((FlowKt__ErrorsKt.catchImpl..inlined.collect.1.1)localObject1).label & 0x80000000) != 0)
      {
        ((FlowKt__ErrorsKt.catchImpl..inlined.collect.1.1)localObject1).label += -2147483648;
        paramContinuation = (Continuation)localObject1;
        break label53;
      }
    }
    paramContinuation = new FlowKt__ErrorsKt.catchImpl..inlined.collect.1.1(this, paramContinuation);
    label53:
    Object localObject1 = paramContinuation.result;
    Object localObject2 = IntrinsicsKt.getCOROUTINE_SUSPENDED();
    int i = paramContinuation.label;
    if (i != 0)
    {
      if (i == 1)
      {
        paramObject = paramContinuation.L$3;
        paramObject = (Continuation)paramContinuation.L$2;
        paramObject = paramContinuation.L$1;
        paramObject = (1)paramContinuation.L$0;
        try
        {
          ResultKt.throwOnFailure(localObject1);
        }
        catch (Throwable paramContinuation)
        {
          break label195;
        }
      }
      else
      {
        throw new IllegalStateException("call to 'resume' before 'invoke' with coroutine");
      }
    }
    else
    {
      ResultKt.throwOnFailure(localObject1);
      localObject1 = (Continuation)paramContinuation;
    }
    try
    {
      FlowCollector localFlowCollector = this.$collector$inlined;
      paramContinuation.L$0 = this;
      paramContinuation.L$1 = paramObject;
      paramContinuation.L$2 = localObject1;
      paramContinuation.L$3 = paramObject;
      paramContinuation.label = 1;
      paramObject = localFlowCollector.emit(paramObject, paramContinuation);
      if (paramObject == localObject2) {
        return localObject2;
      }
      return Unit.INSTANCE;
    }
    catch (Throwable paramContinuation)
    {
      paramObject = this;
    }
    label195:
    paramObject.$fromDownstream$inlined.element = paramContinuation;
    throw paramContinuation;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes16.jar
 * Qualified Name:     kotlinx.coroutines.flow.FlowKt__ErrorsKt.catchImpl..inlined.collect.1
 * JD-Core Version:    0.7.0.1
 */