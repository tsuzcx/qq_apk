package kotlinx.coroutines.flow;

import kotlin.Metadata;
import kotlin.Unit;
import kotlin.coroutines.Continuation;
import kotlin.coroutines.intrinsics.IntrinsicsKt;
import kotlin.jvm.functions.Function2;
import kotlin.jvm.internal.InlineMarker;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

@Metadata(bv={1, 0, 3}, d1={""}, d2={"kotlinx/coroutines/flow/FlowKt__CollectKt$collect$3", "Lkotlinx/coroutines/flow/FlowCollector;", "emit", "", "value", "(Ljava/lang/Object;Lkotlin/coroutines/Continuation;)Ljava/lang/Object;", "kotlinx-coroutines-core", "kotlinx/coroutines/flow/FlowKt__MergeKt$$special$$inlined$collect$1", "kotlinx/coroutines/flow/FlowKt__MergeKt$unsafeTransform$$inlined$unsafeFlow$1$lambda$1"}, k=1, mv={1, 1, 16})
public final class FlowKt__MergeKt$map$$inlined$unsafeTransform$1$2
  implements FlowCollector<T>
{
  public FlowKt__MergeKt$map$$inlined$unsafeTransform$1$2(FlowCollector paramFlowCollector, FlowKt__MergeKt.map..inlined.unsafeTransform.1 param1) {}
  
  @Nullable
  public Object emit(Object paramObject, @NotNull Continuation paramContinuation)
  {
    paramObject = this.$this_unsafeFlow$inlined.emit(this.this$0.$transform$inlined$1.invoke(paramObject, paramContinuation), paramContinuation);
    if (paramObject == IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
      return paramObject;
    }
    return Unit.INSTANCE;
  }
  
  @Nullable
  public Object emit$$forInline(Object paramObject, @NotNull Continuation paramContinuation)
  {
    InlineMarker.mark(4);
    new FlowKt__MergeKt.map..inlined.unsafeTransform.1.2.1(this, paramContinuation);
    InlineMarker.mark(5);
    FlowCollector localFlowCollector = this.$this_unsafeFlow$inlined;
    paramObject = this.this$0.$transform$inlined$1.invoke(paramObject, paramContinuation);
    InlineMarker.mark(0);
    paramObject = localFlowCollector.emit(paramObject, paramContinuation);
    InlineMarker.mark(2);
    InlineMarker.mark(1);
    return paramObject;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes16.jar
 * Qualified Name:     kotlinx.coroutines.flow.FlowKt__MergeKt.map..inlined.unsafeTransform.1.2
 * JD-Core Version:    0.7.0.1
 */