  ...
  extends MSFServlet
{
  public void onReceive(Intent paramIntent, FromServiceMsg paramFromServiceMsg)
  {
    ...
    if (i == 1000)
    {
      ...
      doufu_piece_rsp localdoufu_piece_rsp = bmxg.a(paramFromServiceMsg, new int[1]);
      if (localdoufu_piece_rsp != null)
      {
        paramIntent.putInt("ret", 0);
        paramIntent.putSerializable("data", localdoufu_piece_rsp);
        notifyObserver(null, 1009, true, paramIntent, ayxo.class);
        return;
      }
      ...
      notifyObserver(null, 1009, false, paramIntent, ayxo.class);
      return;
    }
    if (QLog.isColorLevel()) {
  ...
  {
    bmxg localbmxg = new bmxg(Long.valueOf(paramIntent.getLongExtra("selfuin", 0L)).longValue(), new HashMap());
    byte[] arrayOfByte = localbmxg.encode();
    paramIntent = arrayOfByte;
    if (arrayOfByte == null)
    {
      QLog.e("BirthDayNoticeServlet", 1, "onSend request encode result is null.cmd=" + localbmxg.uniKey());
      paramIntent = new byte[4];
    }
    paramPacket.setTimeout(60000L);
    paramPacket.setSSOCommand("SQQzoneSvc." + localbmxg.uniKey());
