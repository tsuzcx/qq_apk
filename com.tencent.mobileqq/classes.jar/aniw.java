import android.os.Bundle;
import android.os.Handler.Callback;
import android.os.Message;
import android.text.TextUtils;
import com.tencent.mobileqq.apollo.trace.sdk.data.TraceData;
import com.tencent.mobileqq.app.ThreadManager;
import com.tencent.qphone.base.util.QLog;
import java.util.Set;
import java.util.concurrent.ConcurrentHashMap;
import java.util.concurrent.CopyOnWriteArraySet;
import java.util.concurrent.atomic.AtomicInteger;

public class aniw
  implements Handler.Callback
{
  private anit jdField_a_of_type_Anit;
  private anjo jdField_a_of_type_Anjo;
  private bkyc jdField_a_of_type_Bkyc = new bkyc(ThreadManager.getSubThreadLooper(), this);
  private Set<Integer> jdField_a_of_type_JavaUtilSet = new CopyOnWriteArraySet();
  private ConcurrentHashMap<Integer, anjp> jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap = new ConcurrentHashMap();
  private AtomicInteger jdField_a_of_type_JavaUtilConcurrentAtomicAtomicInteger = new AtomicInteger(-1);
  private Set<Integer> jdField_b_of_type_JavaUtilSet = new CopyOnWriteArraySet();
  private ConcurrentHashMap<String, anjl> jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap = new ConcurrentHashMap();
  
  public static aniw a()
  {
    return aniy.a();
  }
  
  public static boolean a()
  {
    return anjy.a;
  }
  
  private void b(int paramInt, Bundle paramBundle)
  {
    anjp localanjp = (anjp)this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.get(Integer.valueOf(paramInt));
    if (localanjp != null)
    {
      localanjp.a();
      this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.remove(Integer.valueOf(paramInt), localanjp);
    }
    localanjp = new anjp(paramInt);
    paramBundle.putBoolean("isSampleFocus", a(paramInt));
    localanjp.a(paramBundle);
    this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.put(Integer.valueOf(paramInt), localanjp);
  }
  
  private void c(int paramInt, Bundle paramBundle)
  {
    anjp localanjp = (anjp)this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.get(Integer.valueOf(paramInt));
    if (localanjp == null)
    {
      QLog.e("TraceReport" + this.jdField_a_of_type_Anit.a(paramInt), 1, new Object[] { "already end the trace, featureId:", Integer.valueOf(paramInt) });
      return;
    }
    localanjp.c(paramBundle);
    paramBundle = paramBundle.getString("featureKey");
    a().a(paramBundle);
    this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.remove(Integer.valueOf(paramInt), localanjp);
    this.jdField_b_of_type_JavaUtilSet.add(Integer.valueOf(paramInt));
  }
  
  private void d(int paramInt, Bundle paramBundle)
  {
    anjp localanjp = (anjp)this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.get(Integer.valueOf(paramInt));
    if (localanjp == null) {
      return;
    }
    localanjp.b(paramBundle);
  }
  
  private void e(int paramInt, Bundle paramBundle)
  {
    anjp localanjp = (anjp)this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.get(Integer.valueOf(paramInt));
    if (localanjp == null) {
      return;
    }
    localanjp.d(paramBundle);
  }
  
  public int a(int paramInt, String paramString)
  {
    if (TextUtils.isEmpty(paramString)) {}
    while (!this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.containsKey(paramString)) {
      return paramInt;
    }
    return ((anjl)this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.get(paramString)).a;
  }
  
  public int a(int paramInt, String paramString, boolean paramBoolean)
  {
    if (TextUtils.isEmpty(paramString)) {}
    do
    {
      return paramInt;
      if (this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.containsKey(paramString)) {
        return ((anjl)this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.get(paramString)).b;
      }
    } while (!paramBoolean);
    if (this.jdField_a_of_type_JavaUtilConcurrentAtomicAtomicInteger.get() <= -2147483648) {
      this.jdField_a_of_type_JavaUtilConcurrentAtomicAtomicInteger.set(-1);
    }
    int i = this.jdField_a_of_type_JavaUtilConcurrentAtomicAtomicInteger.decrementAndGet();
    anjl localanjl = new anjl(paramInt, i);
    this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.put(paramString, localanjl);
    return i;
  }
  
  public anit a()
  {
    return this.jdField_a_of_type_Anit;
  }
  
  public void a()
  {
    this.jdField_a_of_type_Bkyc.sendEmptyMessage(4);
    this.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.clear();
    this.jdField_a_of_type_JavaUtilSet.clear();
    this.jdField_b_of_type_JavaUtilSet.clear();
    this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.clear();
    if (this.jdField_a_of_type_Anit != null) {
      this.jdField_a_of_type_Anit.a().a();
    }
    this.jdField_a_of_type_Bkyc.removeMessages(6);
  }
  
  public void a(int paramInt, Bundle paramBundle)
  {
    if (this.jdField_a_of_type_Anit == null) {
      return;
    }
    paramBundle.putInt("featureId", a(paramInt, paramBundle.getString("featureKey"), false));
    this.jdField_a_of_type_Bkyc.obtainMessage(5, paramBundle).sendToTarget();
  }
  
  public void a(int paramInt, String paramString, Bundle paramBundle)
  {
    if (this.jdField_a_of_type_Anit == null) {
      a(null);
    }
    if (this.jdField_a_of_type_Anit == null) {}
    int i;
    do
    {
      return;
      i = a(paramInt, paramString, true);
    } while (this.jdField_a_of_type_JavaUtilSet.contains(Integer.valueOf(i)));
    this.jdField_b_of_type_JavaUtilSet.remove(Integer.valueOf(i));
    paramBundle.putInt("featureId", i);
    if (a(paramInt))
    {
      paramBundle.putInt("initJavaHeap", anjc.c());
      paramBundle.putInt("initNativeHeap", anjc.d());
    }
    this.jdField_a_of_type_Bkyc.obtainMessage(1, paramBundle).sendToTarget();
  }
  
  public void a(int paramInt, boolean paramBoolean)
  {
    if (this.jdField_a_of_type_Anit == null) {
      return;
    }
    if (!paramBoolean)
    {
      this.jdField_a_of_type_JavaUtilSet.add(Integer.valueOf(paramInt));
      return;
    }
    this.jdField_a_of_type_JavaUtilSet.remove(Integer.valueOf(paramInt));
  }
  
  public void a(anit paramanit)
  {
    anit localanit = paramanit;
    if (paramanit == null)
    {
      localanit = anjc.a();
      QLog.e("TraceReport", 1, "init error, outside not init config");
    }
    if (localanit == null) {
      return;
    }
    this.jdField_a_of_type_Anit = localanit;
    if (this.jdField_a_of_type_Anjo == null) {
      this.jdField_a_of_type_Anjo = new anjo();
    }
    this.jdField_a_of_type_Anjo.a(localanit);
    this.jdField_a_of_type_Bkyc.sendEmptyMessage(0);
  }
  
  public void a(TraceData paramTraceData)
  {
    if ((!a()) || (paramTraceData.featureId < 0)) {}
    do
    {
      return;
      if ((this.jdField_a_of_type_Anjo != null) && (this.jdField_a_of_type_Anit != null))
      {
        if (this.jdField_a_of_type_Anjo.a(paramTraceData))
        {
          this.jdField_a_of_type_Bkyc.removeMessages(6);
          this.jdField_a_of_type_Bkyc.sendEmptyMessageDelayed(6, anjo.a);
          return;
        }
        this.jdField_a_of_type_Bkyc.removeMessages(6);
        return;
      }
    } while (!QLog.isColorLevel());
    QLog.d("TraceReport", 2, "sendTraceReport mTraceReportControl or config is null");
  }
  
  public void a(String paramString)
  {
    if (TextUtils.isEmpty(paramString)) {
      return;
    }
    this.jdField_b_of_type_JavaUtilConcurrentConcurrentHashMap.remove(paramString);
  }
  
  public boolean a(int paramInt)
  {
    if (this.jdField_a_of_type_Anit == null) {
      return false;
    }
    return this.jdField_a_of_type_Anit.a(paramInt);
  }
  
  public void b(int paramInt, String paramString, Bundle paramBundle)
  {
    if (this.jdField_a_of_type_Anit == null) {
      a(null);
    }
    if (this.jdField_a_of_type_Anit == null) {}
    int i;
    do
    {
      return;
      i = a(paramInt, paramString, false);
    } while (this.jdField_a_of_type_JavaUtilSet.contains(Integer.valueOf(i)));
    paramBundle.putInt("featureId", i);
    if (a(paramInt))
    {
      paramBundle.putInt("endJavaHeap", anjc.c());
      paramBundle.putInt("endNativeHeap", anjc.d());
    }
    this.jdField_a_of_type_Bkyc.obtainMessage(2, paramBundle).sendToTarget();
  }
  
  public void c(int paramInt, String paramString, Bundle paramBundle)
  {
    if (this.jdField_a_of_type_Anit == null) {
      a(null);
    }
    if (this.jdField_a_of_type_Anit == null) {}
    do
    {
      return;
      paramInt = a(paramInt, paramString, false);
    } while ((this.jdField_a_of_type_JavaUtilSet.contains(Integer.valueOf(paramInt))) || (this.jdField_b_of_type_JavaUtilSet.contains(Integer.valueOf(paramInt))));
    paramBundle.putInt("featureId", paramInt);
    this.jdField_a_of_type_Bkyc.obtainMessage(3, paramBundle).sendToTarget();
  }
  
  public boolean handleMessage(Message paramMessage)
  {
    int i = paramMessage.what;
    paramMessage = (Bundle)paramMessage.obj;
    switch (i)
    {
    }
    for (;;)
    {
      return false;
      if (this.jdField_a_of_type_Anjo != null)
      {
        this.jdField_a_of_type_Anjo.a();
        continue;
        b(paramMessage.getInt("featureId"), paramMessage);
        continue;
        c(paramMessage.getInt("featureId"), paramMessage);
        continue;
        d(paramMessage.getInt("featureId"), paramMessage);
        continue;
        if (this.jdField_a_of_type_Anjo != null)
        {
          this.jdField_a_of_type_Anjo.c();
          continue;
          e(paramMessage.getInt("featureId"), paramMessage);
          continue;
          if (this.jdField_a_of_type_Anjo != null) {
            this.jdField_a_of_type_Anjo.b();
          }
        }
      }
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes3.jar
 * Qualified Name:     aniw
 * JD-Core Version:    0.7.0.1
 */