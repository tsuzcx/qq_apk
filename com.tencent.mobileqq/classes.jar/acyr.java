  ...
  extends aczg
{
    ...
    for (;;)
    {
      ...
      if ((localElem.life_online.has()) && (localElem.life_online.uint32_ack.has()) && (localElem.life_online.uint32_ack.get() == 1))
      {
        if (localElem.life_online.uint64_unique_id.has())
        {
          ...
          nzz.a().a(BaseApplication.getContext(), localMessageHandler.app, l1, l2, l3, l4, 1, null);
        }
      }
      else
      {
        ...
        parambepr = localElem.life_online.gdt_cli_data.get().toStringUtf8();
        ...
        if (parambepr.length() <= 100) {
          break label1719;
        }
        paramStringBuilder = parambepr.substring(0, 100);
        i2 = j;
      }
      label1408:
      label1664:
          ...
          if ((localElem.life_online.uint64_bitmap.get() & 0x1000) == 4096L)
          {
            ...
            if (i1 != 0)
            {
              ...
              if (QLog.isColorLevel())
              {
                ...
                parambepr = Long.valueOf(localElem.life_online.uint64_bitmap.get());
                QLog.d("LifeOnlineElemDecoder", 2, parambepr);
              }
              ThreadManager.getSubThreadHandler().postDelayed(new LifeOnlineElemDecoder.1(this, localQQAppInterface), 1000L);
            }
            if (!paramList1.isEmpty())
            {
              parambepr = (MessageRecord)paramList1.get(0);
              ...
              if (m != 0)
              {
                ...
                label1138:
                parambepr.saveExtInfoToExtStr("gdt_singleAd", paramStringBuilder);
                parambepr = (MessageRecord)paramList1.get(0);
                if (i2 == 0) {
                  break label1594;
                }
                paramStringBuilder = "1";
                parambepr.saveExtInfoToExtStr("gdt_mulAd", paramStringBuilder);
                parambepr = (MessageRecord)paramList1.get(0);
                if (k == 0) {
                  break label1601;
                }
                paramStringBuilder = "1";
                parambepr.saveExtInfoToExtStr("gdt_followAd", paramStringBuilder);
  ...
  {
    a(paramList, paramList1, paramStringBuilder, paramMsg, parambepr);
    return false;
  }
