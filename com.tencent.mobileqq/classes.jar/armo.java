import com.tencent.mobileqq.miniapp.ui.MiniAppActivity;

public abstract class armo
{
  public armr a;
  public MiniAppActivity a;
  
  public armo(MiniAppActivity paramMiniAppActivity, String paramString, int paramInt)
  {
    this.jdField_a_of_type_ComTencentMobileqqMiniappUiMiniAppActivity = paramMiniAppActivity;
  }
  
  public abstract void a();
  
  public void a(armr paramarmr)
  {
    this.jdField_a_of_type_Armr = paramarmr;
    if (this.jdField_a_of_type_ComTencentMobileqqMiniappUiMiniAppActivity != null) {
      this.jdField_a_of_type_ComTencentMobileqqMiniappUiMiniAppActivity.jdField_a_of_type_Armr = paramarmr;
    }
  }
  
  public abstract boolean a();
  
  public abstract void b();
  
  public void c()
  {
    this.jdField_a_of_type_ComTencentMobileqqMiniappUiMiniAppActivity = null;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes10.jar
 * Qualified Name:     armo
 * JD-Core Version:    0.7.0.1
 */