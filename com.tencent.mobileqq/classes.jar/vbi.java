...
public class vbi
  implements Runnable
{
  public vbi(ReplyTextItemBuilder paramReplyTextItemBuilder, MessageForReplyText paramMessageForReplyText, int paramInt, vbp paramvbp) {}
    ...
    for (;;)
    {
      try
      {
        ...
        if (((MessageRecord)localArrayList.get(i)).shmsgseq == this.jdField_a_of_type_ComTencentMobileqqDataMessageForReplyText.mSourceMsgInfo.mSourceMsgSeq)
        {
          ...
          ThreadManager.getUIHandler().post(new vbj(this, localMessageRecord));
          return;
        }
      }
