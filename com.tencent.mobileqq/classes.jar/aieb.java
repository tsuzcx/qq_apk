class aieb
  extends bjub
{
  aieb(aiea paramaiea, int paramInt1, int paramInt2, int[] paramArrayOfInt1, int paramInt3, int[] paramArrayOfInt2, int[] paramArrayOfInt3, int[] paramArrayOfInt4)
  {
    super(paramInt1, paramInt2, paramArrayOfInt1, paramInt3, paramArrayOfInt2, paramArrayOfInt3, paramArrayOfInt4);
  }
  
  public void a(int paramInt, Object paramObject, bjua[] paramArrayOfbjua)
  {
    paramInt = 1;
    if ((paramArrayOfbjua == null) || (paramArrayOfbjua.length <= 0)) {}
    for (;;)
    {
      return;
      int i = this.a.a();
      if ((paramArrayOfbjua.length < 0) && ((i & 0xF) == 1))
      {
        paramArrayOfbjua[0].b = 0;
        paramArrayOfbjua[0].a = 0;
      }
      while (paramInt < paramArrayOfbjua.length)
      {
        paramArrayOfbjua[paramInt].b = -1;
        paramArrayOfbjua[paramInt].a = -1;
        paramInt += 1;
        continue;
        paramInt = 0;
      }
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes2.jar
 * Qualified Name:     aieb
 * JD-Core Version:    0.7.0.1
 */