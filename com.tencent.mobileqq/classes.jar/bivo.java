  ...
  implements ITMAssistantExchangeURLListenner
{
  ...
  {
    bisy.b(bivl.jdField_a_of_type_JavaLangString, "onExchangedURLSucceed --- ");
    if ((paramBoolean) && (??? != null) && (???.size() > 0))
    {
      ...
      while (???.hasNext())
      {
        ...
        if ((localObject1 instanceof AppSimpleDetail))
        {
          ...
          if (i > 0) {
            bivl.jdField_a_of_type_JavaUtilConcurrentConcurrentHashMap.put(((AppSimpleDetail)localObject1).packageName, Integer.valueOf(i));
          }
        }
      }
    }
    synchronized (bivl.jdField_a_of_type_JavaLangObject)
    {
      bivl.jdField_a_of_type_JavaLangObject.notify();
      return;
    }
  }
}
