  ...
  extends pro
{
  ...
  public static pro b(oidb_cmd0xbc9.BannerItem paramBannerItem)
  {
    ...
    for (prq localprq = new prq(paramBannerItem.uint32_banner_type.get());; localprq = new prq(4))
    {
      if (localContentBannerItem.bytes_image_url.has()) {
        localprq.c = localContentBannerItem.bytes_image_url.get().toStringUtf8();
      }
      if (localContentBannerItem.bytes_link_url.has()) {
        localprq.f = localContentBannerItem.bytes_link_url.get().toStringUtf8();
      }
      if (localContentBannerItem.bytes_rowkey.has()) {
        localprq.e = localContentBannerItem.bytes_rowkey.get().toStringUtf8();
      }
      a(localprq, paramBannerItem);
      a(paramBannerItem, localprq);
      return localprq;
    }
  }
