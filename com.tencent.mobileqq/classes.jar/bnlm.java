...
public class bnlm
{
  private static final MutableLiveData<bnlq> jdField_a_of_type_AndroidArchLifecycleMutableLiveData;
  public static final bnlr a;
  private static final List<Feature> jdField_a_of_type_JavaUtilList;
  private static final List<Pair<String, Integer>> b;
  private static final List<bnlq> c;
  
  static
  {
    jdField_a_of_type_Bnlr = new bnlr();
  ...
  public static List<bnlq> a(@Nullable bnlq parambnlq)
  {
    LinkedList localLinkedList = new LinkedList(c);
    if ((parambnlq == null) || (TextUtils.isEmpty(parambnlq.jdField_a_of_type_JavaLangString))) {
      return localLinkedList;
    }
    Iterator localIterator = localLinkedList.iterator();
    while (localIterator.hasNext())
    {
      bnlq localbnlq = (bnlq)localIterator.next();
      if ((localbnlq != null) && (!TextUtils.isEmpty(localbnlq.jdField_a_of_type_JavaLangString)) && (localbnlq.jdField_a_of_type_JavaLangString.equals(parambnlq.jdField_a_of_type_JavaLangString)))
      {
        localbnlq.b = parambnlq.b;
        localbnlq.c = parambnlq.c;
      }
    }
    return localLinkedList;
  }
  ...
  public static void a(@Nullable bnmd parambnmd, long paramLong)
  {
    ThreadManager.getUIHandler().postDelayed(new AEDashboardUtil.1(parambnmd), paramLong);
  }
  ...
  private static void b(@Nullable bnmd parambnmd)
  {
    ThreadManager.excute(new AEDashboardUtil.2(parambnmd), 64, null, true);
  }
  
  private static void b(@Nullable bnmd parambnmd, @NonNull List<bnlp> paramList, @NonNull List<bnlo> paramList1)
  {
    if (parambnmd == null) {
      return;
    }
    ThreadManager.getUIHandler().post(new AEDashboardUtil.3(parambnmd, paramList, paramList1));
  }
  
  private static List<bnlp> c()
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      try
      {
        localbnlp.jdField_a_of_type_JavaLangString = ((Feature)localObject1).getName();
        localbnlp.jdField_a_of_type_Boolean = ((Feature)localObject1).isFunctionReady();
        ...
        {
          localStringBuilder.append((String)localObject1);
          localbnlp.c = localStringBuilder.toString();
          localLinkedList.add(localbnlp);
          break;
          localObject1 = "GPU";
          break label94;
        }
      ...
      {
        localbnlp.jdField_a_of_type_Boolean = false;
        localbnlp.b = localException.getMessage();
        localLinkedList.add(localbnlp);
      }
      finally
      {
    ...
    Collections.sort(localLinkedList, new bnln());
    return localLinkedList;
  }
  
  private static void c()
  {
    c.add(new bnlq("CameraModuleSvc.GetCameraConfig"));
    c.add(new bnlq("CameraModuleSvc.GetCompressedCategoryMaterial.MqStoryCamera"));
    c.add(new bnlq("CameraModuleSvc.GetCompressedCategoryMaterial.MqEmoCamera"));
    c.add(new bnlq("CameraModuleSvc.GetPlayShowCatMatTree"));
  }
  
  private static List<bnlo> d()
  {
    LinkedList localLinkedList = new LinkedList();
    ...
    while (localIterator.hasNext())
    {
      ...
      localbnlo.jdField_a_of_type_JavaLangString = ((String)localPair.first);
      localbnlo.b = bnyp.a().a((String)localPair.first, "null", ((Integer)localPair.second).intValue());
      localLinkedList.add(localbnlo);
    }
    return localLinkedList;
  }
}
