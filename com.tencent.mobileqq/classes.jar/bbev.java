...
class bbev
{
  ...
  private Bitmap a(bbfb parambbfb)
  {
    if (parambbfb != null)
    {
      BitmapFactory.Options localOptions = new BitmapFactory.Options();
      localOptions.inScaled = false;
      if ((!ThemeUtil.isDefaultTheme()) && (!TextUtils.isEmpty(parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFilePath)))
      {
        if (QLog.isColorLevel()) {
          QLog.d("ResourceGrabModel", 2, "get theme package resource");
        }
        return BitmapFactory.decodeFile(parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFilePath, localOptions);
      }
      parambbfb = parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFileName.substring(0, parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFileName.indexOf("."));
      BaseApplication localBaseApplication = BaseApplicationImpl.getContext();
      String str = localBaseApplication.getPackageName();
      int i = localBaseApplication.getResources().getIdentifier(parambbfb, "drawable", str);
      if (i > 0)
      {
        if (QLog.isColorLevel()) {
  ...
  private void a(bbfb parambbfb, HashSet<String> paramHashSet)
  {
    if (parambbfb.jdField_a_of_type_Int == 3)
    {
      b(parambbfb, paramHashSet);
      return;
    }
    c(parambbfb, paramHashSet);
  }
  ...
  private void a(String paramString1, HashSet<String> paramHashSet, String paramString2)
  {
    ...
    //   38: aload_0
    //   39: aload_3
    //   40: invokespecial 251	bbev:b	(Ljava/lang/String;)Ljava/io/File;
    //   43: astore 5
    //   45: aload_2
    //   46: aload 5
  ...
  private void b(bbfb parambbfb, HashSet<String> paramHashSet)
  {
    StringBuilder localStringBuilder = new StringBuilder();
    if ((parambbfb.jdField_a_of_type_Array2dOfInt != null) && (parambbfb.jdField_a_of_type_ArrayOfInt != null) && (parambbfb.jdField_a_of_type_Array2dOfInt.length == parambbfb.jdField_a_of_type_ArrayOfInt.length))
    {
      ...
      int i = 0;
      while (i < parambbfb.jdField_a_of_type_ArrayOfInt.length)
      {
        localStringBuilder.append("    <item ");
        int j = 0;
        while (j < parambbfb.jdField_a_of_type_Array2dOfInt[i].length)
        {
          int k = parambbfb.jdField_a_of_type_Array2dOfInt[i][j];
        ...
        j = parambbfb.jdField_a_of_type_ArrayOfInt[i];
    ...
    a(localStringBuilder.toString(), paramHashSet, parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFileName);
  }
  
  private void c(bbfb parambbfb, HashSet<String> paramHashSet)
  {
    Bitmap localBitmap = a(parambbfb);
    if (localBitmap != null)
    {
      localBitmap = a(localBitmap, parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFileName);
      parambbfb = a(parambbfb.jdField_a_of_type_ComTencentThemeSkinData.mFileName);
      paramHashSet.add(parambbfb.getAbsolutePath());
    }
    try
    {
      if (!parambbfb.exists())
      {
        if (!parambbfb.getParentFile().exists()) {
          parambbfb.getParentFile().mkdir();
        }
        parambbfb.createNewFile();
      }
      parambbfb = new FileOutputStream(parambbfb);
      localBitmap.compress(Bitmap.CompressFormat.PNG, 100, parambbfb);
      parambbfb.flush();
      parambbfb.close();
      return;
    }
    catch (Exception parambbfb)
    {
      QLog.d("ResourceGrabModel", 1, parambbfb, new Object[0]);
    }
  }
  ...
  void a(List<bbfb> paramList, bbfa parambbfa)
  {
    if (!paramList.isEmpty())
    {
      HashSet localHashSet = new HashSet();
      Observable.from(paramList).distinct(new bbez(this)).subscribeOn(Schedulers.io()).subscribe(new bbew(this, localHashSet), new bbex(this), new bbey(this, parambbfa, localHashSet));
    }
  }
}
