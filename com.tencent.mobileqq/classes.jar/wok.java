...
public class wok
{
  ...
  public static void a(Context paramContext, SpannableStringBuilder paramSpannableStringBuilder)
  {
    try
    {
      ...
      int i = 0;
      while (i < wou.b.length)
      {
        for (int j = str.indexOf(wou.b[i]); j >= 0; j = str.indexOf(wou.b[i], j + wou.b[i].length())) {
          paramSpannableStringBuilder.setSpan(new VerticalCenterImageSpan(a(i, f, paramContext, null), 0), j, wou.b[i].length() + j, 33);
        }
        i += 1;
      }
      return;
