...
public abstract class zcl
{
  ...
  public static zcl a(String paramString, JSONObject paramJSONObject)
  {
    if ("dex".equals(paramString))
    {
      ...
      do
      {
        ...
        if (PatchCommonUtil.isDalvik()) {
          return new zco(paramJSONObject);
        }
        if (PatchCommonUtil.isArtLM()) {
          return new zcn(paramJSONObject);
        }
      } while (!PatchCommonUtil.isArtGeN());
      return new zcm(paramJSONObject);
    }
    if ("Native".equals(paramString)) {
      return new zcp(paramJSONObject);
    }
