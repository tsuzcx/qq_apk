...
public class axas
{
  ...
  public static void a(QQAppInterface paramQQAppInterface, String paramString)
  {
    ...
    if (paramString != null)
    {
      ...
      {
        i = j;
        if (akpy.a((MessageRecord)paramString.get(paramString.size() - 1), localMessageRecord, false))
        {
    ...
    if (i < paramString.length())
    {
      if ((paramString.charAt(i) == '\024') && (i + 2 < paramString.length()))
      {
        ...
        if (k >= 0)
        {
          j = i;
          if (k < b.length) {
            localStringBuilder.append(String.valueOf(new char[] { '\024', (char)b[k] }));
          }
        }
      }
      for (int j = i + 1 + 1;; j = i)
