...
public class PluginPreloader
{
  ...
  public static void a(PluginPreloadStrategy paramPluginPreloadStrategy, long paramLong)
  {
    ...
    a.postDelayed(new PluginPreloader.1(paramPluginPreloadStrategy), paramLong);
  }
  
  static void a(AppRuntime paramAppRuntime, PluginPreloadStrategy paramPluginPreloadStrategy, int paramInt, PluginPreloader.ExtraResult paramExtraResult)
  {
    paramPluginPreloadStrategy.a(paramExtraResult);
    if (paramPluginPreloadStrategy.jdField_b_of_type_JavaLangString != null)
    {
      IPluginManager localIPluginManager = (IPluginManager)paramAppRuntime.getManager(QQManagerFactory.MGR_PLUGIN);
      ...
      {
        if (QLog.isColorLevel()) {
          QLog.d("PluginPreloader", 2, "plugin already installed, do preload. ComicPlugin.pluginInfo.version=" + localPluginInfo.mVersion);
        }
      ...
      {
        localIPluginManager.installPlugin(paramPluginPreloadStrategy.jdField_b_of_type_JavaLangString, new PluginPreloader.2(paramAppRuntime, paramPluginPreloadStrategy, paramExtraResult, paramInt));
        return;
      }
      if (QLog.isColorLevel()) {
