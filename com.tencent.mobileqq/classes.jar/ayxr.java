  ...
  extends MSFServlet
{
  public void onReceive(Intent paramIntent, FromServiceMsg paramFromServiceMsg)
  {
    ...
    for (;;)
    {
      ...
      paramFromServiceMsg = bdku.b(paramFromServiceMsg.getWupBuffer());
      try
      {
        paramFromServiceMsg = ((WNSStream)localObject).unpack(paramFromServiceMsg);
        if (paramFromServiceMsg != null)
        {
          paramFromServiceMsg = (ClientOnlineNotfiyRsp)bihk.a(ClientOnlineNotfiyRsp.class, paramFromServiceMsg.BusiBuff);
          if (paramFromServiceMsg != null)
          {
  ...
  public void onSend(Intent paramIntent, Packet paramPacket)
  {
    ...
    byte[] arrayOfByte = bdcv.a(paramIntent);
    if (QLog.isDevelopLevel()) {
      QLog.d("NotifyQZoneServer", 4, "onSend lastPushMsgTime:" + l + ",attachinfo:" + paramIntent);
    }
    biyj localbiyj = new biyj(l, arrayOfByte);
    arrayOfByte = localbiyj.encode();
    paramIntent = arrayOfByte;
    if (arrayOfByte == null)
    {
      QLog.e("NotifyQZoneServer", 1, "onSend request encode result is null.cmd=" + localbiyj.uniKey());
      paramIntent = new byte[4];
    }
    paramPacket.setTimeout(30000L);
    paramPacket.setSSOCommand("SQQzoneSvc." + localbiyj.uniKey());
