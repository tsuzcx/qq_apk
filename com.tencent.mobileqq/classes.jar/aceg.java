  ...
  extends acew
{
  private void a(List<im_msg_body.Elem> paramList, List<MessageRecord> paramList1, StringBuilder paramStringBuilder, msg_comm.Msg paramMsg)
  {
    ...
    for (;;)
    {
      ...
      if (amwh.a(localMessageHandler.app, 3000, String.valueOf(l1), String.valueOf(l4), l6, l7)) {
        continue;
      }
      paramStringBuilder = "";
      try
      {
        ...
        if (paramMsg.msg_body.has())
        {
          ...
          if (paramMsg.rich_text.attr.has()) {
            l2 = bblk.a(((im_msg_body.Attr)paramMsg.rich_text.attr.get()).random.get());
          }
        }
      ...
      catch (UnsupportedEncodingException localInvalidProtocolBufferMicroException)
      {
        ...
        catch (InvalidProtocolBufferMicroException localInvalidProtocolBufferMicroException)
        {
          try
          {
            for (;;)
            {
              paramMsg.fileName = aszt.a(new String(paramList.bytes_filename.get().toByteArray(), "UTF-8"));
              paramList1.add(paramMsg);
              ((bcws)localQQAppInterface.getManager(272)).a(paramMsg);
              if (!QLog.isColorLevel()) {
                break;
              }
