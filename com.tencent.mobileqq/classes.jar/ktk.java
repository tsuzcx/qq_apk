...
public class ktk
{
  ...
  public static ktl a(byte[] paramArrayOfByte)
  {
    ktl localktl = new ktl();
    ...
    for (;;)
    {
      try
      {
        ...
        if (paramArrayOfByte.msg_video_head.has())
        {
          ...
          localktl.jdField_a_of_type_JavaLangString = ((hd_video.VideoHead)localObject).str_from_mobile.get();
          localktl.jdField_b_of_type_JavaLangString = ((hd_video.VideoHead)localObject).str_from_nation.get();
          localktl.jdField_a_of_type_Int = ((hd_video.VideoHead)localObject).enum_body_type.get();
          localktl.jdField_b_of_type_Long = ((hd_video.VideoHead)localObject).uint64_uin.get();
          if (((hd_video.VideoHead)localObject).int32_sub_service_type.has())
          {
            i = ((hd_video.VideoHead)localObject).int32_sub_service_type.get();
            localktl.jdField_b_of_type_Int = i;
            localktl.jdField_c_of_type_Int = ((hd_video.VideoHead)localObject).uint32_seq.get();
            localktl.jdField_a_of_type_Long = ((hd_video.VideoHead)localObject).uint64_room_id.get();
            if (paramArrayOfByte.msg_invite_body.has())
            {
              ...
              localktl.jdField_c_of_type_Long = paramArrayOfByte.uint64_from_uin.get();
              localktl.jdField_d_of_type_Int = paramArrayOfByte.uint32_new_business_flag.get();
              localktl.jdField_a_of_type_Boolean = paramArrayOfByte.bool_terminal_switch_flag.get();
              if (paramArrayOfByte.msg_temp_session.has())
              {
                paramArrayOfByte = paramArrayOfByte.msg_temp_session;
                localktl.jdField_d_of_type_Long = paramArrayOfByte.uint64_group_uin.get();
                localktl.e = paramArrayOfByte.uint32_relationship_type.get();
              }
            }
            QLog.d("VideoPackageUtils", 1, " --- parse video message " + localktl.toString());
            return localktl;
          }
        }
        else
