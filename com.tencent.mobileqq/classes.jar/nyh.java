    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if ((localObject != null) && (((JSONArray)localObject).length() > 0))
        {
          ...
          {
            nyd localnyd = new nyd(((JSONArray)localObject).getJSONObject(i1));
            this.jdField_a_of_type_JavaUtilList.add(localnyd);
            if (!QLog.isColorLevel()) {
              break label584;
            }
            QLog.d("AdvertisementExtInfo", 2, "adPKImageData = " + localnyd.toString());
            break label584;
          }
        }
