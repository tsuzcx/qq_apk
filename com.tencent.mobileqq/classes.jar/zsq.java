  ...
  protected void a(JobContext paramJobContext, armr paramarmr)
  {
    ...
    do
    {
      ...
      try
      {
        ...
        localReqBody.get_privilege_req.app_type.set(paramarmr.jdField_a_of_type_Int);
        if (paramarmr.jdField_a_of_type_Int == 1)
        {
          oidb_0xb60.ClientInfo localClientInfo = new oidb_0xb60.ClientInfo();
          localClientInfo.platform.set(1);
          if (!TextUtils.isEmpty(paramarmr.k)) {
            localClientInfo.sdk_version.set(paramarmr.k);
          }
          if (!TextUtils.isEmpty(paramarmr.i)) {
            localClientInfo.android_package_name.set(paramarmr.i);
          }
          if (!TextUtils.isEmpty(paramarmr.j)) {
            localClientInfo.android_signature.set(paramarmr.j);
          }
          localReqBody.client_info.set(localClientInfo);
        }
        if (QLog.isColorLevel()) {
          QLog.i("DoraemonOpenAPI.permissionHelper.jobApiPermission", 2, "send type=" + paramarmr.jdField_a_of_type_Int + ", appid=" + paramarmr.jdField_a_of_type_JavaLangString);
        }
        mmj.a(paramJobContext, new zsr(this, paramarmr), localReqBody.toByteArray(), "OidbSvc.0xb60_1", 2912, 1, null, 0L);
        return;
      }
