...
public class zgr
{
  ...
  public static Drawable a(int paramInt, float paramFloat, Context paramContext, Drawable.Callback paramCallback)
  {
    ...
    try
    {
      paramInt = zhb.jdField_b_of_type_ArrayOfInt[paramInt];
  ...
  public static void a(Context paramContext, SpannableStringBuilder paramSpannableStringBuilder)
  {
    try
    {
      ...
      int i = 0;
      while (i < zhb.jdField_b_of_type_ArrayOfJavaLangString.length)
      {
        for (int j = str.indexOf(zhb.jdField_b_of_type_ArrayOfJavaLangString[i]); j >= 0; j = str.indexOf(zhb.jdField_b_of_type_ArrayOfJavaLangString[i], j + zhb.jdField_b_of_type_ArrayOfJavaLangString[i].length())) {
          paramSpannableStringBuilder.setSpan(new VerticalCenterImageSpan(a(i, f, paramContext, null), 0), j, zhb.jdField_b_of_type_ArrayOfJavaLangString[i].length() + j, 33);
        }
        i += 1;
      }
      return;
