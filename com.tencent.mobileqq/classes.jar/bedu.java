...
public class bedu
{
  ...
  public static bedu a(aptx[] paramArrayOfaptx)
  {
    bedu localbedu = new bedu();
    int i = 0;
    if (i < paramArrayOfaptx.length)
    {
      ...
      for (;;)
      {
        ...
        try
        {
          ...
          localbedu.jdField_a_of_type_Int = ((JSONObject)localObject).optInt("limitMemberNum", 0);
          if (((JSONObject)localObject).optInt("globalOpenWithNoCheckGrayUin", 0) > 0) {}
          for (boolean bool = true;; bool = false)
          {
            localbedu.jdField_a_of_type_Boolean = bool;
            ...
            {
              localbedu.jdField_a_of_type_JavaUtilArrayList.add(String.valueOf(((JSONArray)localObject).get(j)));
              j += 1;
            }
          }
        }
    ...
    if (QLog.isColorLevel()) {
      QLog.i("TroopEssenceMsgEntryConfig", 2, "config:" + localbedu.toString());
    }
    return localbedu;
  }
