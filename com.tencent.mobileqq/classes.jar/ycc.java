...
public class ycc
{
  ...
  public ycc(qqstory_struct.FeedSeqInfo paramFeedSeqInfo)
  {
    ...
    for (;;)
    {
      ...
      try
      {
        for (;;)
        {
          if (!TextUtils.isEmpty(this.c)) {
            this.jdField_a_of_type_Long = yck.a().parse(this.c).getTime();
          }
          zdl.a(this.jdField_a_of_type_JavaLangString);
          return;
          bool = false;
          break;
      ...
      catch (Exception paramFeedSeqInfo)
      {
        for (;;)
        {
          ykq.c("Q.qqstory.home", "parse date " + this.c, paramFeedSeqInfo);
        }
      }
    }
  }
  
  public ycc(@NonNull String paramString1, int paramInt, String paramString2, String paramString3)
  {
    this.jdField_b_of_type_Int = -1;
    ...
    catch (ParseException paramString2)
    {
      for (;;)
      {
        ykq.c("Q.qqstory.home", "parse date", paramString2);
      }
    }
  }
  
  public static int a(List<ycc> paramList, String paramString)
  {
    paramList = paramList.iterator();
    int i = 0;
    while (paramList.hasNext())
    {
      if (((ycc)paramList.next()).jdField_a_of_type_JavaLangString.equals(paramString)) {
        return i;
      }
      i += 1;
    }
  ...
  public qqstory_struct.FeedSeqInfo a()
  {
    ...
    try
    {
      Integer localInteger1 = Integer.valueOf(yck.a().format(new Date(NetConnInfoCenter.getServerTimeMillis())));
      localFeedSeqInfo.date.set(localInteger1.intValue());
      return localFeedSeqInfo;
    }
    catch (Exception localException1)
    {
      ykq.b("FeedIdListSeqInfo", "exception ", localException1);
      try
      {
        Integer localInteger2 = Integer.valueOf(yck.a().format(new Date()));
        localFeedSeqInfo.date.set(localInteger2.intValue());
        return localFeedSeqInfo;
      }
      catch (Exception localException2)
      {
        ykq.b("FeedIdListSeqInfo", "exception ", localException2);
      }
    }
    return localFeedSeqInfo;
  }
