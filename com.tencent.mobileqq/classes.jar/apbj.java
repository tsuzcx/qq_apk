...
{
  public apbj(AppInterface paramAppInterface)
  {
    super(paramAppInterface);
  }
  
  private static apjr a(oidb_cmd0xb49.LBSPoint paramLBSPoint)
  {
    int j = 0;
    apjr localapjr = new apjr();
    int i;
    if (paramLBSPoint.int32_lat.has())
    {
      i = paramLBSPoint.int32_lat.get();
      localapjr.jdField_a_of_type_Int = i;
      if (!paramLBSPoint.int32_lon.has()) {
        break label141;
      }
      i = paramLBSPoint.int32_lon.get();
      label53:
      localapjr.jdField_b_of_type_Int = i;
      if (!paramLBSPoint.str_name.has()) {
        break label146;
      }
      str = paramLBSPoint.str_name.get();
      label77:
      localapjr.jdField_a_of_type_JavaLangString = str;
    ...
    {
      localapjr.jdField_b_of_type_JavaLangString = str;
      i = j;
    ...
    if (paramFromServiceMsg.isSuccess())
    {
      ...
      if (paramToServiceMsg.msg_ar_classify_rsp.has())
      {
        ...
        if (paramFromServiceMsg.uint32_lbs_result.has())
        {
          ...
          localapjq.jdField_a_of_type_Int = i;
          localapjq.jdField_a_of_type_JavaLangString = paramToServiceMsg;
          if (!paramFromServiceMsg.msg_nearest_point.has()) {
            break label291;
          }
          paramToServiceMsg = a(paramFromServiceMsg.msg_nearest_point);
          label241:
          localapjq.jdField_a_of_type_Apjr = paramToServiceMsg;
        }
      }
      else
      {
        QLog.i("ARLBSHandler", 1, "onReceiveLBSLocation. resp is success. retCode = " + localapjq.jdField_a_of_type_Int);
      }
    }
    for (;;)
    {
      notifyUI(1, true, localapjq);
      return;
      i = -1;
      break;
      label291:
