  ...
  extends rsa
{
  ...
  public static rsa b(oidb_cmd0xbc9.BannerItem paramBannerItem)
  {
    ...
    for (rsc localrsc = new rsc(paramBannerItem.uint32_banner_type.get());; localrsc = new rsc(4))
    {
      if (localContentBannerItem.bytes_image_url.has()) {
        localrsc.c = localContentBannerItem.bytes_image_url.get().toStringUtf8();
      }
      if (localContentBannerItem.bytes_link_url.has()) {
        localrsc.f = localContentBannerItem.bytes_link_url.get().toStringUtf8();
      }
      if (localContentBannerItem.bytes_rowkey.has()) {
        localrsc.e = localContentBannerItem.bytes_rowkey.get().toStringUtf8();
      }
      a(localrsc, paramBannerItem);
      a(paramBannerItem, localrsc);
      return localrsc;
    }
  }
