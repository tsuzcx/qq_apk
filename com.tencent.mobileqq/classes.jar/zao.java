import android.text.TextUtils;
import com.tencent.ad.tangram.net.AdHttp;
import com.tencent.gdtad.aditem.GdtAd;
import com.tencent.gdtad.views.canvas.GdtCanvasData;
import com.tencent.gdtad.views.form.GdtFormData;
import java.io.UnsupportedEncodingException;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

class zao
{
  private static zaq a(GdtAd paramGdtAd, GdtFormData paramGdtFormData)
  {
    zaq localzaq = new zaq(null);
    localzaq.setUrl("https://h5.gdt.qq.com/player/api/form/getSaveTokenForNative");
    localzaq.method = "POST";
    localzaq.contentType = "application/json";
    localzaq.connectTimeoutMillis = 5000;
    localzaq.readTimeoutMillis = 5000;
    localzaq.requestData = a(paramGdtAd, paramGdtFormData);
    return localzaq;
  }
  
  public static void a(GdtAd paramGdtAd, GdtFormData paramGdtFormData)
  {
    if (!a(paramGdtAd, paramGdtFormData))
    {
      yxs.d("GdtFormPrefetchTokenUtil", "prefetch error");
      return;
    }
    int i = 0;
    for (;;)
    {
      zaq localzaq;
      if (i < 3)
      {
        if (!a(paramGdtAd, paramGdtFormData))
        {
          yxs.d("GdtFormPrefetchTokenUtil", "prefetch error");
          return;
        }
        localzaq = a(paramGdtAd, paramGdtFormData);
        if (localzaq != null) {}
      }
      else
      {
        yxs.d("GdtFormPrefetchTokenUtil", "prefetch error");
        return;
      }
      AdHttp.send(localzaq);
      a(paramGdtAd, paramGdtFormData, localzaq);
      if ((localzaq.isSuccess()) && (localzaq.a == 0)) {
        break;
      }
      i += 1;
    }
  }
  
  private static void a(GdtAd paramGdtAd, GdtFormData paramGdtFormData, zaq paramzaq)
  {
    if ((paramzaq == null) || (!paramzaq.isSuccess()) || (!a(paramGdtAd, paramGdtFormData)))
    {
      yxs.d("GdtFormPrefetchTokenUtil", "onResponse error");
      return;
    }
    try
    {
      paramGdtAd = new JSONObject(new String(paramzaq.responseData, "UTF-8"));
      paramzaq.a = paramGdtAd.getInt("code");
      String str = paramGdtAd.optString("message");
      yxs.b("GdtFormPrefetchTokenUtil", "onResponse code:" + paramzaq.a + " message:" + str);
      if (paramzaq.a == 0)
      {
        paramGdtFormData.tokenForUpload = paramGdtAd.getJSONObject("data").getString("token");
        return;
      }
    }
    catch (UnsupportedEncodingException paramGdtAd)
    {
      yxs.d("GdtFormPrefetchTokenUtil", "onResponse", paramGdtAd);
      return;
      yxs.d("GdtFormPrefetchTokenUtil", "onResponse error");
      return;
    }
    catch (JSONException paramGdtAd)
    {
      yxs.d("GdtFormPrefetchTokenUtil", "onResponse", paramGdtAd);
    }
  }
  
  private static boolean a(GdtAd paramGdtAd, GdtFormData paramGdtFormData)
  {
    return (paramGdtAd != null) && (paramGdtAd.isValid()) && (!TextUtils.isEmpty(paramGdtAd.getTraceId())) && (paramGdtFormData != null) && (paramGdtFormData.isValid()) && (!TextUtils.isEmpty(paramGdtAd.getTraceId()));
  }
  
  private static byte[] a(GdtAd paramGdtAd, GdtFormData paramGdtFormData)
  {
    JSONArray localJSONArray = zau.a(paramGdtFormData);
    if ((!a(paramGdtAd, paramGdtFormData)) || (localJSONArray == null) || (localJSONArray == JSONObject.NULL))
    {
      yxs.d("GdtFormPrefetchTokenUtil", "getRequestData error");
      return null;
    }
    Object localObject = yzi.a(paramGdtAd);
    long l;
    if (localObject != null) {
      if (!((GdtCanvasData)localObject).isValid()) {
        l = -2147483648L;
      }
    }
    for (;;)
    {
      try
      {
        localObject = new JSONObject();
        if (l != -2147483648L) {
          ((JSONObject)localObject).put("pageId", l);
        }
        ((JSONObject)localObject).put("traceId", paramGdtAd.getTraceId());
        ((JSONObject)localObject).put("formId", paramGdtFormData.formId);
        ((JSONObject)localObject).put("formValue", localJSONArray.toString());
        paramGdtAd = ((JSONObject)localObject).toString().getBytes("UTF-8");
        return paramGdtAd;
      }
      catch (JSONException paramGdtAd)
      {
        yxs.d("GdtFormPrefetchTokenUtil", "getRequestData", paramGdtAd);
        return null;
      }
      catch (UnsupportedEncodingException paramGdtAd)
      {
        yxs.d("GdtFormPrefetchTokenUtil", "getRequestData", paramGdtAd);
        return null;
      }
      l = ((GdtCanvasData)localObject).pageId;
      continue;
      l = -2147483648L;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes8.jar
 * Qualified Name:     zao
 * JD-Core Version:    0.7.0.1
 */