...
public class aqkw
{
  ...
  {
    paramQQAppInterface = (aqjm)paramQQAppInterface.getManager(QQManagerFactory.BUSINESS_CARD_MANAGER);
  ...
  public static BusinessCard a(QQAppInterface paramQQAppInterface, azrb paramazrb)
  {
    aqjm localaqjm = (aqjm)paramQQAppInterface.getManager(QQManagerFactory.BUSINESS_CARD_MANAGER);
    if (paramazrb.jdField_a_of_type_ComTencentMobileqqActivityProfileActivity$AllInOne.jdField_a_of_type_Int == 0) {
      paramQQAppInterface = localaqjm.a();
    }
    for (;;)
    {
      paramazrb = paramQQAppInterface;
      if (paramQQAppInterface == null) {
        paramazrb = new BusinessCard();
      }
      return paramazrb;
      if (paramazrb.jdField_a_of_type_ComTencentMobileqqDataCard != null)
      {
        paramQQAppInterface = paramazrb.jdField_a_of_type_ComTencentMobileqqDataCard.getCardInfo();
        if ((paramazrb.jdField_a_of_type_ComTencentMobileqqDataCard.hasCardInfo()) && (!TextUtils.isEmpty(paramQQAppInterface.cardId))) {
          localaqjm.a(paramazrb.jdField_a_of_type_ComTencentMobileqqActivityProfileActivity$AllInOne.jdField_a_of_type_JavaLangString, paramQQAppInterface);
        } else {
          paramQQAppInterface = localaqjm.b(paramazrb.jdField_a_of_type_ComTencentMobileqqActivityProfileActivity$AllInOne.jdField_a_of_type_JavaLangString);
        }
      }
      else
      {
  ...
  public static void a(String paramString, URLImageView paramURLImageView, int paramInt1, int paramInt2)
  {
    ...
    do
    {
      ...
      try
      {
        ...
        localURLDrawableOptions.mLoadingDrawable = paramURLImageView.getContext().getResources().getDrawable(2130844907);
        paramURLImageView.setImageDrawable(URLDrawable.getDrawable(paramString, localURLDrawableOptions));
        paramURLImageView.setURLDrawableDownListener(new aqkx(paramURLImageView));
        return;
      }
      catch (Exception paramString) {}
