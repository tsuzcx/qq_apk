...
public class QQTextParseLinkUtil
{
  ...
  static
  {
    ...
    LINK_REGEX = bhfp.c.pattern() + "|" + bhfp.d.pattern() + "|" + "[a-zA-Z0-9\\+\\.\\_\\%\\-\\+]{1,256}\\@[a-zA-Z0-9][a-zA-Z0-9\\-]{0,64}(\\.[a-zA-Z0-9][a-zA-Z0-9\\-]{0,25})+" + "|" + "(?=.*,)(?=.*#)(\\+*[0-9]+[\\-,]*)?(\\([0-9]+\\)[\\-,]*)?([0-9][0-9\\-,][0-9\\-,#]+#)" + "|" + "[\\d]{5,15}" + "|" + QQ_PHONE;
  }
  ...
  static List<begs> parseLinkSpan(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (bhfp.b.matcher(paramString).find())
      {
        ...
        while (localMatcher1.find())
        {
          ...
          if (bhfp.c.matcher(str).find()) {
            localArrayList.add(new begs(str, i, j));
          } else if (bhfp.d.matcher(str).find()) {
            localArrayList.add(new begs(str, i, j));
