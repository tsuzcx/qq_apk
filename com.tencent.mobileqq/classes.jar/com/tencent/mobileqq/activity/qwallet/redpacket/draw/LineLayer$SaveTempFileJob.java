  ...
  implements Runnable
{
  ...
  private WeakReference<aize> jdField_a_of_type_JavaLangRefWeakReference;
  private int b = -1;
  
  public LineLayer$SaveTempFileJob(LineLayer paramLineLayer, int paramInt1, int paramInt2, Bitmap paramBitmap, aize paramaize)
  {
    this.jdField_a_of_type_JavaLangString = (aljq.cm + "temp" + File.separator);
    QLog.d("SaveTempFileJob", 2, "SaveTempFileJob begin:");
    this.jdField_a_of_type_JavaLangRefWeakReference = new WeakReference(paramaize);
    if (paramBitmap == null) {
      return;
    }
