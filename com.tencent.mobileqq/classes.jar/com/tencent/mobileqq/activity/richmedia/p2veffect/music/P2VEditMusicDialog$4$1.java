package com.tencent.mobileqq.activity.richmedia.p2veffect.music;

import ahks;
import com.tencent.qphone.base.util.QLog;
import soy;
import spb;
import sph;
import sqg;
import urk;
import veo;

public class P2VEditMusicDialog$4$1
  implements Runnable
{
  public P2VEditMusicDialog$4$1(ahks paramahks, spb paramspb) {}
  
  public void run()
  {
    urk.b(P2VEditMusicDialog.a(), "address update, refresh ui.");
    if (((soy)((sph)sqg.a(20)).a(1)).a(this.jdField_a_of_type_Spb))
    {
      QLog.d(P2VEditMusicDialog.a(), 2, "isInternationalUser --------------------2");
      this.jdField_a_of_type_Ahks.a.a.a();
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes11.jar
 * Qualified Name:     com.tencent.mobileqq.activity.richmedia.p2veffect.music.P2VEditMusicDialog.4.1
 * JD-Core Version:    0.7.0.1
 */