  ...
  implements Runnable
{
  ...
  private WeakReference<afbn> jdField_a_of_type_JavaLangRefWeakReference;
  private int b = -1;
  
  public LineLayer$SaveTempFileJob(LineLayer paramLineLayer, int paramInt1, int paramInt2, Bitmap paramBitmap, afbn paramafbn)
  {
    this.jdField_a_of_type_JavaLangString = (alof.cm + "temp" + File.separator);
    QLog.d("SaveTempFileJob", 2, "SaveTempFileJob begin:");
    this.jdField_a_of_type_JavaLangRefWeakReference = new WeakReference(paramafbn);
    if (paramBitmap == null) {
      return;
    }
