  ...
  implements Runnable
{
  ...
  private WeakReference<aghz> jdField_a_of_type_JavaLangRefWeakReference;
  private int b = -1;
  
  public LineLayer$SaveTempFileJob(LineLayer paramLineLayer, int paramInt1, int paramInt2, Bitmap paramBitmap, aghz paramaghz)
  {
    this.jdField_a_of_type_JavaLangString = (anhk.cp + "temp" + File.separator);
    QLog.d("SaveTempFileJob", 2, "SaveTempFileJob begin:");
    this.jdField_a_of_type_JavaLangRefWeakReference = new WeakReference(paramaghz);
    if (paramBitmap == null) {
      return;
    }
