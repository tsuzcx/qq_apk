  ...
  implements Runnable
{
  ...
  public void run()
  {
    ...
    ??? = this.jdField_a_of_type_ComTencentMobileqqMagicfaceDrawablePngFrameManager$RandomDrawableParam.jdField_a_of_type_ComTencentMobileqqActivityAioItemMarketFaceItemBuilder$Holder.a.emoticon.eId;
    ??? = new DownloadTask(EmoticonUtils.emoticonAIOPreviewExtensionUrl.replace("[eIdSub]", ((String)???).substring(0, 2)).replace("[eId]", (CharSequence)???).replace("[width]", "200").replace("[height]", "200"), new File(this.jdField_a_of_type_JavaLangString));
    ((DownloadTask)???).n = true;
    this.this$0.jdField_a_of_type_ComTencentMobileqqVipDownloaderInterface.a((DownloadTask)???, new PngFrameManager.3.1(this), null);
  }
}
