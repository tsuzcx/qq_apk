...
public class ScribbleMsgUtils
{
  ...
  {
    a();
    new ScribbleMsgUtils.SendAfterCombinePicAndDataTask(paramQQAppInterface, paramString, paramInt1, paramBitmap, paramInt2, paramCombineCallback).execute(new Void[0]);
  }
  ...
  public static boolean a(QQAppInterface paramQQAppInterface, MessageForScribble paramMessageForScribble)
  {
    ...
    if (paramMessageForScribble.isSendFromLocal()) {
      ((ITransFileController)paramQQAppInterface.getRuntimeService(ITransFileController.class)).removeProcessor(BaseTransFileController.makeKey(paramMessageForScribble.frienduin, paramMessageForScribble.uniseq));
    }
    ScribbleBaseOperator localScribbleBaseOperator = new ScribbleBaseOperator(paramQQAppInterface);
