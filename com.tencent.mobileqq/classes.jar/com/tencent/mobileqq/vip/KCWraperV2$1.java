...
public class KCWraperV2$1
  implements Runnable
{
  public KCWraperV2$1(biid parambiid, File paramFile, Context paramContext) {}
    ...
    for (;;)
    {
      synchronized (this.this$0.a)
      {
        ...
        if (QLog.isColorLevel()) {
          KcSdkShellManager.getInstance().setLogPrint(new biie(this));
        }
        KcSdkShellManager.getInstance().setTMSDKLogEnable(QLog.isColorLevel());
        KcSdkShellManager.getInstance().setPhoneInfoBridge(new biif(this));
        if (biim.a(this.jdField_a_of_type_AndroidContentContext))
        {
          ...
          {
            KcSdkShellManager.getInstance().getKingCardInterface().registerOnChangeListener(new biig());
            i = bool;
          }
          if (i != 0) {
            break label268;
