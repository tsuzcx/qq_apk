  ...
  extends ClearableEditText
{
  ...
  private ArrayList<bckp> jdField_a_of_type_JavaUtilArrayList = new ArrayList();
  ...
  private PointF a()
  {
    ...
    {
      Object localObject = (bckp)this.jdField_a_of_type_JavaUtilArrayList.get(i);
      switch (((bckp)localObject).c)
      {
      }
      for (;;)
      {
        i += 1;
        break;
        localPointF.x += getPaint().measureText(((bckp)localObject).jdField_a_of_type_JavaLangString);
        getPaint().getTextBounds(((bckp)localObject).jdField_a_of_type_JavaLangString, 0, ((bckp)localObject).jdField_a_of_type_JavaLangString.length(), localRect);
        f = Math.max(f, localRect.height());
        continue;
        localObject = (aykr)((bckp)localObject).jdField_a_of_type_AndroidTextStyleCharacterStyle;
        localPointF.x += ((aykr)localObject).a().getBounds().width();
        f = Math.max(f, ((aykr)localObject).a().getBounds().height());
      }
    }
    localPointF.y = f;
    return localPointF;
  ...
  public static void a(List<bckp> paramList, bckq parambckq, Spannable paramSpannable)
  {
    paramList.clear();
    ...
    {
      paramList.add(new bckp(1, 0, paramSpannable.length(), paramSpannable.toString(), null));
      return;
    }
    parambckq.a(paramSpannable);
    Arrays.sort(arrayOfCharacterStyle, parambckq);
    int j = 0;
    int i = 0;
    label76:
    int m;
    ...
    if (i < arrayOfCharacterStyle.length)
    {
      ...
      if (m > j) {
        paramList.add(new bckp(1, j, m, paramSpannable.subSequence(j, m).toString(), null));
      }
      parambckq = paramSpannable.subSequence(m, k).toString();
      if (!(arrayOfCharacterStyle[i] instanceof aykr)) {
        break label279;
      }
    }
    label279:
    for (j = 2;; j = 3)
    {
      paramList.add(new bckp(j, m, k, parambckq, arrayOfCharacterStyle[i]));
      if ((i == arrayOfCharacterStyle.length - 1) && (k < paramSpannable.length()))
      {
        parambckq = paramSpannable.subSequence(k, paramSpannable.length()).toString();
        paramList.add(new bckp(1, k, paramSpannable.length(), parambckq, null));
      }
      i += 1;
      j = k;
      break label76;
  ...
  public void a()
  {
    a(this.jdField_a_of_type_JavaUtilArrayList, this.jdField_a_of_type_Bckq, getText());
  }
  ...
  protected void onDraw(Canvas paramCanvas)
  {
    ...
    if (this.jdField_a_of_type_Int != 0)
    {
      ...
      case 2: 
        ...
        {
          localbckp = (bckp)this.jdField_a_of_type_JavaUtilArrayList.get(j);
          switch (localbckp.c)
          {
          default: 
            j += 1;
          ...
          case 1: 
            ...
            getPaint().getTextBounds(localbckp.jdField_a_of_type_JavaLangString, 0, localbckp.jdField_a_of_type_JavaLangString.length(), this.jdField_a_of_type_AndroidGraphicsRect);
    ...
    for (;;)
    {
      int k = 0;
      float f2;
      if (k < localbckp.jdField_a_of_type_JavaLangString.length())
      {
        float f7 = getPaint().measureText(localbckp.jdField_a_of_type_JavaLangString, k, k + 1);
      ...
      break label196;
      f1 += ((aykr)localbckp.jdField_a_of_type_AndroidTextStyleCharacterStyle).a().getBounds().width();
      break label196;
      ...
      if (j < this.jdField_a_of_type_JavaUtilArrayList.size())
      {
        ...
        for (;;)
        {
          ...
          break;
          k = 0;
          while (k < localbckp.jdField_a_of_type_JavaLangString.length())
          {
            f2 = getPaint().measureText(localbckp.jdField_a_of_type_JavaLangString, k, k + 1);
