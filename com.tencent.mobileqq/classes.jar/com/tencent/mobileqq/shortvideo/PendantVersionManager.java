...
public class PendantVersionManager
{
  ...
  {
    ThreadManager.post(new aidb(), 5, null, false);
  }
  ...
  public static boolean b(String paramString1, String paramString2)
  {
    ...
    if (i < paramString2.size())
    {
      ...
      do
      {
        ...
        localObject = aies.a(str);
        j = ((aiet)localObject).a('|');
        if (j != 0) {
          break label407;
        }
        str = paramString1 + ((aiet)localObject).c();
        File localFile = new File(str);
        if (!localFile.exists())
        {
        ...
        j = Integer.parseInt(((aiet)localObject).d());
        l = localFile.length();
      } while (j == l);
