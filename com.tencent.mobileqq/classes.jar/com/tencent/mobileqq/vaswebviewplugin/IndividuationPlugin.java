    ...
    for (int i = 1;; i = -40) {
      try
      {
        ...
        if ("600".equals(localObject))
        {
          ...
          if (!BaseApplicationImpl.IS_SUPPORT_THEME)
          {
            Toast.makeText(this.mActivity.getApplicationContext(), this.mActivity.getString(2131719888), 0).show();
            return;
          }
          if (!Utils.a())
          {
            Toast.makeText(this.mActivity.getApplicationContext(), this.mActivity.getString(2131718859), 0).show();
            return;
          }
        }
      }
      catch (Exception paramJSONObject)
      {
        ...
        if (QLog.isColorLevel())
        {
          ...
          if ("500".equals(localObject))
          {
            ...
            paramJSONObject.putExtra("selfSet_leftViewText", this.mActivity.getString(2131690778));
          ...
          if ("0".equals(localObject))
          {
            ...
            paramJSONObject = URLDecoder.decode(paramJSONObject.getString("detailId"), "UTF-8").replace("{uin}", this.mBrowserApp.getCurrentAccountUin()).replace("{client}", "androidQQ").replace("{version}", "8.5.5.5105").replace("{platformId}", "2").replace("{device}", Build.DEVICE).replace("{system}", Build.VERSION.RELEASE).replace("{systemInt}", Integer.toString(Build.VERSION.SDK_INT)).replace("{adtag}", "mvip.gongneng.anroid.individuation.web").replace("{updateFlag}", "false").replace("{density}", ThemeUtil.getThemeDensity(this.mBrowserApp.getApplication().getApplicationContext()));
      ...
      try
      {
        ...
        if (i == 2)
        {
          ...
          if (paramJSONObject != null)
          {
            ...
            paramJSONObject.setText(2131694960);
