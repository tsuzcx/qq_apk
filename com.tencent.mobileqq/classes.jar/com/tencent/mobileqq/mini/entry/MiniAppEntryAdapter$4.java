...
class MiniAppEntryAdapter$4
  implements Runnable
{
  MiniAppEntryAdapter$4(MiniAppEntryAdapter paramMiniAppEntryAdapter, BaseActivity paramBaseActivity, MiniAppInfo paramMiniAppInfo) {}
  ...
  public void run()
  {
    ...
    if (BaseApplicationImpl.getApplication().getSharedPreferences(this.val$miniAppInfo.appId + "_" + (String)localObject, 4).edit().clear().commit()) {
      QLog.d("MiniAppEntryAdapter", 1, "clear authorize info. " + this.val$miniAppInfo.appId);
    }
    if (StorageUtil.getPreference().edit().putBoolean(this.val$miniAppInfo.appId + "_debug", false).commit()) {
      QLog.d("MiniAppEntryAdapter", 1, "clear debug info. " + this.val$miniAppInfo.appId);
    }
    ...
    if (localObject != null)
    {
      ...
      QLog.d("MiniAppEntryAdapter", 1, "kill process. " + this.val$miniAppInfo.appId + "; " + ((AppBrandLaunchManager.MiniAppSubProcessorInfo)localObject).processName);
    }
  }
}
 ...
 * Qualified Name:     com.tencent.mobileqq.mini.entry.MiniAppEntryAdapter.4
