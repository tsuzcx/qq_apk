package com.tencent.mobileqq.mini.widget.media;

import android.graphics.Bitmap;
import com.tencent.mobileqq.app.ThreadManagerV2;
import com.tencent.mobileqq.mini.widget.media.live.TXLivePushListenerReflect.ITXSnapshotListener;

class MiniAppLivePusher$3
  implements TXLivePushListenerReflect.ITXSnapshotListener
{
  MiniAppLivePusher$3(MiniAppLivePusher paramMiniAppLivePusher, String paramString, int paramInt) {}
  
  public void onSnapshot(Bitmap paramBitmap)
  {
    ThreadManagerV2.excute(new MiniAppLivePusher.3.1(this, paramBitmap), 64, null, true);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes10.jar
 * Qualified Name:     com.tencent.mobileqq.mini.widget.media.MiniAppLivePusher.3
 * JD-Core Version:    0.7.0.1
 */