package com.tencent.qcircle.tavcut.session.config;

public class StickerEditViewIconConfig
{
  protected int adjustTimeRangeIconResId = -1;
  protected int deleteIconResId = -1;
  protected int editIconResId = -1;
  protected int zoomIconResId = -1;
  
  public int getAdjustTimeRangeIconResId()
  {
    return this.adjustTimeRangeIconResId;
  }
  
  public int getDeleteIconResId()
  {
    return this.deleteIconResId;
  }
  
  public int getEditIconResId()
  {
    return this.editIconResId;
  }
  
  public int getZoomIconResId()
  {
    return this.zoomIconResId;
  }
  
  public void setAdjustTimeRangeIconResId(int paramInt)
  {
    this.adjustTimeRangeIconResId = paramInt;
  }
  
  public void setDeleteIconResId(int paramInt)
  {
    this.deleteIconResId = paramInt;
  }
  
  public void setEditIconResId(int paramInt)
  {
    this.editIconResId = paramInt;
  }
  
  public void setZoomIconResId(int paramInt)
  {
    this.zoomIconResId = paramInt;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes13.jar
 * Qualified Name:     com.tencent.qcircle.tavcut.session.config.StickerEditViewIconConfig
 * JD-Core Version:    0.7.0.1
 */