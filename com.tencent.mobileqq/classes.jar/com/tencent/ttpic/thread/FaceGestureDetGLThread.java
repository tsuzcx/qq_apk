...
public class FaceGestureDetGLThread
{
  ...
  private float[] getRGBGain(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
  {
      ...
      while (i < 5)
      {
        ...
        while (j < i1 / 5)
        {
          ...
          while (k < i2 / 5)
          {
            ...
            i3 = i3 * paramInt2 + j;
            m = m * i + k;
            if ((this.matrixCbs[paramInt2][i][j][k] - (d3 + d2) >= 1.5D * d2) || (this.matrixCrs[paramInt2][i][j][k] - (1.5D * d4 + d1) >= 1.5D * d1)) {
    ...
    for (paramInt2 = k / j;; paramInt2 = 0)
    {
      ...
      {
        f3 = Math.max(Math.max(Math.abs(f6), Math.abs(f5)), Math.abs(f4));
        f1 = f6 / f3 * 0.05F;
        f2 = f5 / f3 * 0.05F;
        f3 = f4 / f3 * 0.05F;
      }
      localObject[0] = (f1 + 1.0F);
      localObject[1] = (f2 + 1.0F);
      localObject[2] = (f3 + 1.0F);
      return localObject;
      i = 0;
    }
  }
