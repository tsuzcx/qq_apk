...
{
  private static final String TAG = Audio3DFilter.class.getSimpleName();
  private int faceCount;
  private GameFilter gameFilter;
  private Frame gameFrame = new Frame();
  ...
  public void render(Frame paramFrame, AIAttr paramAIAttr, PTFaceAttr paramPTFaceAttr)
  {
    int i;
    ...
    for (;;)
    {
      ...
      try
      {
        if (FeatureManager.Features.GAMEPLAY.isFunctionReady())
        {
          ...
          this.gameFrame.bindFrame(this.gameFilter.getGameRefTextureID(), this.gameFilter.getGameRefTextureWidth(), this.gameFilter.getGameRefTextureHeight(), 0.0D);
          ...
          RenderProcess(paramFrame.getTextureId(), this.gameFilter.getGameRefTextureWidth(), this.gameFilter.getGameRefTextureHeight(), -1, 0.0D, this.gameFrame);
          this.gameFilter.updateAndRender(paramFrame, paramAIAttr, paramPTFaceAttr);
        }
      ...
      catch (UnsatisfiedLinkError paramFrame)
      {
        for (;;)
        {
          paramFrame.printStackTrace();
        }
      }
    }
  }
  ...
  public void updateGameFilterRotation(int paramInt)
  {
    this.gameFilter.updateVideoSize(this.width, this.height, paramInt);
  }
