...
import com.tencent.ttpic.openapi.filter.EyeBagBlurFilter;
...
public class BeautyEffectCombineFilter
  extends VideoFilterBase
{
  ...
  private float alphaValue = 0.09F;
  ...
  float[] mCgCrLeft = new float[2];
  float[] mCgCrRight = new float[2];
  private BaseFilter mCopyFilter = new BaseFilter("precision highp float;\nvarying vec2 textureCoordinate;\nuniform sampler2D inputImageTexture;\nvoid main() \n{\ngl_FragColor = texture2D (inputImageTexture, textureCoordinate);\n}\n");
  private Frame mCopyFrame = new Frame();
  byte[] mData = null;
  float[] mRGBLeft = new float[3];
  float[] mRGBRight = new float[3];
  ...
  public BeautyEffectCombineFilter()
  {
  ...
  void getAvgColor(Frame paramFrame, float paramFloat1, List<PointF> paramList, float paramFloat2)
  {
    ...
    this.mCgCrLeft[0] = 114.0F;
    this.mCgCrLeft[1] = 112.0F;
    this.mRGBLeft[0] = -1.0F;
    this.mRGBLeft[1] = -1.0F;
    this.mRGBLeft[2] = -1.0F;
    ...
    if (j == 0)
    {
      this.mRGBLeft[0] = (f6 / n);
      this.mRGBLeft[1] = (f5 / n);
      this.mRGBLeft[2] = (f4 / n);
      this.mCgCrLeft[0] = ((-81085.0F * this.mRGBLeft[0] + 112000.0F * this.mRGBLeft[1] - 30915.0F * this.mRGBLeft[2]) / 256000.0F + 128.0F);
      this.mCgCrLeft[1] = ((-37797.0F * this.mRGBLeft[0] - 74203.0F * this.mRGBLeft[1] + 112000.0F * this.mRGBLeft[2]) / 256000.0F + 128.0F);
      this.mRGBLeft[0] /= 255.0F;
      this.mRGBLeft[1] /= 255.0F;
      this.mRGBLeft[2] /= 255.0F;
      ...
      this.mCgCrRight[0] = 114.0F;
      this.mCgCrRight[1] = 112.0F;
      this.mRGBRight[0] = -1.0F;
      this.mRGBRight[1] = -1.0F;
      this.mRGBRight[2] = -1.0F;
      f2 = 0.0F;
      k = (j - i) * (k - m);
      paramFloat1 = 0.0F;
      paramFloat2 = 0.0F;
    ...
    for (;;)
    {
      i += 4;
      break label1212;
      this.mRGBLeft[0] = (f3 / j);
      this.mRGBLeft[1] = (f2 / j);
      this.mRGBLeft[2] = (f1 / j);
      break;
      label1365:
      f1 += localObject[0];
    ...
    if (j == 0)
    {
      this.mRGBRight[0] = (f4 / k);
      this.mRGBRight[1] = (f3 / k);
      this.mRGBRight[2] = (f2 / k);
    }
    for (;;)
    {
      this.mCgCrRight[0] = ((-81085 * localObject[0] + 112000 * localObject[1] - localObject[2] * 30915) / 256000.0F + 128.0F);
      this.mCgCrRight[1] = ((-37797 * localObject[0] - 74203 * localObject[1] + 112000 * localObject[2]) / 256000.0F + 128.0F);
      this.mRGBRight[0] /= 255.0F;
      this.mRGBRight[1] /= 255.0F;
      this.mRGBRight[2] /= 255.0F;
      return;
      this.mRGBRight[0] = (f1 / j);
      this.mRGBRight[1] = (paramFloat2 / j);
      this.mRGBRight[2] = (paramFloat1 / j);
    }
  }
  ...
  public void setEyePouchSmoothImage(int paramInt, float[] paramArrayOfFloat1, float[] paramArrayOfFloat2, float[] paramArrayOfFloat3, float[] paramArrayOfFloat4)
  {
    ...
    addParam(new UniformParam.FloatsParam("rgbLeft", paramArrayOfFloat1));
    addParam(new UniformParam.FloatsParam("rgbRight", paramArrayOfFloat2));
    addParam(new UniformParam.FloatsParam("cgcrLeft", paramArrayOfFloat3));
    addParam(new UniformParam.FloatsParam("cgcrRight", paramArrayOfFloat4));
  }
  ...
  public Frame updateAndRender(Frame paramFrame, List<PointF> paramList, float[] paramArrayOfFloat)
  {
    if (getSmoothOpacity() > 0.0F)
    {
      this.mCopyFilter.RenderProcess(paramFrame.getTextureId(), paramFrame.width / 2, paramFrame.height / 2, -1, 0.0D, this.mCopyFrame);
      this.mBlurFilter1.updateParam(paramList, paramArrayOfFloat);
      this.mBlurFilter1.updateSize(1.0F / this.mCopyFrame.width, 0.0F);
      this.mBlurFilter1.RenderProcess(this.mCopyFrame.getTextureId(), this.mCopyFrame.width, this.mCopyFrame.height, -1, 0.0D, this.mBlurFrame1);
      this.mBlurFilter1.updateSize(0.0F, 1.0F / this.mCopyFrame.height);
      ...
      getAvgColor(this.mCopyFrame, 0.5F, paramList, 1.0F);
    }
    setEyePouchSmoothImage(this.mBlurFrame2.getLastRenderTextureId(), this.mRGBLeft, this.mRGBRight, this.mCgCrLeft, this.mCgCrRight);
    this.mCopyFilter.RenderProcess(paramFrame.getTextureId(), paramFrame.width, paramFrame.height, -1, 0.0D, this.mCopyFrame);
    updateParam(paramList, paramArrayOfFloat);
    OnDrawFrameGLSL();
 ...
 * Qualified Name:     com.tencent.ttpic.filter.BeautyEffectCombineFilter
