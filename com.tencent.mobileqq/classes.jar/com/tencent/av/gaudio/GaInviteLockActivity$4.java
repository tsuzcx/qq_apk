    ...
    do
    {
      ...
      while (i < paramContext.rpt_msg_member_quit.size())
      {
        ...
        do
        {
          ...
          localObject = (Memberinfo)paramIntent.next();
        } while (((Memberinfo)localObject).jdField_a_of_type_Long != l1);
        GaInviteLockActivity.a(this.a).remove(localObject);
        if (QLog.isColorLevel()) {
          QLog.d(this.a.jdField_b_of_type_JavaLangString, 2, "mMemberChangeEventReceiver remove member UIN:" + ((Memberinfo)localObject).jdField_a_of_type_Long);
        }
        k = j;
        if (n != 2) {
          k = 1;
