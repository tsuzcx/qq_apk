    ...
    if (Math.round(paramFloat) != this.mRadiusInPixels)
    {
      ...
      runOnDraw(new Runnable()
      {
        public void run()
        {
          ...
          {
            int i = 0;
            while (i < QQAVImageDenoiseGaussianBlurFilter.this.mFilters.size())
            {
              ((QQAVImageFilter)QQAVImageDenoiseGaussianBlurFilter.this.mFilters.get(i)).init();
              ((QQAVImageFilter)QQAVImageDenoiseGaussianBlurFilter.this.mFilters.get(i)).onOutputSizeChanged(QQAVImageDenoiseGaussianBlurFilter.this.mOutputWidth, QQAVImageDenoiseGaussianBlurFilter.this.mOutputHeight);
