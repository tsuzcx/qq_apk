...
public class BeautysRealFilter
  extends BaseFilter
{
  ...
  {
    float f = 2.0F * (4.0F * paramFloat * this.scale + 1.0F);
    this.radius = (2.5F * Math.max(0.0F, Math.min(paramFloat, 1.0F)) * this.scale);
    int i = 0;
    while (i < 7)
    {
      this.coef[i] = ((float)Math.exp(-(this.radius * i) * (this.radius * i) / (2.0F * f * f)));
      i += 1;
    }
    paramFloat = this.coef[0];
    i = 1;
    while (i < 7)
    {
      paramFloat += this.coef[i] * 2.0F;
      i += 1;
    }
    i = 0;
    while (i < 7)
    {
      float[] arrayOfFloat = this.coef;
      arrayOfFloat[i] /= paramFloat;
      i += 1;
    }
    if (this.smootfilter2 != null)
    {
      this.smootfilter2.addParam(new Param.FloatParam("radius", this.radius));
      this.smootfilter2.addParam(new Param.Float1sParam("coef", this.coef));
      this.smootfilter1.addParam(new Param.Float1sParam("coef", this.coef));
      this.smootfilter1.addParam(new Param.FloatParam("radius", this.radius));
    }
  }
  ...
  public void ApplyGLSLFilter(boolean paramBoolean, float paramFloat1, float paramFloat2)
  {
    ...
    this.smootfilter1 = new BaseFilter(GLSLRender.FILTER_SHADER_BEAUTY_HORIZONTAL_REAL);
    this.smootfilter1.addParam(new Param.FloatParam("radius", this.radius));
    this.smootfilter1.addParam(new Param.Float1sParam("coef", this.coef));
    setNextFilter(this.smootfilter1, null);
    this.smootfilter2 = new BaseFilter(GLSLRender.FILTER_SHADER_BEAUTY_VERTICAL_REAL);
    this.smootfilter2.addParam(new Param.FloatParam("radius", this.radius));
    this.smootfilter2.addParam(new Param.Float1sParam("coef", this.coef));
 ...
 * Qualified Name:     com.tencent.filter.ttpic.BeautysRealFilter
