    ...
    if (localObject1.length == 5)
    {
      ...
      for (;;)
      {
        try
        {
          ...
          localObject3 = localObject1[1].split("&");
          ...
          if (k != -1)
          {
            if (!"mqq_tt".equals(localObject3[i].substring(0, k)))
            {
              new StringBuilder().append(str).append(".").append((String)localObject2).toString();
              ((List)localObject1).add(URLDecoder.decode(localObject3[i].substring(k + 1)));
            }
            i += 1;
          }
          else
