package com.tencent.aelight.camera.aeeditor;

import com.tencent.aelight.camera.ae.AEPathBase.BASE;
import java.io.File;

public class AEEditorPath$EDITOR$FILES
{
  public static final String a;
  public static final String b;
  public static final String c;
  public static final String d;
  public static final String e;
  public static final String f;
  public static final String g;
  
  static
  {
    StringBuilder localStringBuilder = new StringBuilder();
    localStringBuilder.append(AEPathBase.BASE.b);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("ae_editor");
    a = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(a);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("resources");
    b = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(a);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("effect");
    c = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(a);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("generated_images");
    d = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(a);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("generated_videos");
    e = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(AEPathBase.BASE.a);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("ae_editor");
    f = localStringBuilder.toString();
    localStringBuilder = new StringBuilder();
    localStringBuilder.append(f);
    localStringBuilder.append(File.separator);
    localStringBuilder.append("generated_videos");
    localStringBuilder.append(File.separator);
    localStringBuilder.append("cover");
    g = localStringBuilder.toString();
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes17.jar
 * Qualified Name:     com.tencent.aelight.camera.aeeditor.AEEditorPath.EDITOR.FILES
 * JD-Core Version:    0.7.0.1
 */