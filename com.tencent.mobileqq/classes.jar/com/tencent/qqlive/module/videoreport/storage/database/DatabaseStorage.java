  ...
  implements IDataStorage
{
  ...
  private DatabaseStorage(Context paramContext)
  {
    this.mCache = DbCache.getInstance(paramContext);
    this.mAnnotationProcessor = AnnotationProcessor.getInstance();
  }
  
  public static DatabaseStorage getInstance(Context paramContext)
  {
    if (sInstance == null) {
      try
      {
        if (sInstance == null) {
          sInstance = new DatabaseStorage(paramContext);
        }
      }
      finally {}
    }
  ...
  {
    paramList = this.mCache.getObjects(paramClass, new DatabaseStorage.1(this, paramList));
    paramClass = new ArrayList();
 ...
 * Qualified Name:     com.tencent.qqlive.module.videoreport.storage.database.DatabaseStorage
