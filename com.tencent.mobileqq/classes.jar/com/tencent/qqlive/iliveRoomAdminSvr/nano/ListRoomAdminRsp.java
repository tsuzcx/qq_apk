package com.tencent.qqlive.iliveRoomAdminSvr.nano;

import com.google.protobuf.nano.CodedInputByteBufferNano;
import com.google.protobuf.nano.CodedOutputByteBufferNano;
import com.google.protobuf.nano.ExtendableMessageNano;
import com.google.protobuf.nano.MessageNano;
import com.google.protobuf.nano.WireFormatNano;

public final class ListRoomAdminRsp
  extends ExtendableMessageNano<ListRoomAdminRsp>
{
  public int a;
  public String b;
  public int c;
  public AdminDetail[] d;
  public boolean e;
  
  public ListRoomAdminRsp()
  {
    a();
  }
  
  public static ListRoomAdminRsp a(byte[] paramArrayOfByte)
  {
    return (ListRoomAdminRsp)MessageNano.mergeFrom(new ListRoomAdminRsp(), paramArrayOfByte);
  }
  
  public ListRoomAdminRsp a()
  {
    this.a = 0;
    this.b = "";
    this.c = 0;
    this.d = AdminDetail.a();
    this.e = false;
    this.unknownFieldData = null;
    this.cachedSize = -1;
    return this;
  }
  
  public ListRoomAdminRsp a(CodedInputByteBufferNano paramCodedInputByteBufferNano)
  {
    for (;;)
    {
      int i = paramCodedInputByteBufferNano.readTag();
      if (i == 0) {
        break;
      }
      if (i != 8)
      {
        if (i != 18)
        {
          if (i != 24)
          {
            if (i != 34)
            {
              if (i != 40)
              {
                if (!storeUnknownField(paramCodedInputByteBufferNano, i)) {
                  return this;
                }
              }
              else {
                this.e = paramCodedInputByteBufferNano.readBool();
              }
            }
            else
            {
              int j = WireFormatNano.getRepeatedFieldArrayLength(paramCodedInputByteBufferNano, 34);
              AdminDetail[] arrayOfAdminDetail = this.d;
              if (arrayOfAdminDetail == null) {
                i = 0;
              } else {
                i = arrayOfAdminDetail.length;
              }
              arrayOfAdminDetail = new AdminDetail[j + i];
              j = i;
              if (i != 0)
              {
                System.arraycopy(this.d, 0, arrayOfAdminDetail, 0, i);
                j = i;
              }
              while (j < arrayOfAdminDetail.length - 1)
              {
                arrayOfAdminDetail[j] = new AdminDetail();
                paramCodedInputByteBufferNano.readMessage(arrayOfAdminDetail[j]);
                paramCodedInputByteBufferNano.readTag();
                j += 1;
              }
              arrayOfAdminDetail[j] = new AdminDetail();
              paramCodedInputByteBufferNano.readMessage(arrayOfAdminDetail[j]);
              this.d = arrayOfAdminDetail;
            }
          }
          else {
            this.c = paramCodedInputByteBufferNano.readUInt32();
          }
        }
        else {
          this.b = paramCodedInputByteBufferNano.readString();
        }
      }
      else {
        this.a = paramCodedInputByteBufferNano.readInt32();
      }
    }
    return this;
  }
  
  protected int computeSerializedSize()
  {
    int j = super.computeSerializedSize();
    int k = this.a;
    int i = j;
    if (k != 0) {
      i = j + CodedOutputByteBufferNano.computeInt32Size(1, k);
    }
    j = i;
    if (!this.b.equals("")) {
      j = i + CodedOutputByteBufferNano.computeStringSize(2, this.b);
    }
    k = this.c;
    i = j;
    if (k != 0) {
      i = j + CodedOutputByteBufferNano.computeUInt32Size(3, k);
    }
    Object localObject = this.d;
    j = i;
    if (localObject != null)
    {
      j = i;
      if (localObject.length > 0)
      {
        k = 0;
        for (;;)
        {
          localObject = this.d;
          j = i;
          if (k >= localObject.length) {
            break;
          }
          localObject = localObject[k];
          j = i;
          if (localObject != null) {
            j = i + CodedOutputByteBufferNano.computeMessageSize(4, (MessageNano)localObject);
          }
          k += 1;
          i = j;
        }
      }
    }
    boolean bool = this.e;
    i = j;
    if (bool) {
      i = j + CodedOutputByteBufferNano.computeBoolSize(5, bool);
    }
    return i;
  }
  
  public void writeTo(CodedOutputByteBufferNano paramCodedOutputByteBufferNano)
  {
    int i = this.a;
    if (i != 0) {
      paramCodedOutputByteBufferNano.writeInt32(1, i);
    }
    if (!this.b.equals("")) {
      paramCodedOutputByteBufferNano.writeString(2, this.b);
    }
    i = this.c;
    if (i != 0) {
      paramCodedOutputByteBufferNano.writeUInt32(3, i);
    }
    Object localObject = this.d;
    if ((localObject != null) && (localObject.length > 0))
    {
      i = 0;
      for (;;)
      {
        localObject = this.d;
        if (i >= localObject.length) {
          break;
        }
        localObject = localObject[i];
        if (localObject != null) {
          paramCodedOutputByteBufferNano.writeMessage(4, (MessageNano)localObject);
        }
        i += 1;
      }
    }
    boolean bool = this.e;
    if (bool) {
      paramCodedOutputByteBufferNano.writeBool(5, bool);
    }
    super.writeTo(paramCodedOutputByteBufferNano);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes13.jar
 * Qualified Name:     com.tencent.qqlive.iliveRoomAdminSvr.nano.ListRoomAdminRsp
 * JD-Core Version:    0.7.0.1
 */