package com.tencent.biz.pubaccount.readinjoy.pts;

import com.tencent.biz.pubaccount.readinjoy.pts.util.PTSReport;
import com.tencent.biz.pubaccount.readinjoy.pts.util.PTSReport.R5Builder;
import com.tencent.pts.core.itemview.PTSItemData;

class PTSItemViewBuilder$2
  implements Runnable
{
  public void run()
  {
    PTSReport.a("0X800A9DC", "", "", "", new PTSReport.R5Builder().a("page_name", this.jdField_a_of_type_ComTencentPtsCoreItemviewPTSItemData.getPageName()).a("get_view_cost", String.valueOf(this.jdField_a_of_type_Long)).a());
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes3.jar
 * Qualified Name:     com.tencent.biz.pubaccount.readinjoy.pts.PTSItemViewBuilder.2
 * JD-Core Version:    0.7.0.1
 */