...
public class EmoWindow
{
  ...
  public static void a(Context paramContext, SpannableStringBuilder paramSpannableStringBuilder)
  {
    try
    {
      ...
      int i = 0;
      while (i < Patterns.jdField_b_of_type_ArrayOfJavaLangString.length)
      {
        for (int j = str.indexOf(Patterns.jdField_b_of_type_ArrayOfJavaLangString[i]); j >= 0; j = str.indexOf(Patterns.jdField_b_of_type_ArrayOfJavaLangString[i], j + Patterns.jdField_b_of_type_ArrayOfJavaLangString[i].length())) {
          paramSpannableStringBuilder.setSpan(new VerticalCenterImageSpan(a(i, f, paramContext, null), 0), j, Patterns.jdField_b_of_type_ArrayOfJavaLangString[i].length() + j, 33);
        }
        i += 1;
      }
      return;
