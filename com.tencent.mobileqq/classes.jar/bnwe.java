import android.content.Context;
import com.tencent.mobileqq.app.ThreadManager;
import dov.com.qq.im.aeeditor.module.aifilter.BatchImageAIFilterProxy.1;
import java.util.ArrayList;
import java.util.List;
import mqq.os.MqqHandler;

public class bnwe
  extends bnvy
{
  private static final String jdField_a_of_type_JavaLangString = bnwe.class.getSimpleName();
  private bnwd jdField_a_of_type_Bnwd;
  private int b;
  private List<String> c;
  private List<bnsw> d;
  
  public bnwe(List<String> paramList, List<bnsw> paramList1, int paramInt, bnwd parambnwd)
  {
    this.c = paramList;
    this.d = paramList1;
    this.jdField_b_of_type_Int = paramInt;
    this.jdField_a_of_type_Bnwd = parambnwd;
    this.jdField_a_of_type_Int = (paramList.size() - 1);
  }
  
  protected bnwd a(List<bnwc> paramList)
  {
    int j = 0;
    bnwf localbnwf = new bnwf();
    int i;
    if ((paramList == null) || (paramList.size() == 0)) {
      i = 1;
    }
    for (;;)
    {
      if (i != 0)
      {
        i = j;
        for (;;)
        {
          if (i < this.jdField_a_of_type_Int)
          {
            paramList = a(null);
            paramList.jdField_a_of_type_Boolean = true;
            localbnwf.jdField_a_of_type_JavaUtilList.add(paramList);
            i += 1;
            continue;
            if (paramList.size() == this.jdField_b_of_type_JavaUtilList.size()) {
              break label210;
            }
            bnrh.d(jdField_a_of_type_JavaLangString, "batch image responseList size not match image size");
            i = 1;
            break;
          }
        }
        localbnwf.jdField_a_of_type_Boolean = true;
      }
      for (;;)
      {
        localbnwf.jdField_a_of_type_JavaUtilList.add(this.jdField_b_of_type_Int, (bnwm)this.jdField_a_of_type_Bnwd);
        return localbnwf;
        i = 0;
        while (i < this.jdField_a_of_type_Int)
        {
          Object localObject = (bnwc)paramList.get(i);
          this.jdField_a_of_type_JavaUtilList.clear();
          this.jdField_a_of_type_JavaUtilList.add(this.jdField_b_of_type_JavaUtilList.get(i));
          localObject = a((bnwc)localObject);
          localbnwf.jdField_a_of_type_JavaUtilList.add(localObject);
          i += 1;
        }
      }
      label210:
      i = 0;
    }
  }
  
  protected void a(Context paramContext, bnwb parambnwb)
  {
    ArrayList localArrayList = new ArrayList(this.c);
    localArrayList.remove(this.jdField_b_of_type_Int);
    ThreadManager.getSubThreadHandler().post(new BatchImageAIFilterProxy.1(this, localArrayList, paramContext, parambnwb));
  }
  
  public boolean a()
  {
    return false;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes5.jar
 * Qualified Name:     bnwe
 * JD-Core Version:    0.7.0.1
 */