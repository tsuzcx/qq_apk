    ...
    for (;;)
    {
      ...
      if ((paramContext.equalsIgnoreCase("tencent.video.q2v.GvideoMemInviteUpdate")) && (bool))
      {
        ...
        try
        {
          ...
          {
            paramIntent.add(Long.valueOf(((lnv)((Iterator)localObject).next()).jdField_a_of_type_Long));
            continue;
            bool = false;
          }
        }
    ...
    do
    {
      ...
      if (j < paramContext.rpt_msg_member_join.size())
      {
        ...
        for (int m = 1;; m = 0)
        {
          ...
          if (!paramIntent.contains(Long.valueOf(l1)))
          {
            ...
            if (m != 0)
            {
              GaInviteLockActivity.a(this.a).add(new lnv(((submsgtype0x116.MemberInfo)localObject).uint64_member_uin.get(), ((submsgtype0x116.MemberInfo)localObject).uint32_invite_timestamp.get()));
              i = 1;
              k = i;
              if (QLog.isColorLevel())
              {
      ...
      while (i < paramContext.rpt_msg_member_quit.size())
      {
        ...
        do
        {
          ...
          localObject = (lnv)paramIntent.next();
        } while (((lnv)localObject).jdField_a_of_type_Long != l1);
        GaInviteLockActivity.a(this.a).remove(localObject);
        if (QLog.isColorLevel()) {
          QLog.d(this.a.jdField_b_of_type_JavaLangString, 2, "mMemberChangeEventReceiver remove member UIN:" + ((lnv)localObject).jdField_a_of_type_Long);
        }
        k = j;
        if (n != 2) {
          k = 1;
