    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if ((localObject != null) && (((JSONArray)localObject).length() > 0))
        {
          ...
          {
            obh localobh = new obh(((JSONArray)localObject).getJSONObject(i1));
            this.jdField_a_of_type_JavaUtilList.add(localobh);
            if (!QLog.isColorLevel()) {
              break label608;
            }
            QLog.d("AdvertisementExtInfo", 2, "adPKImageData = " + localobh.toString());
            break label608;
          }
        }
