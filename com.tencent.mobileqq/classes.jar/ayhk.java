  ...
  public static ayhl a(YoutuOcr.YoutuOcrRsp paramYoutuOcrRsp)
  {
    ...
    if (paramYoutuOcrRsp.youtu_ocr_errorcode.has())
    {
      ...
      if (paramYoutuOcrRsp.youtu_orc_detail.has())
      {
        ...
        label93:
        localayhl.jdField_b_of_type_Int = i;
        if (!((YoutuOcr.YoutuOcrDetail)localObject).errormsg.has()) {
          break label226;
        }
        paramYoutuOcrRsp = ((YoutuOcr.YoutuOcrDetail)localObject).errormsg.get();
        label116:
        localayhl.jdField_b_of_type_JavaLangString = paramYoutuOcrRsp;
        if (((YoutuOcr.YoutuOcrDetail)localObject).language.has()) {
          localayhl.e = ((YoutuOcr.YoutuOcrDetail)localObject).language.get();
        }
        if (((YoutuOcr.YoutuOcrDetail)localObject).ocr_language_list.has())
        {
          localayhl.jdField_a_of_type_JavaUtilArrayList = new ArrayList();
          localayhl.jdField_a_of_type_JavaUtilArrayList.addAll(((YoutuOcr.YoutuOcrDetail)localObject).ocr_language_list.get());
        }
    ...
    for (paramYoutuOcrRsp = ((YoutuOcr.YoutuOcrDetail)localObject).ocr_item.get();; paramYoutuOcrRsp = null)
    {
      if (paramYoutuOcrRsp != null) {
        localayhl.d = a(paramYoutuOcrRsp);
      }
      return localayhl;
      i = 0;
      break;
