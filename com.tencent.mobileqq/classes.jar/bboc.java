...
public class bboc
{
  ...
  public bboc(AppInterface paramAppInterface)
  {
  ...
  private boolean h(ToServiceMsg paramToServiceMsg, UniPacket paramUniPacket)
  {
    ...
    String str = TextUtils.emoticonToText(paramToServiceMsg);
    try
    {
      byte[] arrayOfByte = str.getBytes("utf-8");
      if (arrayOfByte.length > 120) {}
      for (localModifyGroupInfoReq.strFingerMemo = npm.a(arrayOfByte, 120);; localModifyGroupInfoReq.strFingerMemo = str)
      {
        localModifyGroupInfoReq.strRichFingerMemo = HttpUtil.escape(paramToServiceMsg);
