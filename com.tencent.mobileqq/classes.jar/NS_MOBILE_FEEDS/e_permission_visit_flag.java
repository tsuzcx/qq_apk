...
{
  private static e_permission_visit_flag[] __values = new e_permission_visit_flag[9];
  ...
  public static e_permission_visit_flag convert(int paramInt)
  {
    ...
    for (;;)
    {
      e_permission_visit_flag[] arrayOfe_permission_visit_flag = __values;
      if (i >= arrayOfe_permission_visit_flag.length) {
        break;
      }
      if (arrayOfe_permission_visit_flag[i].value() == paramInt) {
        return __values[i];
      }
      i += 1;
    }
  ...
  public static e_permission_visit_flag convert(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (arrayOfe_permission_visit_flag[i].toString().equals(paramString)) {
        return __values[i];
      }
      i += 1;
    }
