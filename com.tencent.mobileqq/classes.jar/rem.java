...
public class rem
{
  ...
  public static rem a(feeds_info.AccountProfile paramAccountProfile)
  {
    Object localObject;
    ...
    do
    {
      return localObject;
      localrem = new rem();
      localrem.jdField_a_of_type_Long = paramAccountProfile.uint64_uin.get();
      localrem.jdField_a_of_type_Int = paramAccountProfile.uint32_account_type.get();
      if (paramAccountProfile.bytes_desc.has()) {
        localrem.c = paramAccountProfile.bytes_desc.get().toStringUtf8();
      }
      if (paramAccountProfile.bytes_profile_photo_url.has()) {
        localrem.b = paramAccountProfile.bytes_profile_photo_url.get().toStringUtf8();
      }
      if (paramAccountProfile.bytes_nick.has()) {
        localrem.jdField_a_of_type_JavaLangString = paramAccountProfile.bytes_nick.get().toStringUtf8();
      }
      localObject = localrem;
    } while (!paramAccountProfile.bytes_home_page_url.has());
    localrem.d = paramAccountProfile.bytes_home_page_url.get().toStringUtf8();
    return localrem;
  }
