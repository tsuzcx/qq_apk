...
public class xat
  implements woy<xcg<qqstory_service.ReqBatchStoryPollData>, xee>
{
  ...
  static
  {
    jdField_a_of_type_JavaLangString = wnu.a("StorySvc.batch_poll_data");
  }
  
  public xat(List<String> paramList)
  {
  ...
  public static void a(@NonNull List<String> paramList)
  {
    ...
    while (i < j)
    {
      new xat(paramList.subList(i, Math.min(i + 5, j))).a();
      i += 5;
    }
  }
  ...
  public void a(@NonNull xcg<qqstory_service.ReqBatchStoryPollData> paramxcg, @Nullable xee paramxee, @NonNull ErrorMessage paramErrorMessage)
  {
    if ((paramErrorMessage.isFail()) || (paramxee == null))
    {
      yuk.c("Q.qqstory.pollData.GetVidPollInfoHandler", "Error onCmdRespond:%s", paramErrorMessage);
      return;
    }
    paramxcg = new qqstory_service.RspBatchStoryPollData();
    Object localObject;
    ...
    for (;;)
    {
      try
      {
        ...
        paramxee = (wte)wth.a(5);
        paramErrorMessage = new xau();
        i = 0;
        if (i >= paramxcg.poll_info_list.size()) {
          break;
        }
        localObject = (qqstory_struct.VidPollInfo)paramxcg.poll_info_list.get(i);
        localStoryVideoItem = paramxee.a(((qqstory_struct.VidPollInfo)localObject).vid.get().toStringUtf8());
      ...
      {
        yuk.c("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond Request parse Error!", paramxcg);
        return;
      }
      ...
      paramxee.a(localStoryVideoItem);
      label271:
      paramErrorMessage.a.add(localObject);
      i += 1;
    }
    int i = 0;
    while (i < paramxcg.rate_info_list.size())
    {
      localObject = (qqstory_struct.VidRateInfo)paramxcg.rate_info_list.get(i);
      yuk.b("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond, VidRateInfo:[vid=%s, selfRateResult=%s, totalRateCount=%s, totalRateScore=%s]", ((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8(), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).self_rate_result.get()), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_count.get()), Long.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_score.get()));
      localStoryVideoItem = paramxee.a(((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8());
      if (localStoryVideoItem != null)
      {
