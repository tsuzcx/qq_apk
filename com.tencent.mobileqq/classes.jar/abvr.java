...
public class abvr
{
  public static abvq a(Context paramContext, String paramString)
  {
    ...
    for (;;)
    {
      try
      {
        if (TextUtils.isEmpty(paramContext))
        {
          ...
          if (i < paramContext.length())
          {
            ...
            paramString.add(new abvq(localJSONObject));
            break label130;
          }
        }
        else
      ...
      catch (JSONException paramContext)
      {
        ...
        if (paramString.size() > 1) {
          Collections.sort(paramString, new abvs());
        }
        if (paramString.size() > 0) {
          return (abvq)paramString.get(0);
        }
        return null;
      }
      label125:
    ...
    for (;;)
    {
      try
      {
        ...
        if (TextUtils.isEmpty((CharSequence)localObject))
        {
          ...
          ((JSONArray)localObject).put(new JSONObject(paramabvq.e()));
      ...
      {
        localObject = new abvq(((JSONArray)localObject).getJSONObject(0));
        if (localObject != null)
        {
          int j = ((abvq)localObject).a();
          int k = paramabvq.a();
          if (j >= k) {}
        }
        else
    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if (TextUtils.isEmpty(paramString))
        {
          ...
          localObject3 = new abvq((JSONObject)localObject2).a();
