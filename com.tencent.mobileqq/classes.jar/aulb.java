import android.content.ContentValues;
import android.database.Cursor;
import com.tencent.mobileqq.data.PhoneContact;
import com.tencent.mobileqq.persistence.NoColumnError;

public class aulb
  extends auky
{
  public aulb()
  {
    this.a = 38;
  }
  
  public aukm a(aukm paramaukm, Cursor paramCursor, boolean paramBoolean, aukx paramaukx)
  {
    boolean bool2 = true;
    boolean bool1 = true;
    paramaukm = (PhoneContact)paramaukm;
    if (paramaukx == null)
    {
      paramaukm.mobileNo = paramCursor.getString(paramCursor.getColumnIndex("mobileNo"));
      paramaukm.uin = paramCursor.getString(paramCursor.getColumnIndex("uin"));
      paramaukm.name = paramCursor.getString(paramCursor.getColumnIndex("name"));
      paramaukm.bindingDate = paramCursor.getLong(paramCursor.getColumnIndex("bindingDate"));
      paramaukm.isRecommend = paramCursor.getInt(paramCursor.getColumnIndex("isRecommend"));
      paramaukm.nationCode = paramCursor.getString(paramCursor.getColumnIndex("nationCode"));
      paramaukm.mobileCode = paramCursor.getString(paramCursor.getColumnIndex("mobileCode"));
      paramaukm.unifiedCode = paramCursor.getString(paramCursor.getColumnIndex("unifiedCode"));
      paramaukm.nickName = paramCursor.getString(paramCursor.getColumnIndex("nickName"));
      paramaukm.originBinder = paramCursor.getLong(paramCursor.getColumnIndex("originBinder"));
      paramaukm.ability = paramCursor.getInt(paramCursor.getColumnIndex("ability"));
      paramaukm.contactID = paramCursor.getInt(paramCursor.getColumnIndex("contactID"));
      paramaukm.faceUrl = paramCursor.getString(paramCursor.getColumnIndex("faceUrl"));
      paramaukm.remark = paramCursor.getString(paramCursor.getColumnIndex("remark"));
      paramaukm.pinyinAll = paramCursor.getString(paramCursor.getColumnIndex("pinyinAll"));
      paramaukm.pinyinInitial = paramCursor.getString(paramCursor.getColumnIndex("pinyinInitial"));
      paramaukm.lastScanTime = paramCursor.getLong(paramCursor.getColumnIndex("lastScanTime"));
      if (1 == paramCursor.getShort(paramCursor.getColumnIndex("isUploaded")))
      {
        paramBoolean = true;
        paramaukm.isUploaded = paramBoolean;
        paramaukm.sortWeight = paramCursor.getInt(paramCursor.getColumnIndex("sortWeight"));
        if (1 != paramCursor.getShort(paramCursor.getColumnIndex("hasSendAddReq"))) {
          break label737;
        }
        paramBoolean = true;
        label385:
        paramaukm.hasSendAddReq = paramBoolean;
        if (1 != paramCursor.getShort(paramCursor.getColumnIndex("isNewRecommend"))) {
          break label742;
        }
        paramBoolean = true;
        label410:
        paramaukm.isNewRecommend = paramBoolean;
        paramaukm.highLightTimeStamp = paramCursor.getLong(paramCursor.getColumnIndex("highLightTimeStamp"));
        if (1 != paramCursor.getShort(paramCursor.getColumnIndex("isHiden"))) {
          break label747;
        }
      }
      label737:
      label742:
      label747:
      for (paramBoolean = bool1;; paramBoolean = false)
      {
        paramaukm.isHiden = paramBoolean;
        paramaukm.md5 = paramCursor.getString(paramCursor.getColumnIndex("md5"));
        paramaukm.type = paramCursor.getInt(paramCursor.getColumnIndex("type"));
        paramaukm.label = paramCursor.getString(paramCursor.getColumnIndex("label"));
        paramaukm.detalStatusFlag = ((byte)paramCursor.getShort(paramCursor.getColumnIndex("detalStatusFlag")));
        paramaukm.iTermType = paramCursor.getInt(paramCursor.getColumnIndex("iTermType"));
        paramaukm.eNetworkType = paramCursor.getInt(paramCursor.getColumnIndex("eNetworkType"));
        paramaukm.richTime = paramCursor.getLong(paramCursor.getColumnIndex("richTime"));
        paramaukm.richBuffer = paramCursor.getBlob(paramCursor.getColumnIndex("richBuffer"));
        paramaukm.sex = paramCursor.getInt(paramCursor.getColumnIndex("sex"));
        paramaukm.age = paramCursor.getInt(paramCursor.getColumnIndex("age"));
        paramaukm.samFriend = paramCursor.getInt(paramCursor.getColumnIndex("samFriend"));
        paramaukm.abilityBits = paramCursor.getLong(paramCursor.getColumnIndex("abilityBits"));
        paramaukm.strTermDesc = paramCursor.getString(paramCursor.getColumnIndex("strTermDesc"));
        paramaukm.netTypeIconIdIphoneOrWphoneNoWifi = paramCursor.getInt(paramCursor.getColumnIndex("netTypeIconIdIphoneOrWphoneNoWifi"));
        paramaukm.netTypeIconId = paramCursor.getInt(paramCursor.getColumnIndex("netTypeIconId"));
        return paramaukm;
        paramBoolean = false;
        break;
        paramBoolean = false;
        break label385;
        paramBoolean = false;
        break label410;
      }
    }
    int i = paramCursor.getColumnIndex("mobileNo");
    if (i == -1)
    {
      paramaukx.a(new NoColumnError("mobileNo", String.class));
      i = paramCursor.getColumnIndex("uin");
      if (i != -1) {
        break label2084;
      }
      paramaukx.a(new NoColumnError("uin", String.class));
      label820:
      i = paramCursor.getColumnIndex("name");
      if (i != -1) {
        break label2099;
      }
      paramaukx.a(new NoColumnError("name", String.class));
      label854:
      i = paramCursor.getColumnIndex("bindingDate");
      if (i != -1) {
        break label2114;
      }
      paramaukx.a(new NoColumnError("bindingDate", Long.TYPE));
      label889:
      i = paramCursor.getColumnIndex("isRecommend");
      if (i != -1) {
        break label2129;
      }
      paramaukx.a(new NoColumnError("isRecommend", Integer.TYPE));
      label924:
      i = paramCursor.getColumnIndex("nationCode");
      if (i != -1) {
        break label2144;
      }
      paramaukx.a(new NoColumnError("nationCode", String.class));
      label958:
      i = paramCursor.getColumnIndex("mobileCode");
      if (i != -1) {
        break label2159;
      }
      paramaukx.a(new NoColumnError("mobileCode", String.class));
      label992:
      i = paramCursor.getColumnIndex("unifiedCode");
      if (i != -1) {
        break label2174;
      }
      paramaukx.a(new NoColumnError("unifiedCode", String.class));
      label1026:
      i = paramCursor.getColumnIndex("nickName");
      if (i != -1) {
        break label2189;
      }
      paramaukx.a(new NoColumnError("nickName", String.class));
      label1060:
      i = paramCursor.getColumnIndex("originBinder");
      if (i != -1) {
        break label2204;
      }
      paramaukx.a(new NoColumnError("originBinder", Long.TYPE));
      label1095:
      i = paramCursor.getColumnIndex("ability");
      if (i != -1) {
        break label2219;
      }
      paramaukx.a(new NoColumnError("ability", Integer.TYPE));
      label1130:
      i = paramCursor.getColumnIndex("contactID");
      if (i != -1) {
        break label2234;
      }
      paramaukx.a(new NoColumnError("contactID", Integer.TYPE));
      label1165:
      i = paramCursor.getColumnIndex("faceUrl");
      if (i != -1) {
        break label2249;
      }
      paramaukx.a(new NoColumnError("faceUrl", String.class));
      label1199:
      i = paramCursor.getColumnIndex("remark");
      if (i != -1) {
        break label2264;
      }
      paramaukx.a(new NoColumnError("remark", String.class));
      label1233:
      i = paramCursor.getColumnIndex("pinyinAll");
      if (i != -1) {
        break label2279;
      }
      paramaukx.a(new NoColumnError("pinyinAll", String.class));
      label1267:
      i = paramCursor.getColumnIndex("pinyinInitial");
      if (i != -1) {
        break label2294;
      }
      paramaukx.a(new NoColumnError("pinyinInitial", String.class));
      label1301:
      i = paramCursor.getColumnIndex("lastScanTime");
      if (i != -1) {
        break label2309;
      }
      paramaukx.a(new NoColumnError("lastScanTime", Long.TYPE));
      label1336:
      i = paramCursor.getColumnIndex("isUploaded");
      if (i != -1) {
        break label2324;
      }
      paramaukx.a(new NoColumnError("isUploaded", Boolean.TYPE));
      i = paramCursor.getColumnIndex("sortWeight");
      if (i != -1) {
        break label2351;
      }
      paramaukx.a(new NoColumnError("sortWeight", Integer.TYPE));
      label1406:
      i = paramCursor.getColumnIndex("hasSendAddReq");
      if (i != -1) {
        break label2366;
      }
      paramaukx.a(new NoColumnError("hasSendAddReq", Boolean.TYPE));
      i = paramCursor.getColumnIndex("isNewRecommend");
      if (i != -1) {
        break label2393;
      }
      paramaukx.a(new NoColumnError("isNewRecommend", Boolean.TYPE));
      i = paramCursor.getColumnIndex("highLightTimeStamp");
      if (i != -1) {
        break label2420;
      }
      paramaukx.a(new NoColumnError("highLightTimeStamp", Long.TYPE));
      label1511:
      i = paramCursor.getColumnIndex("isHiden");
      if (i != -1) {
        break label2435;
      }
      paramaukx.a(new NoColumnError("isHiden", Boolean.TYPE));
      i = paramCursor.getColumnIndex("md5");
      if (i != -1) {
        break label2463;
      }
      paramaukx.a(new NoColumnError("md5", String.class));
      label1580:
      i = paramCursor.getColumnIndex("type");
      if (i != -1) {
        break label2478;
      }
      paramaukx.a(new NoColumnError("type", Integer.TYPE));
      label1615:
      i = paramCursor.getColumnIndex("label");
      if (i != -1) {
        break label2493;
      }
      paramaukx.a(new NoColumnError("label", String.class));
      label1649:
      i = paramCursor.getColumnIndex("detalStatusFlag");
      if (i != -1) {
        break label2508;
      }
      paramaukx.a(new NoColumnError("detalStatusFlag", Byte.TYPE));
      label1684:
      i = paramCursor.getColumnIndex("iTermType");
      if (i != -1) {
        break label2524;
      }
      paramaukx.a(new NoColumnError("iTermType", Integer.TYPE));
      label1719:
      i = paramCursor.getColumnIndex("eNetworkType");
      if (i != -1) {
        break label2539;
      }
      paramaukx.a(new NoColumnError("eNetworkType", Integer.TYPE));
      label1754:
      i = paramCursor.getColumnIndex("richTime");
      if (i != -1) {
        break label2554;
      }
      paramaukx.a(new NoColumnError("richTime", Long.TYPE));
      label1789:
      i = paramCursor.getColumnIndex("richBuffer");
      if (i != -1) {
        break label2569;
      }
      paramaukx.a(new NoColumnError("richBuffer", [B.class));
      label1823:
      i = paramCursor.getColumnIndex("sex");
      if (i != -1) {
        break label2584;
      }
      paramaukx.a(new NoColumnError("sex", Integer.TYPE));
      label1858:
      i = paramCursor.getColumnIndex("age");
      if (i != -1) {
        break label2599;
      }
      paramaukx.a(new NoColumnError("age", Integer.TYPE));
      label1893:
      i = paramCursor.getColumnIndex("samFriend");
      if (i != -1) {
        break label2614;
      }
      paramaukx.a(new NoColumnError("samFriend", Integer.TYPE));
      label1928:
      i = paramCursor.getColumnIndex("abilityBits");
      if (i != -1) {
        break label2629;
      }
      paramaukx.a(new NoColumnError("abilityBits", Long.TYPE));
      label1963:
      i = paramCursor.getColumnIndex("strTermDesc");
      if (i != -1) {
        break label2644;
      }
      paramaukx.a(new NoColumnError("strTermDesc", String.class));
      label1997:
      i = paramCursor.getColumnIndex("netTypeIconIdIphoneOrWphoneNoWifi");
      if (i != -1) {
        break label2659;
      }
      paramaukx.a(new NoColumnError("netTypeIconIdIphoneOrWphoneNoWifi", Integer.TYPE));
    }
    for (;;)
    {
      i = paramCursor.getColumnIndex("netTypeIconId");
      if (i != -1) {
        break label2674;
      }
      paramaukx.a(new NoColumnError("netTypeIconId", Integer.TYPE));
      return paramaukm;
      paramaukm.mobileNo = paramCursor.getString(i);
      break;
      label2084:
      paramaukm.uin = paramCursor.getString(i);
      break label820;
      label2099:
      paramaukm.name = paramCursor.getString(i);
      break label854;
      label2114:
      paramaukm.bindingDate = paramCursor.getLong(i);
      break label889;
      label2129:
      paramaukm.isRecommend = paramCursor.getInt(i);
      break label924;
      label2144:
      paramaukm.nationCode = paramCursor.getString(i);
      break label958;
      label2159:
      paramaukm.mobileCode = paramCursor.getString(i);
      break label992;
      label2174:
      paramaukm.unifiedCode = paramCursor.getString(i);
      break label1026;
      label2189:
      paramaukm.nickName = paramCursor.getString(i);
      break label1060;
      label2204:
      paramaukm.originBinder = paramCursor.getLong(i);
      break label1095;
      label2219:
      paramaukm.ability = paramCursor.getInt(i);
      break label1130;
      label2234:
      paramaukm.contactID = paramCursor.getInt(i);
      break label1165;
      label2249:
      paramaukm.faceUrl = paramCursor.getString(i);
      break label1199;
      label2264:
      paramaukm.remark = paramCursor.getString(i);
      break label1233;
      label2279:
      paramaukm.pinyinAll = paramCursor.getString(i);
      break label1267;
      label2294:
      paramaukm.pinyinInitial = paramCursor.getString(i);
      break label1301;
      label2309:
      paramaukm.lastScanTime = paramCursor.getLong(i);
      break label1336;
      label2324:
      if (1 == paramCursor.getShort(i)) {}
      for (paramBoolean = true;; paramBoolean = false)
      {
        paramaukm.isUploaded = paramBoolean;
        break;
      }
      label2351:
      paramaukm.sortWeight = paramCursor.getInt(i);
      break label1406;
      label2366:
      if (1 == paramCursor.getShort(i)) {}
      for (paramBoolean = true;; paramBoolean = false)
      {
        paramaukm.hasSendAddReq = paramBoolean;
        break;
      }
      label2393:
      if (1 == paramCursor.getShort(i)) {}
      for (paramBoolean = true;; paramBoolean = false)
      {
        paramaukm.isNewRecommend = paramBoolean;
        break;
      }
      label2420:
      paramaukm.highLightTimeStamp = paramCursor.getLong(i);
      break label1511;
      label2435:
      if (1 == paramCursor.getShort(i)) {}
      for (paramBoolean = bool2;; paramBoolean = false)
      {
        paramaukm.isHiden = paramBoolean;
        break;
      }
      label2463:
      paramaukm.md5 = paramCursor.getString(i);
      break label1580;
      label2478:
      paramaukm.type = paramCursor.getInt(i);
      break label1615;
      label2493:
      paramaukm.label = paramCursor.getString(i);
      break label1649;
      label2508:
      paramaukm.detalStatusFlag = ((byte)paramCursor.getShort(i));
      break label1684;
      label2524:
      paramaukm.iTermType = paramCursor.getInt(i);
      break label1719;
      label2539:
      paramaukm.eNetworkType = paramCursor.getInt(i);
      break label1754;
      label2554:
      paramaukm.richTime = paramCursor.getLong(i);
      break label1789;
      label2569:
      paramaukm.richBuffer = paramCursor.getBlob(i);
      break label1823;
      label2584:
      paramaukm.sex = paramCursor.getInt(i);
      break label1858;
      label2599:
      paramaukm.age = paramCursor.getInt(i);
      break label1893;
      label2614:
      paramaukm.samFriend = paramCursor.getInt(i);
      break label1928;
      label2629:
      paramaukm.abilityBits = paramCursor.getLong(i);
      break label1963;
      label2644:
      paramaukm.strTermDesc = paramCursor.getString(i);
      break label1997;
      label2659:
      paramaukm.netTypeIconIdIphoneOrWphoneNoWifi = paramCursor.getInt(i);
    }
    label2674:
    paramaukm.netTypeIconId = paramCursor.getInt(i);
    return paramaukm;
  }
  
  public String a(String paramString)
  {
    StringBuilder localStringBuilder = new StringBuilder("CREATE TABLE IF NOT EXISTS ");
    localStringBuilder.append(paramString);
    localStringBuilder.append(" (_id INTEGER PRIMARY KEY AUTOINCREMENT ,mobileNo TEXT UNIQUE ,uin TEXT ,name TEXT ,bindingDate INTEGER ,isRecommend INTEGER ,nationCode TEXT ,mobileCode TEXT ,unifiedCode TEXT ,nickName TEXT ,originBinder INTEGER ,ability INTEGER ,contactID INTEGER ,faceUrl TEXT ,remark TEXT ,pinyinAll TEXT ,pinyinInitial TEXT ,lastScanTime INTEGER ,isUploaded INTEGER ,sortWeight INTEGER ,hasSendAddReq INTEGER ,isNewRecommend INTEGER ,highLightTimeStamp INTEGER ,isHiden INTEGER ,md5 TEXT ,type INTEGER ,label TEXT ,detalStatusFlag INTEGER ,iTermType INTEGER ,eNetworkType INTEGER ,richTime INTEGER ,richBuffer BLOB ,sex INTEGER ,age INTEGER ,samFriend INTEGER ,abilityBits INTEGER ,strTermDesc TEXT ,netTypeIconIdIphoneOrWphoneNoWifi INTEGER ,netTypeIconId INTEGER)");
    return localStringBuilder.toString();
  }
  
  public void a(aukm paramaukm, ContentValues paramContentValues)
  {
    paramaukm = (PhoneContact)paramaukm;
    paramContentValues.put("mobileNo", paramaukm.mobileNo);
    paramContentValues.put("uin", paramaukm.uin);
    paramContentValues.put("name", paramaukm.name);
    paramContentValues.put("bindingDate", Long.valueOf(paramaukm.bindingDate));
    paramContentValues.put("isRecommend", Integer.valueOf(paramaukm.isRecommend));
    paramContentValues.put("nationCode", paramaukm.nationCode);
    paramContentValues.put("mobileCode", paramaukm.mobileCode);
    paramContentValues.put("unifiedCode", paramaukm.unifiedCode);
    paramContentValues.put("nickName", paramaukm.nickName);
    paramContentValues.put("originBinder", Long.valueOf(paramaukm.originBinder));
    paramContentValues.put("ability", Integer.valueOf(paramaukm.ability));
    paramContentValues.put("contactID", Integer.valueOf(paramaukm.contactID));
    paramContentValues.put("faceUrl", paramaukm.faceUrl);
    paramContentValues.put("remark", paramaukm.remark);
    paramContentValues.put("pinyinAll", paramaukm.pinyinAll);
    paramContentValues.put("pinyinInitial", paramaukm.pinyinInitial);
    paramContentValues.put("lastScanTime", Long.valueOf(paramaukm.lastScanTime));
    paramContentValues.put("isUploaded", Boolean.valueOf(paramaukm.isUploaded));
    paramContentValues.put("sortWeight", Integer.valueOf(paramaukm.sortWeight));
    paramContentValues.put("hasSendAddReq", Boolean.valueOf(paramaukm.hasSendAddReq));
    paramContentValues.put("isNewRecommend", Boolean.valueOf(paramaukm.isNewRecommend));
    paramContentValues.put("highLightTimeStamp", Long.valueOf(paramaukm.highLightTimeStamp));
    paramContentValues.put("isHiden", Boolean.valueOf(paramaukm.isHiden));
    paramContentValues.put("md5", paramaukm.md5);
    paramContentValues.put("type", Integer.valueOf(paramaukm.type));
    paramContentValues.put("label", paramaukm.label);
    paramContentValues.put("detalStatusFlag", Byte.valueOf(paramaukm.detalStatusFlag));
    paramContentValues.put("iTermType", Integer.valueOf(paramaukm.iTermType));
    paramContentValues.put("eNetworkType", Integer.valueOf(paramaukm.eNetworkType));
    paramContentValues.put("richTime", Long.valueOf(paramaukm.richTime));
    paramContentValues.put("richBuffer", paramaukm.richBuffer);
    paramContentValues.put("sex", Integer.valueOf(paramaukm.sex));
    paramContentValues.put("age", Integer.valueOf(paramaukm.age));
    paramContentValues.put("samFriend", Integer.valueOf(paramaukm.samFriend));
    paramContentValues.put("abilityBits", Long.valueOf(paramaukm.abilityBits));
    paramContentValues.put("strTermDesc", paramaukm.strTermDesc);
    paramContentValues.put("netTypeIconIdIphoneOrWphoneNoWifi", Integer.valueOf(paramaukm.netTypeIconIdIphoneOrWphoneNoWifi));
    paramContentValues.put("netTypeIconId", Integer.valueOf(paramaukm.netTypeIconId));
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes.jar
 * Qualified Name:     aulb
 * JD-Core Version:    0.7.0.1
 */