    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if ((localObject != null) && (((JSONArray)localObject).length() > 0))
        {
          ...
          {
            omm localomm = new omm(((JSONArray)localObject).getJSONObject(i1));
            this.jdField_a_of_type_JavaUtilList.add(localomm);
            if (!QLog.isColorLevel()) {
              break label608;
            }
            QLog.d("AdvertisementExtInfo", 2, "adPKImageData = " + localomm.toString());
            break label608;
          }
        }
