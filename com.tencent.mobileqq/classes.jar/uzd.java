...
public class uzd
  implements uni<vaq<qqstory_service.ReqBatchStoryPollData>, vco>
{
  ...
  static
  {
    jdField_a_of_type_JavaLangString = ume.a("StorySvc.batch_poll_data");
  }
  
  public uzd(List<String> paramList)
  {
  ...
  public static void a(@NonNull List<String> paramList)
  {
    ...
    while (i < j)
    {
      new uzd(paramList.subList(i, Math.min(i + 5, j))).a();
      i += 5;
    }
  }
  ...
  public void a(@NonNull vaq<qqstory_service.ReqBatchStoryPollData> paramvaq, @Nullable vco paramvco, @NonNull ErrorMessage paramErrorMessage)
  {
    if ((paramErrorMessage.isFail()) || (paramvco == null))
    {
      wsv.c("Q.qqstory.pollData.GetVidPollInfoHandler", "Error onCmdRespond:%s", paramErrorMessage);
      return;
    }
    paramvaq = new qqstory_service.RspBatchStoryPollData();
    Object localObject;
    ...
    for (;;)
    {
      try
      {
        ...
        paramvco = (uro)urr.a(5);
        paramErrorMessage = new uze();
        i = 0;
        if (i >= paramvaq.poll_info_list.size()) {
          break;
        }
        localObject = (qqstory_struct.VidPollInfo)paramvaq.poll_info_list.get(i);
        localStoryVideoItem = paramvco.a(((qqstory_struct.VidPollInfo)localObject).vid.get().toStringUtf8());
      ...
      {
        wsv.c("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond Request parse Error!", paramvaq);
        return;
      }
      ...
      paramvco.a(localStoryVideoItem);
      label271:
      paramErrorMessage.a.add(localObject);
      i += 1;
    }
    int i = 0;
    while (i < paramvaq.rate_info_list.size())
    {
      localObject = (qqstory_struct.VidRateInfo)paramvaq.rate_info_list.get(i);
      wsv.b("Q.qqstory.pollData.GetVidPollInfoHandler", "onCmdRespond, VidRateInfo:[vid=%s, selfRateResult=%s, totalRateCount=%s, totalRateScore=%s]", ((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8(), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).self_rate_result.get()), Integer.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_count.get()), Long.valueOf(((qqstory_struct.VidRateInfo)localObject).total_rate_score.get()));
      localStoryVideoItem = paramvco.a(((qqstory_struct.VidRateInfo)localObject).vid.get().toStringUtf8());
      if (localStoryVideoItem != null)
      {
