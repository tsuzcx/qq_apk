...
{
  public mhe(MultiIncomingCallsActivity paramMultiIncomingCallsActivity) {}
      ...
      if (j < paramContext.rpt_msg_member_join.size())
      {
        ...
        for (int m = 1;; m = 0)
        {
          ...
          if (!paramIntent.contains(Long.valueOf(l1)))
          {
            ...
            if (m != 0)
            {
              MultiIncomingCallsActivity.a(this.a).add(new lqk(((submsgtype0x116.MemberInfo)localObject).uint64_member_uin.get(), ((submsgtype0x116.MemberInfo)localObject).uint32_invite_timestamp.get()));
              i = 1;
              k = i;
              if (QLog.isColorLevel())
              {
