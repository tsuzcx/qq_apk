...
public class akiy
  extends ajfb
{
  public akiy(AppInterface paramAppInterface)
  {
    super(paramAppInterface);
  }
  
  private static akrl a(oidb_cmd0xb49.LBSPoint paramLBSPoint)
  {
    int j = 0;
    akrl localakrl = new akrl();
    int i;
    if (paramLBSPoint.int32_lat.has())
    {
      i = paramLBSPoint.int32_lat.get();
      localakrl.jdField_a_of_type_Int = i;
      if (!paramLBSPoint.int32_lon.has()) {
        break label141;
      }
      i = paramLBSPoint.int32_lon.get();
      label53:
      localakrl.jdField_b_of_type_Int = i;
      if (!paramLBSPoint.str_name.has()) {
        break label146;
      }
      str = paramLBSPoint.str_name.get();
      label77:
      localakrl.jdField_a_of_type_JavaLangString = str;
    ...
    {
      localakrl.jdField_b_of_type_JavaLangString = str;
      i = j;
    ...
    if (paramFromServiceMsg.isSuccess())
    {
      ...
      if (paramToServiceMsg.msg_ar_classify_rsp.has())
      {
        ...
        if (paramFromServiceMsg.uint32_lbs_result.has())
        {
          ...
          localakrk.jdField_a_of_type_Int = i;
          localakrk.jdField_a_of_type_JavaLangString = paramToServiceMsg;
          if (!paramFromServiceMsg.msg_nearest_point.has()) {
            break label291;
          }
          paramToServiceMsg = a(paramFromServiceMsg.msg_nearest_point);
          label241:
          localakrk.jdField_a_of_type_Akrl = paramToServiceMsg;
        }
      }
      else
      {
        QLog.i("ARLBSHandler", 1, "onReceiveLBSLocation. resp is success. retCode = " + localakrk.jdField_a_of_type_Int);
      }
    }
    for (;;)
    {
      notifyUI(1, true, localakrk);
      return;
      i = -1;
      break;
      label291:
