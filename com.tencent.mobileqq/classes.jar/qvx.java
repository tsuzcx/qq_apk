...
public class qvx
{
  public ArrayList<qvy> a;
  articlesummary.AggregatedList a;
  
  public static qvx a(articlesummary.AggregatedList paramAggregatedList)
  {
    if ((paramAggregatedList != null) && (paramAggregatedList.rpt_article_summary.has()))
    {
      qvx localqvx = new qvx();
      Object localObject = paramAggregatedList.rpt_article_summary.get();
      localqvx.jdField_a_of_type_JavaUtilArrayList = new ArrayList(((List)localObject).size());
      localqvx.jdField_a_of_type_TencentImOidbArticlesummaryArticlesummary$AggregatedList = paramAggregatedList;
      ...
      while (paramAggregatedList.hasNext())
      {
        ...
        qvy localqvy = new qvy();
        localqvy.jdField_a_of_type_Long = ((articlesummary.ArticleSummary)localObject).msg_feeds_info.msg_social_feeds_info.uint64_feeds_id.get();
        localqvy.jdField_b_of_type_Long = ((articlesummary.ArticleSummary)localObject).uint64_algorithm_id.get();
        if (localFeedsInfo.feeds_type.has()) {
          localqvy.jdField_a_of_type_Int = localFeedsInfo.feeds_type.get();
        }
        if (((articlesummary.ArticleSummary)localObject).uint32_strategy_id.has()) {
          localqvy.jdField_b_of_type_Int = ((articlesummary.ArticleSummary)localObject).uint32_strategy_id.get();
        }
        if (((articlesummary.ArticleSummary)localObject).uint64_recommend_seq.has()) {
          localqvy.c = ((articlesummary.ArticleSummary)localObject).uint64_recommend_seq.get();
        }
        localqvx.jdField_a_of_type_JavaUtilArrayList.add(localqvy);
      }
      return localqvx;
    }
    return null;
