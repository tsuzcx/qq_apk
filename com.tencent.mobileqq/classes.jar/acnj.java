  ...
  implements AdIPCManager.Adapter
{
  ...
  {
    acvc.b("GdtIPCAdapter", "init");
    QIPCClientHelper.getInstance().register(acnp.a());
  }
  ...
  public boolean register(String paramString, AdIPCManager.Handler paramHandler)
  {
    ...
    for (;;)
    {
      acvc.b("GdtIPCAdapter", String.format("register action:%s result:%b", new Object[] { paramString, Boolean.valueOf(bool) }));
      return bool;
  ...
  public AdIPCManager.Result send(Context paramContext, AdIPCManager.Params paramParams)
  {
    ...
    for (String str = paramParams.getToProcessName();; str = null)
    {
      ...
      acvc.d("GdtIPCAdapter", String.format("send error actionad:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
      return null;
      localObject1 = null;
      break;
    }
    label113:
    acvc.b("GdtIPCAdapter", String.format("send action:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
    ...
    acvc.d("GdtIPCAdapter", String.format("send error action:%s from:%s to:%s", new Object[] { localObject1, paramContext, str }));
    return null;
  }
  ...
  public void send(Context paramContext, AdIPCManager.Params paramParams, WeakReference<AdIPCManager.Callback> paramWeakReference)
  {
    ...
    if (paramParams != null)
    {
      ...
      acvc.d("GdtIPCAdapter", String.format("send error action:%s from:%s to:%s", new Object[] { str1, paramContext, str2 }));
    }
    label97:
    do
    {
      ...
      str1 = null;
      break;
      acvc.b("GdtIPCAdapter", String.format("send action:%s from:%s to:%s", new Object[] { str1, paramContext, str2 }));
    ...
    {
      QIPCClientHelper.getInstance().getClient().callServer("gdt_ipc_async_module_client_to_server", str1, paramParams.bundle, new acnk(this, paramWeakReference, paramParams));
      return;
    }
    QIPCClientHelper.getInstance().getClient().callServer("gdt_ipc_async_module_client_to_server", str1, paramParams.bundle, new acnl(this, paramWeakReference, paramParams));
  }
}
