...
public class ahpg
{
  ...
  private ahpi[] jdField_a_of_type_ArrayOfAhpi = { new agfc(), new ahph(), new benn(), new nzf(), new bfjw() };
  private int b = -1;
    ...
    if (paramInt >= 0)
    {
      ...
      if ((paramListView != null) && (paramInt < paramListView.getCount()))
      {
        ...
        if ((paramListView instanceof MessageRecord))
        {
          ...
          ahpi[] arrayOfahpi = this.jdField_a_of_type_ArrayOfAhpi;
          int i = arrayOfahpi.length;
          paramInt = 0;
          while (paramInt < i)
          {
            arrayOfahpi[paramInt].a(paramQQAppInterface, paramListView);
            paramInt += 1;
          }
        }
      }
