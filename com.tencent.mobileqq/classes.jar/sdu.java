...
public class sdu
{
  ...
  public static Readinjoy.ArticleSummary a(BaseVideoArticleInfo paramBaseVideoArticleInfo)
  {
    ...
    for (;;)
    {
      ...
      catch (Throwable paramBaseVideoArticleInfo)
      {
        ...
        QLog.e("VideoProtoHelper", 0, "videoArticleinfoCovert2LocalPB error:" + pay.a(paramBaseVideoArticleInfo));
      }
  ...
  public static Readinjoy.VideoData a(rgi paramrgi)
  {
    Object localObject;
    if (paramrgi == null) {
      localObject = null;
    ...
    do
    {
      ...
      localVideoData.uint32_busi_type.set(paramrgi.jdField_a_of_type_Int);
      localVideoData.uint32_duration.set(paramrgi.jdField_b_of_type_Int);
      localVideoData.uint32_width.set(paramrgi.c);
      localVideoData.uint32_height.set(paramrgi.d);
      if (!TextUtils.isEmpty(paramrgi.jdField_a_of_type_JavaLangString)) {
        localVideoData.bytes_vid.set(ByteStringMicro.copyFromUtf8(paramrgi.jdField_a_of_type_JavaLangString));
      }
      localVideoData.uint32_file_size.set(paramrgi.e);
      localObject = localVideoData;
    } while (TextUtils.isEmpty(paramrgi.jdField_b_of_type_JavaLangString));
    localVideoData.bytes_video_url.set(ByteStringMicro.copyFromUtf8(paramrgi.jdField_b_of_type_JavaLangString));
    return localVideoData;
  }
  
  public static rgi a(Readinjoy.VideoData paramVideoData)
  {
    ...
    do
    {
      ...
      if (paramVideoData.uint32_busi_type.has()) {
        localrgi.jdField_a_of_type_Int = paramVideoData.uint32_busi_type.get();
      }
      if (paramVideoData.uint32_duration.has()) {
        localrgi.jdField_b_of_type_Int = paramVideoData.uint32_duration.get();
      }
      if (paramVideoData.uint32_width.has()) {
        localrgi.c = paramVideoData.uint32_width.get();
      }
      if (paramVideoData.uint32_height.has()) {
        localrgi.d = paramVideoData.uint32_height.get();
      }
      if (paramVideoData.bytes_vid.has()) {
        localrgi.jdField_a_of_type_JavaLangString = paramVideoData.bytes_vid.get().toStringUtf8();
      }
      if (paramVideoData.uint32_file_size.has()) {
        localrgi.e = paramVideoData.uint32_file_size.get();
      }
      localObject = localrgi;
    } while (!paramVideoData.bytes_video_url.has());
    localrgi.jdField_b_of_type_JavaLangString = paramVideoData.bytes_video_url.get().toStringUtf8();
    return localrgi;
  }
}
