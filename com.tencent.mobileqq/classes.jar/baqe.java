import android.os.Bundle;
import android.os.Handler;
import android.os.Looper;
import android.text.TextUtils;
import com.tencent.common.app.AppInterface;
import com.tencent.mobileqq.app.SignatureManager;
import com.tencent.mobileqq.app.ThreadManager;
import com.tencent.mobileqq.richstatus.RichStatus;
import com.tencent.mobileqq.vas.VasResEngine.VasResDrawable;
import com.tencent.mobileqq.vas.adapter.SignatureAdapter.2;
import com.tencent.mobileqq.vas.adapter.SignatureAdapter.3;
import com.tencent.qphone.base.util.QLog;
import java.util.Observer;
import java.util.concurrent.atomic.AtomicBoolean;

public class baqe
  implements bapo, bapu
{
  private bapp jdField_a_of_type_Bapp;
  private RichStatus jdField_a_of_type_ComTencentMobileqqRichstatusRichStatus;
  private VasResDrawable jdField_a_of_type_ComTencentMobileqqVasVasResEngineVasResDrawable;
  private String jdField_a_of_type_JavaLangString;
  private Observer jdField_a_of_type_JavaUtilObserver = new baqf(this);
  private boolean jdField_a_of_type_Boolean;
  private RichStatus jdField_b_of_type_ComTencentMobileqqRichstatusRichStatus = new RichStatus(null);
  private boolean jdField_b_of_type_Boolean;
  
  public baqe(VasResDrawable paramVasResDrawable, AppInterface paramAppInterface)
  {
    this.jdField_a_of_type_ComTencentMobileqqVasVasResEngineVasResDrawable = paramVasResDrawable;
    this.jdField_a_of_type_Bapp = paramVasResDrawable.a();
    paramVasResDrawable.a().jdField_a_of_type_Int = 2130846329;
    paramVasResDrawable.a().b = 2130845183;
    paramVasResDrawable.a().jdField_a_of_type_JavaLangString = "sig_cover";
    paramVasResDrawable.a().e = 103;
    if (paramAppInterface != null) {
      paramVasResDrawable.a("my_uin", paramAppInterface.getAccount());
    }
  }
  
  public String a()
  {
    return this.jdField_a_of_type_JavaLangString;
  }
  
  public void a()
  {
    int i = this.jdField_a_of_type_Bapp.a();
    avep.a().addObserver(this.jdField_a_of_type_JavaUtilObserver);
    baoi localbaoi = avep.a().a(i);
    if (!localbaoi.jdField_a_of_type_JavaUtilConcurrentAtomicAtomicBoolean.get())
    {
      if ((localbaoi.jdField_a_of_type_Baok == null) || (TextUtils.isEmpty(localbaoi.jdField_a_of_type_Baok.c))) {
        break label98;
      }
      this.jdField_a_of_type_JavaLangString = localbaoi.jdField_a_of_type_Baok.c;
    }
    for (;;)
    {
      avep.a().deleteObserver(this.jdField_a_of_type_JavaUtilObserver);
      if (!SignatureManager.a(localbaoi)) {
        break;
      }
      ThreadManager.executeOnFileThread(new SignatureAdapter.2(this, i, localbaoi));
      return;
      label98:
      if (!TextUtils.isEmpty(localbaoi.s)) {
        this.jdField_a_of_type_JavaLangString = localbaoi.s;
      } else {
        this.jdField_a_of_type_JavaLangString = localbaoi.k;
      }
    }
    this.jdField_a_of_type_Boolean = false;
    this.jdField_a_of_type_Bapp.a().sendEmptyMessage(10001);
  }
  
  public void a(int paramInt, Bundle paramBundle)
  {
    int i = 0;
    boolean bool;
    if (QLog.isColorLevel())
    {
      StringBuilder localStringBuilder = new StringBuilder().append("downloadDone isMainThread:");
      if (Looper.getMainLooper().getThread().getId() == Thread.currentThread().getId())
      {
        bool = true;
        QLog.d("SignatureAdapter", 2, bool);
      }
    }
    else
    {
      if (paramBundle != null) {
        i = paramBundle.getInt("resType");
      }
      switch (i)
      {
      }
    }
    do
    {
      return;
      bool = false;
      break;
    } while ((3 != paramInt) && (paramInt != 0));
    ThreadManager.executeOnFileThread(new SignatureAdapter.3(this, paramBundle));
  }
  
  public void a(boolean paramBoolean)
  {
    this.jdField_b_of_type_Boolean = paramBoolean;
  }
  
  public boolean a()
  {
    return this.jdField_a_of_type_Boolean;
  }
  
  public void b()
  {
    avep.a().deleteObserver(this.jdField_a_of_type_JavaUtilObserver);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mobileqq\classes8.jar
 * Qualified Name:     baqe
 * JD-Core Version:    0.7.0.1
 */