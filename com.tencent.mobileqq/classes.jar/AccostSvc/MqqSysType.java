  ...
  implements Serializable
{
  ...
  private static MqqSysType[] a = new MqqSysType[9];
  ...
  public static MqqSysType convert(int paramInt)
  {
    ...
    for (;;)
    {
      MqqSysType[] arrayOfMqqSysType = a;
      if (i >= arrayOfMqqSysType.length) {
        break;
      }
      if (arrayOfMqqSysType[i].value() == paramInt) {
        return a[i];
      }
      i += 1;
    }
  ...
  public static MqqSysType convert(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (arrayOfMqqSysType[i].toString().equals(paramString)) {
        return a[i];
      }
      i += 1;
    }
 ...
 * Qualified Name:     AccostSvc.MqqSysType
