...
public class akxj
  extends ajtb
{
  public akxj(AppInterface paramAppInterface)
  {
    super(paramAppInterface);
  }
  
  private static alfw a(oidb_cmd0xb49.LBSPoint paramLBSPoint)
  {
    int j = 0;
    ...
    if (paramLBSPoint.int32_lat.has())
    {
      ...
      label53:
      localalfw.jdField_b_of_type_Int = i;
      if (!paramLBSPoint.str_name.has()) {
        break label146;
      }
      str = paramLBSPoint.str_name.get();
      label77:
      localalfw.jdField_a_of_type_JavaLangString = str;
    ...
    {
      localalfw.jdField_b_of_type_JavaLangString = str;
      i = j;
    ...
    if (paramFromServiceMsg.isSuccess())
    {
      ...
      if (paramToServiceMsg.msg_ar_classify_rsp.has())
      {
        ...
        if (paramFromServiceMsg.uint32_lbs_result.has())
        {
          ...
          localalfv.jdField_a_of_type_Int = i;
          localalfv.jdField_a_of_type_JavaLangString = paramToServiceMsg;
          if (!paramFromServiceMsg.msg_nearest_point.has()) {
            break label291;
          }
          paramToServiceMsg = a(paramFromServiceMsg.msg_nearest_point);
          label241:
          localalfv.jdField_a_of_type_Alfw = paramToServiceMsg;
        }
      }
      else
      {
        QLog.i("ARLBSHandler", 1, "onReceiveLBSLocation. resp is success. retCode = " + localalfv.jdField_a_of_type_Int);
      }
    }
    for (;;)
    {
      notifyUI(1, true, localalfv);
      return;
      i = -1;
      break;
      label291:
