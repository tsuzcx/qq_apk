...
public class blrg
{
  private static final MutableLiveData<blrk> jdField_a_of_type_AndroidArchLifecycleMutableLiveData;
  public static final blrl a;
  private static final List<Feature> jdField_a_of_type_JavaUtilList;
  private static final List<Pair<String, Integer>> b;
  private static final List<blrk> c;
  
  static
  {
    jdField_a_of_type_Blrl = new blrl();
  ...
  public static MutableLiveData<blrk> a()
  {
    return jdField_a_of_type_AndroidArchLifecycleMutableLiveData;
  }
  
  public static List<blrk> a(@Nullable blrk paramblrk)
  {
    LinkedList localLinkedList = new LinkedList(c);
    if ((paramblrk == null) || (TextUtils.isEmpty(paramblrk.jdField_a_of_type_JavaLangString))) {
      return localLinkedList;
    }
    Iterator localIterator = localLinkedList.iterator();
    while (localIterator.hasNext())
    {
      blrk localblrk = (blrk)localIterator.next();
      if ((localblrk != null) && (!TextUtils.isEmpty(localblrk.jdField_a_of_type_JavaLangString)) && (localblrk.jdField_a_of_type_JavaLangString.equals(paramblrk.jdField_a_of_type_JavaLangString)))
      {
        localblrk.b = paramblrk.b;
        localblrk.c = paramblrk.c;
      }
    }
    return localLinkedList;
  }
  ...
  public static void a(@Nullable blrx paramblrx, long paramLong)
  {
    ThreadManager.getUIHandler().postDelayed(new AEDashboardUtil.1(paramblrx), paramLong);
  }
  ...
  private static void b(@Nullable blrx paramblrx)
  {
    ThreadManager.excute(new AEDashboardUtil.2(paramblrx), 64, null, true);
  }
  
  private static void b(@Nullable blrx paramblrx, @NonNull List<blrj> paramList, @NonNull List<blri> paramList1)
  {
    if (paramblrx == null) {
      return;
    }
    ThreadManager.getUIHandler().post(new AEDashboardUtil.3(paramblrx, paramList, paramList1));
  }
  
  private static List<blrj> c()
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      blrj localblrj = new blrj();
      try
      {
        localblrj.jdField_a_of_type_JavaLangString = ((Feature)localObject1).getName();
        localblrj.jdField_a_of_type_Boolean = ((Feature)localObject1).isFunctionReady();
        ...
        {
          localStringBuilder.append((String)localObject1);
          localblrj.c = localStringBuilder.toString();
          localLinkedList.add(localblrj);
          break;
          localObject1 = "GPU";
          break label94;
        }
      ...
      {
        localblrj.jdField_a_of_type_Boolean = false;
        localblrj.b = localException.getMessage();
        localLinkedList.add(localblrj);
      }
      finally
      {
    ...
    Collections.sort(localLinkedList, new blrh());
    return localLinkedList;
  }
  
  private static void c()
  {
    c.add(new blrk("CameraModuleSvc.GetCameraConfig"));
    c.add(new blrk("CameraModuleSvc.GetCompressedCategoryMaterial.MqStoryCamera"));
    c.add(new blrk("CameraModuleSvc.GetCompressedCategoryMaterial.MqEmoCamera"));
    c.add(new blrk("CameraModuleSvc.GetPlayShowCatMatTree"));
  }
  
  private static List<blri> d()
  {
    LinkedList localLinkedList = new LinkedList();
    ...
    while (localIterator.hasNext())
    {
      ...
      blri localblri = new blri();
      localblri.jdField_a_of_type_JavaLangString = ((String)localPair.first);
      localblri.b = bmbk.a().a((String)localPair.first, "null", ((Integer)localPair.second).intValue());
      localLinkedList.add(localblri);
    }
    return localLinkedList;
  }
}
