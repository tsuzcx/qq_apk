    ...
    try
    {
      ...
      if (!SettingCloneUtil.readValue(BaseApplicationImpl.getApplication(), Long.toString(l), null, "qqsetting_pcactive_key", false))
      {
        ...
        if (((SubMsgType0x6b.MsgBody)localObject).bytes_no_text.has()) {
          parambbyn = new String(((SubMsgType0x6b.MsgBody)localObject).bytes_no_text.get().toByteArray(), "utf-8");
        }
        BaseApplicationImpl.getApplication().setPCActiveNotice(Long.toString(l), paramMsgType0x210, parambbyn, paramList);
        ...
        ((Intent)localObject).putExtra("lButton", parambbyn);
