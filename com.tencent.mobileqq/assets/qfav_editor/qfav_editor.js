...
QfavEditor.updateFileInfo = function (fileName, sha1, md5, fid, src, uid, bid, size, category) {
    console.log('updateFileInfo fileName:' + fileName + ' sha1:' + sha1 + 'md5:' + md5 + 'fid:' + fid);
    var elmts = document.getElementsByName(fileName);
    var modifyCount = 0;
    for (i = 0; i < elmts.length; ++i) {
        var item = elmts[i];
        var eleFid = item.getAttribute("fid");
        if (item == undefined || (eleFid != undefined && eleFid.length > 5)) {
            continue;
        }

        var eleName = item.getAttribute("name");
        var eleMd5 = item.getAttribute("md5");
        var eleSha1 = item.getAttribute("sha1");
        console.log('updateFileInfo item info: fileName:' + eleName + ' sha1:' + eleSha1 + 'md5:' + eleMd5 + 'fid:' + eleFid);
        if (eleName == fileName && eleMd5 == md5) {
            item.setAttribute("fid", fid);
            item.setAttribute("src", src);
            item.setAttribute("uid", uid);
            item.setAttribute("bid", bid);
    ...
    if (modifyCount > 0) {
        QfavEditor._editor._onDomSubtreeModified();
    }
}
...
function notifyPreventFinish(prevent) {
    QfavUtil.notify('needPreventFinish', { prevent: prevent });
}
