...
final class aq
{
  static int a(RecyclerView.s params, aj paramaj, View paramView1, View paramView2, RecyclerView.LayoutManager paramLayoutManager, boolean paramBoolean)
  {
    ...
    int i = paramaj.bp(paramView2);
    int j = paramaj.bo(paramView1);
    return Math.min(paramaj.kJ(), i - j);
  }
  
  static int a(RecyclerView.s params, aj paramaj, View paramView1, View paramView2, RecyclerView.LayoutManager paramLayoutManager, boolean paramBoolean1, boolean paramBoolean2)
  {
    int i = 0;
    int j = i;
    ...
    for (i = Math.max(0, params.getItemCount() - j - 1);; i = Math.max(0, i))
    {
      ...
      j = Math.abs(paramaj.bp(paramView2) - paramaj.bo(paramView1));
      int k = Math.abs(paramLayoutManager.getPosition(paramView1) - paramLayoutManager.getPosition(paramView2));
      float f = j / (k + 1);
      return Math.round(i * f + (paramaj.kH() - paramaj.bo(paramView1)));
    }
  }
  
  static int b(RecyclerView.s params, aj paramaj, View paramView1, View paramView2, RecyclerView.LayoutManager paramLayoutManager, boolean paramBoolean)
  {
    if ((paramLayoutManager.getChildCount() == 0) || (params.getItemCount() == 0) || (paramView1 == null) || (paramView2 == null)) {
      return 0;
    }
