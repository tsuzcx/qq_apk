package kotlinx.coroutines.a;

import com.tencent.matrix.trace.core.AppMethodBeat;
import d.l;
import java.util.concurrent.atomic.AtomicReferenceFieldUpdater;

@l(fvt={1, 1, 16}, fvu={""}, fvv={"Lkotlinx/coroutines/internal/LockFreeTaskQueue;", "", "E", "", "singleConsumer", "<init>", "(Z)V", "element", "addLast", "(Ljava/lang/Object;)Z", "", "close", "()V", "isClosed", "()Z", "R", "Lkotlin/Function1;", "transform", "", "map", "(Lkotlin/jvm/functions/Function1;)Ljava/util/List;", "removeFirstOrNull", "()Ljava/lang/Object;", "isEmpty", "", "getSize", "()I", "size", "kotlinx-coroutines-core"})
public class h<E>
{
  private static final AtomicReferenceFieldUpdater Kfq;
  private volatile Object _cur;
  
  static
  {
    AppMethodBeat.i(118130);
    Kfq = AtomicReferenceFieldUpdater.newUpdater(h.class, Object.class, "_cur");
    AppMethodBeat.o(118130);
  }
  
  public h()
  {
    AppMethodBeat.i(118129);
    this._cur = new i(8, false);
    AppMethodBeat.o(118129);
  }
  
  public final void close()
  {
    AppMethodBeat.i(118126);
    for (;;)
    {
      i locali = (i)this._cur;
      if (locali.vE())
      {
        AppMethodBeat.o(118126);
        return;
      }
      Kfq.compareAndSet(this, locali, locali.fLR());
    }
  }
  
  public final E fLJ()
  {
    AppMethodBeat.i(118128);
    for (;;)
    {
      i locali = (i)this._cur;
      Object localObject = locali.fLJ();
      if (localObject != i.Kfu)
      {
        AppMethodBeat.o(118128);
        return localObject;
      }
      Kfq.compareAndSet(this, locali, locali.fLR());
    }
  }
  
  public final boolean fN(E paramE)
  {
    AppMethodBeat.i(118127);
    for (;;)
    {
      i locali = (i)this._cur;
      switch (locali.fO(paramE))
      {
      default: 
        break;
      case 0: 
        AppMethodBeat.o(118127);
        return true;
      case 2: 
        AppMethodBeat.o(118127);
        return false;
      case 1: 
        Kfq.compareAndSet(this, locali, locali.fLR());
      }
    }
  }
  
  public final int getSize()
  {
    long l = ((i)this._cur)._state;
    int i = (int)((0x3FFFFFFF & l) >> 0);
    return (int)((l & 0xC0000000) >> 30) - i & 0x3FFFFFFF;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes3.jar
 * Qualified Name:     kotlinx.coroutines.a.h
 * JD-Core Version:    0.7.0.1
 */