...
public final class i
{
      ...
      if (i < paramInt1 + 0)
      {
        ...
        if (j > 0)
        {
          if ((m != 0) && (arrayOfInt[j] != 0)) {
            arrayOfChar[j] = ((char)(arrayOfChar[(j - 1)] ^ bIZ[((bIY[m] + bIY[arrayOfInt[j]]) % 255)]));
          }
          for (;;)
          {
            j -= 1;
        ...
        if ((m != 0) && (arrayOfInt[0] != 0)) {
          arrayOfChar[0] = ((char)bIZ[((bIY[m] + bIY[arrayOfInt[0]]) % 255)]);
        }
        for (;;)
        {
          i += 1;
  ...
  public static String a(String paramString, k paramk)
  {
    ...
    if (paramString.length() != paramk.bJd)
    {
      paramString = new IllegalArgumentException("The number of codewords does not match the selected symbol");
      AppMethodBeat.o(12341);
      throw paramString;
    }
    StringBuilder localStringBuilder = new StringBuilder(paramk.bJd + paramk.bJe);
    localStringBuilder.append(paramString);
    int m = paramk.xX();
    if (m == 1) {
      localStringBuilder.append(c(paramString, paramk.bJe));
    }
    for (;;)
    {
      ...
      while (i < m)
      {
        arrayOfInt1[i] = paramk.fT(i + 1);
        arrayOfInt2[i] = paramk.bJj;
        localObject[i] = 0;
        if (i > 0) {
      ...
      while (i < m)
      {
        ...
        {
          localStringBuilder.setCharAt(paramk.bJd + k, ((String)localObject).charAt(j));
          k += m;
          j += 1;
        }
        i += 1;
