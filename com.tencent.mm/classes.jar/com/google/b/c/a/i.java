...
public final class i
{
  ...
  {
    int i = 1;
    cdI = new int[] { 5, 7, 10, 11, 12, 14, 18, 20, 24, 28, 36, 42, 48, 56, 62, 68 };
    ...
    int[] arrayOfInt9 = { 245, 127, 242, 218, 130, 250, 162, 181, 102, 120, 84, 179, 220, 251, 80, 182, 229, 18, 2, 4, 68, 33, 101, 137, 95, 119, 115, 44, 175, 184, 59, 25, 225, 98, 81, 112 };
    int[] arrayOfInt10 = { 245, 132, 172, 223, 96, 32, 117, 22, 238, 133, 238, 231, 205, 188, 237, 87, 191, 106, 16, 147, 118, 23, 37, 90, 170, 205, 131, 88, 120, 100, 66, 138, 186, 240, 82, 44, 176, 87, 187, 147, 160, 175, 69, 213, 92, 253, 225, 19 };
    int[] arrayOfInt11 = { 175, 9, 223, 238, 12, 17, 220, 208, 100, 29, 175, 170, 230, 192, 215, 235, 150, 159, 36, 223, 38, 200, 132, 54, 228, 146, 218, 234, 117, 203, 29, 232, 144, 238, 22, 150, 201, 117, 62, 207, 164, 13, 137, 245, 127, 67, 247, 28, 155, 43, 203, 107, 233, 53, 143, 46 };
    int[] arrayOfInt12 = { 220, 228, 173, 89, 251, 149, 159, 56, 89, 33, 147, 244, 154, 36, 73, 127, 213, 136, 248, 180, 234, 197, 158, 177, 68, 122, 93, 213, 15, 160, 227, 236, 66, 139, 153, 185, 202, 167, 179, 25, 220, 232, 96, 210, 231, 136, 223, 239, 181, 241, 59, 52, 172, 25, 49, 232, 211, 189, 64, 54, 108, 153, 132, 63, 96, 103, 82, 186 };
    cdJ = new int[][] { arrayOfInt1, arrayOfInt2, arrayOfInt3, { 175, 138, 205, 12, 194, 168, 39, 245, 60, 97, 120 }, arrayOfInt4, arrayOfInt5, arrayOfInt6, arrayOfInt7, arrayOfInt8, { 211, 231, 43, 97, 71, 96, 103, 174, 37, 151, 170, 53, 75, 34, 249, 121, 17, 138, 110, 213, 141, 136, 120, 151, 233, 168, 93, 255 }, arrayOfInt9, { 77, 193, 137, 31, 19, 38, 22, 153, 247, 105, 122, 2, 245, 133, 242, 8, 175, 95, 100, 9, 167, 105, 214, 111, 57, 121, 21, 1, 253, 57, 54, 101, 248, 202, 69, 50, 150, 177, 226, 5, 9, 5 }, arrayOfInt10, arrayOfInt11, { 242, 93, 169, 50, 144, 210, 39, 118, 202, 188, 201, 189, 143, 108, 196, 37, 185, 112, 134, 230, 245, 63, 197, 190, 250, 106, 185, 221, 175, 64, 114, 71, 161, 44, 147, 6, 27, 218, 51, 63, 87, 10, 40, 130, 188, 17, 163, 31, 176, 170, 4, 107, 232, 7, 94, 166, 224, 124, 86, 47, 11, 204 }, arrayOfInt12 };
    cdK = new int[256];
    cdL = new int['ÿ'];
    int j = 0;
    while (j < 255)
      ...
      if (i < paramInt1 + 0)
      {
        ...
        if (j > 0)
        {
          if ((m != 0) && (arrayOfInt[j] != 0)) {
            arrayOfChar[j] = ((char)(arrayOfChar[(j - 1)] ^ cdL[((cdK[m] + cdK[arrayOfInt[j]]) % 255)]));
          }
          for (;;)
          {
            j -= 1;
        ...
        if ((m != 0) && (arrayOfInt[0] != 0)) {
          arrayOfChar[0] = ((char)cdL[((cdK[m] + cdK[arrayOfInt[0]]) % 255)]);
        }
        for (;;)
        {
          i += 1;
  ...
  public static String a(String paramString, k paramk)
  {
    ...
    if (paramString.length() != paramk.cdP)
    {
      paramString = new IllegalArgumentException("The number of codewords does not match the selected symbol");
      AppMethodBeat.o(12341);
      throw paramString;
    }
    StringBuilder localStringBuilder = new StringBuilder(paramk.cdP + paramk.cdQ);
    localStringBuilder.append(paramString);
    int m = paramk.Ik();
    if (m == 1) {
      localStringBuilder.append(c(paramString, paramk.cdQ));
    }
    for (;;)
    {
      ...
      while (i < m)
      {
        arrayOfInt1[i] = paramk.hh(i + 1);
        arrayOfInt2[i] = paramk.cdV;
        localObject[i] = 0;
        if (i > 0) {
      ...
      while (i < m)
      {
        ...
        {
          localStringBuilder.setCharAt(paramk.cdP + k, ((String)localObject).charAt(j));
          k += m;
          j += 1;
        }
        i += 1;
