...
final class f
  implements g
{
  private static String w(CharSequence paramCharSequence)
  {
    int k = 0;
  ...
  public final void a(h paramh)
  {
    ...
    for (;;)
    {
      ...
      paramh.cs(w((CharSequence)localObject));
    ...
    int k = i - 1;
    String str = w(localCharSequence);
    if (!paramh.In()) {
      i = 1;
    }
    for (;;)
    {
      ...
      if (k <= 2)
      {
        paramh.hi(paramh.cdE.length() + k);
        j = i;
        if (paramh.cdG.cdP - paramh.cdE.length() >= 3)
        {
          paramh.hi(paramh.cdE.length() + str.length());
          j = 0;
        }
      }
      if (j != 0)
