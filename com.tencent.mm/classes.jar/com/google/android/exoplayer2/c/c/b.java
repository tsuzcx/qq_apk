package com.google.android.exoplayer2.c.c;

import com.google.android.exoplayer2.Format;
import com.google.android.exoplayer2.c.e;
import com.google.android.exoplayer2.c.f;
import com.google.android.exoplayer2.c.g;
import com.google.android.exoplayer2.c.h;
import com.google.android.exoplayer2.c.i;
import com.google.android.exoplayer2.c.j;
import com.google.android.exoplayer2.c.k;
import com.google.android.exoplayer2.c.l;
import com.google.android.exoplayer2.i.x;
import com.google.android.exoplayer2.metadata.Metadata;
import com.google.android.exoplayer2.metadata.id3.a.a;
import com.google.android.exoplayer2.o;
import com.tencent.matrix.trace.core.AppMethodBeat;
import java.io.EOFException;

public final class b
  implements e
{
  public static final h aRP;
  private static final int aTU;
  private static final int aTV;
  private static final int aTW;
  private Metadata aMO;
  private final com.google.android.exoplayer2.i.m aRR;
  private g aRV;
  private final long aTX;
  private final j aTY;
  private final i aTZ;
  private com.google.android.exoplayer2.c.m aUa;
  private int aUb;
  private a aUc;
  private long aUd;
  private long aUe;
  private int aUf;
  private final int flags;
  
  static
  {
    AppMethodBeat.i(92045);
    aRP = new h()
    {
      public final e[] ss()
      {
        AppMethodBeat.i(92037);
        b localb = new b();
        AppMethodBeat.o(92037);
        return new e[] { localb };
      }
    };
    aTU = x.bU("Xing");
    aTV = x.bU("Info");
    aTW = x.bU("VBRI");
    AppMethodBeat.o(92045);
  }
  
  public b()
  {
    this(0);
  }
  
  public b(int paramInt)
  {
    this(paramInt, -9223372036854775807L);
  }
  
  public b(int paramInt, long paramLong)
  {
    AppMethodBeat.i(92038);
    this.flags = paramInt;
    this.aTX = paramLong;
    this.aRR = new com.google.android.exoplayer2.i.m(10);
    this.aTY = new j();
    this.aTZ = new i();
    this.aUd = -9223372036854775807L;
    AppMethodBeat.o(92038);
  }
  
  private boolean a(f paramf, boolean paramBoolean)
  {
    AppMethodBeat.i(92042);
    int m;
    int n;
    int k;
    int i;
    int j;
    if (paramBoolean)
    {
      m = 16384;
      paramf.sp();
      if (paramf.getPosition() != 0L) {
        break label326;
      }
      e(paramf);
      n = (int)paramf.sq();
      if (!paramBoolean) {
        paramf.ed(n);
      }
      k = 0;
      i = 0;
      j = 0;
    }
    for (;;)
    {
      label65:
      byte[] arrayOfByte = this.aRR.data;
      boolean bool;
      if (j > 0) {
        bool = true;
      }
      for (;;)
      {
        if (paramf.b(arrayOfByte, 0, 4, bool))
        {
          this.aRR.setPosition(0);
          int i1 = this.aRR.readInt();
          int i2;
          if ((i == 0) || (j(i1, i)))
          {
            i2 = j.ek(i1);
            if (i2 != -1) {}
          }
          else
          {
            i = k + 1;
            if (k == m)
            {
              if (!paramBoolean)
              {
                paramf = new o("Searched too many bytes.");
                AppMethodBeat.o(92042);
                throw paramf;
                m = 131072;
                break;
                bool = false;
                continue;
              }
              AppMethodBeat.o(92042);
              return false;
            }
            if (paramBoolean)
            {
              paramf.sp();
              paramf.ee(n + i);
              k = i;
              i = 0;
              j = 0;
              break label65;
            }
            paramf.ed(1);
            k = i;
            i = 0;
            j = 0;
            break label65;
          }
          j += 1;
          if (j == 1)
          {
            j.a(i1, this.aTY);
            i = i1;
          }
          while (j != 4)
          {
            paramf.ee(i2 - 4);
            break;
          }
        }
      }
      if (paramBoolean) {
        paramf.ed(n + k);
      }
      for (;;)
      {
        this.aUb = i;
        AppMethodBeat.o(92042);
        return true;
        paramf.sp();
      }
      label326:
      k = 0;
      n = 0;
      i = 0;
      j = 0;
    }
  }
  
  private void e(f paramf)
  {
    AppMethodBeat.i(92043);
    int i = 0;
    paramf.b(this.aRR.data, 0, 10);
    this.aRR.setPosition(0);
    if (this.aRR.vj() == com.google.android.exoplayer2.metadata.id3.a.aZL)
    {
      this.aRR.fu(3);
      int j = this.aRR.vn();
      int k = j + 10;
      a.a locala;
      if (this.aMO == null)
      {
        byte[] arrayOfByte = new byte[k];
        System.arraycopy(this.aRR.data, 0, arrayOfByte, 0, 10);
        paramf.b(arrayOfByte, 10, j);
        if ((this.flags & 0x2) != 0)
        {
          locala = i.aRA;
          label120:
          this.aMO = new com.google.android.exoplayer2.metadata.id3.a(locala).d(arrayOfByte, k);
          if (this.aMO != null) {
            this.aTZ.c(this.aMO);
          }
        }
      }
      for (;;)
      {
        i += k;
        break;
        locala = null;
        break label120;
        paramf.ee(j);
      }
    }
    paramf.sp();
    paramf.ee(i);
    AppMethodBeat.o(92043);
  }
  
  private a f(f paramf)
  {
    AppMethodBeat.i(92044);
    paramf.b(this.aRR.data, 0, 4);
    this.aRR.setPosition(0);
    j.a(this.aRR.readInt(), this.aTY);
    paramf = new a(paramf.getPosition(), this.aTY.bitrate, paramf.sr());
    AppMethodBeat.o(92044);
    return paramf;
  }
  
  private static boolean j(int paramInt, long paramLong)
  {
    return (0xFFFE0C00 & paramInt) == (0xFFFE0C00 & paramLong);
  }
  
  public final int a(f paramf, k paramk)
  {
    AppMethodBeat.i(92041);
    if (this.aUb == 0) {}
    label796:
    label1064:
    for (;;)
    {
      int j;
      Object localObject1;
      int k;
      int m;
      int n;
      int i1;
      Object localObject2;
      try
      {
        a(paramf, false);
        if (this.aUc == null)
        {
          paramk = new com.google.android.exoplayer2.i.m(this.aTY.aOj);
          paramf.b(paramk.data, 0, this.aTY.aOj);
          if ((this.aTY.version & 0x1) == 0) {
            break label568;
          }
          if (this.aTY.channels != 1)
          {
            i = 36;
            if (paramk.limit < i + 4) {
              break label591;
            }
            paramk.setPosition(i);
            j = paramk.readInt();
            if ((j != aTU) && (j != aTV)) {
              break label591;
            }
            if ((j != aTU) && (j != aTV)) {
              break label748;
            }
            localObject1 = this.aTY;
            l2 = paramf.getPosition();
            l1 = paramf.sr();
            k = ((j)localObject1).aRJ;
            m = ((j)localObject1).sampleRate;
            l2 += ((j)localObject1).aOj;
            n = paramk.readInt();
            if ((n & 0x1) == 1)
            {
              i1 = paramk.vo();
              if (i1 != 0) {
                break label630;
              }
            }
            paramk = null;
            if ((paramk != null) && (!this.aTZ.su()))
            {
              paramf.sp();
              paramf.ee(i + 141);
              paramf.b(this.aRR.data, 0, 3);
              this.aRR.setPosition(0);
              localObject1 = this.aTZ;
              k = this.aRR.vj();
              i = k >> 12;
              k &= 0xFFF;
              if ((i > 0) || (k > 0))
              {
                ((i)localObject1).aNa = i;
                ((i)localObject1).aNb = k;
              }
            }
            paramf.ed(this.aTY.aOj);
            localObject1 = paramk;
            if (paramk != null)
            {
              localObject1 = paramk;
              if (!paramk.so())
              {
                localObject1 = paramk;
                if (j == aTV) {
                  localObject1 = f(paramf);
                }
              }
            }
            this.aUc = ((a)localObject1);
            if ((this.aUc == null) || ((!this.aUc.so()) && ((this.flags & 0x1) != 0))) {
              this.aUc = f(paramf);
            }
            this.aRV.a(this.aUc);
            localObject1 = this.aUa;
            localObject2 = this.aTY.mimeType;
            i = this.aTY.channels;
            j = this.aTY.sampleRate;
            k = this.aTZ.aNa;
            m = this.aTZ.aNb;
            if ((this.flags & 0x2) == 0) {
              break label1094;
            }
            paramk = null;
            ((com.google.android.exoplayer2.c.m)localObject1).f(Format.a(null, (String)localObject2, null, -1, 4096, i, j, -1, k, m, null, null, 0, null, paramk));
          }
        }
        else
        {
          if (this.aUf != 0) {
            break label1250;
          }
          paramf.sp();
          if (paramf.b(this.aRR.data, 0, 4, true)) {
            break;
          }
          AppMethodBeat.o(92041);
          return -1;
        }
      }
      catch (EOFException paramf)
      {
        AppMethodBeat.o(92041);
        return -1;
      }
      i = 21;
      continue;
      label568:
      if (this.aTY.channels != 1)
      {
        i = 21;
      }
      else
      {
        i = 13;
        continue;
        label591:
        if (paramk.limit >= 40)
        {
          paramk.setPosition(36);
          if (paramk.readInt() == aTW)
          {
            j = aTW;
            continue;
          }
        }
        j = 0;
        continue;
        label630:
        long l3 = x.b(i1, k * 1000000L, m);
        if ((n & 0x6) != 6)
        {
          paramk = new d(l2, l3, l1);
        }
        else
        {
          long l4 = paramk.vo();
          paramk.fu(1);
          localObject2 = new long[99];
          k = 0;
          while (k < 99)
          {
            localObject2[k] = paramk.readUnsignedByte();
            k += 1;
          }
          paramk = new d(l2, l3, l1, (long[])localObject2, l4, ((j)localObject1).aOj);
          continue;
          label748:
          if (j == aTW)
          {
            localObject1 = this.aTY;
            l1 = paramf.getPosition();
            l3 = paramf.sr();
            paramk.fu(10);
            i = paramk.readInt();
            if (i <= 0) {
              localObject1 = null;
            }
            for (;;)
            {
              paramf.ed(this.aTY.aOj);
              break;
              j = ((j)localObject1).sampleRate;
              l2 = i;
              if (j >= 32000) {}
              for (i = 1152;; i = 576)
              {
                l4 = x.b(l2, i * 1000000L, j);
                k = paramk.readUnsignedShort();
                m = paramk.readUnsignedShort();
                n = paramk.readUnsignedShort();
                paramk.fu(2);
                l1 += ((j)localObject1).aOj;
                localObject1 = new long[k + 1];
                localObject2 = new long[k + 1];
                localObject1[0] = 0L;
                localObject2[0] = l1;
                j = 1;
                if (j >= localObject1.length) {
                  break label1064;
                }
                switch (n)
                {
                default: 
                  localObject1 = null;
                  break label796;
                }
              }
              i = paramk.readUnsignedByte();
              l1 += i * m;
              localObject1[j] = (j * l4 / k);
              if (l3 == -1L) {}
              for (l2 = l1;; l2 = Math.min(l3, l1))
              {
                localObject2[j] = l2;
                j += 1;
                break;
                i = paramk.readUnsignedShort();
                break label974;
                i = paramk.vj();
                break label974;
                i = paramk.vo();
                break label974;
              }
              localObject1 = new c((long[])localObject1, (long[])localObject2, l4);
            }
          }
          label974:
          localObject1 = null;
          paramf.sp();
          continue;
          label1094:
          paramk = this.aMO;
        }
      }
    }
    this.aRR.setPosition(0);
    int i = this.aRR.readInt();
    if ((!j(i, this.aUb)) || (j.ek(i) == -1))
    {
      paramf.ed(1);
      this.aUb = 0;
      AppMethodBeat.o(92041);
      return 0;
    }
    j.a(i, this.aTY);
    if (this.aUd == -9223372036854775807L)
    {
      this.aUd = this.aUc.Q(paramf.getPosition());
      if (this.aTX != -9223372036854775807L)
      {
        l1 = this.aUc.Q(0L);
        l2 = this.aUd;
        this.aUd = (this.aTX - l1 + l2);
      }
    }
    this.aUf = this.aTY.aOj;
    label1250:
    i = this.aUa.a(paramf, this.aUf, true);
    if (i == -1)
    {
      AppMethodBeat.o(92041);
      return -1;
    }
    this.aUf -= i;
    if (this.aUf > 0)
    {
      AppMethodBeat.o(92041);
      return 0;
    }
    long l1 = this.aUd;
    long l2 = this.aUe * 1000000L / this.aTY.sampleRate;
    this.aUa.a(l2 + l1, 1, this.aTY.aOj, 0, null);
    this.aUe += this.aTY.aRJ;
    this.aUf = 0;
    AppMethodBeat.o(92041);
    return 0;
  }
  
  public final void a(g paramg)
  {
    AppMethodBeat.i(92040);
    this.aRV = paramg;
    this.aUa = this.aRV.ej(0);
    this.aRV.st();
    AppMethodBeat.o(92040);
  }
  
  public final boolean a(f paramf)
  {
    AppMethodBeat.i(92039);
    boolean bool = a(paramf, true);
    AppMethodBeat.o(92039);
    return bool;
  }
  
  public final void g(long paramLong1, long paramLong2)
  {
    this.aUb = 0;
    this.aUd = -9223372036854775807L;
    this.aUe = 0L;
    this.aUf = 0;
  }
  
  static abstract interface a
    extends l
  {
    public abstract long Q(long paramLong);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes9.jar
 * Qualified Name:     com.google.android.exoplayer2.c.c.b
 * JD-Core Version:    0.7.0.1
 */