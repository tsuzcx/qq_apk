...
public class SwitchFaceUtil
{
  ...
  {
    if ((!TextUtils.isEmpty(paramString)) && (paramString.startsWith("assets://"))) {}
    for (paramString = VideoBitmapUtil.decodeSampleBitmapFromAssets(VideoGlobalContext.getContext(), VideoUtil.getRealPath(paramString) + File.separator + paramFaceItem.faceExchangeImage, 2147483647, 2147483647);; paramString = VideoBitmapUtil.decodeSampledBitmapFromFile(paramString + File.separator + paramFaceItem.faceExchangeImage, 2147483647, 2147483647))
    {
  ...
  {
    if ((VideoUtil.isEmpty(paramList)) || (paramList.size() < 90)) {
    ...
    while (i > 0)
    {
      double d2 = i * 0.523333348929882D;
  ...
  public static Bitmap getGrayBitmap(SwitchFaceUtil.FEATURE_TYPE paramFEATURE_TYPE)
  {
    ...
    if (paramFEATURE_TYPE.equals(SwitchFaceUtil.FEATURE_TYPE.NO_EYE)) {
      localBitmap = VideoBitmapUtil.decodeSampleBitmap(VideoGlobalContext.getContext(), GRAY_FILE_NO_EYE, 2147483647, 2147483647);
    }
    do
    {
      return localBitmap;
      if (paramFEATURE_TYPE.equals(SwitchFaceUtil.FEATURE_TYPE.HAS_EYE)) {
        return VideoBitmapUtil.decodeSampleBitmap(VideoGlobalContext.getContext(), GRAY_FILE_HAS_EYE, 2147483647, 2147483647);
      }
    } while (!paramFEATURE_TYPE.equals(SwitchFaceUtil.FEATURE_TYPE.ALL_GRAY));
    return VideoBitmapUtil.decodeSampleBitmap(VideoGlobalContext.getContext(), GRAY_FILE_ALL, 2147483647, 2147483647);
  }
  
  public static List<PointF> getGrayCoords(SwitchFaceUtil.FEATURE_TYPE paramFEATURE_TYPE)
  {
    switch (SwitchFaceUtil.1.$SwitchMap$com$tencent$ttpic$util$SwitchFaceUtil$FEATURE_TYPE[paramFEATURE_TYPE.ordinal()])
    {
    }
    for (;;)
    {
  ...
  {
    float[] arrayOfFloat;
    if ((VideoUtil.isEmpty(paramList)) || (paramArrayOfFloat == null) || (paramInt1 <= 0) || (paramInt2 <= 0) || (paramArrayOfFloat.length != 276))
    {
  ...
  private static List<PointF> loadTexCoords(String paramString1, String paramString2)
  {
    ...
    {
      return localArrayList;
      paramString1 = VideoFileUtil.load(paramString1, paramString2);
 ...
 * Qualified Name:     com.tencent.ttpic.util.SwitchFaceUtil
