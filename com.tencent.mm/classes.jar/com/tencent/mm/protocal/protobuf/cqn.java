package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import e.a.a.b;
import java.util.LinkedList;

public final class cqn
  extends buy
{
  public int xGN;
  public int xGP;
  public cre xZe;
  public String xgY;
  
  public final int op(int paramInt, Object... paramVarArgs)
  {
    AppMethodBeat.i(28697);
    if (paramInt == 0)
    {
      paramVarArgs = (e.a.a.c.a)paramVarArgs[0];
      if (this.xZe == null)
      {
        paramVarArgs = new b("Not all required fields were included: Piece");
        AppMethodBeat.o(28697);
        throw paramVarArgs;
      }
      if (this.BaseRequest != null)
      {
        paramVarArgs.iQ(1, this.BaseRequest.computeSize());
        this.BaseRequest.writeFields(paramVarArgs);
      }
      if (this.xZe != null)
      {
        paramVarArgs.iQ(2, this.xZe.computeSize());
        this.xZe.writeFields(paramVarArgs);
      }
      paramVarArgs.aO(3, this.xGN);
      paramVarArgs.aO(4, this.xGP);
      if (this.xgY != null) {
        paramVarArgs.e(5, this.xgY);
      }
      AppMethodBeat.o(28697);
      return 0;
    }
    if (paramInt == 1) {
      if (this.BaseRequest == null) {
        break label673;
      }
    }
    label673:
    for (paramInt = e.a.a.a.iP(1, this.BaseRequest.computeSize()) + 0;; paramInt = 0)
    {
      int i = paramInt;
      if (this.xZe != null) {
        i = paramInt + e.a.a.a.iP(2, this.xZe.computeSize());
      }
      i = i + e.a.a.b.b.a.bl(3, this.xGN) + e.a.a.b.b.a.bl(4, this.xGP);
      paramInt = i;
      if (this.xgY != null) {
        paramInt = i + e.a.a.b.b.a.f(5, this.xgY);
      }
      AppMethodBeat.o(28697);
      return paramInt;
      if (paramInt == 2)
      {
        paramVarArgs = new e.a.a.a.a((byte[])paramVarArgs[0], unknownTagHandler);
        for (paramInt = buy.getNextFieldNumber(paramVarArgs); paramInt > 0; paramInt = buy.getNextFieldNumber(paramVarArgs)) {
          if (!super.populateBuilderWithField(paramVarArgs, this, paramInt)) {
            paramVarArgs.eqQ();
          }
        }
        if (this.xZe == null)
        {
          paramVarArgs = new b("Not all required fields were included: Piece");
          AppMethodBeat.o(28697);
          throw paramVarArgs;
        }
        AppMethodBeat.o(28697);
        return 0;
      }
      if (paramInt == 3)
      {
        Object localObject1 = (e.a.a.a.a)paramVarArgs[0];
        cqn localcqn = (cqn)paramVarArgs[1];
        paramInt = ((Integer)paramVarArgs[2]).intValue();
        Object localObject2;
        boolean bool;
        switch (paramInt)
        {
        default: 
          AppMethodBeat.o(28697);
          return -1;
        case 1: 
          paramVarArgs = ((e.a.a.a.a)localObject1).Wp(paramInt);
          i = paramVarArgs.size();
          paramInt = 0;
          while (paramInt < i)
          {
            localObject2 = (byte[])paramVarArgs.get(paramInt);
            localObject1 = new hq();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = ((hq)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bv.a)localObject1, buy.getNextFieldNumber((e.a.a.a.a)localObject2))) {}
            localcqn.BaseRequest = ((hq)localObject1);
            paramInt += 1;
          }
          AppMethodBeat.o(28697);
          return 0;
        case 2: 
          paramVarArgs = ((e.a.a.a.a)localObject1).Wp(paramInt);
          i = paramVarArgs.size();
          paramInt = 0;
          while (paramInt < i)
          {
            localObject2 = (byte[])paramVarArgs.get(paramInt);
            localObject1 = new cre();
            localObject2 = new e.a.a.a.a((byte[])localObject2, unknownTagHandler);
            for (bool = true; bool; bool = ((cre)localObject1).populateBuilderWithField((e.a.a.a.a)localObject2, (com.tencent.mm.bv.a)localObject1, buy.getNextFieldNumber((e.a.a.a.a)localObject2))) {}
            localcqn.xZe = ((cre)localObject1);
            paramInt += 1;
          }
          AppMethodBeat.o(28697);
          return 0;
        case 3: 
          localcqn.xGN = ((e.a.a.a.a)localObject1).CLY.sl();
          AppMethodBeat.o(28697);
          return 0;
        case 4: 
          localcqn.xGP = ((e.a.a.a.a)localObject1).CLY.sl();
          AppMethodBeat.o(28697);
          return 0;
        }
        localcqn.xgY = ((e.a.a.a.a)localObject1).CLY.readString();
        AppMethodBeat.o(28697);
        return 0;
      }
      AppMethodBeat.o(28697);
      return -1;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes8.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.cqn
 * JD-Core Version:    0.7.0.1
 */