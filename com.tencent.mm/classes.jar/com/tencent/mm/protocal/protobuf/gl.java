package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;
import g.a.a.b;
import java.util.LinkedList;

public final class gl
  extends dyy
{
  public String RFk;
  public boolean RLw;
  public String RLx;
  
  public final int op(int paramInt, Object... paramVarArgs)
  {
    AppMethodBeat.i(189229);
    if (paramInt == 0)
    {
      paramVarArgs = (g.a.a.c.a)paramVarArgs[0];
      if (this.BaseResponse == null)
      {
        paramVarArgs = new b("Not all required fields were included: BaseResponse");
        AppMethodBeat.o(189229);
        throw paramVarArgs;
      }
      if (this.BaseResponse != null)
      {
        paramVarArgs.oE(1, this.BaseResponse.computeSize());
        this.BaseResponse.writeFields(paramVarArgs);
      }
      if (this.RFk != null) {
        paramVarArgs.f(2, this.RFk);
      }
      paramVarArgs.co(3, this.RLw);
      if (this.RLx != null) {
        paramVarArgs.f(4, this.RLx);
      }
      AppMethodBeat.o(189229);
      return 0;
    }
    if (paramInt == 1) {
      if (this.BaseResponse == null) {
        break label500;
      }
    }
    label500:
    for (paramInt = g.a.a.a.oD(1, this.BaseResponse.computeSize()) + 0;; paramInt = 0)
    {
      int i = paramInt;
      if (this.RFk != null) {
        i = paramInt + g.a.a.b.b.a.g(2, this.RFk);
      }
      i += g.a.a.b.b.a.gL(3) + 1;
      paramInt = i;
      if (this.RLx != null) {
        paramInt = i + g.a.a.b.b.a.g(4, this.RLx);
      }
      AppMethodBeat.o(189229);
      return paramInt;
      if (paramInt == 2)
      {
        paramVarArgs = new g.a.a.a.a((byte[])paramVarArgs[0], unknownTagHandler);
        for (paramInt = dyy.getNextFieldNumber(paramVarArgs); paramInt > 0; paramInt = dyy.getNextFieldNumber(paramVarArgs)) {
          if (!super.populateBuilderWithField(paramVarArgs, this, paramInt)) {
            paramVarArgs.iUs();
          }
        }
        if (this.BaseResponse == null)
        {
          paramVarArgs = new b("Not all required fields were included: BaseResponse");
          AppMethodBeat.o(189229);
          throw paramVarArgs;
        }
        AppMethodBeat.o(189229);
        return 0;
      }
      if (paramInt == 3)
      {
        Object localObject = (g.a.a.a.a)paramVarArgs[0];
        gl localgl = (gl)paramVarArgs[1];
        paramInt = ((Integer)paramVarArgs[2]).intValue();
        switch (paramInt)
        {
        default: 
          AppMethodBeat.o(189229);
          return -1;
        case 1: 
          paramVarArgs = ((g.a.a.a.a)localObject).aGc(paramInt);
          i = paramVarArgs.size();
          paramInt = 0;
          while (paramInt < i)
          {
            localObject = (byte[])paramVarArgs.get(paramInt);
            jh localjh = new jh();
            if ((localObject != null) && (localObject.length > 0)) {
              localjh.parseFrom((byte[])localObject);
            }
            localgl.BaseResponse = localjh;
            paramInt += 1;
          }
          AppMethodBeat.o(189229);
          return 0;
        case 2: 
          localgl.RFk = ((g.a.a.a.a)localObject).abFh.readString();
          AppMethodBeat.o(189229);
          return 0;
        case 3: 
          localgl.RLw = ((g.a.a.a.a)localObject).abFh.AB();
          AppMethodBeat.o(189229);
          return 0;
        }
        localgl.RLx = ((g.a.a.a.a)localObject).abFh.readString();
        AppMethodBeat.o(189229);
        return 0;
      }
      AppMethodBeat.o(189229);
      return -1;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes2.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.gl
 * JD-Core Version:    0.7.0.1
 */