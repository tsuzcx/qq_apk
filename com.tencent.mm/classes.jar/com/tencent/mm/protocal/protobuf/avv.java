package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;

public final class avv
  extends com.tencent.mm.bv.a
{
  public String eAx;
  public double fOc;
  public int major;
  public int minor;
  
  public final int op(int paramInt, Object... paramVarArgs)
  {
    AppMethodBeat.i(10191);
    if (paramInt == 0)
    {
      paramVarArgs = (e.a.a.c.a)paramVarArgs[0];
      if (this.eAx != null) {
        paramVarArgs.e(1, this.eAx);
      }
      paramVarArgs.aO(2, this.major);
      paramVarArgs.aO(3, this.minor);
      paramVarArgs.f(4, this.fOc);
      AppMethodBeat.o(10191);
      return 0;
    }
    if (paramInt == 1) {
      if (this.eAx == null) {
        break label363;
      }
    }
    label363:
    for (paramInt = e.a.a.b.b.a.f(1, this.eAx) + 0;; paramInt = 0)
    {
      int i = e.a.a.b.b.a.bl(2, this.major);
      int j = e.a.a.b.b.a.bl(3, this.minor);
      int k = e.a.a.b.b.a.eW(4);
      AppMethodBeat.o(10191);
      return paramInt + i + j + (k + 8);
      if (paramInt == 2)
      {
        paramVarArgs = new e.a.a.a.a((byte[])paramVarArgs[0], unknownTagHandler);
        for (paramInt = com.tencent.mm.bv.a.getNextFieldNumber(paramVarArgs); paramInt > 0; paramInt = com.tencent.mm.bv.a.getNextFieldNumber(paramVarArgs)) {
          if (!super.populateBuilderWithField(paramVarArgs, this, paramInt)) {
            paramVarArgs.eqQ();
          }
        }
        AppMethodBeat.o(10191);
        return 0;
      }
      if (paramInt == 3)
      {
        e.a.a.a.a locala = (e.a.a.a.a)paramVarArgs[0];
        avv localavv = (avv)paramVarArgs[1];
        switch (((Integer)paramVarArgs[2]).intValue())
        {
        default: 
          AppMethodBeat.o(10191);
          return -1;
        case 1: 
          localavv.eAx = locala.CLY.readString();
          AppMethodBeat.o(10191);
          return 0;
        case 2: 
          localavv.major = locala.CLY.sl();
          AppMethodBeat.o(10191);
          return 0;
        case 3: 
          localavv.minor = locala.CLY.sl();
          AppMethodBeat.o(10191);
          return 0;
        }
        localavv.fOc = Double.longBitsToDouble(locala.CLY.emy());
        AppMethodBeat.o(10191);
        return 0;
      }
      AppMethodBeat.o(10191);
      return -1;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes3.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.avv
 * JD-Core Version:    0.7.0.1
 */