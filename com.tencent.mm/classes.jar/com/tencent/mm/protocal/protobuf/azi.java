package com.tencent.mm.protocal.protobuf;

import com.tencent.matrix.trace.core.AppMethodBeat;

public final class azi
  extends com.tencent.mm.bw.a
{
  public String dJw;
  public int scene;
  public String sessionId;
  
  public final int op(int paramInt, Object... paramVarArgs)
  {
    AppMethodBeat.i(196089);
    if (paramInt == 0)
    {
      paramVarArgs = (g.a.a.c.a)paramVarArgs[0];
      paramVarArgs.aM(1, this.scene);
      if (this.dJw != null) {
        paramVarArgs.e(2, this.dJw);
      }
      if (this.sessionId != null) {
        paramVarArgs.e(3, this.sessionId);
      }
      AppMethodBeat.o(196089);
      return 0;
    }
    if (paramInt == 1)
    {
      int i = g.a.a.b.b.a.bu(1, this.scene) + 0;
      paramInt = i;
      if (this.dJw != null) {
        paramInt = i + g.a.a.b.b.a.f(2, this.dJw);
      }
      i = paramInt;
      if (this.sessionId != null) {
        i = paramInt + g.a.a.b.b.a.f(3, this.sessionId);
      }
      AppMethodBeat.o(196089);
      return i;
    }
    if (paramInt == 2)
    {
      paramVarArgs = new g.a.a.a.a((byte[])paramVarArgs[0], unknownTagHandler);
      for (paramInt = com.tencent.mm.bw.a.getNextFieldNumber(paramVarArgs); paramInt > 0; paramInt = com.tencent.mm.bw.a.getNextFieldNumber(paramVarArgs)) {
        if (!super.populateBuilderWithField(paramVarArgs, this, paramInt)) {
          paramVarArgs.hPl();
        }
      }
      AppMethodBeat.o(196089);
      return 0;
    }
    if (paramInt == 3)
    {
      g.a.a.a.a locala = (g.a.a.a.a)paramVarArgs[0];
      azi localazi = (azi)paramVarArgs[1];
      switch (((Integer)paramVarArgs[2]).intValue())
      {
      default: 
        AppMethodBeat.o(196089);
        return -1;
      case 1: 
        localazi.scene = locala.UbS.zi();
        AppMethodBeat.o(196089);
        return 0;
      case 2: 
        localazi.dJw = locala.UbS.readString();
        AppMethodBeat.o(196089);
        return 0;
      }
      localazi.sessionId = locala.UbS.readString();
      AppMethodBeat.o(196089);
      return 0;
    }
    AppMethodBeat.o(196089);
    return -1;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes2.jar
 * Qualified Name:     com.tencent.mm.protocal.protobuf.azi
 * JD-Core Version:    0.7.0.1
 */