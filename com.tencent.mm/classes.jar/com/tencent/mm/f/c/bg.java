package com.tencent.mm.f.c;

import android.content.ContentValues;
import android.database.Cursor;
import com.tencent.mm.sdk.storage.IAutoDBItem;
import com.tencent.mm.sdk.storage.sql.Column;
import com.tencent.mm.sdk.storage.sql.SingleTable;

public abstract class bg
  extends IAutoDBItem
{
  public static final Column C_ID;
  public static final Column C_ROWID;
  public static final Column C_UPDATETIME;
  public static final String[] INDEX_CREATE = new String[0];
  public static final SingleTable TABLE = new SingleTable("DynamicMsgCacheData");
  public static final Column hFi;
  public static final Column hFj;
  private static final int hFm;
  private static final int hFn;
  public static final Column hnz;
  public static final Column hoE;
  private static final int hoM;
  private static final int hoh;
  private static final int id_HASHCODE;
  private static final int rowid_HASHCODE = "rowid".hashCode();
  private static final int updateTime_HASHCODE;
  private boolean __hadSetid = true;
  private boolean __hadSetupdateTime = true;
  public String field_appId;
  public String field_cacheKey;
  public String field_data;
  public String field_id;
  public int field_interval;
  public long field_updateTime;
  private boolean hFk = true;
  private boolean hFl = true;
  private boolean hnQ = true;
  private boolean hoI = true;
  
  static
  {
    C_ROWID = new Column("rowid", "long", "DynamicMsgCacheData", "");
    C_ID = new Column("id", "string", "DynamicMsgCacheData", "");
    hFi = new Column("cachekey", "string", "DynamicMsgCacheData", "");
    hnz = new Column("appid", "string", "DynamicMsgCacheData", "");
    hoE = new Column("data", "string", "DynamicMsgCacheData", "");
    hFj = new Column("interval", "int", "DynamicMsgCacheData", "");
    C_UPDATETIME = new Column("updatetime", "long", "DynamicMsgCacheData", "");
    id_HASHCODE = "id".hashCode();
    hFm = "cacheKey".hashCode();
    hoh = "appId".hashCode();
    hoM = "data".hashCode();
    hFn = "interval".hashCode();
    updateTime_HASHCODE = "updateTime".hashCode();
  }
  
  public void convertFrom(Cursor paramCursor)
  {
    String[] arrayOfString = paramCursor.getColumnNames();
    if (arrayOfString == null) {
      return;
    }
    int i = 0;
    int j = arrayOfString.length;
    label20:
    int k;
    if (i < j)
    {
      k = arrayOfString[i].hashCode();
      if (id_HASHCODE != k) {
        break label60;
      }
      this.field_id = paramCursor.getString(i);
    }
    for (;;)
    {
      i += 1;
      break label20;
      break;
      label60:
      if (hFm == k) {
        this.field_cacheKey = paramCursor.getString(i);
      } else if (hoh == k) {
        this.field_appId = paramCursor.getString(i);
      } else if (hoM == k) {
        this.field_data = paramCursor.getString(i);
      } else if (hFn == k) {
        this.field_interval = paramCursor.getInt(i);
      } else if (updateTime_HASHCODE == k) {
        this.field_updateTime = paramCursor.getLong(i);
      } else if (rowid_HASHCODE == k) {
        this.systemRowid = paramCursor.getLong(i);
      }
    }
  }
  
  public ContentValues convertTo()
  {
    ContentValues localContentValues = new ContentValues();
    if (this.__hadSetid) {
      localContentValues.put("id", this.field_id);
    }
    if (this.hFk) {
      localContentValues.put("cacheKey", this.field_cacheKey);
    }
    if (this.hnQ) {
      localContentValues.put("appId", this.field_appId);
    }
    if (this.hoI) {
      localContentValues.put("data", this.field_data);
    }
    if (this.hFl) {
      localContentValues.put("interval", Integer.valueOf(this.field_interval));
    }
    if (this.__hadSetupdateTime) {
      localContentValues.put("updateTime", Long.valueOf(this.field_updateTime));
    }
    if (this.systemRowid > 0L) {
      localContentValues.put("rowid", Long.valueOf(this.systemRowid));
    }
    return localContentValues;
  }
  
  public String[] getIndexCreateSQL()
  {
    return INDEX_CREATE;
  }
  
  public String getTableName()
  {
    return "DynamicMsgCacheData";
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes.jar
 * Qualified Name:     com.tencent.mm.f.c.bg
 * JD-Core Version:    0.7.0.1
 */