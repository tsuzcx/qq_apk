package com.tencent.mm.ay;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.g.a.lv;
import com.tencent.mm.g.a.lv.b;
import com.tencent.mm.sdk.b.b;
import com.tencent.mm.sdk.platformtools.ap;
import java.util.List;

public final class a
{
  public static final void a(f paramf)
  {
    AppMethodBeat.i(136995);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136988);
        lv locallv = new lv();
        locallv.dnO.action = 6;
        locallv.dnO.dnQ = this.hNJ;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136988);
      }
    });
    AppMethodBeat.o(136995);
  }
  
  public static final void aGr()
  {
    AppMethodBeat.i(136991);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136981);
        lv locallv = new lv();
        locallv.dnO.action = 1;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136981);
      }
    });
    AppMethodBeat.o(136991);
  }
  
  public static final void aGs()
  {
    AppMethodBeat.i(136992);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136983);
        lv locallv = new lv();
        locallv.dnO.action = 2;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136983);
      }
    });
    AppMethodBeat.o(136992);
  }
  
  public static final void aGt()
  {
    AppMethodBeat.i(136993);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136984);
        lv locallv = new lv();
        locallv.dnO.action = -1;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136984);
      }
    });
    AppMethodBeat.o(136993);
  }
  
  public static final void aGu()
  {
    AppMethodBeat.i(136994);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136987);
        lv locallv = new lv();
        locallv.dnO.action = 11;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136987);
      }
    });
    AppMethodBeat.o(136994);
  }
  
  public static boolean aGv()
  {
    AppMethodBeat.i(136996);
    lv locallv = new lv();
    locallv.dnO.action = -3;
    com.tencent.mm.sdk.b.a.GpY.l(locallv);
    boolean bool = locallv.dnP.result;
    AppMethodBeat.o(136996);
    return bool;
  }
  
  public static boolean aGw()
  {
    AppMethodBeat.i(136997);
    lv locallv = new lv();
    locallv.dnO.action = 10;
    com.tencent.mm.sdk.b.a.GpY.l(locallv);
    boolean bool = locallv.dnP.result;
    AppMethodBeat.o(136997);
    return bool;
  }
  
  public static boolean aGx()
  {
    AppMethodBeat.i(136998);
    lv locallv = new lv();
    locallv.dnO.action = 9;
    com.tencent.mm.sdk.b.a.GpY.l(locallv);
    boolean bool = locallv.dnP.result;
    AppMethodBeat.o(136998);
    return bool;
  }
  
  public static f aGy()
  {
    AppMethodBeat.i(136999);
    Object localObject = new lv();
    ((lv)localObject).dnO.action = -2;
    com.tencent.mm.sdk.b.a.GpY.l((b)localObject);
    localObject = ((lv)localObject).dnP.dnQ;
    AppMethodBeat.o(136999);
    return localObject;
  }
  
  public static c aGz()
  {
    AppMethodBeat.i(137003);
    Object localObject = new lv();
    ((lv)localObject).dnO.action = 8;
    com.tencent.mm.sdk.b.a.GpY.l((b)localObject);
    localObject = ((lv)localObject).dnP.dnU;
    AppMethodBeat.o(137003);
    return localObject;
  }
  
  public static void b(f paramf)
  {
    AppMethodBeat.i(191306);
    lv locallv = new lv();
    locallv.dnO.action = 16;
    locallv.dnO.dnQ = paramf;
    com.tencent.mm.sdk.b.a.GpY.l(locallv);
    AppMethodBeat.o(191306);
  }
  
  public static void c(f paramf)
  {
    AppMethodBeat.i(137000);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136989);
        lv locallv = new lv();
        locallv.dnO.action = 0;
        locallv.dnO.dnQ = this.hNF;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136989);
      }
    });
    AppMethodBeat.o(137000);
  }
  
  public static void d(f paramf)
  {
    AppMethodBeat.i(137001);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136982);
        lv locallv = new lv();
        locallv.dnO.action = 3;
        locallv.dnO.dnQ = this.hNF;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136982);
      }
    });
    AppMethodBeat.o(137001);
  }
  
  public static boolean e(f paramf)
  {
    if (paramf == null) {
      return false;
    }
    switch (paramf.hNQ)
    {
    default: 
      return false;
    }
    return true;
  }
  
  public static void f(List<f> paramList, final int paramInt)
  {
    AppMethodBeat.i(182547);
    ap.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(136990);
        lv locallv = new lv();
        locallv.dnO.action = 4;
        locallv.dnO.dhL = this.hNK;
        locallv.dnO.dnR = paramInt;
        com.tencent.mm.sdk.b.a.GpY.l(locallv);
        AppMethodBeat.o(136990);
      }
    });
    AppMethodBeat.o(182547);
  }
  
  public static boolean pl(int paramInt)
  {
    AppMethodBeat.i(137002);
    lv locallv = new lv();
    locallv.dnO.action = 7;
    locallv.dnO.position = paramInt;
    com.tencent.mm.sdk.b.a.GpY.l(locallv);
    boolean bool = locallv.dnP.result;
    AppMethodBeat.o(137002);
    return bool;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes4.jar
 * Qualified Name:     com.tencent.mm.ay.a
 * JD-Core Version:    0.7.0.1
 */