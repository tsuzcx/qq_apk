package com.tencent.mm.view.b;

import android.animation.PropertyValuesHolder;
import android.animation.ValueAnimator;
import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.Canvas;
import android.graphics.Matrix;
import android.graphics.PointF;
import android.graphics.PorterDuff.Mode;
import android.graphics.Rect;
import android.graphics.RectF;
import android.view.MotionEvent;
import android.view.animation.LinearInterpolator;
import com.tencent.mm.b.d;
import com.tencent.mm.b.d.1;
import com.tencent.mm.b.d.2;
import com.tencent.mm.sdk.platformtools.y;

public final class b
  extends a
{
  private PointF wxA = new PointF();
  private float wxB = 0.0F;
  private float wxC = 0.0F;
  private boolean wxD = false;
  public boolean wxE = false;
  private d wxz = new d(this);
  
  public b(Context paramContext, com.tencent.mm.bt.b paramb)
  {
    super(paramContext, paramb);
  }
  
  private void translate(float paramFloat1, float paramFloat2)
  {
    RectF localRectF = getCurImageRect();
    float f2 = paramFloat1 - this.wxA.x;
    float f1 = paramFloat2 - this.wxA.y;
    if (f2 > 0.0F) {
      if (getBoardRect().left <= localRectF.left)
      {
        paramFloat1 = f2 * 0.5F;
        if (f1 <= 0.0F) {
          break label261;
        }
        if (getBoardRect().top > localRectF.top) {
          break label216;
        }
        paramFloat2 = f1 * 0.5F;
      }
    }
    for (;;)
    {
      getMainMatrix().postTranslate(paramFloat1, paramFloat2);
      return;
      paramFloat1 = f2;
      if (Math.abs(f2) <= Math.abs(localRectF.left - getBoardRect().left)) {
        break;
      }
      paramFloat1 = getBoardRect().left - localRectF.left;
      break;
      if (getBoardRect().right >= localRectF.right)
      {
        paramFloat1 = f2 * 0.5F;
        break;
      }
      paramFloat1 = f2;
      if (Math.abs(f2) <= Math.abs(getBoardRect().right - localRectF.right)) {
        break;
      }
      paramFloat1 = getBoardRect().right - localRectF.right;
      break;
      label216:
      paramFloat2 = f1;
      if (Math.abs(f1) > Math.abs(localRectF.top - getBoardRect().top))
      {
        paramFloat2 = getBoardRect().top - localRectF.top;
        continue;
        label261:
        if (getBoardRect().bottom >= localRectF.bottom)
        {
          paramFloat2 = f1 * 0.5F;
        }
        else
        {
          paramFloat2 = f1;
          if (Math.abs(f1) > Math.abs(getBoardRect().bottom - localRectF.bottom)) {
            paramFloat2 = getBoardRect().bottom - localRectF.bottom;
          }
        }
      }
    }
  }
  
  protected final void P(MotionEvent paramMotionEvent)
  {
    float f4 = paramMotionEvent.getX(paramMotionEvent.getPointerCount() - 1);
    float f5 = paramMotionEvent.getY(paramMotionEvent.getPointerCount() - 1);
    RectF localRectF;
    Rect localRect;
    float f2;
    float f1;
    float f6;
    float f7;
    float f3;
    label308:
    int i;
    label326:
    int j;
    switch (paramMotionEvent.getActionMasked())
    {
    case 3: 
    case 4: 
    default: 
    case 0: 
    case 1: 
      do
      {
        do
        {
          for (;;)
          {
            this.wxA.x = f4;
            this.wxA.y = f5;
            return;
            this.wxA.set(f4, f5);
            this.wxB = 0.0F;
            this.wxC = getCurScale();
            paramMotionEvent = this.wxz;
            y.d("MicroMsg.StickRoundAnim", "[cancel]");
            if ((paramMotionEvent.jQ != null) && ((paramMotionEvent.jQ.isRunning()) || (paramMotionEvent.jQ.isStarted()))) {
              paramMotionEvent.jQ.cancel();
            }
            this.wxE = false;
          }
          this.wxB = 0.0F;
        } while (!this.wxE);
        paramMotionEvent = this.wxz;
      } while (!paramMotionEvent.bur);
      y.d("MicroMsg.StickRoundAnim", "[play] start");
      localRectF = paramMotionEvent.buz.getCurImageRect();
      localRect = paramMotionEvent.buz.getBoardRect();
      paramMotionEvent.buA = localRectF.centerX();
      paramMotionEvent.buB = localRectF.centerY();
      f2 = localRect.centerY() - localRectF.centerY();
      f1 = localRect.centerX() - localRectF.centerX();
      f6 = paramMotionEvent.buz.getCurScale();
      f7 = paramMotionEvent.buz.getMaxScale();
      f3 = paramMotionEvent.buz.getMinScale();
      if (f6 > f7)
      {
        paramMotionEvent.mScale = f7;
        paramMotionEvent.buC = true;
        if (localRect.height() >= localRectF.height()) {
          break label631;
        }
        i = 1;
        if (localRect.width() >= localRectF.width()) {
          break label637;
        }
        j = 1;
        label344:
        if (i != 0) {
          f2 = 0.0F;
        }
        if (j != 0) {
          f1 = 0.0F;
        }
        if ((localRectF.top <= localRect.top) || (i == 0)) {
          break label643;
        }
        f2 = localRect.top - localRectF.top;
      }
      break;
    }
    label391:
    label425:
    label722:
    label982:
    for (;;)
    {
      if ((localRectF.left > localRect.left) && (j != 0))
      {
        f3 = localRect.left - localRectF.left;
        y.d("MicroMsg.StickRoundAnim", "%s %s ", new Object[] { Float.valueOf(f3), Float.valueOf(f2) });
        if (f6 <= f7) {
          break label722;
        }
      }
      for (paramMotionEvent.buD = true;; paramMotionEvent.buD = false)
      {
        paramMotionEvent.jQ = ValueAnimator.ofPropertyValuesHolder(new PropertyValuesHolder[] { PropertyValuesHolder.ofFloat("deltaY", new float[] { 0.0F, f2 }), PropertyValuesHolder.ofFloat("deltaX", new float[] { 0.0F, f3 }), PropertyValuesHolder.ofFloat("scale", new float[] { 0.0F, 80.0F }) });
        paramMotionEvent.jQ.addUpdateListener(new d.1(paramMotionEvent, f6));
        paramMotionEvent.jQ.addListener(new d.2(paramMotionEvent));
        paramMotionEvent.jQ.setInterpolator(new LinearInterpolator());
        paramMotionEvent.jQ.setDuration(80L);
        paramMotionEvent.jQ.start();
        break;
        if (f6 < f3)
        {
          paramMotionEvent.mScale = f3;
          paramMotionEvent.buC = true;
          break label308;
        }
        paramMotionEvent.buC = false;
        break label308;
        label631:
        i = 0;
        break label326;
        label637:
        j = 0;
        break label344;
        if ((localRectF.bottom >= localRect.bottom) || (i == 0)) {
          break label982;
        }
        f2 = localRect.bottom - localRectF.bottom;
        break label391;
        f3 = f1;
        if (localRectF.right >= localRect.right) {
          break label425;
        }
        f3 = f1;
        if (j == 0) {
          break label425;
        }
        f3 = localRect.right - localRectF.right;
        break label425;
      }
      this.wxB = 0.0F;
      this.wxC = getCurScale();
      break;
      this.wxB = 0.0F;
      this.wxD = true;
      break;
      if (this.wxD)
      {
        this.wxD = false;
        return;
      }
      if (paramMotionEvent.getPointerCount() == 2)
      {
        this.wxE = true;
        f1 = paramMotionEvent.getX(0) - paramMotionEvent.getX(1);
        f2 = paramMotionEvent.getY(0) - paramMotionEvent.getY(1);
        f1 = (float)Math.sqrt(f1 * f1 + f2 * f2);
        if (this.wxB == 0.0F)
        {
          this.wxB = f1;
          break;
        }
        f2 = f1 / this.wxB * this.wxC;
        if (this.wxq * 0.5F <= f2)
        {
          f1 = f2;
          if (f2 > this.wxp)
          {
            f1 = this.wxp;
            f1 = (f2 - this.wxp) * 0.2F + f1;
          }
          getMainMatrix().postScale(f1 / getCurScale(), f1 / getCurScale(), f4, f5);
        }
        this.wxz.bur = true;
        translate(f4, f5);
        postInvalidate();
        break;
      }
      if ((paramMotionEvent.getPointerCount() != 1) || (!cLA())) {
        break;
      }
      this.wxE = true;
      this.wxz.bur = true;
      translate(f4, f5);
      postInvalidate();
      break;
    }
  }
  
  public final boolean cLB()
  {
    return true;
  }
  
  public final float getMinScale()
  {
    if (getPresenter().coI().tG() == com.tencent.mm.e.a.bzS)
    {
      float f1 = getBoardRect().width() / getCurImageRect().width();
      float f2 = getBoardRect().height() / getCurImageRect().height();
      if (f1 > f2) {}
      for (;;)
      {
        return f1 * getCurScale();
        f1 = f2;
      }
    }
    return super.getMinScale();
  }
  
  protected final void onDraw(Canvas paramCanvas)
  {
    paramCanvas.drawColor(0, PorterDuff.Mode.CLEAR);
    paramCanvas.setMatrix(getMainMatrix());
    paramCanvas.save();
    paramCanvas.clipRect(getAliveRect());
    Bitmap localBitmap = getPresenter().coJ();
    if ((localBitmap != null) && (!localBitmap.isRecycled())) {
      paramCanvas.drawBitmap(localBitmap, 0.0F, 0.0F, null);
    }
    paramCanvas.restore();
    getPresenter().onDraw(paramCanvas);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes7.jar
 * Qualified Name:     com.tencent.mm.view.b.b
 * JD-Core Version:    0.7.0.1
 */