...
{
  private final HashMap<c, Vector<Integer>> aaP;
  private b shX;
  ...
  private void a(Vector<Integer> paramVector, c paramc)
  {
    ...
    while ((paramVector == null) || (paramVector.length == 0))
    {
      ...
      i = 0;
      while (i < paramVector.shS.length)
      {
        ((StringBuilder)localObject).append(" " + paramVector.shS[i]);
        i += 1;
      }
      Log.d("MicroMsg.exdevice.GaussianFilter", ((StringBuilder)localObject).toString());
      d1 = i.x(paramVector.shS);
      d2 = i.y(paramVector.shS);
      d3 = i.h(0.15D, d2).add(new BigDecimal(d1)).doubleValue();
      double d4 = i.h(3.09D, d2).add(new BigDecimal(d1)).doubleValue();
      Log.d("MicroMsg.exdevice.GaussianFilter", "Mean = " + d1 + "SD = " + d2 + "LowerLimit = " + d3 + " UpperLimit = " + d4);
      i = 0;
      localObject = new int[paramVector.shS.length];
      int j = 0;
      while (j < paramVector.shS.length)
      {
        int k = i;
        if (paramVector.shS[j] >= d3)
        {
          k = i;
          if (paramVector.shS[j] <= d4)
          {
            localObject[i] = paramVector.shS[j];
            k = i + 1;
          }
        }
        j += 1;
    ...
    paramc.shV = new BigDecimal(d2).divide(new BigDecimal(1), 3, 4).doubleValue();
    double d3 = paramc.shU.shZ.siF;
  ...
  public final void a(int paramInt, String paramString, e parame)
  {
    ...
    localc.shT = null;
    localc.shU = parame;
    localc.shW = paramString;
    if (!this.aaP.containsKey(localc))
    {
      paramString = new Vector();
      paramString.add(Integer.valueOf(paramInt));
      this.aaP.put(localc, paramString);
      AppMethodBeat.o(22440);
      return;
    }
    paramString = (Vector)this.aaP.get(localc);
    paramString.add(Integer.valueOf(paramInt));
