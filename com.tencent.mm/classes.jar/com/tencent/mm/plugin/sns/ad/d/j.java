package com.tencent.mm.plugin.sns.ad.d;

import android.database.Cursor;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.plugin.expt.b.b;
import com.tencent.mm.plugin.expt.b.b.a;
import com.tencent.mm.plugin.sns.ad.i.d;
import com.tencent.mm.plugin.sns.data.l;
import com.tencent.mm.plugin.sns.model.aj;
import com.tencent.mm.plugin.sns.storage.c;
import com.tencent.mm.plugin.sns.storage.g;
import com.tencent.mm.plugin.sns.storage.i;
import com.tencent.mm.sdk.platformtools.BuildInfo;
import java.util.ArrayList;
import java.util.Collection;
import java.util.Iterator;
import java.util.List;
import org.json.JSONObject;

public final class j
{
  private static boolean Jzw = false;
  
  public static Cursor a(Cursor paramCursor, com.tencent.mm.storagebase.h paramh, String paramString)
  {
    AppMethodBeat.i(197678);
    Object localObject;
    label95:
    ArrayList localArrayList;
    long l1;
    label150:
    long l2;
    int i;
    if (paramCursor == null)
    {
      int j;
      int k;
      do
      {
        for (;;)
        {
          try
          {
            com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "the cursor is null!");
            AppMethodBeat.o(197678);
            return null;
          }
          catch (Throwable paramCursor)
          {
            com.tencent.mm.sdk.platformtools.Log.e("ConsecutiveAdHelper", "there is something wrong in removeConsecutiveAd: " + android.util.Log.getStackTraceString(paramCursor));
            l.a(l.JQS, "", 0, 0, "");
            paramCursor = f(paramh, paramString);
            AppMethodBeat.o(197678);
            return paramCursor;
          }
          if (!d.isEmpty((Collection)localObject)) {
            break label258;
          }
          paramCursor.moveToFirst();
          AppMethodBeat.o(197678);
          return paramCursor;
          j = paramCursor.getColumnIndex("snsId");
          k = paramCursor.getColumnIndex("sourceType");
          if ((j >= 0) && (k >= 0)) {
            break;
          }
          com.tencent.mm.sdk.platformtools.Log.w("ConsecutiveAdHelper", "the snsid index or source type index is invalid!");
          localObject = null;
        }
        localArrayList = new ArrayList();
        l1 = 0L;
        localObject = localArrayList;
      } while (!paramCursor.moveToNext());
      l2 = paramCursor.getLong(j);
      if ((paramCursor.getInt(k) & 0x20) <= 0) {
        break label341;
      }
      i = 1;
    }
    label258:
    label341:
    label344:
    for (;;)
    {
      if (l1 == 0L)
      {
        com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "there is first ad sns id is ".concat(String.valueOf(l2)));
        l1 = l2;
        break label150;
      }
      com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "there is consecutive ad sns id is ".concat(String.valueOf(l2)));
      localArrayList.add(new b(l1, l2));
      break label150;
      paramCursor.close();
      ht((List)localObject);
      com.tencent.mm.plugin.report.service.h.IzE.p(1612L, 30L, ((List)localObject).size());
      com.tencent.mm.plugin.sns.ad.i.j.a(new a((List)localObject));
      com.tencent.mm.sdk.platformtools.Log.d("ConsecutiveAdHelper", "it need to query again!!");
      paramCursor = f(paramh, paramString);
      AppMethodBeat.o(197678);
      return paramCursor;
      if (paramCursor != null) {
        break label95;
      }
      localObject = null;
      break;
      for (;;)
      {
        if (i != 0) {
          break label344;
        }
        l1 = 0L;
        break;
        i = 0;
      }
    }
  }
  
  private static Cursor f(com.tencent.mm.storagebase.h paramh, String paramString)
  {
    AppMethodBeat.i(197686);
    if (paramh != null)
    {
      paramh = paramh.rawQuery(paramString, null);
      AppMethodBeat.o(197686);
      return paramh;
    }
    AppMethodBeat.o(197686);
    return null;
  }
  
  public static void fJW()
  {
    Jzw = true;
  }
  
  public static boolean fJX()
  {
    AppMethodBeat.i(197656);
    if ((BuildInfo.DEBUG) || (BuildInfo.IS_FLAVOR_RED) || (BuildInfo.IS_FLAVOR_PURPLE))
    {
      com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "this is debug mode， isFilterValueInDebug is " + Jzw);
      boolean bool = Jzw;
      AppMethodBeat.o(197656);
      return bool;
    }
    AppMethodBeat.o(197656);
    return true;
  }
  
  public static boolean fJY()
  {
    AppMethodBeat.i(197657);
    try
    {
      int i = ((b)com.tencent.mm.kernel.h.ae(b.class)).a(b.a.vvj, 0);
      com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "isConsecutiveFilter exptValue =".concat(String.valueOf(i)));
      if (i > 0)
      {
        AppMethodBeat.o(197657);
        return true;
      }
      AppMethodBeat.o(197657);
      return false;
    }
    catch (Throwable localThrowable)
    {
      AppMethodBeat.o(197657);
    }
    return true;
  }
  
  private static void ht(List<b> paramList)
  {
    AppMethodBeat.i(197684);
    if (d.isEmpty(paramList))
    {
      AppMethodBeat.o(197684);
      return;
    }
    paramList = paramList.iterator();
    while (paramList.hasNext())
    {
      long l = ((b)paramList.next()).fWg;
      if (l != 0L)
      {
        com.tencent.mm.sdk.platformtools.Log.i("ConsecutiveAdHelper", "delete sns id ".concat(String.valueOf(l)));
        aj.fOL().delete(l);
        aj.fON().Rb(l);
        g.Ra(l);
      }
    }
    AppMethodBeat.o(197684);
  }
  
  static final class a
    extends com.tencent.mm.plugin.sns.ad.i.j.b
  {
    final List<j.b> Jzx;
    
    a(List<j.b> paramList)
    {
      this.Jzx = paramList;
    }
    
    public final void bw(JSONObject paramJSONObject)
    {
      AppMethodBeat.i(255519);
      try
      {
        paramJSONObject.putOpt("snsId", "");
        paramJSONObject.putOpt("uxinfo", "");
        paramJSONObject.putOpt("adExtInfo", "");
        paramJSONObject.putOpt("scene", Integer.valueOf(0));
        AppMethodBeat.o(255519);
        return;
      }
      catch (Throwable paramJSONObject)
      {
        AppMethodBeat.o(255519);
      }
    }
    
    /* Error */
    public final void bx(JSONObject paramJSONObject)
    {
      // Byte code:
      //   0: ldc 58
      //   2: invokestatic 31	com/tencent/matrix/trace/core/AppMethodBeat:i	(I)V
      //   5: aload_0
      //   6: getfield 17	com/tencent/mm/plugin/sns/ad/d/j$a:Jzx	Ljava/util/List;
      //   9: invokestatic 64	com/tencent/mm/plugin/sns/ad/i/d:isEmpty	(Ljava/util/Collection;)Z
      //   12: istore_3
      //   13: iload_3
      //   14: ifeq +9 -> 23
      //   17: ldc 58
      //   19: invokestatic 56	com/tencent/matrix/trace/core/AppMethodBeat:o	(I)V
      //   22: return
      //   23: new 66	org/json/JSONArray
      //   26: dup
      //   27: invokespecial 67	org/json/JSONArray:<init>	()V
      //   30: astore 4
      //   32: aload_1
      //   33: ldc 69
      //   35: aload_0
      //   36: getfield 17	com/tencent/mm/plugin/sns/ad/d/j$a:Jzx	Ljava/util/List;
      //   39: invokeinterface 75 1 0
      //   44: invokestatic 53	java/lang/Integer:valueOf	(I)Ljava/lang/Integer;
      //   47: invokevirtual 41	org/json/JSONObject:putOpt	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
      //   50: pop
      //   51: aload_1
      //   52: ldc 77
      //   54: aload 4
      //   56: invokevirtual 41	org/json/JSONObject:putOpt	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
      //   59: pop
      //   60: aload_0
      //   61: getfield 17	com/tencent/mm/plugin/sns/ad/d/j$a:Jzx	Ljava/util/List;
      //   64: invokeinterface 81 1 0
      //   69: astore_1
      //   70: iconst_0
      //   71: istore_2
      //   72: aload_1
      //   73: invokeinterface 87 1 0
      //   78: ifeq +78 -> 156
      //   81: aload_1
      //   82: invokeinterface 91 1 0
      //   87: checkcast 93	com/tencent/mm/plugin/sns/ad/d/j$b
      //   90: astore 5
      //   92: aload 5
      //   94: ifnull +52 -> 146
      //   97: new 37	org/json/JSONObject
      //   100: dup
      //   101: invokespecial 94	org/json/JSONObject:<init>	()V
      //   104: astore 6
      //   106: aload 6
      //   108: ldc 33
      //   110: aload 5
      //   112: getfield 98	com/tencent/mm/plugin/sns/ad/d/j$b:fWg	J
      //   115: invokestatic 104	com/tencent/mm/plugin/sns/data/t:Qu	(J)Ljava/lang/String;
      //   118: invokevirtual 41	org/json/JSONObject:putOpt	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
      //   121: pop
      //   122: aload 6
      //   124: ldc 106
      //   126: aload 5
      //   128: getfield 109	com/tencent/mm/plugin/sns/ad/d/j$b:Jzy	J
      //   131: invokestatic 104	com/tencent/mm/plugin/sns/data/t:Qu	(J)Ljava/lang/String;
      //   134: invokevirtual 41	org/json/JSONObject:putOpt	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
      //   137: pop
      //   138: aload 4
      //   140: aload 6
      //   142: invokevirtual 113	org/json/JSONArray:put	(Ljava/lang/Object;)Lorg/json/JSONArray;
      //   145: pop
      //   146: iload_2
      //   147: iconst_1
      //   148: iadd
      //   149: istore_2
      //   150: iload_2
      //   151: bipush 10
      //   153: if_icmplt +21 -> 174
      //   156: ldc 58
      //   158: invokestatic 56	com/tencent/matrix/trace/core/AppMethodBeat:o	(I)V
      //   161: return
      //   162: astore_1
      //   163: ldc 58
      //   165: invokestatic 56	com/tencent/matrix/trace/core/AppMethodBeat:o	(I)V
      //   168: return
      //   169: astore 5
      //   171: goto -25 -> 146
      //   174: goto -102 -> 72
      // Local variable table:
      //   start	length	slot	name	signature
      //   0	177	0	this	a
      //   0	177	1	paramJSONObject	JSONObject
      //   71	83	2	i	int
      //   12	2	3	bool	boolean
      //   30	109	4	localJSONArray	org.json.JSONArray
      //   90	37	5	localb	j.b
      //   169	1	5	localThrowable	Throwable
      //   104	37	6	localJSONObject	JSONObject
      // Exception table:
      //   from	to	target	type
      //   5	13	162	java/lang/Throwable
      //   23	70	162	java/lang/Throwable
      //   72	92	162	java/lang/Throwable
      //   97	146	169	java/lang/Throwable
    }
    
    public final String fJK()
    {
      return "sns_ad_remove_consecutive_ad_report";
    }
  }
  
  static final class b
  {
    final long Jzy;
    final long fWg;
    
    b(long paramLong1, long paramLong2)
    {
      this.Jzy = paramLong1;
      this.fWg = paramLong2;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes4.jar
 * Qualified Name:     com.tencent.mm.plugin.sns.ad.d.j
 * JD-Core Version:    0.7.0.1
 */