package com.tencent.mm.plugin.sns.k;

import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.plugin.report.service.h;
import com.tencent.mm.sdk.platformtools.Log;
import com.tencent.mm.sdk.platformtools.Util;
import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

public final class c
{
  private static Map<String, a> DUD;
  private static volatile long DUE;
  
  static
  {
    AppMethodBeat.i(96202);
    DUD = new ConcurrentHashMap();
    DUE = 0L;
    AppMethodBeat.o(96202);
  }
  
  public static void a(String paramString1, String paramString2, int paramInt1, String paramString3, int paramInt2, int paramInt3, int paramInt4, long paramLong1, long paramLong2)
  {
    AppMethodBeat.i(96200);
    try
    {
      a locala = new a((byte)0);
      locala.DUF = aPP(paramString2);
      locala.DUG = paramInt1;
      locala.mMimeType = paramString3;
      locala.mWidth = paramInt2;
      locala.mHeight = paramInt3;
      locala.DUH = paramInt4;
      locala.baM = paramLong1;
      locala.DUI = paramLong2;
      DUD.put(paramString1, locala);
      AppMethodBeat.o(96200);
      return;
    }
    catch (Exception paramString1)
    {
      AppMethodBeat.o(96200);
    }
  }
  
  private static String aPP(String paramString)
  {
    AppMethodBeat.i(96198);
    int i = paramString.indexOf('?');
    String str = paramString;
    if (i >= 0) {
      str = paramString.substring(0, i);
    }
    i = str.indexOf("://");
    paramString = str;
    if (i >= 0) {
      paramString = str.substring(i + 3);
    }
    i = paramString.indexOf('/');
    if (i >= 0)
    {
      paramString = paramString.substring(i + 1);
      AppMethodBeat.o(96198);
      return paramString;
    }
    AppMethodBeat.o(96198);
    return paramString;
  }
  
  public static void bn(String paramString, long paramLong)
  {
    AppMethodBeat.i(96201);
    try
    {
      paramString = (a)DUD.remove(paramString);
      if (paramString == null) {
        return;
      }
      paramString.DUJ = paramLong;
      StringBuilder localStringBuilder = new StringBuilder(1024);
      localStringBuilder.append(paramString.DUF).append(',').append(paramString.DUG).append(',').append(paramString.mMimeType).append(',').append(paramString.mWidth).append(',').append(paramString.mHeight).append(',').append(paramString.DUH).append(',').append(paramString.baM).append(',').append(paramString.DUI).append(',').append(paramString.DUJ);
      paramString = localStringBuilder.toString();
      Log.i("MicroMsg.SnsImgStats", "report dl: ".concat(String.valueOf(paramString)));
      h.CyF.kvStat(13513, paramString);
      return;
    }
    catch (Exception paramString) {}finally
    {
      fcr();
      AppMethodBeat.o(96201);
    }
  }
  
  public static void c(String paramString1, String paramString2, int paramInt1, int paramInt2, int paramInt3, long paramLong)
  {
    AppMethodBeat.i(96199);
    try
    {
      b localb = new b((byte)0);
      localb.DUF = aPP(paramString1);
      localb.mMimeType = paramString2;
      localb.mWidth = paramInt1;
      localb.mHeight = paramInt2;
      localb.DUH = paramInt3;
      localb.baM = paramLong;
      paramString1 = new StringBuilder(1024);
      paramString1.append(localb.DUF).append(',').append(localb.mMimeType).append(',').append(localb.mWidth).append(',').append(localb.mHeight).append(',').append(localb.DUH).append(',').append(localb.baM);
      paramString1 = paramString1.toString();
      Log.i("MicroMsg.SnsImgStats", "report up: ".concat(String.valueOf(paramString1)));
      h.CyF.kvStat(13512, paramString1);
      AppMethodBeat.o(96199);
      return;
    }
    catch (Exception paramString1)
    {
      AppMethodBeat.o(96199);
    }
  }
  
  private static void fcr()
  {
    AppMethodBeat.i(96197);
    synchronized (DUD)
    {
      if (Util.ticksToNow(DUE) <= 120000L)
      {
        AppMethodBeat.o(96197);
        return;
      }
      DUE = Util.currentTicks();
      DUD.clear();
      AppMethodBeat.o(96197);
      return;
    }
  }
  
  static final class a
  {
    String DUF = "";
    int DUG = 0;
    int DUH = -1;
    long DUI = -1L;
    long DUJ = -1L;
    long baM = -1L;
    int mHeight = -1;
    String mMimeType = "";
    int mWidth = -1;
  }
  
  static final class b
  {
    String DUF = "";
    int DUH = -1;
    long baM = -1L;
    int mHeight = -1;
    String mMimeType = "";
    int mWidth = -1;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes3.jar
 * Qualified Name:     com.tencent.mm.plugin.sns.k.c
 * JD-Core Version:    0.7.0.1
 */