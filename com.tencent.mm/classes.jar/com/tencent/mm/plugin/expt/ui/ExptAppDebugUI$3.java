package com.tencent.mm.plugin.expt.ui;

import android.view.View;
import android.view.View.OnClickListener;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.hellhoundlib.b.b;
import com.tencent.mm.sdk.platformtools.bt;
import com.tencent.mm.ui.widget.picker.c;
import com.tencent.mm.ui.widget.picker.c.a;
import java.util.ArrayList;

final class ExptAppDebugUI$3
  implements View.OnClickListener
{
  ExptAppDebugUI$3(ExptAppDebugUI paramExptAppDebugUI) {}
  
  public final void onClick(final View paramView)
  {
    AppMethodBeat.i(210545);
    Object localObject = new b();
    ((b)localObject).bd(paramView);
    com.tencent.mm.hellhoundlib.a.a.b("com/tencent/mm/plugin/expt/ui/ExptAppDebugUI$3", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V", this, ((b)localObject).ahq());
    com.tencent.mm.plugin.expt.f.a.cqZ();
    localObject = com.tencent.mm.plugin.expt.f.a.cre();
    if (((ArrayList)localObject).size() > 0)
    {
      paramView = new String[((ArrayList)localObject).size()];
      int i = 0;
      while (i < paramView.length)
      {
        paramView[i] = ((ArrayList)localObject).get(i);
        i += 1;
      }
      localObject = new c(this.rbX.getContext(), paramView);
      ((c)localObject).afD(0);
      ((c)localObject).LaQ = new c.a()
      {
        public final void onResult(boolean paramAnonymousBoolean, Object paramAnonymousObject1, Object paramAnonymousObject2)
        {
          AppMethodBeat.i(210544);
          this.nqO.hide();
          int i = this.nqO.fMM();
          paramAnonymousObject1 = paramView[i];
          ExptAppDebugUI.a(ExptAppDebugUI.3.this.rbX, bt.getInt(paramAnonymousObject1, 0));
          AppMethodBeat.o(210544);
        }
      };
      ((c)localObject).show();
    }
    com.tencent.mm.hellhoundlib.a.a.a(this, "com/tencent/mm/plugin/expt/ui/ExptAppDebugUI$3", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V");
    AppMethodBeat.o(210545);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes4.jar
 * Qualified Name:     com.tencent.mm.plugin.expt.ui.ExptAppDebugUI.3
 * JD-Core Version:    0.7.0.1
 */