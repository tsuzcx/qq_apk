...
{
  private final HashMap<c, Vector<Integer>> mMap;
  private b pfO;
  ...
  private void a(Vector<Integer> paramVector, c paramc)
  {
    ...
    while ((paramVector == null) || (paramVector.length == 0))
    {
      ...
      i = 0;
      while (i < paramVector.pfK.length)
      {
        ((StringBuilder)localObject).append(" " + paramVector.pfK[i]);
        i += 1;
      }
      Log.d("MicroMsg.exdevice.GaussianFilter", ((StringBuilder)localObject).toString());
      d1 = i.x(paramVector.pfK);
      d2 = i.y(paramVector.pfK);
      ...
      i = 0;
      localObject = new int[paramVector.pfK.length];
      int j = 0;
      while (j < paramVector.pfK.length)
      {
        int k = i;
        if (paramVector.pfK[j] >= d3)
        {
          k = i;
          if (paramVector.pfK[j] <= d4)
          {
            localObject[i] = paramVector.pfK[j];
            k = i + 1;
          }
        }
        j += 1;
    ...
    double d2 = i.x(paramVector);
    paramc.pfN = new BigDecimal(d2).divide(new BigDecimal(1), 3, 4).doubleValue();
    double d3 = paramc.pfM.pfQ.pgu;
  ...
  public final void a(int paramInt, String paramString, e parame)
  {
    ...
    localc.pfL = null;
    localc.pfM = parame;
    localc.dfJ = paramString;
    if (!this.mMap.containsKey(localc))
    {
      paramString = new Vector();
      paramString.add(Integer.valueOf(paramInt));
      this.mMap.put(localc, paramString);
      AppMethodBeat.o(22440);
      return;
    }
    paramString = (Vector)this.mMap.get(localc);
    paramString.add(Integer.valueOf(paramInt));
