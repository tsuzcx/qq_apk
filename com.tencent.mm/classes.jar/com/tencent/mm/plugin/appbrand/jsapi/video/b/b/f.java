  ...
  extends Reader
{
  private e koR;
  private boolean koS;
  private CharsetDecoder koT;
  private final ByteBuffer koU;
  ...
  public f(e parame, String paramString)
  {
    ...
    this.koS = false;
    this.koU = ByteBuffer.allocate(8192);
    this.koR = parame;
    try
    {
      this.koT = Charset.forName(paramString).newDecoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE);
      this.koU.limit(0);
      ...
      for (;;)
      {
        try
        {
          ...
          if (paramArrayOfChar == CoderResult.UNDERFLOW)
          {
            ...
            if (this.koS)
            {
              localObject1 = this.koT.decode(this.koU, localCharBuffer, true);
              this.koT.flush(localCharBuffer);
              this.koT.reset();
            }
          }
        ...
        if (!this.koS)
        {
          paramInt2 = this.koU.capacity();
          i = this.koU.limit();
          int j = this.koU.arrayOffset();
          int k = this.koU.limit();
          paramInt2 = this.koR.read(this.koU.array(), j + k, paramInt2 - i);
          if (paramInt2 == -1)
          {
            this.koS = true;
          }
          ...
          {
            this.koU.limit(paramInt2 + this.koU.limit());
            localCoderResult = this.koT.decode(this.koU, localCharBuffer, false);
            paramArrayOfChar = localCoderResult;
            if (localCoderResult.isUnderflow())
            {
              if (this.koU.limit() != this.koU.capacity()) {
                break label509;
              }
              this.koU.compact();
              this.koU.limit(this.koU.position());
              this.koU.position(0);
              break label509;
              label490:
              paramInt2 = localCharBuffer.position();
