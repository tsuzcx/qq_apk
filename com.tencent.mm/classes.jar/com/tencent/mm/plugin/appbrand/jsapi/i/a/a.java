package com.tencent.mm.plugin.appbrand.jsapi.i.a;

import android.animation.Animator;
import android.animation.Animator.AnimatorListener;
import android.animation.AnimatorSet;
import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.Matrix;
import android.graphics.Point;
import android.graphics.SurfaceTexture;
import android.os.Bundle;
import android.os.Looper;
import android.text.TextUtils;
import android.view.LayoutInflater;
import android.view.MotionEvent;
import android.view.SurfaceView;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.view.animation.Animation;
import android.widget.FrameLayout;
import android.widget.FrameLayout.LayoutParams;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import com.tencent.luggage.a.e;
import com.tencent.mapsdk.raster.model.BitmapDescriptor;
import com.tencent.mapsdk.raster.model.CameraPosition;
import com.tencent.mapsdk.raster.model.CameraPosition.Builder;
import com.tencent.mapsdk.raster.model.Circle;
import com.tencent.mapsdk.raster.model.CircleOptions;
import com.tencent.mapsdk.raster.model.CustomLayer;
import com.tencent.mapsdk.raster.model.CustomLayerOptions;
import com.tencent.mapsdk.raster.model.IndoorBuilding;
import com.tencent.mapsdk.raster.model.IndoorInfo;
import com.tencent.mapsdk.raster.model.IndoorLevel;
import com.tencent.mapsdk.raster.model.IndoorMapPoi;
import com.tencent.mapsdk.raster.model.LatLngBounds.Builder;
import com.tencent.mapsdk.raster.model.MapPoi;
import com.tencent.mapsdk.raster.model.Marker;
import com.tencent.mapsdk.raster.model.MarkerOptions;
import com.tencent.mapsdk.raster.model.OverlayLevel;
import com.tencent.mapsdk.raster.model.Polygon;
import com.tencent.mapsdk.raster.model.PolygonOptions;
import com.tencent.mapsdk.raster.model.Polyline;
import com.tencent.mapsdk.raster.model.PolylineOptions;
import com.tencent.mapsdk.raster.model.VisibleRegion;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.plugin.appbrand.widget.d.c;
import com.tencent.mm.plugin.appbrand.y.m;
import com.tencent.mm.plugin.location_soso.api.SoSoMapView;
import com.tencent.mm.sdk.platformtools.ae;
import com.tencent.mm.sdk.platformtools.ar;
import com.tencent.mm.sdk.platformtools.bu;
import com.tencent.mm.sdk.platformtools.h;
import com.tencent.tencentmap.mapsdk.map.CameraUpdate;
import com.tencent.tencentmap.mapsdk.map.CameraUpdateFactory;
import com.tencent.tencentmap.mapsdk.map.CancelableCallback;
import com.tencent.tencentmap.mapsdk.map.Projection;
import com.tencent.tencentmap.mapsdk.map.TencentMap;
import com.tencent.tencentmap.mapsdk.map.TencentMap.InfoWindowAdapter;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnIndoorStateChangeListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnInfoWindowClickListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnMapCameraChangeListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnMapClickListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnMapLoadedListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnMapPoiClickListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnMarkerClickListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.OnScreenShotListener;
import com.tencent.tencentmap.mapsdk.map.TencentMap.TencentMapGestureListener;
import com.tencent.tencentmap.mapsdk.map.TencentMapOptions;
import com.tencent.tencentmap.mapsdk.map.UiSettings;
import com.tencent.tencentmap.mapsdk.maps.model.BitmapDescriptorFactory;
import com.tencent.tencentmap.mapsdk.maps.model.GroundOverlay;
import com.tencent.tencentmap.mapsdk.maps.model.GroundOverlayOptions;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.animation.MarkerTranslateAnimator;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.Cluster;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.ClusterItem;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.ClusterManager;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.ClusterManager.OnClusterClickListener;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.ClusterManager.OnClusterItemClickListener;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.algo.Algorithm;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.algo.NonHierarchicalDistanceBasedAlgorithm;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.algo.PreCachingAlgorithmDecorator;
import com.tencent.tencentmap.mapsdk.vector.compat.utils.clustering.view.DefaultClusterRenderer;
import java.util.ArrayList;
import java.util.Collection;
import java.util.HashMap;
import java.util.Iterator;
import java.util.LinkedList;
import java.util.List;
import java.util.Map;
import java.util.Queue;
import java.util.Set;
import java.util.concurrent.ConcurrentHashMap;
import java.util.concurrent.atomic.AtomicBoolean;
import java.util.concurrent.atomic.AtomicInteger;

public class a
  implements b
{
  private static long kTU;
  private static final String kTx;
  FrameLayout container;
  Context context;
  private ImageView dtJ;
  private int kTA;
  private boolean kTB;
  private boolean kTC;
  private volatile HashMap<String, MarkerTranslateAnimator> kTD;
  private volatile HashMap<String, MarkerTranslateAnimator> kTE;
  private volatile ClusterManager<f> kTF;
  private DefaultClusterRenderer<f> kTG;
  private boolean kTH;
  SurfaceView kTI;
  private Runnable kTJ;
  private b.m kTK;
  private b.y kTL;
  private b.z kTM;
  private boolean kTN;
  private b.w kTO;
  private Runnable kTP;
  private b.x kTQ;
  private b.f kTR;
  private Map<String, CustomLayer> kTS;
  private ArrayList<String> kTT;
  private Queue<Runnable> kTV;
  private CancelableCallback kTW;
  public ArrayList<Circle> kTX;
  public ArrayList<b> kTY;
  private Map<String, Polyline> kTZ;
  private String kTq;
  protected final SoSoMapView kTr;
  private boolean kTs;
  private b.l kTt;
  private b.p kTu;
  private b.o kTv;
  private b.s kTw;
  private String kTy;
  private String kTz;
  private a kUa;
  private Map<String, g> kUb;
  public ArrayList<c> kUc;
  public ArrayList<Polygon> kUd;
  private boolean kUe;
  private d kUf;
  private com.tencent.mm.plugin.appbrand.utils.b.a.b kUg;
  private final int kUh;
  private AtomicBoolean kUi;
  private AtomicBoolean kUj;
  private b.q kUk;
  private ArrayList<f> kUl;
  private ArrayList<String> kUm;
  private ArrayList<Integer> kUn;
  private AtomicInteger kUo;
  private HashMap<Integer, Cluster<f>> kUp;
  private HashMap<String, b.v> kUq;
  private com.tencent.mm.plugin.appbrand.f.a kUr;
  boolean kUs;
  boolean kUt;
  private HashMap<String, GroundOverlay> kUu;
  private long lastCheckTime;
  private int mMapType;
  
  static
  {
    AppMethodBeat.i(146554);
    kTx = com.tencent.mm.plugin.appbrand.y.b.eu("TencentMapSubKey", "E6FBZ-OLSCQ-UIU5C-GWLJ7-ABUPT-V7FJX");
    kTU = 500L;
    AppMethodBeat.o(146554);
  }
  
  public a(Context paramContext, String paramString, Map<String, Object> paramMap)
  {
    AppMethodBeat.i(146490);
    this.kTD = new HashMap();
    this.kTE = new HashMap();
    this.kTH = false;
    this.kTJ = new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(146485);
        a.this.kTI.setVisibility(4);
        AppMethodBeat.o(146485);
      }
    };
    this.kTN = false;
    this.kTS = new ConcurrentHashMap();
    this.kTT = new ArrayList();
    this.kTV = new LinkedList();
    this.kTW = new CancelableCallback()
    {
      public final void onCancel()
      {
        AppMethodBeat.i(146463);
        a.s(a.this).poll();
        if (!a.s(a.this).isEmpty()) {
          ((Runnable)a.s(a.this).peek()).run();
        }
        AppMethodBeat.o(146463);
      }
      
      public final void onFinish()
      {
        AppMethodBeat.i(146462);
        a.s(a.this).poll();
        if (!a.s(a.this).isEmpty()) {
          ((Runnable)a.s(a.this).peek()).run();
        }
        AppMethodBeat.o(146462);
      }
    };
    this.kTX = new ArrayList();
    this.kTY = new ArrayList();
    this.kTZ = new ConcurrentHashMap();
    this.kUa = new a();
    this.kUb = new ConcurrentHashMap();
    this.kUc = new ArrayList();
    this.kUd = new ArrayList();
    this.kUg = new com.tencent.mm.plugin.appbrand.utils.b.a.b()
    {
      public final void a(int paramAnonymousInt, String paramAnonymousString, final com.tencent.mm.plugin.appbrand.utils.b.a.a paramAnonymousa)
      {
        AppMethodBeat.i(212553);
        if (paramAnonymousInt != 0)
        {
          ae.e("MicroMsg.DefaultTencentMapView", "errCode:%d, errStr:%s", new Object[] { Integer.valueOf(paramAnonymousInt), paramAnonymousString });
          AppMethodBeat.o(212553);
          return;
        }
        ae.d("MicroMsg.DefaultTencentMapView", "refresh location latitude = %f, longitude = %f", new Object[] { Double.valueOf(paramAnonymousa.latitude), Double.valueOf(paramAnonymousa.longitude) });
        if (!a.blO())
        {
          ae.w("MicroMsg.DefaultTencentMapView", "refresh location fail, no perrmission");
          AppMethodBeat.o(212553);
          return;
        }
        if ((!a.r(a.this)) || (a.x(a.this)))
        {
          AppMethodBeat.o(212553);
          return;
        }
        m.runOnUiThread(new Runnable()
        {
          public final void run()
          {
            AppMethodBeat.i(212552);
            d locald;
            double d1;
            double d2;
            if (a.this.kTr != null)
            {
              if (a.d(a.this) == null)
              {
                a.a(a.this, new d(a.u(a.this)));
                locald = a.d(a.this);
                localObject = a.this.kTr;
                com.tencent.mm.plugin.appbrand.utils.b.a.a locala = paramAnonymousa;
                MarkerOptions localMarkerOptions = new MarkerOptions();
                localMarkerOptions.anchor(0.5F, 0.5F);
                localMarkerOptions.position(new com.tencent.mapsdk.raster.model.LatLng(locala.latitude, locala.longitude));
                localMarkerOptions.markerView(locald);
                localMarkerOptions.zIndex(2147483647);
                localMarkerOptions.flat(true);
                localMarkerOptions.tag("AnchorPoint");
                locald.kVc = ((SoSoMapView)localObject).getMap().addMarker(localMarkerOptions);
              }
              locald = a.d(a.this);
              d1 = paramAnonymousa.latitude;
              d2 = paramAnonymousa.longitude;
              Object localObject = paramAnonymousa.provider;
              double d3 = paramAnonymousa.dgG;
              if ((locald.kWa != -1.0D) || (locald.kWb != -1.0D)) {
                break label398;
              }
              locald.kWa = d1;
              locald.kVY = d1;
              locald.kWb = d2;
              locald.kVZ = d2;
              if ((locald.kWa != -1.0D) && (locald.kWb != -1.0D)) {
                locald.kWf = f.a(locald.kWb, locald.kWa, d2, d1, (String)localObject, d3);
              }
              if ((locald.kWd != 900.0F) && (locald.kWc != 900.0F)) {
                break label433;
              }
              float f = (float)f.h(d1, d2);
              locald.kWc = f;
              locald.kWd = f;
            }
            for (;;)
            {
              if (locald.kVc != null) {
                locald.kVc.setPosition(new com.tencent.mapsdk.raster.model.LatLng(d1, d2));
              }
              AppMethodBeat.o(212552);
              return;
              label398:
              locald.kWa = locald.kVY;
              locald.kWb = locald.kVZ;
              locald.kVZ = d2;
              locald.kVY = d1;
              break;
              label433:
              locald.kWd = locald.kWc;
              locald.kWc = ((float)f.h(d1, d2));
            }
          }
        });
        AppMethodBeat.o(212553);
      }
    };
    this.lastCheckTime = 0L;
    this.kUh = 500;
    this.kUi = new AtomicBoolean(false);
    this.kUj = new AtomicBoolean(false);
    this.kUl = new ArrayList();
    this.kUm = new ArrayList();
    this.kUn = new ArrayList();
    this.kUo = new AtomicInteger(100);
    this.kUp = new HashMap();
    this.kUq = new HashMap();
    this.kUu = new HashMap();
    this.context = paramContext;
    this.kTq = paramString;
    this.container = new FrameLayout(paramContext);
    paramString = new FrameLayout.LayoutParams(-1, -1);
    this.container.setBackgroundColor(0);
    this.container.setLayoutParams(paramString);
    this.kTr = new SoSoMapView(paramContext, M(paramMap));
    this.container.addView(this.kTr, new FrameLayout.LayoutParams(-1, -1));
    this.dtJ = new ImageView(paramContext);
    this.dtJ.setScaleType(ImageView.ScaleType.FIT_XY);
    this.container.addView(this.dtJ, new ViewGroup.LayoutParams(-1, -1));
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s, init", new Object[] { this });
    this.kTr.getMap().setInfoWindowAdapter(this.kUa);
    this.kTr.getMap().setMapAnchor(0.5F, 0.5F);
    this.kTr.getMap().enableMultipleInfowindow(true);
    this.kTr.getMap().setMapStyle(this.kTA);
    paramContext = this.kTr.getMap();
    if (this.kTC) {}
    for (int i = 1008;; i = 1000)
    {
      paramContext.setMapType(i);
      this.kTr.getMap().setHandDrawMapEnable(this.kTB);
      this.kTr.getMap().setOnInfoWindowClickListener(new TencentMap.OnInfoWindowClickListener()
      {
        public final void onInfoWindowClick(Marker paramAnonymousMarker)
        {
          AppMethodBeat.i(146458);
          if (paramAnonymousMarker == null)
          {
            AppMethodBeat.o(146458);
            return;
          }
          paramAnonymousMarker = a.this.Qt((String)paramAnonymousMarker.getTag());
          if (paramAnonymousMarker == null)
          {
            ae.e("MicroMsg.DefaultTencentMapView", "[onInfoWindowClick] map:%s appbrandMarker is null, return", new Object[] { this });
            AppMethodBeat.o(146458);
            return;
          }
          if (a.a(a.this) == null)
          {
            ae.e("MicroMsg.DefaultTencentMapView", "[onInfoWindowClick] map:%s mapCalloutClick is null, return", new Object[] { this });
            AppMethodBeat.o(146458);
            return;
          }
          a.a(a.this).a(paramAnonymousMarker);
          AppMethodBeat.o(146458);
        }
      });
      this.kTr.getMap().setOnMarkerClickListener(new TencentMap.OnMarkerClickListener()
      {
        public final boolean onMarkerClick(Marker paramAnonymousMarker)
        {
          AppMethodBeat.i(146468);
          if (paramAnonymousMarker == null)
          {
            AppMethodBeat.o(146468);
            return false;
          }
          ae.i("MicroMsg.DefaultTencentMapView", "map:%s markerid:%s, onMarkerClick", new Object[] { this, paramAnonymousMarker.getTag() });
          if ((bu.isNullOrNil((String)paramAnonymousMarker.getTag())) && (a.b(a.this) != null) && (((a.e)a.b(a.this).getRenderer()).getCluster(paramAnonymousMarker) != null))
          {
            paramAnonymousMarker = ((a.e)a.b(a.this).getRenderer()).getCluster(paramAnonymousMarker);
            bool = a.this.b(paramAnonymousMarker);
            AppMethodBeat.o(146468);
            return bool;
          }
          if (bu.isNullOrNil((String)paramAnonymousMarker.getTag()))
          {
            ae.w("MicroMsg.DefaultTencentMapView", "realMarker.getTag is null, return");
            AppMethodBeat.o(146468);
            return false;
          }
          if (((String)paramAnonymousMarker.getTag()).endsWith("AnchorPoint"))
          {
            if ((a.c(a.this) != null) && (a.d(a.this) != null))
            {
              ae.w("MicroMsg.DefaultTencentMapView", "click anchor point");
              a.c(a.this).d(a.d(a.this).getLatitude(), a.d(a.this).getLongitude());
            }
            AppMethodBeat.o(146468);
            return true;
          }
          a.g localg = a.this.Qt((String)paramAnonymousMarker.getTag());
          if (localg == null)
          {
            ae.e("MicroMsg.DefaultTencentMapView", "[onMarkerClickListener] map:%s appbrandMarker is null, return", new Object[] { this });
            AppMethodBeat.o(146468);
            return false;
          }
          if (a.e(a.this) == null)
          {
            ae.e("MicroMsg.DefaultTencentMapView", "[onMarkerClickListener] map:%s mapCalloutClick is null, return", new Object[] { this });
            AppMethodBeat.o(146468);
            return false;
          }
          if (localg.kVD != null)
          {
            if (!((String)paramAnonymousMarker.getTag()).endsWith("#label")) {
              break label443;
            }
            ae.w("MicroMsg.DefaultTencentMapView", "realMarker.getTag is label marker, do nothing");
          }
          for (;;)
          {
            if (localg.kVc != null) {
              localg.kVc.set2Top();
            }
            if (localg.kVi != null) {
              localg.kVi.set2Top();
            }
            Object localObject;
            if (a.d(a.this) != null)
            {
              localObject = a.d(a.this);
              if (((d)localObject).kVc != null) {
                ((d)localObject).kVc.set2Top();
              }
            }
            if (localg.kVD.kVh <= 0) {
              break;
            }
            a.a(a.this, localg.kVD.kVh);
            AppMethodBeat.o(146468);
            return true;
            label443:
            b.v.a locala = localg.kVD.kVI;
            if (localg.kVc != null) {
              localObject = localg.kVc;
            }
            for (;;)
            {
              if ((locala != null) && (locala.kVP == b.v.a.kVS) && (localObject != null))
              {
                if (!((Marker)localObject).isInfoWindowShown())
                {
                  a.a(a.this, (Marker)localObject);
                  break;
                  localObject = paramAnonymousMarker;
                  continue;
                }
                ((Marker)localObject).hideInfoWindow();
                break;
              }
            }
            if ((locala != null) && (localObject != null)) {
              a.a(a.this, (Marker)localObject);
            }
          }
          if (((String)paramAnonymousMarker.getTag()).endsWith("#label"))
          {
            ae.w("MicroMsg.DefaultTencentMapView", "realMarker.getTag is label marker, return");
            bool = a.f(a.this).c(localg);
            AppMethodBeat.o(146468);
            return bool;
          }
          boolean bool = a.e(a.this).b(localg);
          AppMethodBeat.o(146468);
          return bool;
        }
      });
      this.kTr.getMap().setOnMapClickListener(new TencentMap.OnMapClickListener()
      {
        public final void onMapClick(com.tencent.mapsdk.raster.model.LatLng paramAnonymousLatLng)
        {
          AppMethodBeat.i(146478);
          Iterator localIterator = a.g(a.this).values().iterator();
          while (localIterator.hasNext())
          {
            a.g localg = (a.g)localIterator.next();
            if ((localg.kVD != null) && (localg.kVD.kVI != null) && (localg.kVD.kVI.kVP == b.v.a.kVS) && (localg.kVc.isInfoWindowShown())) {
              localg.kVc.hideInfoWindow();
            }
          }
          if (a.h(a.this) != null)
          {
            a.h(a.this).e(paramAnonymousLatLng.getLongitude(), paramAnonymousLatLng.getLatitude());
            AppMethodBeat.o(146478);
            return;
          }
          AppMethodBeat.o(146478);
        }
      });
      this.kTr.getMap().setOnMapLoadedListener(new TencentMap.OnMapLoadedListener()
      {
        public final void onMapLoaded()
        {
          AppMethodBeat.i(146479);
          if (a.i(a.this) != null) {
            a.i(a.this);
          }
          a.j(a.this);
          a.k(a.this);
          if (a.l(a.this).size() > 0)
          {
            Object localObject = new ArrayList();
            ((ArrayList)localObject).addAll(a.l(a.this));
            localObject = ((ArrayList)localObject).iterator();
            while (((Iterator)localObject).hasNext())
            {
              String str = (String)((Iterator)localObject).next();
              a.this.Qq(str);
            }
            a.l(a.this).clear();
          }
          AppMethodBeat.o(146479);
        }
      });
      this.kTr.getMap().setOnMapPoiClickListener(new TencentMap.OnMapPoiClickListener()
      {
        public final void onClicked(MapPoi paramAnonymousMapPoi)
        {
          AppMethodBeat.i(146480);
          if (a.m(a.this) != null)
          {
            b.t localt = new b.t();
            localt.latitude = paramAnonymousMapPoi.getPosition().getLatitude();
            localt.longitude = paramAnonymousMapPoi.getPosition().getLongitude();
            localt.name = paramAnonymousMapPoi.getName();
            if (((paramAnonymousMapPoi instanceof IndoorMapPoi)) && (!bu.isNullOrNil(((IndoorMapPoi)paramAnonymousMapPoi).getBuildingId())))
            {
              localt.buildingId = ((IndoorMapPoi)paramAnonymousMapPoi).getBuildingId();
              localt.floorName = ((IndoorMapPoi)paramAnonymousMapPoi).getFloorName();
            }
            a.m(a.this).a(localt);
          }
          AppMethodBeat.o(146480);
        }
      });
      this.kTr.getMap().setOnIndoorStateChangeListener(new TencentMap.OnIndoorStateChangeListener()
      {
        public final boolean onIndoorBuildingDeactivated()
        {
          AppMethodBeat.i(146483);
          ae.v("MicroMsg.DefaultTencentMapView", "[onIndoorBuildingDeactivated]");
          if (a.n(a.this) != null)
          {
            b.f localf = a.blM();
            a.a(a.this, localf);
            a.n(a.this).a(localf);
          }
          AppMethodBeat.o(146483);
          return true;
        }
        
        public final boolean onIndoorBuildingFocused()
        {
          AppMethodBeat.i(146481);
          ae.v("MicroMsg.DefaultTencentMapView", "[onIndoorBuildingFocused]");
          AppMethodBeat.o(146481);
          return true;
        }
        
        public final boolean onIndoorLevelActivated(IndoorBuilding paramAnonymousIndoorBuilding)
        {
          AppMethodBeat.i(146482);
          ae.v("MicroMsg.DefaultTencentMapView", "[onIndoorLevelActivated]indoorBuilding:%s", new Object[] { paramAnonymousIndoorBuilding });
          if (a.n(a.this) != null)
          {
            b.f localf = new b.f();
            localf.buildingId = paramAnonymousIndoorBuilding.getBuidlingId();
            localf.buildingName = paramAnonymousIndoorBuilding.getBuildingName();
            localf.kVr = new LinkedList();
            Iterator localIterator = paramAnonymousIndoorBuilding.getLevels().iterator();
            while (localIterator.hasNext())
            {
              IndoorLevel localIndoorLevel = (IndoorLevel)localIterator.next();
              b.g localg = new b.g();
              localg.floorName = localIndoorLevel.getName();
              localf.kVr.add(localg);
            }
            localf.kVs = paramAnonymousIndoorBuilding.getActiveLevelIndex();
            a.a(a.this, localf);
            a.n(a.this).a(localf);
          }
          AppMethodBeat.o(146482);
          return true;
        }
      });
      ae.i("MicroMsg.DefaultTencentMapView", "map:%s onCreate", new Object[] { this });
      this.kTr.onCreate(null);
      onResume();
      AppMethodBeat.o(146490);
      return;
    }
  }
  
  private f Qw(String paramString)
  {
    AppMethodBeat.i(212582);
    Iterator localIterator = this.kUl.iterator();
    while (localIterator.hasNext())
    {
      f localf = (f)localIterator.next();
      if (paramString.contains(f.a(localf)))
      {
        AppMethodBeat.o(212582);
        return localf;
      }
    }
    AppMethodBeat.o(212582);
    return null;
  }
  
  private static int Qx(String paramString)
  {
    AppMethodBeat.i(212590);
    if (TextUtils.isEmpty(paramString))
    {
      AppMethodBeat.o(212590);
      return -1;
    }
    int i;
    if (paramString.equals("abovelabels"))
    {
      i = OverlayLevel.OverlayLevelAboveLabels;
      AppMethodBeat.o(212590);
      return i;
    }
    if (paramString.equals("abovebuildings"))
    {
      i = OverlayLevel.OverlayLevelAboveBuildings;
      AppMethodBeat.o(212590);
      return i;
    }
    if (paramString.equals("aboveroads"))
    {
      i = OverlayLevel.OverlayLevelAboveRoads;
      AppMethodBeat.o(212590);
      return i;
    }
    AppMethodBeat.o(212590);
    return -1;
  }
  
  private void V(Runnable paramRunnable)
  {
    AppMethodBeat.i(146524);
    this.kTV.add(paramRunnable);
    if (this.kTV.size() == 1) {
      paramRunnable.run();
    }
    AppMethodBeat.o(146524);
  }
  
  private static CameraUpdate a(TencentMap paramTencentMap, float paramFloat1, float paramFloat2, float paramFloat3, double paramDouble1, double paramDouble2)
  {
    AppMethodBeat.i(146523);
    if ((paramFloat1 == -1.0F) && (paramFloat2 == -1.0F) && (paramFloat3 == -1.0F) && (paramDouble1 != -1.0D) && (paramDouble2 != -1.0D))
    {
      paramTencentMap = CameraUpdateFactory.newLatLng(new com.tencent.mapsdk.raster.model.LatLng(paramDouble1, paramDouble2));
      AppMethodBeat.o(146523);
      return paramTencentMap;
    }
    Object localObject = paramTencentMap.getCameraPosition();
    localObject = new CameraPosition.Builder((CameraPosition)localObject).bearing(((CameraPosition)localObject).getBearing()).skew(((CameraPosition)localObject).getSkew());
    ae.v("MicroMsg.DefaultTencentMapView", "tencentMap.getCameraPosition:%s rotate:%f skew:%f", new Object[] { paramTencentMap.getCameraPosition(), Float.valueOf(paramTencentMap.getCameraPosition().getBearing()), Float.valueOf(paramTencentMap.getCameraPosition().getSkew()) });
    if (paramFloat1 != -1.0F) {
      ((CameraPosition.Builder)localObject).zoom(paramFloat1);
    }
    if (paramFloat2 != -1.0F) {
      ((CameraPosition.Builder)localObject).bearing(paramFloat2);
    }
    if (paramFloat3 != -1.0F) {
      ((CameraPosition.Builder)localObject).skew(paramFloat3);
    }
    if ((paramDouble1 != -1.0D) && (paramDouble2 != -1.0D)) {
      ((CameraPosition.Builder)localObject).target(new com.tencent.mapsdk.raster.model.LatLng(paramDouble1, paramDouble2));
    }
    paramTencentMap = CameraUpdateFactory.newCameraPosition(((CameraPosition.Builder)localObject).build());
    AppMethodBeat.o(146523);
    return paramTencentMap;
  }
  
  /* Error */
  private org.json.JSONObject a(Cluster<f> paramCluster, boolean paramBoolean)
  {
    // Byte code:
    //   0: ldc_w 666
    //   3: invokestatic 239	com/tencent/matrix/trace/core/AppMethodBeat:i	(I)V
    //   6: iconst_0
    //   7: istore_3
    //   8: iload_2
    //   9: ifeq +211 -> 220
    //   12: aload_0
    //   13: getfield 341	com/tencent/mm/plugin/appbrand/jsapi/i/a/a:kUo	Ljava/util/concurrent/atomic/AtomicInteger;
    //   16: invokevirtual 669	java/util/concurrent/atomic/AtomicInteger:getAndIncrement	()I
    //   19: istore_3
    //   20: aload_0
    //   21: getfield 335	com/tencent/mm/plugin/appbrand/jsapi/i/a/a:kUn	Ljava/util/ArrayList;
    //   24: iload_3
    //   25: invokestatic 674	java/lang/Integer:valueOf	(I)Ljava/lang/Integer;
    //   28: invokevirtual 675	java/util/ArrayList:add	(Ljava/lang/Object;)Z
    //   31: pop
    //   32: aload_0
    //   33: getfield 343	com/tencent/mm/plugin/appbrand/jsapi/i/a/a:kUp	Ljava/util/HashMap;
    //   36: iload_3
    //   37: invokestatic 674	java/lang/Integer:valueOf	(I)Ljava/lang/Integer;
    //   40: aload_1
    //   41: invokevirtual 678	java/util/HashMap:put	(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;
    //   44: pop
    //   45: aload_1
    //   46: invokeinterface 684 1 0
    //   51: invokeinterface 687 1 0
    //   56: astore 4
    //   58: aload 4
    //   60: invokeinterface 509 1 0
    //   65: ifeq +29 -> 94
    //   68: aload 4
    //   70: invokeinterface 513 1 0
    //   75: checkcast 113	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f
    //   78: iload_3
    //   79: putfield 690	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f:kVh	I
    //   82: goto -24 -> 58
    //   85: astore_1
    //   86: ldc_w 666
    //   89: invokestatic 258	com/tencent/matrix/trace/core/AppMethodBeat:o	(I)V
    //   92: aconst_null
    //   93: areturn
    //   94: new 692	org/json/JSONObject
    //   97: dup
    //   98: invokespecial 693	org/json/JSONObject:<init>	()V
    //   101: astore 4
    //   103: aload 4
    //   105: ldc_w 695
    //   108: iload_3
    //   109: invokevirtual 698	org/json/JSONObject:put	(Ljava/lang/String;I)Lorg/json/JSONObject;
    //   112: pop
    //   113: new 692	org/json/JSONObject
    //   116: dup
    //   117: invokespecial 693	org/json/JSONObject:<init>	()V
    //   120: astore 5
    //   122: aload 5
    //   124: ldc_w 700
    //   127: aload_1
    //   128: invokeinterface 704 1 0
    //   133: invokevirtual 708	com/tencent/mapsdk/raster/model/LatLng:getLatitude	()D
    //   136: invokevirtual 711	org/json/JSONObject:put	(Ljava/lang/String;D)Lorg/json/JSONObject;
    //   139: pop
    //   140: aload 5
    //   142: ldc_w 713
    //   145: aload_1
    //   146: invokeinterface 704 1 0
    //   151: invokevirtual 716	com/tencent/mapsdk/raster/model/LatLng:getLongitude	()D
    //   154: invokevirtual 711	org/json/JSONObject:put	(Ljava/lang/String;D)Lorg/json/JSONObject;
    //   157: pop
    //   158: aload 4
    //   160: ldc_w 718
    //   163: aload 5
    //   165: invokevirtual 721	org/json/JSONObject:put	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
    //   168: pop
    //   169: new 653	org/json/JSONArray
    //   172: dup
    //   173: invokespecial 654	org/json/JSONArray:<init>	()V
    //   176: astore 5
    //   178: aload_1
    //   179: invokeinterface 684 1 0
    //   184: invokeinterface 687 1 0
    //   189: astore_1
    //   190: aload_1
    //   191: invokeinterface 509 1 0
    //   196: ifeq +64 -> 260
    //   199: aload 5
    //   201: aload_1
    //   202: invokeinterface 513 1 0
    //   207: checkcast 113	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f
    //   210: invokestatic 516	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f:a	(Lcom/tencent/mm/plugin/appbrand/jsapi/i/a/a$f;)Ljava/lang/String;
    //   213: invokevirtual 661	org/json/JSONArray:put	(Ljava/lang/Object;)Lorg/json/JSONArray;
    //   216: pop
    //   217: goto -27 -> 190
    //   220: aload_1
    //   221: invokeinterface 684 1 0
    //   226: invokeinterface 687 1 0
    //   231: astore 4
    //   233: aload 4
    //   235: invokeinterface 509 1 0
    //   240: ifeq -146 -> 94
    //   243: aload 4
    //   245: invokeinterface 513 1 0
    //   250: checkcast 113	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f
    //   253: getfield 690	com/tencent/mm/plugin/appbrand/jsapi/i/a/a$f:kVh	I
    //   256: istore_3
    //   257: goto -163 -> 94
    //   260: aload 4
    //   262: ldc_w 723
    //   265: aload 5
    //   267: invokevirtual 721	org/json/JSONObject:put	(Ljava/lang/String;Ljava/lang/Object;)Lorg/json/JSONObject;
    //   270: pop
    //   271: ldc_w 666
    //   274: invokestatic 258	com/tencent/matrix/trace/core/AppMethodBeat:o	(I)V
    //   277: aload 4
    //   279: areturn
    //   280: astore_1
    //   281: goto -195 -> 86
    // Local variable table:
    //   start	length	slot	name	signature
    //   0	284	0	this	a
    //   0	284	1	paramCluster	Cluster<f>
    //   0	284	2	paramBoolean	boolean
    //   7	250	3	i	int
    //   56	222	4	localObject1	Object
    //   120	146	5	localObject2	Object
    // Exception table:
    //   from	to	target	type
    //   12	58	85	org/json/JSONException
    //   58	82	85	org/json/JSONException
    //   94	190	85	org/json/JSONException
    //   190	217	85	org/json/JSONException
    //   220	233	85	org/json/JSONException
    //   233	257	85	org/json/JSONException
    //   260	271	85	org/json/JSONException
    //   12	58	280	java/lang/Exception
    //   58	82	280	java/lang/Exception
    //   94	190	280	java/lang/Exception
    //   190	217	280	java/lang/Exception
    //   220	233	280	java/lang/Exception
    //   233	257	280	java/lang/Exception
    //   260	271	280	java/lang/Exception
  }
  
  private static void a(float paramFloat1, float paramFloat2, Bitmap paramBitmap, ImageView paramImageView)
  {
    AppMethodBeat.i(146537);
    if (paramImageView == null)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "markerIcon is null, err");
      AppMethodBeat.o(146537);
      return;
    }
    float f;
    if (paramFloat1 != 0.0F)
    {
      f = paramFloat1;
      paramFloat1 = paramFloat2;
      if (paramFloat2 != 0.0F) {}
    }
    else
    {
      f = com.tencent.mm.plugin.appbrand.y.g.vO(paramBitmap.getWidth());
      paramFloat1 = com.tencent.mm.plugin.appbrand.y.g.vO(paramBitmap.getHeight());
    }
    if ((f > 0.0F) && (paramFloat1 > 0.0F) && ((f != paramBitmap.getWidth()) || (paramFloat1 != paramBitmap.getHeight())))
    {
      paramFloat2 = f / paramBitmap.getWidth();
      paramFloat1 /= paramBitmap.getHeight();
      Matrix localMatrix = new Matrix();
      localMatrix.postScale(paramFloat2, paramFloat1);
      paramImageView.setImageBitmap(Bitmap.createBitmap(paramBitmap, 0, 0, paramBitmap.getWidth(), paramBitmap.getHeight(), localMatrix, true));
      AppMethodBeat.o(146537);
      return;
    }
    paramImageView.setImageBitmap(paramBitmap);
    paramImageView.setScaleType(ImageView.ScaleType.CENTER);
    AppMethodBeat.o(146537);
  }
  
  private void a(String paramString, g paramg)
  {
    AppMethodBeat.i(146538);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s addMarker markerId:%s", new Object[] { this, paramString });
    this.kUb.put(paramString, paramg);
    AppMethodBeat.o(146538);
  }
  
  private void a(String paramString, b.v paramv, g paramg)
  {
    AppMethodBeat.i(212574);
    if (Qt(paramString + "#label") != null)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "addLabelMarker label is exist, label id:%s", new Object[] { paramString + "#label" });
      AppMethodBeat.o(212574);
      return;
    }
    if (paramv.kVJ == null)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "addLabelMarker labelStyle is null, label id:%s", new Object[] { paramString + "#label" });
      AppMethodBeat.o(212574);
      return;
    }
    MarkerOptions localMarkerOptions = new MarkerOptions();
    localMarkerOptions.position(new com.tencent.mapsdk.raster.model.LatLng(paramv.latitude, paramv.longitude));
    Object localObject2 = i.blQ();
    Object localObject1 = localObject2;
    if (localObject2 == null) {
      localObject1 = new com.tencent.mm.plugin.appbrand.widget.d.d(this.context);
    }
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).reset();
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setTextColor(paramv.kVJ.color);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setTextSize(paramv.kVJ.kVK);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setText(paramv.kVJ.content);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setTextPadding(paramv.kVJ.padding);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setGravity(paramv.kVJ.jWV);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).A(paramv.kVJ.kVL, paramv.kVJ.borderWidth, paramv.kVJ.borderColor, paramv.kVJ.bgColor);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setX(paramv.kVJ.x);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).setY(paramv.kVJ.y);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).eA(paramv.kVJ.width, paramv.kVJ.height);
    ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).measure(0, 0);
    localMarkerOptions.anchor(((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).getAnchorX(), ((com.tencent.mm.plugin.appbrand.widget.d.d)localObject1).getAnchorY());
    localMarkerOptions.markerView((View)localObject1);
    localMarkerOptions.visible(true);
    localMarkerOptions.tag(paramString + "#label");
    localMarkerOptions.zIndex(paramv.zIndex + 3);
    localMarkerOptions.level(OverlayLevel.OverlayLevelAboveLabels);
    if ((!bu.isNullOrNil(paramv.buildingId)) && (!bu.isNullOrNil(paramv.floorName))) {
      localMarkerOptions.indoorInfo(new IndoorInfo(paramv.buildingId, paramv.floorName));
    }
    localObject2 = this.kTr.getMap().addMarker(localMarkerOptions);
    localObject1 = paramg;
    if (paramg == null)
    {
      localObject1 = new g();
      ((g)localObject1).data = paramv.data;
      ((g)localObject1).kVD = paramv;
    }
    ((g)localObject1).kVi = ((Marker)localObject2);
    a(paramString + "#label", (g)localObject1);
    AppMethodBeat.o(212574);
  }
  
  private boolean a(b.aa paramaa)
  {
    AppMethodBeat.i(212580);
    if (this.kTr.getMap() == null)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "addMapMarkerCluster map is null");
      AppMethodBeat.o(212580);
      return false;
    }
    if (!blK())
    {
      ae.e("MicroMsg.DefaultTencentMapView", "addMapMarkerCluster mClusterManager is not initialize");
      AppMethodBeat.o(212580);
      return false;
    }
    f localf = b(paramaa);
    this.kUl.add(localf);
    this.kUm.add(f.a(localf));
    this.kTF.addItem(localf);
    this.kTF.cluster();
    ae.i("MicroMsg.DefaultTencentMapView", "addMapMarkerCluster mClusterItems:%d， id:%s， latitude:%f, longitude:%f", new Object[] { Integer.valueOf(this.kUl.size()), paramaa.id, Double.valueOf(localf.getPosition().getLatitude()), Double.valueOf(localf.getPosition().getLongitude()) });
    AppMethodBeat.o(212580);
    return true;
  }
  
  private static f b(b.aa paramaa)
  {
    AppMethodBeat.i(212581);
    com.tencent.mapsdk.raster.model.LatLng localLatLng = new com.tencent.mapsdk.raster.model.LatLng(paramaa.kVU.latitude, paramaa.kVU.longitude);
    paramaa = new f(paramaa.id, localLatLng);
    AppMethodBeat.o(212581);
    return paramaa;
  }
  
  private void b(final Marker paramMarker)
  {
    AppMethodBeat.i(212576);
    if (Looper.myLooper() == Looper.getMainLooper())
    {
      paramMarker.showInfoWindow();
      AppMethodBeat.o(212576);
      return;
    }
    ar.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(212538);
        paramMarker.showInfoWindow();
        AppMethodBeat.o(212538);
      }
    });
    AppMethodBeat.o(212576);
  }
  
  private Bundle blJ()
  {
    AppMethodBeat.i(146545);
    Bundle localBundle = new Bundle();
    if (!kTx.equals(this.kTy))
    {
      String str = bu.nullAsNil(localBundle.getString("smallAppKey"));
      localBundle.putString("smallAppKey", str + this.kTy + "#" + this.kTz + ";");
    }
    AppMethodBeat.o(146545);
    return localBundle;
  }
  
  private boolean blL()
  {
    AppMethodBeat.i(212583);
    ae.i("MicroMsg.DefaultTencentMapView", "removeAllMapCluster");
    if (this.kTF != null)
    {
      Iterator localIterator = this.kUl.iterator();
      while (localIterator.hasNext())
      {
        f localf = (f)localIterator.next();
        this.kTF.removeItem(localf);
      }
      this.kUm.clear();
      this.kUl.clear();
      this.kTF.cluster();
      this.kUn.clear();
      this.kUq.clear();
      AppMethodBeat.o(212583);
      return true;
    }
    AppMethodBeat.o(212583);
    return false;
  }
  
  private static b.f blw()
  {
    AppMethodBeat.i(146492);
    b.f localf = new b.f();
    localf.buildingId = "";
    localf.buildingName = "";
    localf.kVr = new LinkedList();
    localf.kVs = -1;
    AppMethodBeat.o(146492);
    return localf;
  }
  
  private void blx()
  {
    AppMethodBeat.i(146496);
    if (this.kTI == null) {
      this.kTI = getSurfaceView();
    }
    if (this.kTI != null)
    {
      ar.ay(this.kTJ);
      this.kTI.setVisibility(0);
      AppMethodBeat.o(146496);
      return;
    }
    ae.e("MicroMsg.DefaultTencentMapView", "showTencentMap err");
    AppMethodBeat.o(146496);
  }
  
  private void c(String paramString, b.v paramv, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    Object localObject3 = null;
    AppMethodBeat.i(212575);
    Object localObject1;
    if ((paramv.kVh > 0) && (this.kUn.contains(Integer.valueOf(paramv.kVh))))
    {
      localObject1 = (Cluster)this.kUp.get(Integer.valueOf(paramv.kVh));
      if (localObject1 == null) {
        break label506;
      }
      localObject1 = ((e)this.kTF.getRenderer()).getMarker((Cluster)localObject1);
    }
    for (;;)
    {
      final ImageView localImageView;
      Object localObject2;
      if (localObject1 != null)
      {
        if (paramv.kVE) {
          ((Marker)localObject1).setPosition(new com.tencent.mapsdk.raster.model.LatLng(paramv.latitude, paramv.longitude));
        }
        if (!bu.isNullOrNil(paramv.title)) {
          ((Marker)localObject1).setTitle(paramv.title);
        }
        if (!bu.isNullOrNil(paramv.kVH)) {
          ((Marker)localObject1).setContentDescription(paramv.kVH);
        }
        ((Marker)localObject1).setRotation(paramv.kVl);
        ((Marker)localObject1).setAlpha(paramv.alpha);
        final FrameLayout localFrameLayout = (FrameLayout)((LayoutInflater)this.context.getSystemService("layout_inflater")).inflate(2131493687, null);
        localImageView = (ImageView)localFrameLayout.findViewById(2131302158);
        final float f1 = paramv.kVF;
        final float f2 = paramv.kVG;
        localObject2 = localObject3;
        if (parama != null)
        {
          if (!bu.isNullOrNil(paramv.kVn))
          {
            if (localImageView != null) {
              localImageView.setImageBitmap(null);
            }
            localObject2 = parama.a(paramv.kVn, null, new com.tencent.mm.plugin.appbrand.f.a.b()
            {
              public final void N(Bitmap paramAnonymousBitmap)
              {
                AppMethodBeat.i(212535);
                if (paramAnonymousBitmap != null)
                {
                  a.b(f1, f2, paramAnonymousBitmap, localImageView);
                  if (this.kUL != null)
                  {
                    ae.v("MicroMsg.DefaultTencentMapView", "setMarkerView");
                    this.kUL.setMarkerView(localFrameLayout);
                  }
                }
                AppMethodBeat.o(212535);
              }
            });
          }
        }
        else
        {
          label282:
          if (localObject2 == null) {
            break label482;
          }
          a(f1, f2, (Bitmap)localObject2, localImageView);
          label298:
          ((Marker)localObject1).setMarkerView(localFrameLayout);
          ((Marker)localObject1).setAnchor(paramv.anchorX, paramv.anchorY);
          ((Marker)localObject1).setTag(paramString);
          ((Marker)localObject1).setInfoWindowHideAnimation(new Animation() {});
          ((Marker)localObject1).setInfoWindowShowAnimation(new Animation() {});
          ((Marker)localObject1).setVisible(true);
          ((Marker)localObject1).setZIndex(paramv.zIndex + 2);
          ((Marker)localObject1).setLevel(OverlayLevel.OverlayLevelAboveLabels);
        }
      }
      else
      {
        AppMethodBeat.o(212575);
        return;
        if (!paramv.kVE) {
          break label500;
        }
        localObject1 = Qw(paramString);
        if (localObject1 == null) {
          break label494;
        }
      }
      label482:
      label494:
      for (localObject1 = ((e)this.kTF.getRenderer()).getMarker((ClusterItem)localObject1);; localObject1 = null)
      {
        this.kUq.put(paramString, paramv);
        if (this.kUr == null) {
          this.kUr = parama;
        }
        break;
        localObject2 = localObject3;
        if (localImageView == null) {
          break label282;
        }
        localImageView.setImageBitmap(null);
        localObject2 = localObject3;
        break label282;
        ae.e("MicroMsg.DefaultTencentMapView", "[addMarker] bitmap is null, use default");
        break label298;
      }
      label500:
      localObject1 = null;
      continue;
      label506:
      localObject1 = null;
    }
  }
  
  private SurfaceView getSurfaceView()
  {
    AppMethodBeat.i(146495);
    if (this.kTI != null)
    {
      localSurfaceView = this.kTI;
      AppMethodBeat.o(146495);
      return localSurfaceView;
    }
    this.kTI = i(this.kTr);
    SurfaceView localSurfaceView = this.kTI;
    AppMethodBeat.o(146495);
    return localSurfaceView;
  }
  
  private static SurfaceView i(ViewGroup paramViewGroup)
  {
    AppMethodBeat.i(146548);
    if ((paramViewGroup == null) || (paramViewGroup.getChildCount() == 0))
    {
      AppMethodBeat.o(146548);
      return null;
    }
    int j = paramViewGroup.getChildCount();
    int i = 0;
    for (;;)
    {
      if (i >= j) {
        break label85;
      }
      View localView = paramViewGroup.getChildAt(i);
      if ((localView instanceof SurfaceView))
      {
        paramViewGroup = (SurfaceView)localView;
        AppMethodBeat.o(146548);
        return paramViewGroup;
      }
      if ((localView instanceof ViewGroup))
      {
        paramViewGroup = (ViewGroup)localView;
        break;
      }
      i += 1;
    }
    label85:
    AppMethodBeat.o(146548);
    return null;
  }
  
  private void i(List<com.tencent.mapsdk.raster.model.LatLng> paramList, int paramInt)
  {
    AppMethodBeat.i(212569);
    LatLngBounds.Builder localBuilder = new LatLngBounds.Builder();
    localBuilder.include(paramList);
    this.kTN = false;
    this.kTr.getMap().animateCamera(CameraUpdateFactory.newLatLngBounds(localBuilder.build(), paramInt));
    this.kTP = null;
    AppMethodBeat.o(212569);
  }
  
  public final void H(float paramFloat1, float paramFloat2)
  {
    AppMethodBeat.i(146546);
    if ((!this.kUe) || (this.kTs))
    {
      AppMethodBeat.o(146546);
      return;
    }
    if (this.kUf != null) {
      this.kUf.H(paramFloat1, paramFloat2);
    }
    AppMethodBeat.o(146546);
  }
  
  protected TencentMapOptions M(Map<String, Object> paramMap)
  {
    AppMethodBeat.i(146491);
    TencentMapOptions localTencentMapOptions = new TencentMapOptions();
    if (paramMap == null)
    {
      ae.w("MicroMsg.DefaultTencentMapView", "params is null, return");
      AppMethodBeat.o(146491);
      return localTencentMapOptions;
    }
    String str = f.a(paramMap, "theme", "normal");
    this.kTB = str.equals("handDraw");
    this.mMapType = f.a(paramMap, "mapType", 1);
    localTencentMapOptions.setMapType(this.mMapType);
    this.kTy = f.a(paramMap, "subKey", "");
    this.kTz = f.a(paramMap, "subId", "");
    boolean bool;
    if (f.a(paramMap, "enableDarkMode", 0) == 1)
    {
      bool = true;
      this.kTC = bool;
      if (!bu.isNullOrNil(this.kTy)) {
        break label288;
      }
      this.kTy = kTx;
      this.kTz = "";
    }
    for (;;)
    {
      localTencentMapOptions.setSubInfo(this.kTy, this.kTz);
      this.kTA = f.a(paramMap, "styleId", 0);
      ae.d("MicroMsg.DefaultTencentMapView", "MapReport subId:%s, subKey:%s", new Object[] { this.kTz, this.kTy });
      ae.i("MicroMsg.DefaultTencentMapView", "[createTencentMapOptions]theme:%s isHandDraw:%b, mapType:%d, subId:%s, subKey:%s, styleId:%d enableDarkmode:%b", new Object[] { str, Boolean.valueOf(this.kTB), Integer.valueOf(this.mMapType), this.kTz, this.kTy, Integer.valueOf(this.kTA), Boolean.valueOf(this.kTC) });
      AppMethodBeat.o(146491);
      return localTencentMapOptions;
      bool = false;
      break;
      label288:
      this.kTC = false;
    }
  }
  
  public final void Qq(String paramString)
  {
    AppMethodBeat.i(182814);
    if (!this.kTH)
    {
      if (!this.kTT.contains(paramString)) {
        this.kTT.add(paramString);
      }
      ae.e("MicroMsg.DefaultTencentMapView", "addCustomLayer, wait map loaded callback");
      AppMethodBeat.o(182814);
      return;
    }
    CustomLayer localCustomLayer1 = this.kTr.getMap().addCustomLayer(new CustomLayerOptions().layerId(paramString));
    if (localCustomLayer1 != null)
    {
      ae.v("MicroMsg.DefaultTencentMapView", "addCustomLayer success, layerId:%s", new Object[] { paramString });
      CustomLayer localCustomLayer2 = (CustomLayer)this.kTS.remove(paramString);
      if (localCustomLayer2 != null) {
        localCustomLayer2.remove();
      }
      this.kTS.put(paramString, localCustomLayer1);
      AppMethodBeat.o(182814);
      return;
    }
    ae.e("MicroMsg.DefaultTencentMapView", "addCustomLayer fail, layerId:%s", new Object[] { paramString });
    AppMethodBeat.o(182814);
  }
  
  public final void Qr(String paramString)
  {
    AppMethodBeat.i(182815);
    CustomLayer localCustomLayer = (CustomLayer)this.kTS.remove(paramString);
    if (localCustomLayer != null)
    {
      ae.v("MicroMsg.DefaultTencentMapView", "removeCustomLayer success, layerId:%s", new Object[] { paramString });
      localCustomLayer.remove();
      AppMethodBeat.o(182815);
      return;
    }
    ae.e("MicroMsg.DefaultTencentMapView", "removeCustomLayer fail, layerId:%s", new Object[] { paramString });
    AppMethodBeat.o(182815);
  }
  
  public final boolean Qs(String paramString)
  {
    AppMethodBeat.i(146530);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s removeLine lineId:%s", new Object[] { this, paramString });
    Polyline localPolyline = (Polyline)this.kTZ.get(paramString);
    if (localPolyline == null)
    {
      ae.w("MicroMsg.DefaultTencentMapView", "lineId:%s is null", new Object[] { paramString });
      AppMethodBeat.o(146530);
      return false;
    }
    localPolyline.remove();
    this.kTZ.remove(paramString);
    AppMethodBeat.o(146530);
    return true;
  }
  
  public final g Qt(String paramString)
  {
    AppMethodBeat.i(146533);
    paramString = (g)this.kUb.get(paramString);
    AppMethodBeat.o(146533);
    return paramString;
  }
  
  public final boolean Qu(String paramString)
  {
    AppMethodBeat.i(146535);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s removeMarker markerId:%s", new Object[] { this, paramString });
    if (this.kUm.contains(paramString))
    {
      ae.i("MicroMsg.DefaultTencentMapView", "removeMapMarkerClusterGeometry id:%s", new Object[] { paramString });
      if ((this.kTF != null) && (!TextUtils.isEmpty(paramString)))
      {
        localObject = this.kUl.iterator();
        while (((Iterator)localObject).hasNext())
        {
          f localf = (f)((Iterator)localObject).next();
          if (paramString.contains(f.a(localf)))
          {
            this.kTF.removeItem(localf);
            this.kUl.remove(localf);
            this.kUm.remove(paramString);
            if (localf.kVh > 0)
            {
              Qv(localf.kVh);
              this.kUn.remove(Integer.valueOf(localf.kVh));
              this.kUp.remove(Integer.valueOf(localf.kVh));
            }
            ae.i("MicroMsg.DefaultTencentMapView", "remove MapMarker ClusterGeometry success id:%s", new Object[] { paramString });
            this.kTF.cluster();
          }
        }
      }
    }
    Object localObject = (g)this.kUb.get(paramString);
    if (localObject == null)
    {
      ae.w("MicroMsg.DefaultTencentMapView", "marker:%s is null", new Object[] { paramString });
      AppMethodBeat.o(146535);
      return false;
    }
    if (((g)localObject).kVc != null) {
      ((g)localObject).kVc.remove();
    }
    if (((g)localObject).kVi != null)
    {
      if ((((g)localObject).kVi.getMarkerView() != null) && ((((g)localObject).kVi.getMarkerView() instanceof com.tencent.mm.plugin.appbrand.widget.d.d))) {
        i.a((com.tencent.mm.plugin.appbrand.widget.d.d)((g)localObject).kVi.getMarkerView());
      }
      ((g)localObject).kVi.remove();
      this.kUb.remove(paramString + "#label");
    }
    ((g)localObject).kVj = null;
    this.kUb.remove(paramString);
    AppMethodBeat.o(146535);
    return true;
  }
  
  public final boolean Qv(String paramString)
  {
    AppMethodBeat.i(212572);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s removeLabelMarker markerId:%s", new Object[] { this, paramString });
    g localg = (g)this.kUb.remove(paramString + "#label");
    if (localg == null)
    {
      ae.w("MicroMsg.DefaultTencentMapView", "marker:%s is null", new Object[] { paramString });
      AppMethodBeat.o(212572);
      return false;
    }
    if (localg.kVi != null)
    {
      if ((localg.kVi.getMarkerView() != null) && ((localg.kVi.getMarkerView() instanceof com.tencent.mm.plugin.appbrand.widget.d.d))) {
        i.a((com.tencent.mm.plugin.appbrand.widget.d.d)localg.kVi.getMarkerView());
      }
      localg.kVi.remove();
    }
    AppMethodBeat.o(212572);
    return true;
  }
  
  public final Point a(b.i parami)
  {
    AppMethodBeat.i(212588);
    ae.i("MicroMsg.DefaultTencentMapView", "toScreenLocation");
    parami = new com.tencent.mapsdk.raster.model.LatLng(parami.latitude, parami.longitude);
    parami = this.kTr.getMap().getProjection().toScreenLocation(parami);
    AppMethodBeat.o(212588);
    return parami;
  }
  
  public final void a(double paramDouble1, double paramDouble2, float paramFloat1, float paramFloat2, float paramFloat3)
  {
    AppMethodBeat.i(146521);
    a(paramDouble1, paramDouble2, paramFloat1, paramFloat2, paramFloat3, false);
    AppMethodBeat.o(146521);
  }
  
  public final void a(final double paramDouble1, double paramDouble2, final float paramFloat1, final float paramFloat2, final float paramFloat3, boolean paramBoolean)
  {
    AppMethodBeat.i(146522);
    ae.v("MicroMsg.DefaultTencentMapView", "start map:%s latitude:%f longitude:%f scale:%f rotate:%f skew:%f", new Object[] { this, Double.valueOf(paramDouble1), Double.valueOf(paramDouble2), Float.valueOf(paramFloat1), Float.valueOf(paramFloat2), Float.valueOf(paramFloat3) });
    if ((paramFloat1 == -1.0F) && (paramFloat2 == -1.0F) && (paramFloat3 == -1.0F) && (paramDouble1 == -1.0D) && (paramDouble2 == -1.0D))
    {
      AppMethodBeat.o(146522);
      return;
    }
    final TencentMap localTencentMap = this.kTr.getMap();
    if (localTencentMap != null)
    {
      if (paramBoolean)
      {
        V(new Runnable()
        {
          public final void run()
          {
            AppMethodBeat.i(146464);
            CameraUpdate localCameraUpdate = a.b(localTencentMap, paramFloat1, paramFloat2, paramFloat3, paramDouble1, this.iaH);
            ae.v("MicroMsg.DefaultTencentMapView", "doing map:%s latitude:%f longitude:%f scale:%f rotate:%f skew:%f", new Object[] { this, Double.valueOf(paramDouble1), Double.valueOf(this.iaH), Float.valueOf(paramFloat1), Float.valueOf(paramFloat2), Float.valueOf(paramFloat3) });
            a.a(a.this, false);
            localTencentMap.animateCamera(localCameraUpdate, a.blN(), a.t(a.this));
            AppMethodBeat.o(146464);
          }
        });
        AppMethodBeat.o(146522);
        return;
      }
      CameraUpdate localCameraUpdate = a(localTencentMap, paramFloat1, paramFloat2, paramFloat3, paramDouble1, paramDouble2);
      this.kTN = false;
      localTencentMap.moveCamera(localCameraUpdate);
    }
    AppMethodBeat.o(146522);
  }
  
  public final void a(b.b arg1)
  {
    AppMethodBeat.i(146526);
    Object localObject1 = new CircleOptions();
    ((CircleOptions)localObject1).center(new com.tencent.mapsdk.raster.model.LatLng(???.latitude, ???.longitude));
    ((CircleOptions)localObject1).radius(???.radius);
    ((CircleOptions)localObject1).strokeColor(???.strokeColor);
    ((CircleOptions)localObject1).strokeWidth(???.strokeWidth);
    ((CircleOptions)localObject1).fillColor(???.fillColor);
    int i = Qx(???.kVm);
    if (i >= 0) {
      ((CircleOptions)localObject1).level(i);
    }
    for (;;)
    {
      localObject1 = this.kTr.getMap().addCircle((CircleOptions)localObject1);
      synchronized (this.kTX)
      {
        this.kTX.add(localObject1);
        AppMethodBeat.o(146526);
        return;
        ((CircleOptions)localObject1).level(OverlayLevel.OverlayLevelAboveLabels);
      }
    }
  }
  
  public final void a(b.k paramk, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    int i = 0;
    AppMethodBeat.i(146531);
    Object localObject1 = new PolylineOptions();
    Object localObject2 = new ArrayList();
    Object localObject3 = paramk.kVx.iterator();
    while (((Iterator)localObject3).hasNext())
    {
      b.i locali = (b.i)((Iterator)localObject3).next();
      ((List)localObject2).add(new com.tencent.mapsdk.raster.model.LatLng(locali.latitude, locali.longitude));
    }
    ((PolylineOptions)localObject1).addAll((Iterable)localObject2);
    if ((paramk.kVC != null) && (paramk.kVC.size() > 0))
    {
      ae.i("MicroMsg.DefaultTencentMapView", "colorList size:%d ", new Object[] { Integer.valueOf(paramk.kVC.size()) });
      int j = paramk.kVC.size();
      localObject2 = new int[j];
      localObject3 = new int[j];
      while (i < j)
      {
        localObject2[i] = ((Integer)paramk.kVC.get(i)).intValue();
        localObject3[i] = i;
        i += 1;
      }
      ((PolylineOptions)localObject1).colors((int[])localObject2, (int[])localObject3);
      ((PolylineOptions)localObject1).width(paramk.width);
      ((PolylineOptions)localObject1).setDottedLine(paramk.kVy);
      ((PolylineOptions)localObject1).edgeColor(paramk.borderColor);
      ((PolylineOptions)localObject1).edgeWidth(paramk.borderWidth);
      if (paramk.kVz)
      {
        parama = parama.cA(paramk.kVA);
        ((PolylineOptions)localObject1).arrowTexture(new BitmapDescriptor(parama));
        if (parama == null) {
          ((PolylineOptions)localObject1).arrowTexture(new BitmapDescriptor(BitmapFactory.decodeResource(this.context.getResources(), 2131230986)));
        }
      }
      ((PolylineOptions)localObject1).zIndex(1.0F);
      if ((!bu.isNullOrNil(paramk.buildingId)) && (!bu.isNullOrNil(paramk.floorName))) {
        ((PolylineOptions)localObject1).indoorInfo(new IndoorInfo(paramk.buildingId, paramk.floorName));
      }
      ((PolylineOptions)localObject1).arrowGap(paramk.kVB);
      i = Qx(paramk.kVm);
      if (i < 0) {
        break label472;
      }
      ((PolylineOptions)localObject1).level(i);
    }
    for (;;)
    {
      ((PolylineOptions)localObject1).lineCap(true);
      localObject1 = this.kTr.getMap().addPolyline((PolylineOptions)localObject1);
      if (localObject1 != null) {
        break label484;
      }
      ae.e("MicroMsg.DefaultTencentMapView", "polyline is null, return");
      AppMethodBeat.o(146531);
      return;
      if (paramk.style != -1)
      {
        ((PolylineOptions)localObject1).color(paramk.style);
        break;
      }
      ((PolylineOptions)localObject1).color(paramk.color);
      break;
      label472:
      ((PolylineOptions)localObject1).level(OverlayLevel.OverlayLevelAboveLabels);
    }
    label484:
    if (bu.isNullOrNil(paramk.id)) {}
    for (parama = paramk.hashCode();; parama = paramk.id)
    {
      this.kTZ.put(bu.bI(parama, paramk.toString()), localObject1);
      AppMethodBeat.o(146531);
      return;
    }
  }
  
  public final void a(b.l paraml)
  {
    this.kTt = paraml;
  }
  
  public final void a(b.m paramm)
  {
    this.kTK = paramm;
  }
  
  public final void a(b.o paramo)
  {
    this.kTv = paramo;
  }
  
  public final void a(b.p paramp)
  {
    this.kTu = paramp;
  }
  
  public final void a(b.s params)
  {
    this.kTw = params;
  }
  
  public final void a(b.w paramw)
  {
    AppMethodBeat.i(146499);
    this.kTO = paramw;
    this.kTr.getMap().setTencentMapGestureListener(new TencentMap.TencentMapGestureListener()
    {
      public final boolean onDoubleTap(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
      
      public final boolean onDown(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        AppMethodBeat.i(146486);
        a.a(a.this, true);
        AppMethodBeat.o(146486);
        return false;
      }
      
      public final boolean onFling(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
      
      public final boolean onLongPress(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
      
      public final void onMapStable()
      {
        AppMethodBeat.i(146487);
        AppMethodBeat.o(146487);
      }
      
      public final boolean onScroll(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
      
      public final boolean onSingleTap(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
      
      public final boolean onUp(float paramAnonymousFloat1, float paramAnonymousFloat2)
      {
        return false;
      }
    });
    this.kTr.getMap().setOnMapCameraChangeListener(new TencentMap.OnMapCameraChangeListener()
    {
      public final void onCameraChange(CameraPosition paramAnonymousCameraPosition)
      {
        AppMethodBeat.i(146459);
        if (a.p(a.this) != null)
        {
          b.a locala = new b.a();
          locala.kVk = new b.i(paramAnonymousCameraPosition.getTarget().getLatitude(), paramAnonymousCameraPosition.getTarget().getLongitude());
          locala.zoom = paramAnonymousCameraPosition.getZoom();
          locala.kVl = paramAnonymousCameraPosition.getBearing();
          locala.skew = paramAnonymousCameraPosition.getSkew();
          a.p(a.this).a(locala, a.q(a.this));
        }
        AppMethodBeat.o(146459);
      }
      
      public final void onCameraChangeFinish(CameraPosition paramAnonymousCameraPosition)
      {
        AppMethodBeat.i(146460);
        if (a.p(a.this) != null)
        {
          b.a locala = new b.a();
          locala.kVk = new b.i(paramAnonymousCameraPosition.getTarget().getLatitude(), paramAnonymousCameraPosition.getTarget().getLongitude());
          locala.zoom = paramAnonymousCameraPosition.getZoom();
          locala.kVl = paramAnonymousCameraPosition.getBearing();
          locala.skew = paramAnonymousCameraPosition.getSkew();
          a.p(a.this).b(locala, a.q(a.this));
          a.a(a.this, false);
        }
        a.j(a.this);
        AppMethodBeat.o(146460);
      }
    });
    AppMethodBeat.o(146499);
  }
  
  public final void a(b.x paramx)
  {
    this.kTQ = paramx;
  }
  
  public final void a(b.z paramz)
  {
    this.kTM = paramz;
  }
  
  public final void a(String paramString, int paramInt, b.i parami, boolean paramBoolean)
  {
    AppMethodBeat.i(146532);
    Polyline localPolyline = (Polyline)this.kTZ.get(paramString);
    if (localPolyline == null)
    {
      ae.w("MicroMsg.DefaultTencentMapView", "lineId:%s is null", new Object[] { paramString });
      AppMethodBeat.o(146532);
      return;
    }
    localPolyline.setEraseable(paramBoolean);
    localPolyline.eraseTo(paramInt, new com.tencent.mapsdk.raster.model.LatLng(parami.latitude, parami.longitude));
    AppMethodBeat.o(146532);
  }
  
  public final void a(String paramString, b.v paramv)
  {
    AppMethodBeat.i(182816);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s updateMarker markerId:%s", new Object[] { this, paramString });
    final g localg = Qt(paramString);
    if (localg != null) {
      if (paramv.kVI != null)
      {
        if (paramv.kVI.view == null) {
          break label546;
        }
        localg.kVD.kVI = paramv.kVI;
        if ((localg.kVc == null) || (paramv.kVI.kVP != b.v.a.kVT)) {
          break label500;
        }
        b(localg.kVc);
      }
    }
    label546:
    for (;;)
    {
      if (paramv.kVJ != null)
      {
        localg.kVi.remove();
        Qu(paramString + "#label");
        MarkerOptions localMarkerOptions = new MarkerOptions();
        localMarkerOptions.position(new com.tencent.mapsdk.raster.model.LatLng(paramv.latitude, paramv.longitude));
        com.tencent.mm.plugin.appbrand.widget.d.d locald2 = i.blQ();
        com.tencent.mm.plugin.appbrand.widget.d.d locald1 = locald2;
        if (locald2 == null) {
          locald1 = new com.tencent.mm.plugin.appbrand.widget.d.d(this.context);
        }
        locald1.reset();
        locald1.setTextColor(paramv.kVJ.color);
        locald1.setTextSize(paramv.kVJ.kVK);
        locald1.setText(paramv.kVJ.content);
        locald1.setTextPadding(paramv.kVJ.padding);
        locald1.setGravity(paramv.kVJ.jWV);
        locald1.A(paramv.kVJ.kVL, paramv.kVJ.borderWidth, paramv.kVJ.borderColor, paramv.kVJ.bgColor);
        locald1.setX(paramv.kVJ.x);
        locald1.setY(paramv.kVJ.y);
        locald1.eA(paramv.kVJ.width, paramv.kVJ.height);
        locald1.measure(0, 0);
        localMarkerOptions.anchor(locald1.getAnchorX(), locald1.getAnchorY());
        localMarkerOptions.markerView(locald1);
        localMarkerOptions.visible(true);
        localMarkerOptions.tag(paramString + "#label");
        localMarkerOptions.zIndex(paramv.zIndex + 3);
        localMarkerOptions.level(OverlayLevel.OverlayLevelAboveLabels);
        if ((!bu.isNullOrNil(paramv.buildingId)) && (!bu.isNullOrNil(paramv.floorName))) {
          localMarkerOptions.indoorInfo(new IndoorInfo(paramv.buildingId, paramv.floorName));
        }
        localg.kVi = this.kTr.getMap().addMarker(localMarkerOptions);
        a(paramString + "#label", localg);
      }
      AppMethodBeat.o(182816);
      return;
      label500:
      if (localg.kVc != null) {
        if (Looper.myLooper() == Looper.getMainLooper())
        {
          localg.kVc.refreshInfoWindow();
        }
        else
        {
          ar.f(new Runnable()
          {
            public final void run()
            {
              AppMethodBeat.i(212537);
              localg.kVc.refreshInfoWindow();
              AppMethodBeat.o(212537);
            }
          });
          continue;
          localg.kVj = null;
        }
      }
    }
  }
  
  public final void a(final String paramString, final b.v paramv, final com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(146536);
    if (Looper.myLooper() == Looper.getMainLooper())
    {
      b(paramString, paramv, parama);
      AppMethodBeat.o(146536);
      return;
    }
    ar.f(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(212533);
        a.this.b(paramString, paramv, parama);
        AppMethodBeat.o(212533);
      }
    });
    AppMethodBeat.o(146536);
  }
  
  public final void a(final String paramString, ArrayList<b.i> arg2, long paramLong, boolean paramBoolean, b.r arg6)
  {
    AppMethodBeat.i(212577);
    g localg = Qt(paramString);
    if ((localg == null) || (localg.kVc == null))
    {
      ae.e("MicroMsg.DefaultTencentMapView", "get marker failed!");
      ???.gy(false);
      AppMethodBeat.o(212577);
      return;
    }
    if (???.size() <= 0)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "keyFrame is empty, err, return");
      ???.gy(false);
      AppMethodBeat.o(212577);
      return;
    }
    synchronized (this.kTD)
    {
      if (this.kTD.containsKey(paramString)) {
        m.runOnUiThread(new Runnable()
        {
          public final void run()
          {
            AppMethodBeat.i(212543);
            this.kUR.cancelAnimation();
            AppMethodBeat.o(212543);
          }
        });
      }
      ??? = new com.tencent.mapsdk.raster.model.LatLng[???.size()];
      ??? = ???.iterator();
      int i = 0;
      if (???.hasNext())
      {
        b.i locali = (b.i)???.next();
        ???[i] = new com.tencent.mapsdk.raster.model.LatLng(locali.latitude, locali.longitude);
        i += 1;
      }
    }
    ??? = new MarkerTranslateAnimator(localg.kVc, paramLong, (com.tencent.mapsdk.raster.model.LatLng[])???, paramBoolean);
    ???.addAnimatorListener(new Animator.AnimatorListener()
    {
      public final void onAnimationCancel(Animator arg1)
      {
        AppMethodBeat.i(212546);
        ae.i("MicroMsg.DefaultTencentMapView", "MarkerTranslateAnimator onAnimationCancel");
        if (paramr != null) {
          paramr.blt();
        }
        synchronized (a.v(a.this))
        {
          a.v(a.this).remove(paramString);
          AppMethodBeat.o(212546);
          return;
        }
      }
      
      public final void onAnimationEnd(Animator arg1)
      {
        AppMethodBeat.i(212545);
        ae.i("MicroMsg.DefaultTencentMapView", "MarkerTranslateAnimator onAnimationEnd");
        if (paramr != null) {
          paramr.gy(true);
        }
        synchronized (a.v(a.this))
        {
          a.v(a.this).remove(paramString);
          AppMethodBeat.o(212545);
          return;
        }
      }
      
      public final void onAnimationRepeat(Animator paramAnonymousAnimator) {}
      
      public final void onAnimationStart(Animator paramAnonymousAnimator)
      {
        AppMethodBeat.i(212544);
        ae.i("MicroMsg.DefaultTencentMapView", "MarkerTranslateAnimator onAnimationStart");
        AppMethodBeat.o(212544);
      }
    });
    synchronized (this.kTD)
    {
      this.kTD.put(paramString, ???);
      m.runOnUiThread(new Runnable()
      {
        public final void run()
        {
          AppMethodBeat.i(212547);
          ae.d("MicroMsg.DefaultTencentMapView", "MarkerTranslateAnimator start");
          paramArrayList.startAnimation();
          AppMethodBeat.o(212547);
        }
      });
      if (localg.kVi != null)
      {
        ae.d("MicroMsg.DefaultTencentMapView", "labelMarker do move animator");
        synchronized (this.kTE)
        {
          if (this.kTE.containsKey(paramString)) {
            m.runOnUiThread(new Runnable()
            {
              public final void run()
              {
                AppMethodBeat.i(212548);
                this.kUR.cancelAnimation();
                AppMethodBeat.o(212548);
              }
            });
          }
          ??? = new MarkerTranslateAnimator(localg.kVi, paramLong, (com.tencent.mapsdk.raster.model.LatLng[])???, paramBoolean);
          ???.addAnimatorListener(new Animator.AnimatorListener()
          {
            public final void onAnimationCancel(Animator arg1)
            {
              AppMethodBeat.i(212550);
              synchronized (a.w(a.this))
              {
                a.w(a.this).remove(paramString);
                AppMethodBeat.o(212550);
                return;
              }
            }
            
            public final void onAnimationEnd(Animator arg1)
            {
              AppMethodBeat.i(212549);
              synchronized (a.w(a.this))
              {
                a.w(a.this).remove(paramString);
                AppMethodBeat.o(212549);
                return;
              }
            }
            
            public final void onAnimationRepeat(Animator paramAnonymousAnimator) {}
            
            public final void onAnimationStart(Animator paramAnonymousAnimator) {}
          });
        }
      }
    }
    synchronized (this.kTE)
    {
      this.kTE.put(paramString, ???);
      m.runOnUiThread(new Runnable()
      {
        public final void run()
        {
          AppMethodBeat.i(212551);
          paramr.startAnimation();
          AppMethodBeat.o(212551);
        }
      });
      AppMethodBeat.o(212577);
      return;
      paramString = finally;
      AppMethodBeat.o(212577);
      throw paramString;
      paramString = finally;
      AppMethodBeat.o(212577);
      throw paramString;
    }
  }
  
  public final void a(String paramString, LinkedList<b.h> paramLinkedList, final b.r paramr)
  {
    AppMethodBeat.i(146539);
    paramString = Qt(paramString);
    if ((paramString == null) || (paramString.kVc == null))
    {
      ae.e("MicroMsg.DefaultTencentMapView", "get marker failed!");
      paramr.gy(false);
      AppMethodBeat.o(146539);
      return;
    }
    if (paramLinkedList.size() <= 0)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "keyFrame is empty, err, return");
      paramr.gy(false);
      AppMethodBeat.o(146539);
      return;
    }
    Object localObject = (b.h)paramLinkedList.get(0);
    ((b.h)localObject).kVu = paramString.kVc.getPosition().getLatitude();
    ((b.h)localObject).kVt = paramString.kVc.getPosition().getLongitude();
    int j = paramLinkedList.size();
    int i = 1;
    if (i < j)
    {
      localObject = (b.h)paramLinkedList.get(i - 1);
      b.h localh = (b.h)paramLinkedList.get(i);
      if (((b.h)localObject).kVl == 0.0F) {
        localh.kVt = ((b.h)localObject).longitude;
      }
      for (localh.kVu = ((b.h)localObject).latitude;; localh.kVu = paramString.kVc.getPosition().getLatitude())
      {
        i += 1;
        break;
        localh.kVt = paramString.kVc.getPosition().getLongitude();
      }
    }
    localObject = new c(paramLinkedList, paramString.kVc, this.kTr);
    paramr = new Animator.AnimatorListener()
    {
      public final void onAnimationCancel(Animator paramAnonymousAnimator) {}
      
      public final void onAnimationEnd(Animator paramAnonymousAnimator)
      {
        AppMethodBeat.i(212540);
        ae.d("MicroMsg.DefaultTencentMapView", "MarkerAnimator end");
        if (paramr != null) {
          paramr.gy(true);
        }
        AppMethodBeat.o(212540);
      }
      
      public final void onAnimationRepeat(Animator paramAnonymousAnimator) {}
      
      public final void onAnimationStart(Animator paramAnonymousAnimator)
      {
        AppMethodBeat.i(212539);
        ae.d("MicroMsg.DefaultTencentMapView", "MarkerAnimator start");
        AppMethodBeat.o(212539);
      }
    };
    ((c)localObject).wV.addListener(paramr);
    m.runOnUiThread(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(212541);
        this.kUP.wV.start();
        AppMethodBeat.o(212541);
      }
    });
    if (paramString.kVi != null) {
      m.runOnUiThread(new Runnable()
      {
        public final void run()
        {
          AppMethodBeat.i(212542);
          this.kUQ.wV.start();
          AppMethodBeat.o(212542);
        }
      });
    }
    AppMethodBeat.o(146539);
  }
  
  public final boolean a(final int paramInt, b.e parame, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(212585);
    if (paramInt < 0)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "addGroundOverlay fail, id < 0 or options is null");
      AppMethodBeat.o(212585);
      return false;
    }
    ae.i("MicroMsg.DefaultTencentMapView", "remove GroundOverlay id:%d, first ,and than add again", new Object[] { Integer.valueOf(paramInt) });
    tj(paramInt);
    ae.i("MicroMsg.DefaultTencentMapView", "addGroundOverlay id:%d", new Object[] { Integer.valueOf(paramInt) });
    final GroundOverlayOptions localGroundOverlayOptions = new GroundOverlayOptions();
    if ((parame.kVp != null) && (parame.kVq != null))
    {
      com.tencent.tencentmap.mapsdk.maps.model.LatLng localLatLng1 = new com.tencent.tencentmap.mapsdk.maps.model.LatLng();
      localLatLng1.longitude = parame.kVp.longitude;
      localLatLng1.latitude = parame.kVp.latitude;
      com.tencent.tencentmap.mapsdk.maps.model.LatLng localLatLng2 = new com.tencent.tencentmap.mapsdk.maps.model.LatLng();
      localLatLng2.longitude = parame.kVq.longitude;
      localLatLng2.latitude = parame.kVq.latitude;
      localGroundOverlayOptions.latLngBounds(new com.tencent.tencentmap.mapsdk.maps.model.LatLngBounds(localLatLng2, localLatLng1));
    }
    localGroundOverlayOptions.alpha(parame.alpha);
    localGroundOverlayOptions.visible(parame.visible);
    localGroundOverlayOptions.zIndex(parame.zIndex);
    if ((parama != null) && (!TextUtils.isEmpty(parame.kVn)))
    {
      parame = parama.a(parame.kVn, null, new com.tencent.mm.plugin.appbrand.f.a.b()
      {
        public final void N(Bitmap paramAnonymousBitmap)
        {
          AppMethodBeat.i(212557);
          if (paramAnonymousBitmap != null)
          {
            localGroundOverlayOptions.bitmap(BitmapDescriptorFactory.fromBitmap(paramAnonymousBitmap));
            paramAnonymousBitmap = a.this.kTr.getMap().addGroundOverlay(localGroundOverlayOptions);
            if (paramAnonymousBitmap == null)
            {
              ae.e("MicroMsg.DefaultTencentMapView", "addGroundOverlay fail, groundOverlay is null");
              AppMethodBeat.o(212557);
              return;
            }
            a.E(a.this).put(paramInt, paramAnonymousBitmap);
            ae.i("MicroMsg.DefaultTencentMapView", "addGroundOverlay success id:%d", new Object[] { Integer.valueOf(paramInt) });
            AppMethodBeat.o(212557);
            return;
          }
          ae.e("MicroMsg.DefaultTencentMapView", "groundOverlay load bitmap is null");
          AppMethodBeat.o(212557);
        }
      });
      if (parame != null)
      {
        localGroundOverlayOptions.bitmap(BitmapDescriptorFactory.fromBitmap(parame));
        parame = this.kTr.getMap().addGroundOverlay(localGroundOverlayOptions);
        if (parame == null)
        {
          ae.e("MicroMsg.DefaultTencentMapView", "addGroundOverlay fail, groundOverlay is null");
          AppMethodBeat.o(212585);
          return false;
        }
        this.kUu.put(String.valueOf(paramInt), parame);
        ae.i("MicroMsg.DefaultTencentMapView", "addGroundOverlay success id:%d", new Object[] { Integer.valueOf(paramInt) });
      }
    }
    while (parama == null)
    {
      AppMethodBeat.o(212585);
      return true;
    }
    ae.e("MicroMsg.DefaultTencentMapView", "groundOverlay icon path is null");
    AppMethodBeat.o(212585);
    return false;
  }
  
  public final boolean a(b.ab arg1)
  {
    AppMethodBeat.i(146543);
    Object localObject1 = new PolygonOptions();
    ArrayList localArrayList = new ArrayList();
    Iterator localIterator = ???.kVx.iterator();
    while (localIterator.hasNext())
    {
      b.i locali = (b.i)localIterator.next();
      localArrayList.add(new com.tencent.mapsdk.raster.model.LatLng(locali.latitude, locali.longitude));
    }
    ((PolygonOptions)localObject1).addAll(localArrayList);
    ((PolygonOptions)localObject1).fillColor(???.fillColor);
    ((PolygonOptions)localObject1).strokeColor(???.strokeColor);
    ((PolygonOptions)localObject1).strokeWidth(???.strokeWidth);
    ((PolygonOptions)localObject1).zIndex(???.zIndex);
    int i = Qx(???.kVm);
    if (i >= 0) {
      ((PolygonOptions)localObject1).level(i);
    }
    localObject1 = this.kTr.getMap().addPolygon((PolygonOptions)localObject1);
    synchronized (this.kUd)
    {
      this.kUd.add(localObject1);
      AppMethodBeat.o(146543);
      return true;
    }
  }
  
  public final boolean a(b.c paramc, b.n paramn, final com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(146528);
    if (bu.isNullOrNil(paramc.kVn))
    {
      AppMethodBeat.o(146528);
      return false;
    }
    Object localObject = parama.cA(paramc.kVn);
    if (localObject == null)
    {
      AppMethodBeat.o(146528);
      return false;
    }
    parama = new ImageView(this.context);
    parama.setScaleType(ImageView.ScaleType.FIT_XY);
    parama.setImageBitmap((Bitmap)localObject);
    int k = paramc.left;
    int m = paramc.top;
    int i;
    if (paramc.height == 0)
    {
      i = com.tencent.mm.plugin.appbrand.y.g.vO(((Bitmap)localObject).getHeight());
      if (paramc.width != 0) {
        break label217;
      }
    }
    label217:
    for (int j = com.tencent.mm.plugin.appbrand.y.g.vO(((Bitmap)localObject).getWidth());; j = paramc.width)
    {
      localObject = new FrameLayout.LayoutParams(j, i);
      ((FrameLayout.LayoutParams)localObject).setMargins(k, m, 0, 0);
      boolean bool = paramc.kVo;
      paramc = paramc.data;
      parama.setOnTouchListener(new a.7(this, bool, parama));
      parama.setOnClickListener(new a.8(this, bool, paramn, paramc));
      m.runOnUiThread(new Runnable()
      {
        public final void run()
        {
          AppMethodBeat.i(146469);
          ((ViewGroup)a.this.container).addView(parama, this.kUC);
          synchronized (a.this.kTY)
          {
            a.this.kTY.add(new a.b(a.this, parama));
            AppMethodBeat.o(146469);
            return;
          }
        }
      });
      AppMethodBeat.o(146528);
      return true;
      i = paramc.height;
      break;
    }
  }
  
  public final boolean a(b.d arg1, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(146541);
    if (bu.isNullOrNil(???.kVn))
    {
      AppMethodBeat.o(146541);
      return false;
    }
    MarkerOptions localMarkerOptions = new MarkerOptions();
    localMarkerOptions.position(new com.tencent.mapsdk.raster.model.LatLng(???.latitude, ???.longitude));
    parama = parama.cA(???.kVn);
    if ((parama != null) && (!parama.isRecycled())) {
      localMarkerOptions.icon(new BitmapDescriptor(parama));
    }
    localMarkerOptions.rotation(???.kVl);
    parama = this.kTr.getMap().addMarker(localMarkerOptions);
    synchronized (this.kUc)
    {
      this.kUc.add(new c(parama));
      AppMethodBeat.o(146541);
      return true;
    }
  }
  
  public final boolean a(boolean paramBoolean1, boolean paramBoolean2, final int paramInt1, final int paramInt2, b.q paramq)
  {
    AppMethodBeat.i(212579);
    boolean bool;
    if (this.kTr.getMap() == null)
    {
      bool = false;
      if (bool)
      {
        this.kUk = paramq;
        this.kUs = paramBoolean2;
        this.kUt = paramBoolean1;
        ae.i("MicroMsg.DefaultTencentMapView", "initMapMarkerCluster zoomOnClick:%b, enableDefaultStyle:%b", new Object[] { Boolean.valueOf(paramBoolean2), Boolean.valueOf(paramBoolean1) });
      }
      AppMethodBeat.o(212579);
      return bool;
    }
    Runnable local36 = new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(212556);
        if (a.b(a.this) == null) {
          a.a(a.this, new a.d(a.this, a.u(a.this), a.this.kTr.getMap()));
        }
        if (a.z(a.this) == null)
        {
          a.a(a.this, new a.e(a.this, a.u(a.this), a.this.kTr.getMap(), a.b(a.this)));
          a.z(a.this).setMinClusterSize(paramInt1);
          NonHierarchicalDistanceBasedAlgorithm localNonHierarchicalDistanceBasedAlgorithm = new NonHierarchicalDistanceBasedAlgorithm(a.u(a.this));
          localNonHierarchicalDistanceBasedAlgorithm.setMaxDistanceAtZoom(paramInt2);
          a.b(a.this).setAlgorithm(localNonHierarchicalDistanceBasedAlgorithm);
        }
        for (;;)
        {
          a.b(a.this).setOnClusterClickListener(new ClusterManager.OnClusterClickListener()
          {
            public final boolean onClusterClick(Cluster<a.f> paramAnonymous2Cluster)
            {
              AppMethodBeat.i(212555);
              boolean bool = a.this.b(paramAnonymous2Cluster);
              AppMethodBeat.o(212555);
              return bool;
            }
          });
          a.b(a.this).setOnClusterItemClickListener(new ClusterManager.OnClusterItemClickListener() {});
          ae.i("MicroMsg.DefaultTencentMapView", "init map marker cluster finish");
          a.b(a.this).setRenderer(a.z(a.this));
          a.b(a.this).setClusterEnabled(true);
          a.this.kTr.getMap().setOnMapCameraChangeListener(a.b(a.this));
          AppMethodBeat.o(212556);
          return;
          a.z(a.this).setMinClusterSize(paramInt1);
          if ((a.b(a.this).getAlgorithm() != null) && ((a.b(a.this).getAlgorithm() instanceof NonHierarchicalDistanceBasedAlgorithm))) {
            ((NonHierarchicalDistanceBasedAlgorithm)a.b(a.this).getAlgorithm()).setMaxDistanceAtZoom(paramInt2);
          }
        }
      }
    };
    if (Looper.myLooper() == Looper.getMainLooper()) {
      local36.run();
    }
    for (;;)
    {
      bool = true;
      break;
      ar.f(local36);
    }
  }
  
  public final void b(final String paramString, b.v paramv, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(212573);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s addMarker markerId:%s", new Object[] { this, paramString });
    if (this.kUj.get())
    {
      ae.e("MicroMsg.DefaultTencentMapView", "map:%s addMarker markerId:%s fail", new Object[] { this, paramString });
      AppMethodBeat.o(212573);
      return;
    }
    Object localObject1;
    g localg;
    MarkerOptions localMarkerOptions;
    final FrameLayout localFrameLayout;
    if (paramv.kVh > 0)
    {
      localObject1 = paramv.kVh;
      if (this.kUt)
      {
        ae.i("MicroMsg.DefaultTencentMapView", "cluster item use default style");
        AppMethodBeat.o(212573);
        return;
      }
      c((String)localObject1, paramv, parama);
      localg = new g();
      localg.data = paramv.data;
      localg.kVD = paramv;
      a((String)localObject1, localg);
      if ((!paramv.kVE) && (!TextUtils.isEmpty(paramString)))
      {
        localMarkerOptions = new MarkerOptions();
        localMarkerOptions.position(new com.tencent.mapsdk.raster.model.LatLng(paramv.latitude, paramv.longitude));
        if (!bu.isNullOrNil(paramv.title)) {
          localMarkerOptions.title(paramv.title);
        }
        if (!bu.isNullOrNil(paramv.kVH)) {
          localMarkerOptions.contentDescription(paramv.kVH);
        }
        localMarkerOptions.rotation(paramv.kVl);
        localMarkerOptions.alpha(paramv.alpha);
        localFrameLayout = (FrameLayout)((LayoutInflater)this.context.getSystemService("layout_inflater")).inflate(2131493687, null);
        final ImageView localImageView = (ImageView)localFrameLayout.findViewById(2131302158);
        final float f1 = paramv.kVF;
        final float f2 = paramv.kVG;
        Object localObject3 = null;
        Object localObject2 = localObject3;
        if (parama != null)
        {
          localObject2 = localObject3;
          if (!bu.isNullOrNil(paramv.kVn))
          {
            if (localImageView != null) {
              localImageView.setImageBitmap(null);
            }
            localObject2 = parama.a(paramv.kVn, null, new com.tencent.mm.plugin.appbrand.f.a.b()
            {
              public final void N(Bitmap paramAnonymousBitmap)
              {
                AppMethodBeat.i(212534);
                if (paramAnonymousBitmap != null)
                {
                  a.b(f1, f2, paramAnonymousBitmap, localImageView);
                  if ((a.this.Qt(paramString) != null) && (a.this.Qt(paramString).kVc != null))
                  {
                    paramAnonymousBitmap = a.this.Qt(paramString).kVc;
                    if (paramAnonymousBitmap != null) {
                      paramAnonymousBitmap.setMarkerView(localFrameLayout);
                    }
                    ae.v("MicroMsg.DefaultTencentMapView", "setMarkerView");
                  }
                }
                AppMethodBeat.o(212534);
              }
            });
          }
        }
        if (localObject2 == null) {
          break label691;
        }
        a(f1, f2, (Bitmap)localObject2, localImageView);
      }
    }
    for (;;)
    {
      localMarkerOptions.markerView(localFrameLayout);
      localMarkerOptions.anchor(paramv.anchorX, paramv.anchorY);
      localMarkerOptions.tag(localObject1);
      localMarkerOptions.infoWindowHideAnimation(new Animation() {});
      localMarkerOptions.infoWindowShowAnimation(new Animation() {});
      localMarkerOptions.visible(true);
      localMarkerOptions.zIndex(paramv.zIndex + 2);
      localMarkerOptions.level(OverlayLevel.OverlayLevelAboveLabels);
      if ((!bu.isNullOrNil(paramv.buildingId)) && (!bu.isNullOrNil(paramv.floorName))) {
        localMarkerOptions.indoorInfo(new IndoorInfo(paramv.buildingId, paramv.floorName));
      }
      if (paramv.kVI != null) {
        localMarkerOptions.infoWindowOffset(paramv.kVI.kVQ, paramv.kVI.kVR);
      }
      localg.kVc = this.kTr.getMap().addMarker(localMarkerOptions);
      if ((paramv.kVI != null) && (paramv.kVI.kVP == b.v.a.kVT)) {
        b(localg.kVc);
      }
      if (((!paramv.kVE) || (paramv.kVh != 0)) && (paramv.kVJ != null)) {
        a((String)localObject1, paramv, localg);
      }
      AppMethodBeat.o(212573);
      return;
      if (paramv.kVE)
      {
        localObject1 = new b.aa();
        ((b.aa)localObject1).kVU = new b.i(paramv.latitude, paramv.longitude);
        ((b.aa)localObject1).id = paramString;
        a((b.aa)localObject1);
        c(paramString, paramv, parama);
      }
      localObject1 = paramString;
      break;
      label691:
      ae.e("MicroMsg.DefaultTencentMapView", "[addMarker] bitmap is null, use default");
    }
  }
  
  public final boolean b(final int paramInt, b.e parame, com.tencent.mm.plugin.appbrand.f.a parama)
  {
    AppMethodBeat.i(212587);
    if (!this.kUu.containsKey(String.valueOf(paramInt)))
    {
      ae.e("MicroMsg.DefaultTencentMapView", "updateGroundOverlay fail, groundOverlay not exist");
      AppMethodBeat.o(212587);
      return false;
    }
    final GroundOverlay localGroundOverlay = (GroundOverlay)this.kUu.get(String.valueOf(paramInt));
    if (localGroundOverlay == null)
    {
      ae.e("MicroMsg.DefaultTencentMapView", "updateGroundOverlay fail, groundOverlay is null");
      AppMethodBeat.o(212587);
      return false;
    }
    ae.i("MicroMsg.DefaultTencentMapView", "updateGroundOverlay id:%d", new Object[] { Integer.valueOf(paramInt) });
    if ((parame.kVp != null) && (parame.kVq != null))
    {
      com.tencent.tencentmap.mapsdk.maps.model.LatLng localLatLng1 = new com.tencent.tencentmap.mapsdk.maps.model.LatLng();
      localLatLng1.longitude = parame.kVp.longitude;
      localLatLng1.latitude = parame.kVp.latitude;
      com.tencent.tencentmap.mapsdk.maps.model.LatLng localLatLng2 = new com.tencent.tencentmap.mapsdk.maps.model.LatLng();
      localLatLng2.longitude = parame.kVq.longitude;
      localLatLng2.latitude = parame.kVq.latitude;
      localGroundOverlay.setLatLongBounds(new com.tencent.tencentmap.mapsdk.maps.model.LatLngBounds(localLatLng2, localLatLng1));
    }
    localGroundOverlay.setAlpha(parame.alpha);
    localGroundOverlay.setVisibility(parame.visible);
    localGroundOverlay.setZindex(parame.zIndex);
    if ((parama != null) && (!TextUtils.isEmpty(parame.kVn)))
    {
      parame = parama.a(parame.kVn, null, new com.tencent.mm.plugin.appbrand.f.a.b()
      {
        public final void N(Bitmap paramAnonymousBitmap)
        {
          AppMethodBeat.i(212558);
          if (paramAnonymousBitmap != null)
          {
            localGroundOverlay.setBitmap(BitmapDescriptorFactory.fromBitmap(paramAnonymousBitmap));
            ae.i("MicroMsg.DefaultTencentMapView", "updateGroundOverlay success id:%d", new Object[] { Integer.valueOf(paramInt) });
            AppMethodBeat.o(212558);
            return;
          }
          ae.e("MicroMsg.DefaultTencentMapView", "groundOverlay load bitmap is null");
          AppMethodBeat.o(212558);
        }
      });
      if (parame != null)
      {
        localGroundOverlay.setBitmap(BitmapDescriptorFactory.fromBitmap(parame));
        ae.i("MicroMsg.DefaultTencentMapView", "updateGroundOverlay success id:%d", new Object[] { Integer.valueOf(paramInt) });
      }
    }
    while (parama == null)
    {
      AppMethodBeat.o(212587);
      return true;
    }
    ae.e("MicroMsg.DefaultTencentMapView", "groundOverlay icon path is null");
    AppMethodBeat.o(212587);
    return false;
  }
  
  public final boolean b(Cluster<f> paramCluster)
  {
    AppMethodBeat.i(212578);
    ae.i("MicroMsg.DefaultTencentMapView", "onClusterClick");
    if (this.kUs)
    {
      LinkedList localLinkedList = new LinkedList();
      Iterator localIterator = paramCluster.getItems().iterator();
      while (localIterator.hasNext()) {
        localLinkedList.add(((f)localIterator.next()).getPosition());
      }
      i(localLinkedList, 0);
    }
    if (this.kUk != null) {
      this.kUk.R(a(paramCluster, false));
    }
    AppMethodBeat.o(212578);
    return true;
  }
  
  public final b.f blA()
  {
    AppMethodBeat.i(146515);
    if (this.kTR != null)
    {
      localf = this.kTR;
      AppMethodBeat.o(146515);
      return localf;
    }
    b.f localf = blw();
    AppMethodBeat.o(146515);
    return localf;
  }
  
  public boolean blB()
  {
    return false;
  }
  
  public final b.i blC()
  {
    AppMethodBeat.i(146519);
    Object localObject = this.kTr.getMap().getMapCenter();
    localObject = new b.i(((com.tencent.mapsdk.raster.model.LatLng)localObject).getLatitude(), ((com.tencent.mapsdk.raster.model.LatLng)localObject).getLongitude());
    AppMethodBeat.o(146519);
    return localObject;
  }
  
  public final void blD()
  {
    AppMethodBeat.i(146525);
    synchronized (this.kTX)
    {
      Iterator localIterator = this.kTX.iterator();
      if (localIterator.hasNext()) {
        ((Circle)localIterator.next()).remove();
      }
    }
    this.kTX.clear();
    AppMethodBeat.o(146525);
  }
  
  public final void blE()
  {
    AppMethodBeat.i(146527);
    m.runOnUiThread(new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(146465);
        ViewGroup localViewGroup = (ViewGroup)a.this.container;
        synchronized (a.this.kTY)
        {
          Iterator localIterator = a.this.kTY.iterator();
          if (localIterator.hasNext()) {
            localViewGroup.removeView(((a.b)localIterator.next()).kVb);
          }
        }
        a.this.kTY.clear();
        AppMethodBeat.o(146465);
      }
    });
    AppMethodBeat.o(146527);
  }
  
  public final void blF()
  {
    AppMethodBeat.i(146529);
    Iterator localIterator = this.kTZ.values().iterator();
    while (localIterator.hasNext()) {
      ((Polyline)localIterator.next()).remove();
    }
    this.kTZ.clear();
    AppMethodBeat.o(146529);
  }
  
  public final void blG()
  {
    AppMethodBeat.i(146534);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s removeAllMarker", new Object[] { this });
    Iterator localIterator = this.kUb.values().iterator();
    while (localIterator.hasNext())
    {
      g localg = (g)localIterator.next();
      if (localg.kVc != null) {
        localg.kVc.remove();
      }
      if (localg.kVi != null)
      {
        if ((localg.kVi.getMarkerView() != null) && ((localg.kVi.getMarkerView() instanceof com.tencent.mm.plugin.appbrand.widget.d.d))) {
          i.a((com.tencent.mm.plugin.appbrand.widget.d.d)localg.kVi.getMarkerView());
        }
        localg.kVi.remove();
      }
    }
    this.kUb.clear();
    blL();
    AppMethodBeat.o(146534);
  }
  
  public final void blH()
  {
    AppMethodBeat.i(146540);
    synchronized (this.kUc)
    {
      Iterator localIterator = this.kUc.iterator();
      if (localIterator.hasNext()) {
        ((c)localIterator.next()).kVc.remove();
      }
    }
    this.kUc.clear();
    AppMethodBeat.o(146540);
  }
  
  public final void blI()
  {
    AppMethodBeat.i(146542);
    synchronized (this.kUd)
    {
      Iterator localIterator = this.kUd.iterator();
      if (localIterator.hasNext()) {
        ((Polygon)localIterator.next()).remove();
      }
    }
    this.kUd.clear();
    AppMethodBeat.o(146542);
  }
  
  public final boolean blK()
  {
    return (this.kTF != null) && (this.kTG != null);
  }
  
  public final b.ac bly()
  {
    AppMethodBeat.i(146498);
    Object localObject2 = this.kTr.getProjection().getVisibleRegion().getLatLngBounds();
    Object localObject1 = new b.j();
    ((b.j)localObject1).kVv = new b.i(((com.tencent.mapsdk.raster.model.LatLngBounds)localObject2).getSouthwest().getLatitude(), ((com.tencent.mapsdk.raster.model.LatLngBounds)localObject2).getSouthwest().getLongitude());
    ((b.j)localObject1).kVw = new b.i(((com.tencent.mapsdk.raster.model.LatLngBounds)localObject2).getNortheast().getLatitude(), ((com.tencent.mapsdk.raster.model.LatLngBounds)localObject2).getNortheast().getLongitude());
    localObject2 = new b.ad();
    ((b.ad)localObject2).kVW = ((b.j)localObject1);
    localObject1 = new b.ac();
    ((b.ac)localObject1).kVV = ((b.ad)localObject2);
    AppMethodBeat.o(146498);
    return localObject1;
  }
  
  public final void blz()
  {
    AppMethodBeat.i(146501);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s moveToMapLocation", new Object[] { this });
    this.kTP = new Runnable()
    {
      public final void run()
      {
        AppMethodBeat.i(146461);
        if ((a.d(a.this) != null) && (a.r(a.this)))
        {
          double d1 = a.d(a.this).getLatitude();
          double d2 = a.d(a.this).getLongitude();
          a.this.kTr.getIController().animateTo(d1, d2);
          ae.d("MicroMsg.DefaultTencentMapView", "[moveToMapLocation]latitude:%f, longtitude:%f", new Object[] { Double.valueOf(d1), Double.valueOf(d2) });
        }
        AppMethodBeat.o(146461);
      }
    };
    if ((this.kUf != null) && (this.kUe))
    {
      this.kTP.run();
      this.kTP = null;
    }
    AppMethodBeat.o(146501);
  }
  
  public final void c(float paramFloat1, float paramFloat2, boolean paramBoolean)
  {
    AppMethodBeat.i(146516);
    com.tencent.mapsdk.raster.model.LatLng localLatLng = this.kTr.getMap().getMapCenter();
    this.kTr.setMapAnchor(paramFloat1, paramFloat2);
    a(localLatLng.getLatitude(), localLatLng.getLongitude(), -1.0F, -1.0F, -1.0F, paramBoolean);
    AppMethodBeat.o(146516);
  }
  
  public final b.i d(Point paramPoint)
  {
    AppMethodBeat.i(212589);
    ae.i("MicroMsg.DefaultTencentMapView", "fromScreenLocation");
    paramPoint = this.kTr.getMap().getProjection().fromScreenLocation(paramPoint);
    paramPoint = new b.i(paramPoint.getLatitude(), paramPoint.getLongitude());
    AppMethodBeat.o(212589);
    return paramPoint;
  }
  
  public final void dj(String paramString1, String paramString2)
  {
    AppMethodBeat.i(146514);
    this.kTr.getMap().setIndoorFloor(paramString1, paramString2);
    AppMethodBeat.o(146514);
  }
  
  public final void f(double paramDouble1, double paramDouble2)
  {
    AppMethodBeat.i(146500);
    a(paramDouble1, paramDouble2, -1.0F, -1.0F, -1.0F, true);
    AppMethodBeat.o(146500);
  }
  
  public void g(SurfaceTexture paramSurfaceTexture) {}
  
  public final void gA(boolean paramBoolean)
  {
    AppMethodBeat.i(146506);
    this.kTr.getUiSettings().setScrollGesturesEnabled(paramBoolean);
    AppMethodBeat.o(146506);
  }
  
  public final void gB(boolean paramBoolean)
  {
    AppMethodBeat.i(146507);
    this.kTr.getUiSettings().setRotateGesturesEnabled(paramBoolean);
    AppMethodBeat.o(146507);
  }
  
  public final void gC(boolean paramBoolean)
  {
    AppMethodBeat.i(146508);
    this.kTr.getUiSettings().setCompassEnabled(paramBoolean);
    AppMethodBeat.o(146508);
  }
  
  public final void gD(boolean paramBoolean)
  {
    AppMethodBeat.i(146510);
    this.kTr.getUiSettings().setTiltGesturesEnabled(paramBoolean);
    AppMethodBeat.o(146510);
  }
  
  public final void gE(boolean paramBoolean)
  {
    AppMethodBeat.i(146511);
    this.kTr.getMap().setSatelliteEnabled(paramBoolean);
    AppMethodBeat.o(146511);
  }
  
  public final void gF(boolean paramBoolean)
  {
    AppMethodBeat.i(146512);
    this.kTr.getMap().setTrafficEnabled(paramBoolean);
    AppMethodBeat.o(146512);
  }
  
  public final void gG(boolean paramBoolean)
  {
    AppMethodBeat.i(146513);
    this.kTr.getMap().setIndoorEnabled(paramBoolean);
    AppMethodBeat.o(146513);
  }
  
  public final void gH(boolean paramBoolean)
  {
    AppMethodBeat.i(146517);
    this.kTr.getUiSettings().setIndoorLevelPickerEnabled(paramBoolean);
    AppMethodBeat.o(146517);
  }
  
  public final void gI(boolean paramBoolean)
  {
    AppMethodBeat.i(146518);
    this.kTr.getUiSettings().setScaleControlsEnabled(paramBoolean);
    AppMethodBeat.o(146518);
  }
  
  public final void gJ(boolean paramBoolean)
  {
    AppMethodBeat.i(212567);
    this.kTr.getMap().showBuilding(paramBoolean);
    AppMethodBeat.o(212567);
  }
  
  public final void gK(boolean paramBoolean)
  {
    AppMethodBeat.i(146544);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s show location", new Object[] { this });
    this.kUe = paramBoolean;
    d locald;
    if (paramBoolean)
    {
      if (this.kUf != null)
      {
        locald = this.kUf;
        if (locald.kVc != null) {
          locald.kVc.setVisible(true);
        }
      }
      ((com.tencent.mm.plugin.appbrand.utils.b.a)e.K(com.tencent.mm.plugin.appbrand.utils.b.a.class)).b("gcj02", this.kUg, blJ());
      g.a(this);
      AppMethodBeat.o(146544);
      return;
    }
    if (this.kUf != null)
    {
      locald = this.kUf;
      if (locald.kVc != null) {
        locald.kVc.setVisible(false);
      }
    }
    ((com.tencent.mm.plugin.appbrand.utils.b.a)e.K(com.tencent.mm.plugin.appbrand.utils.b.a.class)).c("gcj02", this.kUg, blJ());
    g.b(this);
    AppMethodBeat.o(146544);
  }
  
  public final float getRotate()
  {
    AppMethodBeat.i(146503);
    float f = this.kTr.getMap().getCameraPosition().getBearing();
    AppMethodBeat.o(146503);
    return f;
  }
  
  public final float getSkew()
  {
    AppMethodBeat.i(146504);
    float f = this.kTr.getMap().getCameraPosition().getSkew();
    AppMethodBeat.o(146504);
    return f;
  }
  
  public final View getView()
  {
    return this.container;
  }
  
  public final float getZoom()
  {
    AppMethodBeat.i(212564);
    float f = this.kTr.getMap().getCameraPosition().getZoom();
    AppMethodBeat.o(212564);
    return f;
  }
  
  public final void gz(boolean paramBoolean)
  {
    AppMethodBeat.i(146505);
    this.kTr.getUiSettings().setZoomGesturesEnabled(paramBoolean);
    AppMethodBeat.o(146505);
  }
  
  public final void h(List<b.i> paramList, int paramInt)
  {
    AppMethodBeat.i(146520);
    LinkedList localLinkedList = new LinkedList();
    paramList = paramList.iterator();
    while (paramList.hasNext())
    {
      b.i locali = (b.i)paramList.next();
      localLinkedList.add(new com.tencent.mapsdk.raster.model.LatLng(locali.latitude, locali.longitude));
    }
    i(localLinkedList, paramInt);
    AppMethodBeat.o(146520);
  }
  
  public final void onDestroy()
  {
    AppMethodBeat.i(146497);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s onDestroy", new Object[] { this });
    this.kUj.set(true);
    this.kTr.getMap().setOnMapCameraChangeListener(null);
    blG();
    blF();
    blF();
    blE();
    blH();
    blI();
    Object localObject = this.kTS.values().iterator();
    while (((Iterator)localObject).hasNext()) {
      ((CustomLayer)((Iterator)localObject).next()).remove();
    }
    this.kTS.clear();
    if (this.kTr != null)
    {
      this.kTr.getMap().setOnMapPoiClickListener(null);
      this.kTr.clean();
      this.kTr.onDestroy();
    }
    if (this.kUf != null)
    {
      localObject = this.kUf;
      if (((d)localObject).kVc != null)
      {
        ((d)localObject).kVc.remove();
        ((d)localObject).kVc = null;
      }
      this.kUf = null;
    }
    AppMethodBeat.o(146497);
  }
  
  public final void onPause()
  {
    AppMethodBeat.i(146494);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s onPause", new Object[] { this });
    if (this.mMapType == 0)
    {
      this.dtJ.setVisibility(0);
      if (this.kTI == null) {
        this.kTI = getSurfaceView();
      }
      if (this.kTI != null)
      {
        ar.o(this.kTJ, 100L);
        this.kTr.setVisibility(4);
        this.kTr.getMap().getScreenShot(new TencentMap.OnScreenShotListener()
        {
          public final void onMapScreenShot(Bitmap paramAnonymousBitmap)
          {
            AppMethodBeat.i(146484);
            if (paramAnonymousBitmap != null) {
              a.o(a.this).setImageBitmap(paramAnonymousBitmap);
            }
            AppMethodBeat.o(146484);
          }
        });
      }
    }
    for (;;)
    {
      this.kTs = true;
      if (this.kUe)
      {
        ((com.tencent.mm.plugin.appbrand.utils.b.a)e.K(com.tencent.mm.plugin.appbrand.utils.b.a.class)).c("gcj02", this.kUg, blJ());
        g.b(this);
      }
      AppMethodBeat.o(146494);
      return;
      ae.e("MicroMsg.DefaultTencentMapView", "hideTencentMap err");
      break;
      this.kTr.onPause();
    }
  }
  
  public final void onResume()
  {
    AppMethodBeat.i(146493);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s onResume", new Object[] { this });
    if (this.mMapType == 0)
    {
      this.dtJ.setVisibility(4);
      this.kTr.setVisibility(0);
      this.kTr.onResume();
      blx();
    }
    for (;;)
    {
      this.kTs = false;
      if (this.kUe)
      {
        ((com.tencent.mm.plugin.appbrand.utils.b.a)e.K(com.tencent.mm.plugin.appbrand.utils.b.a.class)).b("gcj02", this.kUg, blJ());
        g.a(this);
      }
      AppMethodBeat.o(146493);
      return;
      this.kTr.onResume();
    }
  }
  
  public void onSizeChanged(int paramInt1, int paramInt2) {}
  
  public void onTouchEvent(MotionEvent paramMotionEvent) {}
  
  public final void setBuilding3dEffectEnable(boolean paramBoolean)
  {
    AppMethodBeat.i(212568);
    this.kTr.getMap().setBuilding3dEffectEnable(paramBoolean);
    AppMethodBeat.o(212568);
  }
  
  public final void setIndoorMaskColor(int paramInt)
  {
    AppMethodBeat.i(212565);
    this.kTr.getMap().setIndoorMaskColor(paramInt);
    AppMethodBeat.o(212565);
  }
  
  public final void setMapStyle(int paramInt)
  {
    AppMethodBeat.i(182813);
    ae.i("MicroMsg.DefaultTencentMapView", "map:%s setMapStyle: %d", new Object[] { this, Integer.valueOf(paramInt) });
    this.kTr.getMap().setMapStyle(paramInt);
    AppMethodBeat.o(182813);
  }
  
  public final void setMaxZoomLevel(int paramInt)
  {
    AppMethodBeat.i(212570);
    this.kTr.getMap().setMaxZoomLevel(paramInt);
    AppMethodBeat.o(212570);
  }
  
  public final void setMinZoomLevel(int paramInt)
  {
    AppMethodBeat.i(212571);
    this.kTr.getMap().setMinZoomLevel(paramInt);
    AppMethodBeat.o(212571);
  }
  
  public final void setPoisEnabled(boolean paramBoolean)
  {
    AppMethodBeat.i(212566);
    this.kTr.getMap().setPoisEnabled(paramBoolean);
    AppMethodBeat.o(212566);
  }
  
  public final boolean tj(int paramInt)
  {
    AppMethodBeat.i(212586);
    if (!this.kUu.containsKey(String.valueOf(paramInt)))
    {
      ae.e("MicroMsg.DefaultTencentMapView", "removeGroundOverlay fail, groundOverlay not exist");
      AppMethodBeat.o(212586);
      return false;
    }
    ae.i("MicroMsg.DefaultTencentMapView", "removeGroundOverlay id:%d", new Object[] { Integer.valueOf(paramInt) });
    GroundOverlay localGroundOverlay = (GroundOverlay)this.kUu.remove(String.valueOf(paramInt));
    if (localGroundOverlay != null)
    {
      localGroundOverlay.remove();
      AppMethodBeat.o(212586);
      return true;
    }
    AppMethodBeat.o(212586);
    return false;
  }
  
  public String toString()
  {
    AppMethodBeat.i(146547);
    String str = "DefaultTencentMapView{mapId=" + this.kTq + '}';
    AppMethodBeat.o(146547);
    return str;
  }
  
  public final class a
    implements TencentMap.InfoWindowAdapter
  {
    public a() {}
    
    public final View getInfoWindow(Marker paramMarker)
    {
      AppMethodBeat.i(146488);
      if (paramMarker == null)
      {
        AppMethodBeat.o(146488);
        return null;
      }
      if (bu.isNullOrNil((String)paramMarker.getTag()))
      {
        ae.w("MicroMsg.DefaultTencentMapView", "realMarker.getTag is null, return");
        AppMethodBeat.o(146488);
        return null;
      }
      if (((String)paramMarker.getTag()).endsWith("#label"))
      {
        AppMethodBeat.o(146488);
        return null;
      }
      paramMarker = a.this.Qt((String)paramMarker.getTag());
      if (paramMarker == null)
      {
        ae.e("MicroMsg.DefaultTencentMapView", "[getInfoWindow] appbrandMarker is null, return");
        AppMethodBeat.o(146488);
        return null;
      }
      if (paramMarker.kVD == null)
      {
        ae.e("MicroMsg.DefaultTencentMapView", "[getInfoWindow] appBrandMarker.appBrandMarkerOptions is null, return");
        AppMethodBeat.o(146488);
        return null;
      }
      b.v.a locala = paramMarker.kVD.kVI;
      if (locala == null)
      {
        AppMethodBeat.o(146488);
        return null;
      }
      if (locala.view != null) {}
      for (boolean bool = true;; bool = false)
      {
        ae.i("MicroMsg.DefaultTencentMapView", "marker:%s calloutStyle.view is %b", new Object[] { paramMarker, Boolean.valueOf(bool) });
        if (locala.view == null) {
          break;
        }
        if (paramMarker.kVj == null) {
          paramMarker.kVj = new ImageView(a.u(a.this));
        }
        localObject = h.fZ(locala.view);
        if (localObject == null) {
          ae.e("MicroMsg.DefaultTencentMapView", "marker:%s calloutStyle.view exist but bitmap is null");
        }
        paramMarker.kVj.setImageBitmap((Bitmap)localObject);
        paramMarker = paramMarker.kVj;
        AppMethodBeat.o(146488);
        return paramMarker;
      }
      a locala1 = a.this;
      Object localObject = i.blP();
      paramMarker = (Marker)localObject;
      if (localObject == null) {
        paramMarker = new com.tencent.mm.plugin.appbrand.widget.d.b(locala1.context);
      }
      paramMarker.setText("");
      paramMarker.setTextSize(12);
      paramMarker.setTextColor(com.tencent.mm.plugin.appbrand.widget.d.b.noO);
      paramMarker.setTitlePadding(0);
      paramMarker.setGravity("center");
      int i = com.tencent.mm.plugin.appbrand.widget.d.b.noP;
      paramMarker.A(0, 0, i, i);
      paramMarker.setText(locala.content);
      paramMarker.setTextSize(locala.kVK);
      paramMarker.setTextColor(locala.color);
      paramMarker.setTitlePadding(locala.padding);
      paramMarker.setGravity(locala.jWV);
      paramMarker.A(locala.kVL, locala.borderWidth, locala.borderColor, locala.bgColor);
      AppMethodBeat.o(146488);
      return paramMarker;
    }
    
    public final void onInfoWindowDettached(Marker paramMarker, View paramView)
    {
      AppMethodBeat.i(146489);
      if ((paramView instanceof com.tencent.mm.plugin.appbrand.widget.d.b))
      {
        ae.d("MicroMsg.DefaultTencentMapView", "recycler calloutView");
        i.a((com.tencent.mm.plugin.appbrand.widget.d.b)paramView);
      }
      AppMethodBeat.o(146489);
    }
  }
  
  public final class b
  {
    ImageView kVb;
    
    public b(ImageView paramImageView)
    {
      this.kVb = paramImageView;
    }
  }
  
  public final class c
  {
    Marker kVc;
    
    public c(Marker paramMarker)
    {
      this.kVc = paramMarker;
    }
  }
  
  final class d
    extends ClusterManager<a.f>
  {
    public d(Context paramContext, TencentMap paramTencentMap)
    {
      super(paramTencentMap);
    }
    
    public final void onCameraChange(CameraPosition paramCameraPosition)
    {
      AppMethodBeat.i(212559);
      super.onCameraChange(paramCameraPosition);
      if (a.p(a.this) != null)
      {
        b.a locala = new b.a();
        locala.kVk = new b.i(paramCameraPosition.getTarget().getLatitude(), paramCameraPosition.getTarget().getLongitude());
        locala.zoom = paramCameraPosition.getZoom();
        locala.kVl = paramCameraPosition.getBearing();
        locala.skew = paramCameraPosition.getSkew();
        a.p(a.this).a(locala, a.q(a.this));
      }
      AppMethodBeat.o(212559);
    }
    
    public final void onCameraChangeFinish(CameraPosition paramCameraPosition)
    {
      AppMethodBeat.i(212560);
      super.onCameraChangeFinish(paramCameraPosition);
      ae.i("MicroMsg.DefaultTencentMapView", "onCameraChangeFinish");
      if (a.p(a.this) != null)
      {
        b.a locala = new b.a();
        locala.kVk = new b.i(paramCameraPosition.getTarget().getLatitude(), paramCameraPosition.getTarget().getLongitude());
        locala.zoom = paramCameraPosition.getZoom();
        locala.kVl = paramCameraPosition.getBearing();
        locala.skew = paramCameraPosition.getSkew();
        a.p(a.this).b(locala, a.q(a.this));
        a.a(a.this, false);
      }
      a.j(a.this);
      if ((a.b(a.this) != null) && (a.b(a.this).getAlgorithm().getClusters(paramCameraPosition.getZoom()).size() > 0))
      {
        paramCameraPosition = ((NonHierarchicalDistanceBasedAlgorithm)((PreCachingAlgorithmDecorator)a.b(a.this).getAlgorithm()).getAlgorithm()).getClusters(paramCameraPosition.getZoom());
        ae.i("MicroMsg.DefaultTencentMapView", "onCameraChangeFinish cluster size:%d", new Object[] { Integer.valueOf(paramCameraPosition.size()) });
        paramCameraPosition = paramCameraPosition.iterator();
        while (paramCameraPosition.hasNext()) {
          ae.i("MicroMsg.DefaultTencentMapView", "onCameraChangeFinish cluster item size:%d", new Object[] { Integer.valueOf(((Cluster)paramCameraPosition.next()).getItems().size()) });
        }
      }
      AppMethodBeat.o(212560);
    }
  }
  
  final class e
    extends DefaultClusterRenderer<a.f>
  {
    public e(TencentMap paramTencentMap, ClusterManager<a.f> paramClusterManager)
    {
      super(paramClusterManager, localClusterManager);
    }
    
    public final void onBeforeClusterRendered(Cluster<a.f> paramCluster, MarkerOptions paramMarkerOptions)
    {
      AppMethodBeat.i(212562);
      super.onBeforeClusterRendered(paramCluster, paramMarkerOptions);
      ae.i("MicroMsg.DefaultTencentMapView", "onBeforeClusterRendered, cluster size:%d", new Object[] { Integer.valueOf(paramCluster.getItems().size()) });
      paramMarkerOptions = paramCluster.getItems().iterator();
      while (paramMarkerOptions.hasNext())
      {
        a.f localf = (a.f)paramMarkerOptions.next();
        if (localf.kVh > 0) {
          a.this.Qv(localf.kVh);
        } else {
          a.this.Qv(a.f.a(localf));
        }
      }
      if (a.D(a.this) != null) {
        a.D(a.this).p(a.a(a.this, paramCluster));
      }
      AppMethodBeat.o(212562);
    }
  }
  
  static final class f
    implements ClusterItem
  {
    private String id;
    private com.tencent.mapsdk.raster.model.LatLng kVg;
    int kVh;
    
    f(String paramString, com.tencent.mapsdk.raster.model.LatLng paramLatLng)
    {
      this.id = paramString;
      this.kVg = paramLatLng;
    }
    
    public final com.tencent.mapsdk.raster.model.LatLng getPosition()
    {
      return this.kVg;
    }
  }
  
  public static final class g
    extends b.u
  {
    public Marker kVc;
    public Marker kVi;
    public ImageView kVj;
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes7.jar
 * Qualified Name:     com.tencent.mm.plugin.appbrand.jsapi.i.a.a
 * JD-Core Version:    0.7.0.1
 */