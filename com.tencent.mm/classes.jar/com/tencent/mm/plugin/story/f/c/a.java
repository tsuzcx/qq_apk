package com.tencent.mm.plugin.story.f.c;

import com.tencent.matrix.trace.core.AppMethodBeat;
import d.l;

@l(gjZ={1, 1, 16}, gka={""}, gkb={"Lcom/tencent/mm/plugin/story/model/download/LoaderScene;", "", "(Ljava/lang/String;I)V", "COMMON_SCENE", "ALBUM_SCENE", "FAV_SCENE", "plugin-story_release"})
public enum a
{
  static
  {
    AppMethodBeat.i(118888);
    a locala1 = new a("COMMON_SCENE", 0);
    BdG = locala1;
    a locala2 = new a("ALBUM_SCENE", 1);
    BdH = locala2;
    a locala3 = new a("FAV_SCENE", 2);
    BdI = locala3;
    BdJ = new a[] { locala1, locala2, locala3 };
    AppMethodBeat.o(118888);
  }
  
  private a() {}
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes6.jar
 * Qualified Name:     com.tencent.mm.plugin.story.f.c.a
 * JD-Core Version:    0.7.0.1
 */