  ...
  implements com.tencent.mm.network.q
{
  ...
  {
    paramd.setDeviceID(com.tencent.mm.compatible.deviceinfo.q.aaH());
    paramd.setDeviceType(d.hjv);
    paramd.setClientVersion(d.FFH);
    paramd.setUin(j.e.a.FGm.aCk());
    if (g.ajM())
    {
      ...
      int k;
      int i;
      if (((a)g.ab(a.class)).acL().getInt("UseAesGcmSessionKeySwitch", 1) == 0)
      {
        bool = true;
        ae.i("MicroMsg.MMReqRespBase", "summerauths check cgi[%s] accHasReady openSwitch[%s] ", new Object[] { Integer.valueOf(j), Boolean.valueOf(bool) });
        if (bool)
        {
          paramd = ((a)g.ab(a.class)).acL().getValue("UseAesGcmSessionKeyCgiList");
          if (!bu.isNullOrNil(paramd))
          {
            ae.i("MicroMsg.MMReqRespBase", "summerauths check cgi list[%s]", new Object[] { paramd });
      ...
      for (;;)
      {
        ...
        {
          String str = paramd[i];
          if (j == bu.getInt(str, 0))
          {
            paraml.setSingleSession(false);
            ae.i("MicroMsg.MMReqRespBase", "summerauths check cgi list found cgi[%s] singleSession[%s]", new Object[] { str, Boolean.valueOf(paraml.isSingleSession()) });
          }
        }
        else
        {
    ...
    ae.i("MicroMsg.MMReqRespBase", "summerauths check cgi[%s] USE_ECDH[%s] accHasReady[%s] ", new Object[] { Integer.valueOf(paramd.getCmdId()), Boolean.valueOf(f.FFQ), Boolean.valueOf(g.ajM()) });
  }
