package com.tencent.mm.as;

import android.content.Context;
import android.content.SharedPreferences;
import android.os.Process;
import com.tencent.e.h;
import com.tencent.e.i;
import com.tencent.matrix.trace.core.AppMethodBeat;
import com.tencent.mm.compatible.util.g;
import com.tencent.mm.model.cm;
import com.tencent.mm.sdk.platformtools.Log;
import com.tencent.mm.sdk.platformtools.MMApplicationContext;
import com.tencent.mm.vfs.u;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Collections;
import java.util.Date;
import java.util.Iterator;
import java.util.List;
import java.util.concurrent.Future;

public final class a
{
  private static boolean active;
  private static Future fuk;
  private static final c lJM;
  private static boolean lJN;
  private static long lJO;
  private static long lJP;
  
  static
  {
    AppMethodBeat.i(132522);
    active = false;
    lJM = new c();
    lJN = true;
    lJO = 0L;
    lJP = 0L;
    AppMethodBeat.o(132522);
  }
  
  public static void A(int paramInt, long paramLong)
  {
    AppMethodBeat.i(132513);
    if (!lJN)
    {
      AppMethodBeat.o(132513);
      return;
    }
    if (active)
    {
      AppMethodBeat.o(132513);
      return;
    }
    Object localObject = lJM;
    c.c localc = new c.c(cm.bfC(), System.currentTimeMillis(), paramInt, paramLong);
    localObject = ((c)localObject).lKa;
    if (((c.a)localObject).lKi.size() > 30000)
    {
      paramInt = 0;
      for (;;)
      {
        if (paramInt < 100) {
          try
          {
            ((c.a)localObject).lKi.remove(0);
            paramInt += 1;
          }
          catch (Exception localException)
          {
            Log.e("MicroMsg.ActiveDetector.ProcessDetector", "addSendInterProcessList %s", new Object[] { localException.getMessage() });
          }
        }
      }
    }
    ((c.a)localObject).lKi.add(localc);
    AppMethodBeat.o(132513);
  }
  
  public static void B(int paramInt, long paramLong)
  {
    AppMethodBeat.i(132514);
    if (!lJN)
    {
      AppMethodBeat.o(132514);
      return;
    }
    if (active)
    {
      AppMethodBeat.o(132514);
      return;
    }
    Object localObject = lJM;
    c.c localc = new c.c(cm.bfC(), System.currentTimeMillis(), paramInt, paramLong);
    localObject = ((c)localObject).lKa;
    if (((c.a)localObject).lKj.size() > 30000)
    {
      paramInt = 0;
      for (;;)
      {
        if (paramInt < 100) {
          try
          {
            ((c.a)localObject).lKj.remove(0);
            paramInt += 1;
          }
          catch (Exception localException)
          {
            Log.e("MicroMsg.ActiveDetector.ProcessDetector", "addReceiveInterProcessList %s", new Object[] { localException.getMessage() });
          }
        }
      }
    }
    ((c.a)localObject).lKj.add(localc);
    AppMethodBeat.o(132514);
  }
  
  public static boolean GC(long paramLong)
  {
    boolean bool = true;
    if ((lJO <= 0L) || (lJP <= 0L) || (paramLong <= 0L)) {
      bool = false;
    }
    do
    {
      do
      {
        return bool;
        if (lJO < lJP) {
          break;
        }
      } while (paramLong >= lJO);
      return false;
    } while (paramLong < lJP);
    return false;
  }
  
  static String GD(long paramLong)
  {
    AppMethodBeat.i(132519);
    Object localObject = new Date(paramLong);
    localObject = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format((Date)localObject);
    AppMethodBeat.o(132519);
    return localObject;
  }
  
  private static a a(c.c paramc, int paramInt)
  {
    AppMethodBeat.i(132521);
    if (paramc == null)
    {
      AppMethodBeat.o(132521);
      return null;
    }
    a locala = new a();
    locala.lJQ = paramc.lJQ;
    locala.startTime = paramc.time;
    locala.endTime = paramc.time;
    locala.type = paramInt;
    locala.lJT = paramc.type;
    AppMethodBeat.o(132521);
    return locala;
  }
  
  private static a a(c.d paramd, int paramInt)
  {
    AppMethodBeat.i(132520);
    if (paramd == null)
    {
      AppMethodBeat.o(132520);
      return null;
    }
    a locala = new a();
    locala.lJQ = paramd.lKG;
    locala.startTime = paramd.startTime;
    locala.endTime = paramd.endTime;
    locala.type = paramInt;
    locala.pid = paramd.pid;
    locala.lJR = paramd.lJR;
    if (paramInt == 1)
    {
      locala.networkStatus = paramd.networkStatus;
      locala.lJS = paramd.lJS;
    }
    AppMethodBeat.o(132520);
    return locala;
  }
  
  public static void a(int paramInt1, long paramLong1, long paramLong2, long paramLong3, long paramLong4, long paramLong5, String paramString, int paramInt2)
  {
    AppMethodBeat.i(132515);
    if (!lJN)
    {
      AppMethodBeat.o(132515);
      return;
    }
    c localc = lJM;
    paramString = new c.b(Process.myPid(), paramLong1, paramLong2, paramLong3, paramLong4, paramLong5, paramString, paramInt2);
    Log.i("MicroMsg.ActiveDetector.ProcessDetector", "addDelayedMsgInBackground() delayed msg[%s]", new Object[] { paramString.toString() });
    switch (paramInt1)
    {
    }
    for (;;)
    {
      AppMethodBeat.o(132515);
      return;
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKu.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKv.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKw.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKx.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKy.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKz.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKA.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKB.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKC.add(paramString);
      AppMethodBeat.o(132515);
      return;
      localc.lKa.lKD.add(paramString);
    }
  }
  
  private static void a(List<a> paramList, List<c.b> paramList1, boolean paramBoolean)
  {
    AppMethodBeat.i(132518);
    if (paramList1 == null)
    {
      AppMethodBeat.o(132518);
      return;
    }
    Iterator localIterator = paramList1.iterator();
    label146:
    while (localIterator.hasNext())
    {
      c.b localb = (c.b)localIterator.next();
      if (localb == null) {
        paramList1 = null;
      }
      for (;;)
      {
        if (paramList1 == null) {
          break label146;
        }
        paramList.add(paramList1);
        break;
        paramList1 = new a();
        paramList1.pid = localb.pid;
        paramList1.lJQ = localb.lJQ;
        paramList1.startTime = localb.lKE;
        paramList1.endTime = localb.lKE;
        paramList1.type = 4;
        paramList1.lJU = localb.lJU;
        paramList1.lJV = localb.lJV;
        paramList1.lJW = localb.lJW;
        paramList1.dgo = paramBoolean;
      }
    }
    AppMethodBeat.o(132518);
  }
  
  public static void b(int paramInt1, long paramLong1, long paramLong2, long paramLong3, long paramLong4, long paramLong5, String paramString, int paramInt2)
  {
    AppMethodBeat.i(132516);
    if (!lJN)
    {
      AppMethodBeat.o(132516);
      return;
    }
    c localc = lJM;
    paramString = new c.b(Process.myPid(), paramLong1, paramLong2, paramLong3, paramLong4, paramLong5, paramString, paramInt2);
    Log.i("MicroMsg.ActiveDetector.ProcessDetector", "addDelayedMsgInForeground() delayed msg[%s]", new Object[] { paramString.toString() });
    switch (paramInt1)
    {
    }
    for (;;)
    {
      AppMethodBeat.o(132516);
      return;
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKk, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKl, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKm, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKn, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKo, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKp, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKq, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKr, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKs, paramString);
      AppMethodBeat.o(132516);
      return;
      c.a.a(localc.lKa.lKt, paramString);
    }
  }
  
  public static List<a> bkH()
  {
    localObject2 = null;
    AppMethodBeat.i(132517);
    if (!MMApplicationContext.isMMProcess())
    {
      AppMethodBeat.o(132517);
      return null;
    }
    Object localObject1 = lJM.lKe;
    Object localObject3 = (String)localObject1 + "/mm";
    localObject4 = (String)localObject1 + "/push";
    try
    {
      localObject1 = (c.a)b.w(u.Tf((String)localObject3));
      try
      {
        localObject3 = (c.a)b.w(u.Tf((String)localObject4));
        localObject2 = localObject3;
      }
      catch (Exception localException2)
      {
        a locala;
        for (;;)
        {
          Log.e("MicroMsg.ActiveDetector", "%s,read exception:" + localException2.getMessage(), new Object[] { localObject4 });
        }
        localObject4 = localIterator.lKj.iterator();
        while (((Iterator)localObject4).hasNext())
        {
          locala = a((c.c)((Iterator)localObject4).next(), 3);
          if (locala != null) {
            localException2.add(locala);
          }
        }
        a(localException2, localIterator.lKk, true);
        a(localException2, localIterator.lKl, true);
        a(localException2, localIterator.lKm, true);
        a(localException2, localIterator.lKn, true);
        a(localException2, localIterator.lKo, true);
        a(localException2, localIterator.lKp, true);
        a(localException2, localIterator.lKq, true);
        a(localException2, localIterator.lKr, true);
        a(localException2, localIterator.lKs, true);
        a(localException2, localIterator.lKt, true);
        a(localException2, localIterator.lKu, false);
        a(localException2, localIterator.lKv, false);
        a(localException2, localIterator.lKw, false);
        a(localException2, localIterator.lKx, false);
        a(localException2, localIterator.lKy, false);
        a(localException2, localIterator.lKz, false);
        a(localException2, localIterator.lKA, false);
        a(localException2, localIterator.lKB, false);
        a(localException2, localIterator.lKC, false);
        a(localException2, localIterator.lKD, false);
        if (localObject2 == null) {
          break label567;
        }
        Iterator localIterator = ((c.a)localObject2).lKh.iterator();
        while (localIterator.hasNext())
        {
          localObject4 = a((c.d)localIterator.next(), 1);
          if (localObject4 != null) {
            localException2.add(localObject4);
          }
        }
        localIterator = ((c.a)localObject2).lKi.iterator();
        while (localIterator.hasNext())
        {
          localObject2 = a((c.c)localIterator.next(), 2);
          if (localObject2 != null) {
            localException2.add(localObject2);
          }
        }
        Collections.sort(localException2);
        AppMethodBeat.o(132517);
        return localException2;
      }
      localObject3 = new ArrayList();
      if (localObject1 != null)
      {
        localObject4 = ((c.a)localObject1).lKh.iterator();
        while (((Iterator)localObject4).hasNext())
        {
          locala = a((c.d)((Iterator)localObject4).next(), 0);
          if (locala != null) {
            ((List)localObject3).add(locala);
          }
        }
      }
    }
    catch (Exception localException1)
    {
      for (;;)
      {
        Log.e("MicroMsg.ActiveDetector", "%s,read exception:" + localException1.getMessage(), new Object[] { localObject3 });
        localIterator = null;
      }
    }
  }
  
  public static final void gp(boolean paramBoolean)
  {
    AppMethodBeat.i(132512);
    Log.i("MicroMsg.ActiveDetector", "onActive() active:%s", new Object[] { Boolean.valueOf(paramBoolean) });
    if (!MMApplicationContext.getContext().getSharedPreferences("system_config_prefs", g.avK()).getBoolean("msg_delay_close_detect", false)) {}
    for (boolean bool = true;; bool = false)
    {
      lJN = bool;
      if (bool) {
        break;
      }
      AppMethodBeat.o(132512);
      return;
    }
    active = paramBoolean;
    if (paramBoolean)
    {
      Log.i("MicroMsg.ActiveDetector", "[oneliang]active, time%s, pid:%s", new Object[] { Long.valueOf(System.currentTimeMillis()), Integer.valueOf(Process.myPid()) });
      Log.i("MicroMsg.ActiveDetector", "active, interrupt end, time %s", new Object[] { Long.valueOf(System.currentTimeMillis()) });
      if (fuk != null) {
        fuk.cancel(true);
      }
      fuk = null;
      c localc = lJM;
      localc.lKg = false;
      localc.lKc = 0L;
      localc.lKd = 0L;
      Log.i("MicroMsg.ActiveDetector", "active, processDetector.clear end, time %s", new Object[] { Long.valueOf(System.currentTimeMillis()) });
      lJO = cm.bfC();
      AppMethodBeat.o(132512);
      return;
    }
    Log.i("MicroMsg.ActiveDetector", "[oneliang]unactive, time%s, pid:%s", new Object[] { Long.valueOf(System.currentTimeMillis()), Integer.valueOf(Process.myPid()) });
    if (fuk == null)
    {
      fuk = h.ZvG.bh(lJM);
      lJM.lKg = true;
    }
    lJP = cm.bfC();
    AppMethodBeat.o(132512);
  }
  
  public static boolean isActive()
  {
    return active;
  }
  
  public static void vX(int paramInt)
  {
    lJM.lJX = paramInt;
  }
  
  public static final class a
    implements Comparable<a>
  {
    public boolean dgo = false;
    public long endTime = 0L;
    public long lJQ = 0L;
    public boolean lJR = true;
    public boolean lJS = false;
    public int lJT = 0;
    public long lJU = 0L;
    public long lJV = 0L;
    public long lJW = 0L;
    public int networkStatus = 0;
    public int pid = 0;
    public long startTime = 0L;
    public int type = 0;
    
    public final String toString()
    {
      AppMethodBeat.i(132511);
      StringBuilder localStringBuilder = new StringBuilder();
      switch (this.type)
      {
      default: 
      case 0: 
      case 1: 
      case 2: 
      case 3: 
        for (;;)
        {
          localStringBuilder.append("\n");
          str1 = localStringBuilder.toString();
          AppMethodBeat.o(132511);
          return str1;
          localStringBuilder.append(String.format("server time:%s,local start time:%s,local end time:%s,[mm] pid:%s,normal execute:%s", new Object[] { a.GD(this.lJQ), a.GD(this.startTime), a.GD(this.endTime), Integer.valueOf(this.pid), Boolean.valueOf(this.lJR) }));
          continue;
          localStringBuilder.append(String.format("server time:%s,local start time:%s,local end time:%s,[push] pid:%s,network:%s,normal execute:%s", new Object[] { a.GD(this.lJQ), a.GD(this.startTime), a.GD(this.endTime), Integer.valueOf(this.pid), Integer.valueOf(this.networkStatus), Boolean.valueOf(this.lJR) }));
          continue;
          localStringBuilder.append(String.format("server time:%s,local start time:%s,local end time:%s,send broadcast type(push):%s", new Object[] { a.GD(this.lJQ), a.GD(this.startTime), a.GD(this.endTime), Integer.valueOf(this.lJT) }));
          continue;
          localStringBuilder.append(String.format("server time:%s,local start time:%s,local end time:%s,receive broadcast type(mm):%s", new Object[] { a.GD(this.lJQ), a.GD(this.startTime), a.GD(this.endTime), Integer.valueOf(this.lJT) }));
        }
      }
      String str2 = a.GD(this.lJQ);
      String str3 = a.GD(this.startTime);
      String str4 = a.GD(this.endTime);
      int i = this.pid;
      String str5 = a.GD(this.lJU);
      long l1 = this.lJV;
      long l2 = this.lJW;
      if (this.dgo) {}
      for (String str1 = "foreground";; str1 = "background")
      {
        localStringBuilder.append(String.format("server time:%s,local start time:%s,local end time:%s,delayed msg pid:%s, msg server time:%s,interval time:%s, msg server id:%s, %s", new Object[] { str2, str3, str4, Integer.valueOf(i), str5, Long.valueOf(l1), Long.valueOf(l2), str1 }));
        break;
      }
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.mm\classes9.jar
 * Qualified Name:     com.tencent.mm.as.a
 * JD-Core Version:    0.7.0.1
 */