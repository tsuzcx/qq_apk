  ...
  implements h
{
  ...
  private static String q(List<i> paramList)
  {
    ...
    while (i < j)
    {
      String str = ((i)paramList.get(i)).ps();
    ...
    if (this.aJY == null) {
      try
      {
        if (this.aJY == null)
        {
          ...
          this.aJY = Collections.unmodifiableMap(localMap1);
        }
      }
      finally
      {
    ...
    static
    {
      ...
      for (;;)
      {
        ...
        if (!TextUtils.isEmpty(aJZ)) {
          ((Map)localObject).put("User-Agent", Collections.singletonList(new j.b(aJZ)));
        }
        aKa = Collections.unmodifiableMap((Map)localObject);
 ...
 * Qualified Name:     com.bumptech.glide.load.c.j
