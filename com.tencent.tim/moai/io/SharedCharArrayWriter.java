...
public class SharedCharArrayWriter extends Writer {
     ...
     */
    public SharedCharArrayWriter(int initialSize) {
        if (initialSize < 0) {
            throw new IllegalArgumentException("size < 0");
        }
        buf = Caches.chars(initialSize);
        lock = buf;
    }
    ...
    private void expand(int i) {
        ...
        char[] newbuf = Caches.chars(newLen);
    ...
    public void write(char[] buffer, int offset, int len) {
        Indexes.checkOffsetAndCount(buffer.length, offset, len);
        synchronized (lock) {
            expand(len);
    ...
    public void write(String str, int offset, int count) {
        ...
        if ((offset | count) < 0 || offset > str.length() - count) {
            throw new StringIndexOutOfBoundsException("length=" + str.length() +
     ...
     */
    @Override
    public SharedCharArrayWriter append(CharSequence csq) {
        if (csq == null) {
            csq = "null";
        }
     ...
     */
    @Override
    public SharedCharArrayWriter append(CharSequence csq, int start, int end) {
        if (csq == null) {
            csq = "null";
        }
