...
public class adxd
  extends MSFServlet
{
  private String TAG = "RockDownloaderServlet";
  
  public void onReceive(Intent paramIntent, FromServiceMsg paramFromServiceMsg)
  {
    if (QLog.isColorLevel()) {
      QLog.d(this.TAG, 2, "onReceive with code: " + paramFromServiceMsg.getResultCode());
    }
    ...
    try
    {
      ...
      else if ("QQApkSvc.update_download_count".equals(localObject))
      {
        ...
        if (paramFromServiceMsg != null)
        {
          ...
          QLog.d(this.TAG, 2, new Object[] { localObject, " ", Boolean.valueOf(bool), " ", Integer.valueOf(paramIntent.download_num.get()), " ", paramFromServiceMsg.err_msg.get(), " ", Integer.valueOf(paramFromServiceMsg.err_code.get()), " ", paramFromServiceMsg.jump_url.get() });
        }
      }
    }
    catch (Exception paramIntent)
    {
      if (QLog.isColorLevel())
      {
        QLog.d(this.TAG, 2, paramIntent, new Object[0]);
        return;
        if (QLog.isColorLevel()) {
          QLog.d(this.TAG, 2, new Object[] { localObject, " ", Boolean.valueOf(bool), " ", Integer.valueOf(paramIntent.download_num.get()) });
        }
      }
    }
  }
  ...
  public void onSend(Intent paramIntent, Packet paramPacket)
  {
    ...
    if (QLog.isColorLevel()) {
      QLog.d(this.TAG, 2, new Object[] { "cmd=", str });
    }
    Object localObject;
    if ("QQApkSvc.pre_download_apk".equals(str))
    {
      ...
      paramPacket.putSendData(aqnv.I(((ServerApi.ReqPreDownloadRecmd)localObject).toByteArray()));
    }
    for (;;)
    {
      if (QLog.isColorLevel()) {
        QLog.d(this.TAG, 2, "onSendFinish");
      }
      return;
      if ("QQApkSvc.update_download_count".equals(str))
      {
        ...
        paramPacket.setSSOCommand(str);
        paramPacket.putSendData(aqnv.I(((ServerApi.ReqUpdateDownCountRecmd)localObject).toByteArray()));
      }
    }
  }
}
