  ...
  implements Boss, MonitorThreadPoolExecutor.ThreadPoolMonitorListener
{
  ...
  public BossImp(Context paramContext, Looper paramLooper)
  {
    ...
    this.mCpuExecutor = new MonitorThreadPoolExecutor(this.mCpuConfig.getCore(), this.mCpuConfig.getMaximum(), this.mCpuConfig.getAliveTime(), this.mCpuConfig.getTimeUnit(), paramLooper, new AsyncThreadFactory("cpu"));
    this.mCpuExecutor.setName("cpu");
    this.mCpuExecutor.setMonitorListener(this);
    this.mExecutors[0] = this.mCpuExecutor;
    paramLooper = new LinkedBlockingQueue();
    this.mDiskReadExecutor = new MonitorThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS, paramLooper, new AsyncThreadFactory("disk_read"));
    this.mCpuExecutor.setName("disk_read");
    this.mCpuExecutor.setMonitorListener(this);
    this.mExecutors[1] = this.mDiskReadExecutor;
    paramLooper = new LinkedBlockingQueue();
    this.mDiskWriteExecutor = new MonitorThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS, paramLooper, new AsyncThreadFactory("disk_write"));
    ...
    paramLooper = new LinkedBlockingQueue();
    this.mNetworkExecutor = new MonitorThreadPoolExecutor(this.mNetworkConfig.getCore(), this.mNetworkConfig.getMaximum(), this.mNetworkConfig.getAliveTime(), this.mNetworkConfig.getTimeUnit(), paramLooper, new AsyncThreadFactory("network"));
