  ...
  extends ToolRuntimeBase
{
  ...
  public AppRuntime onGetSubRuntime(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if ((!InitSkin.cIJ) && (bool)) {
        InitSkin.dRP();
      }
      return paramString;
      if ("comic_plugin.apk".equals(paramString))
      {
        paramString = avcv.b(localBaseApplicationImpl, MobileQQ.processName);
        bool = false;
      }
      else if ("qqreaderplugin.apk".equals(paramString))
      {
        paramString = avmj.h(localBaseApplicationImpl, MobileQQ.processName);
        bool = false;
      }
      else if ("qqpim_plugin.apk".equals(paramString))
      {
        paramString = avlo.g(localBaseApplicationImpl, MobileQQ.processName);
      }
      ...
      {
        paramString = avkv.f(localBaseApplicationImpl, MobileQQ.processName);
        bool = false;
      }
