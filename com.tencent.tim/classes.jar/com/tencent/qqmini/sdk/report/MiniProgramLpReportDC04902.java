...
public class MiniProgramLpReportDC04902
{
  ...
  public static void reportGameEnd(float paramFloat1, float paramFloat2, String paramString1, String paramString2)
  {
    ...
    if (d3 < 0.01D)
    {
      ...
      d2 = 0.0D;
      QMLog.d("MiniProgramLpReportDC04", "reportGameEnd: minuteFromStart=" + d3 + " jankCount=" + jankCount + " bigJankCount=" + bigJankCount);
      localObject1 = DC_TABLE_VALUE_APPID + '|' + DC_TABLE_VALUE_QUA + '|' + DC_TABLE_VALUE_START_TIME + '|' + DC_TABLE_VALUE_END_TIME + '|' + DC_TABLE_VALUE_F_20 + '|' + DC_TABLE_VALUE_F_33 + '|' + DC_TABLE_VALUE_F_50 + '|' + DC_TABLE_VALUE_F_100 + '|' + DC_TABLE_VALUE_F_LONG + '|' + DC_TABLE_VALUE_AVG_MEM + '|' + DC_TABLE_VALUE_MAX_MEM + '|' + DC_TABLE_VALUE_AVG_CPU + '|' + DC_TABLE_VALUE_MAX_CPU + '|' + DC_TABLE_VALUE_ATTACH_INFO + '|' + DC_TABLE_VALUE_F_MAX_TIME_MILLIS + '|' + DC_TABLE_VALUE_F_MAX + '|' + (int)paramFloat2 + '|' + (int)paramFloat1 + '|' + DC_TABLE_VALUE_TEXTURE_OFFLINE + '|' + MiniProgramReportHelper.getNetworkType() + '|' + DC_TABLE_VALUE_NETWORK_GATEWAY_IP + '|' + NetworkUtil.getCurrentWifiSSID(AppLoaderFactory.g().getMiniAppEnv().getContext()) + '|' + DC_TABLE_VALUE_PLATFORM + '|' + Build.MODEL + '|' + Build.VERSION.RELEASE + '|' + d1 + '|' + d2;
      if (QUAUtil.isQQApp()) {
        break label758;
      }
      str1 = QUAUtil.getQUA();
    ...
    for (;;)
    {
      ...
      for (str1 = "dc04902";; str1 = "dc05389")
      {
        ...
        QMLog.d("MiniProgramLpReportDC04", "reportGameEnd " + (String)localObject1);
  ...
  public static void updateDisplayFrameTime(long paramLong, boolean paramBoolean)
  {
    ...
    for (;;)
    {
      QMLog.d("MiniProgramLpReportDC04", "jankCount=" + jankCount + " bigJankCount=" + bigJankCount + " time=" + (paramLong - startFrameTime) / 1000000L);
      i = totalFrameCount;
      frameTimes[(i % 3)] = l2;
      totalFrameCount += 1;
      break;
