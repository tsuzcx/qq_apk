...
public final class CustomConfiguration
{
  ...
  static
  {
    ...
    DEX_CONFIG = aekv.aT("sdk_dex_config", "");
    try
    {
      if ((!TextUtils.isEmpty(DEX_CONFIG)) || (!TextUtils.isEmpty(DEX_CONFIG)))
      {
        JSONObject localJSONObject = new JSONObject(DEX_CONFIG);
        if (!TextUtils.isEmpty(localJSONObject.optString("ver")))
        {
          localJSONObject.putOpt("app_version", "3.4.4.3058");
          DEX_CONFIG = localJSONObject.toString();
        }
      }
      return;
    }
