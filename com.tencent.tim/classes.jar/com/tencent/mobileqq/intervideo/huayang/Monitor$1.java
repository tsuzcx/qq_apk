  ...
  implements Runnable
{
  ...
  public void run()
  {
    try
    {
      HttpURLConnection localHttpURLConnection = (HttpURLConnection)new URL(this.val$finalUrl).openConnection();
      if (localHttpURLConnection != null)
      {
        ...
        if (localHttpURLConnection.getResponseCode() == 200)
        {
          if (QLog.isColorLevel()) {
            QLog.i("Monitor", 2, " report to monitor success,URL = " + this.val$finalUrl);
          }
        }
        else if (QLog.isColorLevel())
        {
          QLog.i("Monitor", 2, " report to monitor failed,URL = " + this.val$finalUrl);
          return;
        }
      }
    }
