  ...
  public static void preLaunchMiniApp(Context paramContext, MiniAppConfig paramMiniAppConfig)
  {
    ...
    for (;;)
    {
      ...
      for (;;)
      {
        try
        {
          if (aekv.W("mini_sdk_prelaunch_enable", 1) == 1)
          {
            i = 1;
            if (i != 0) {
              break;
      ...
      for (int i = 1; i != 0; i = 0)
      {
        ...
        if (bool)
        {
          ...
          i = j;
          if (!TextUtils.isEmpty(TissueGlobal.tissueEnv.getNativeLibDir()))
          {
            i = j;
            if (TissueGlobal.verifyTissueEngine(TissueGlobal.tissueEnv.getNativeLibDir())) {
              i = 1;
            }
          }
          if ((bool) && (i != 0)) {
            localBundle.putString("tissuenativelibdir", TissueGlobal.tissueEnv.getNativeLibDir());
          }
        }
  ...
  public static void preloadMiniApp(Context paramContext, boolean paramBoolean)
  {
    ...
    for (;;)
    {
      try
      {
        ...
        localBundle.putString("mini_key_preload_type", "preload_app");
        localBundle.putBoolean("sdk_mode", true);
        if ((paramBoolean) && (i != 0) && (TissueGlobal.tissueEnv != null)) {
          localBundle.putString("tissuenativelibdir", TissueGlobal.tissueEnv.getNativeLibDir());
        }
        MiniSDK.preloadMiniApp(paramContext, localBundle);
      }
