...
public class MiniProgramLpReportDC04902
{
  ...
  private static String DC_TABLE_VALUE_QUA = avpq.getQUA3();
  ...
  public static void reportGameEnd(float paramFloat1, float paramFloat2, float paramFloat3)
  {
    ...
    try
    {
      ...
      if (d3 < 0.01D)
      {
        ...
        String str = DC_TABLE_VALUE_APPID + '|' + DC_TABLE_VALUE_QUA + '|' + DC_TABLE_VALUE_START_TIME + '|' + DC_TABLE_VALUE_END_TIME + '|' + DC_TABLE_VALUE_F_20 + '|' + DC_TABLE_VALUE_F_33 + '|' + DC_TABLE_VALUE_F_50 + '|' + DC_TABLE_VALUE_F_100 + '|' + DC_TABLE_VALUE_F_LONG + '|' + DC_TABLE_VALUE_AVG_MEM + '|' + DC_TABLE_VALUE_MAX_MEM + '|' + DC_TABLE_VALUE_AVG_CPU + '|' + DC_TABLE_VALUE_MAX_CPU + '|' + DC_TABLE_VALUE_ATTACH_INFO + '|' + DC_TABLE_VALUE_F_MAX_TIME_MILLIS + '|' + DC_TABLE_VALUE_F_MAX + '|' + (int)paramFloat2 + '|' + (int)paramFloat1 + '|' + DC_TABLE_VALUE_TEXTURE_OFFLINE + '|' + MiniProgramReportHelper.getNetworkType() + '|' + DC_TABLE_VALUE_NETWORK_GATEWAY_IP + '|' + aqiw.getCurrentWifiSSID(aroi.a().getContext()) + '|' + DC_TABLE_VALUE_PLATFORM + '|' + Build.MODEL + '|' + Build.VERSION.RELEASE + '|' + d1 + '|' + d2;
        Object localObject = new Bundle();
        ((Bundle)localObject).putStringArray("data", new String[] { str });
        ((Bundle)localObject).putString("log_key", "dc04902");
        QLog.d("MiniProgramLpReportDC04", 1, new Object[] { "reportGameEnd ", str });
        AppBrandProxy.g().sendCmd("cmd_dc_report_log_key_data", (Bundle)localObject, null);
        hasReportStart = false;
        UserAction.setLogAble(false, false);
        HashMap localHashMap = new HashMap();
        localHashMap.put("appId", DC_TABLE_VALUE_APPID);
        localHashMap.put("qua", DC_TABLE_VALUE_QUA);
        localHashMap.put("osVersion", Build.VERSION.RELEASE);
        localObject = "";
        str = "";
        GameRuntimeLoader localGameRuntimeLoader = GameRuntimeLoaderManager.g().getBindRuntimeLoader(aroi.a().getContext());
        if (localGameRuntimeLoader != null)
        {
          localObject = localGameRuntimeLoader.getJsVersion();
          str = localGameRuntimeLoader.getTritonVersion();
        }
        localHashMap.put("baseLibVersion", localObject);
        localHashMap.put("tritonVersion", str);
        localHashMap.put("isSdk", String.valueOf(0));
        localHashMap.put("jank", String.valueOf(d1));
        localHashMap.put("bigJank", String.valueOf(d2));
        UserAction.onUserActionToTunnel("00000ARN3S3S9UE8", "mini_game_jank", true, -1L, -1L, localHashMap, true, true);
      }
    }
  ...
  public static void updateDisplayFrameTime(long paramLong, boolean paramBoolean)
  {
    ...
    for (;;)
    {
      QLog.e("MiniProgramLpReportDC04", 1, "jankCount=" + jankCount + " bigJankCount=" + bigJankCount + " time=" + (paramLong - startFrameTime) / 1000000L);
      i = totalFrameCount;
      frameTimes[(i % 3)] = l2;
      totalFrameCount += 1;
      break;
