...
public class FaceDetector
{
  ...
  public List<QWalletFaceTracker.ExpressionInfo> parseExpressionConfigFromJson(Context paramContext, String paramString1, String paramString2)
  {
    try
    {
      paramContext = aagd.readFile(paramString1);
      ...
      catch (JSONException paramContext)
      {
        try
        {
          ...
          for (;;)
          {
            ...
            locala = new irl.a();
            paramString1.expressionItem = locala;
            ...
            locala.expressionID = ((JSONObject)localObject).optString("expressionID");
            localJSONArray = ((JSONObject)localObject).optJSONArray("expressionFeat");
            if (localJSONArray == null) {
              break;
            }
            locala.expressionFeat = new ArrayList();
            j = 0;
            while (j < localJSONArray.length() / 2)
            {
              locala.expressionFeat.add(new PointF(localJSONArray.optInt(j * 2), localJSONArray.optInt(j * 2 + 1)));
              j += 1;
            }
          ...
          if ((localJSONArray != null) && (localJSONArray.length() == 3)) {
            locala.expressionAngle = convertJsonAngle2NormalAngle(new float[] { (float)localJSONArray.optDouble(0), (float)localJSONArray.optDouble(1), (float)localJSONArray.optDouble(2) });
          }
          localJSONArray = ((JSONObject)localObject).optJSONArray("expressionWeight");
          if ((localJSONArray != null) && (localJSONArray.length() == 7))
          {
            locala.expressionWeight = new double[7];
            j = 0;
          }
          while (j < 7)
          {
            locala.expressionWeight[j] = localJSONArray.optDouble(j);
            j += 1;
            continue;
            locala.expressionWeight = WEIGHT;
          }
          ...
          {
            locala.cB = new int[3];
            locala.cB[0] = ((JSONArray)localObject).optInt(0);
            locala.cB[1] = ((JSONArray)localObject).optInt(1);
            locala.cB[2] = ((JSONArray)localObject).optInt(2);
          }
