...
public class HandRecognizer
{
  ...
  public void onPreviewFrameHandler(byte[] paramArrayOfByte, int paramInt1, int paramInt2, Camera paramCamera, int paramInt3, OnPreviewFrameHandlerListener paramOnPreviewFrameHandlerListener)
  {
    if (!this.hasSDkInit) {
      return;
    }
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      if (HandRecognizer.this.box.confidence >= 0.01D)
      {
        int j = HandRecognizer.this.doHandAlignment(HandRecognizer.this.box, HandRecognizer.this.box);
        i = j;
        if (j == 0)
        {
          i = j;
          if (HandRecognizer.this.box.confidence >= 0.9908F) {
            i = HandRecognizer.this.doHandClassifyWithGesture(HandRecognizer.this.box, this.goal_label, localYtHandClassifyResult);
          }
        }
      }
      if (i != 0) {
  ...
  {
    public abstract void getHandRecognizeResult(int paramInt);
  }
}
