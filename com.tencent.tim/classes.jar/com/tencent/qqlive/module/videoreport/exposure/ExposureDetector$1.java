  ...
  implements IViewTraverseCallback
{
  ...
  public boolean onEnter(View paramView, int paramInt)
  {
    ...
    this.val$detectionData.exposureRate.set(paramInt, Double.valueOf(-1.0D));
    ...
    for (long l1 = ((Rect)localObject3).height() * i;; l1 = 0L)
    {
      ...
      if (l1 != 0L)
      {
        ...
        double d = (float)l1 * 1.0F / (float)l2;
        this.val$detectionData.exposureRate.set(paramInt, Double.valueOf(d));
        if (this.val$preOrder) {
          this.val$callback.onExposed(paramView, this.val$detectionData, d);
        }
      }
  ...
  {
    double d = ((Double)this.val$detectionData.exposureRate.get(paramInt)).doubleValue();
    if ((d != -1.0D) && (!this.val$preOrder)) {
      this.val$callback.onExposed(paramView, this.val$detectionData, d);
    }
