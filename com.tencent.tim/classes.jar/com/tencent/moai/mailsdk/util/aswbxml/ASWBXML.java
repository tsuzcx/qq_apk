...
public class ASWBXML
{
  ...
  public void loadStream(InputStream paramInputStream, ArrayList<Long> paramArrayList, StreamHandler paramStreamHandler)
    throws Exception
  {
    Object localObject = DocumentBuilderFactory.newInstance();
    ((DocumentBuilderFactory)localObject).setNamespaceAware(true);
    ...
    ASWBXMLStreamQueue localASWBXMLStreamQueue;
    do
    {
      return;
      localASWBXMLStreamQueue = new ASWBXMLStreamQueue(paramInputStream);
      localASWBXMLStreamQueue.Dequeue();
      localASWBXMLStreamQueue.dequeueMultibyteInt();
      if (localASWBXMLStreamQueue.dequeueMultibyteInt() != 106) {
        throw new Exception("EX2010_activesyncfolder_cs only supports UTF-8 encoded XML.");
      }
      if (localASWBXMLStreamQueue.dequeueMultibyteInt() != 0) {
        throw new Exception("WBXML data contains a string table.");
      }
      paramInputStream = this.xmlDoc;
    } while (localASWBXMLStreamQueue.getCount() <= 0);
    int i = localASWBXMLStreamQueue.Dequeue();
    int j;
    label320:
    int k;
    switch (i)
    ...
    case 0: 
      ...
      j = localASWBXMLStreamQueue.Dequeue();
      if ((j >= 0) && (j < 25))
      {
        this.currentCodePage = j;
        ...
        paramStreamHandler.onSwitch(this.codePages[this.currentCodePage].getNamespace());
      }
      break;
    }
    label704:
    ...
    for (;;)
    {
      break;
      throw new Exception(String.format("Unknown code page ID 0x{0:X} encountered in WBXML", new Object[] { Byte.valueOf(i) }));
      if (paramInputStream.getParentNode() != null)
      {
        localObject = paramInputStream.getParentNode();
        paramInputStream = (InputStream)localObject;
        if (paramStreamHandler != null)
        {
          paramStreamHandler.onEnd();
          paramInputStream = (InputStream)localObject;
        }
      }
      else
      {
        throw new Exception("END global token encountered out of sequence");
        j = localASWBXMLStreamQueue.dequeueMultibyteInt();
        if ("Mime".equals(paramInputStream.getNodeName())) {
          localASWBXMLStreamQueue.dequeueStream(j, new ASWBXML.1(this, paramStreamHandler));
        }
        for (;;)
        {
          paramInputStream.appendChild(this.xmlDoc.createCDATASection(""));
          break;
          localASWBXMLStreamQueue.dequeueStream(j, new ASWBXML.2(this, paramStreamHandler));
        }
        j = localASWBXMLStreamQueue.getIndex();
        for (long l = ((Long)paramArrayList.remove(0)).longValue(); l < j; l = ((Long)paramArrayList.remove(0)).longValue()) {}
        localASWBXMLStreamQueue.dequeueStream((int)(l - j), new ASWBXML.3(this, paramStreamHandler));
        localASWBXMLStreamQueue.Dequeue();
        paramInputStream.appendChild(this.xmlDoc.createTextNode(""));
        continue;
        throw new Exception(String.format("Encountered unknown global token 0x{0:X}.", new Object[] { Byte.valueOf(i) }));
        j = 0;
        break label320;
        ...
        String str = this.codePages[this.currentCodePage].getTag(i);
        if (str == null) {
          str = String.format("UNKNOWN_TAG_%02X", new Object[] { Byte.valueOf(i) });
        }
        for (;;)
        {
          localObject = this.xmlDoc.createElementNS(this.codePages[this.currentCodePage].getNamespace(), str);
          if (this.xmlDoc.getDocumentElement() == null)
          {
            this.defaultCodePage = this.currentCodePage;
            paramInputStream.appendChild((Node)localObject);
            this.xmlDoc.getDocumentElement().setAttribute("xmlns", this.codePages[this.currentCodePage].getNamespace());
            if (k == 0) {
              break label959;
            }
          ...
          for (;;)
          {
            ...
            paramStreamHandler.onStart(str);
            paramInputStream = (InputStream)localObject;
            break;
            if (this.currentCodePage != this.defaultCodePage)
            {
  ...
  {
    public abstract void onData(byte paramByte);
    
    public abstract void onData(byte[] paramArrayOfByte, int paramInt);
    
    public abstract void onEnd();
    
    public abstract void onStart(String paramString);
    
    public abstract void onSwitch(String paramString);
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.tim\classes7.jar
 * Qualified Name:     com.tencent.moai.mailsdk.util.aswbxml.ASWBXML
