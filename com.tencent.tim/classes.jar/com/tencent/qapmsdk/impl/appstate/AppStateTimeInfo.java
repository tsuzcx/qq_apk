  ...
  private void beforeReport()
  {
    if ((TraceUtil.appState.get() != AppState.COMMONRESTART.getValue()) && (TraceUtil.appState.get() != AppState.INIT.getValue()) && (TraceUtil.appState.get() != AppState.f.getValue()))
    {
      if (appLaunchDurationTime() != -1L)
      {
        ...
        MonitorAdapter localMonitorAdapter = MonitorAdapter.newOne("QAPM_APPLAUNCH", appLaunchStageName(), PluginCombination.resourcePlugin.threshold, TraceType.ContextType.APP);
        ...
        if ((TraceUtil.appState.get() == AppState.FIRSTSTART.getValue()) || (TraceUtil.appState.get() == AppState.COLDSTART.getValue()))
        {
          localMonitorAdapter.addMonitorUnit(new QAPMTraceUnit("QAPM_APPLAUNCH", "LAUNCH_APPLICATION_INIT", this.appAttachBaseContextBeginTime, this.appAttachBaseContextEndTime, TraceType.ContextType.APP.getValue()));
          localMonitorAdapter.deleteMonitorUnit(false);
          localMonitorAdapter.addMonitorUnit(new QAPMTraceUnit("QAPM_APPLAUNCH", "LAUNCH_MAIN_ACTIVITY_INIT", this.appAttachBaseContextEndTime, this.appCreateEndTime, TraceType.ContextType.APP.getValue()));
          localMonitorAdapter.deleteMonitorUnit(false);
        }
        localMonitorAdapter.addMonitorUnit(new QAPMTraceUnit("QAPM_APPLAUNCH", "LAUNCH_ACTIVITY_LOAD", activityPageStartLoadingTime(), this.activityResumeEndTime, TraceType.ContextType.APP.getValue()));
        localMonitorAdapter.deleteMonitorUnit(false);
