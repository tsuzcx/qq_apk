...
public class GraphicRenderMgr
{
  ...
  public static void loadPtuSO()
  {
    if (iow.uW())
    {
      soloadedPTV = ankq.axX();
      ptuSoVersion = ankq.axY();
    }
    if (QLog.isColorLevel()) {
      QLog.w("GraphicRenderMgr", 1, "loadPtuSO, soloadedPTV[" + soloadedPTV + "], ptuSoVersion[" + ptuSoVersion + "], isSupported[" + iow.uW() + "]");
    }
  }
  ...
  public static boolean loadSo()
  {
    ...
    try
    {
      ...
      SoLoadUtil.loadNativeLibrary(localBaseApplication, "c++_shared", 0, false);
      SoLoadUtil.loadNativeLibrary(localBaseApplication, "xplatform", 0, false);
  ...
  public void setFocusDetectCallback(a parama)
  {
    synchronized (this.mAutoFocusCallbackLock)
    {
      this.mAutoFocusCallback = parama;
      return;
    }
  }
