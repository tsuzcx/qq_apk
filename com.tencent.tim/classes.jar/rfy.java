  ...
  extends rgf
{
  ...
  public rfy(DoodleView paramDoodleView)
  {
  ...
  private void init()
  {
    ...
    if (ayxa.isLiuHaiUseValid()) {
      this.bqS = ayxa.sNotchHeight;
    }
    this.bqT = localDisplayMetrics.heightPixels;
    if (ayxa.isLiuHaiUseValid()) {
      this.bqT = (localDisplayMetrics.heightPixels - ayxa.eMW);
    }
  ...
  protected void an(Canvas paramCanvas)
  {
    ...
    {
      this.path.reset();
      this.path.moveTo(this.bqQ, (this.bqT - this.bqS) / 2 + this.bqS);
      this.path.lineTo(this.bqR, (this.bqT - this.bqS) / 2 + this.bqS);
      paramCanvas.drawPath(this.path, this.aY);
      if (QLog.isColorLevel()) {
