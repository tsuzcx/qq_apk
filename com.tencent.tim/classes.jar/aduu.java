...
public class aduu
{
  private static final int cKj = Build.VERSION.SDK_INT;
  ...
  public static void a(float[] paramArrayOfFloat, SensorEvent paramSensorEvent)
  {
    ...
    if (paramSensorEvent.sensor.getType() == 11)
    {
      if (arrayOfFloat.length > 4)
      {
        if (cKj >= 9)
        {
          getRotationMatrixFromVector(paramArrayOfFloat, (float[])arrayOfFloat.clone());
          return;
        }
        if (cb == null) {
          cb = new float[4];
        }
        System.arraycopy(arrayOfFloat, 0, cb, 0, 4);
        getRotationMatrixFromVector(paramArrayOfFloat, cb);
        return;
      }
      getRotationMatrixFromVector(paramArrayOfFloat, (float[])arrayOfFloat.clone());
      return;
