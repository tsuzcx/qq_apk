public abstract interface azav
{
  public abstract void E(String paramString, int paramInt, long paramLong);
  
  public abstract void Tt(int paramInt);
  
  public abstract void dJV();
  
  public abstract void dJW();
  
  public abstract void js(int paramInt1, int paramInt2);
  
  public abstract void m(String paramString, int paramInt1, int paramInt2, int paramInt3);
  
  public abstract void n(String paramString1, int paramInt1, int paramInt2, String paramString2);
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.tim\classes5.jar
 * Qualified Name:     azav
 * JD-Core Version:    0.7.0.1
 */