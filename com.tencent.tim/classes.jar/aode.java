...
class aode
  extends auve
{
  aode(aodc paramaodc, int paramInt1, int paramInt2, int[] paramArrayOfInt1, int paramInt3, int[] paramArrayOfInt2, int[] paramArrayOfInt3, int[] paramArrayOfInt4, List paramList, int[] paramArrayOfInt5)
  {
  ...
  {
    paramInt = 0;
    Object localObject = this.a.a((aoef)paramObject);
    int i = ((List)localObject).size();
    paramObject = localObject;
    if (i > 3)
    {
      paramObject = ((List)localObject).subList(i - 2, i);
      paramObject.add(0, aodc.a);
    }
    ...
    if (paramInt < paramArrayOfb.length)
    {
      ...
      if (paramInt < i)
      {
        localObject = (aoeg)paramObject.get(paramInt);
        j = this.GU.indexOf(localObject);
        if (j < 0)
        {
          if (QLog.isColorLevel()) {
            QLog.i(this.TAG, 1, "getRightMenuItemInfo error, can not find the menu， menuId[" + ((aoeg)localObject).hF() + "]");
          }
