...
public class avcc
{
  private static avcc a;
  private Pattern pattern = Pattern.compile("(\\d+)\\.(\\d+).(\\d+)_(\\d+)");
    ...
    do
    {
      do
      {
        do
        {
          ...
          } while ((!TextUtils.isEmpty((CharSequence)localObject)) && (!tf((String)localObject)));
  ...
  private boolean tf(String paramString)
  {
    ...
    for (;;)
    {
      ...
      try
      {
        ...
        {
          paramString = r(localObject[0].trim());
          localObject = r(localObject[1].trim());
          if ((paramString != null) && (localObject != null) && (paramString.length >= 4) && (localObject.length >= 4))
          {
            ...
            paramString = this.pattern.matcher(avpq.getQUA3());
            if (paramString.find())
            {
