...
public class tnq
{
  public static tnp a(Context paramContext, String paramString)
  {
    ...
    for (;;)
    {
      try
      {
        if (TextUtils.isEmpty(paramContext))
        {
          ...
          if (i < paramContext.length())
          {
            ...
            paramString.add(new tnp(localJSONObject));
            break label130;
          }
        }
        else
      ...
      catch (JSONException paramContext)
      {
        ...
        if (paramString.size() > 1) {
          Collections.sort(paramString, new tnr());
        }
        if (paramString.size() > 0) {
          return (tnp)paramString.get(0);
        }
        return null;
      }
      label125:
    ...
    for (;;)
    {
      ...
      try
      {
        ...
        if (TextUtils.isEmpty(paramString))
        {
          ...
          localObject3 = new tnp((JSONObject)localObject2).os();
  ...
  public static void c(Context paramContext, tnp paramtnp)
  {
    int i = 0;
    if (paramtnp == null) {
      return;
    ...
    for (;;)
    {
      try
      {
        ...
        if (TextUtils.isEmpty((CharSequence)localObject))
        {
          ...
          ((JSONArray)localObject).put(new JSONObject(paramtnp.or()));
      ...
      {
        localObject = new tnp(((JSONArray)localObject).getJSONObject(0));
        if (localObject != null)
        {
          int j = ((tnp)localObject).uX();
          int k = paramtnp.uX();
          if (j >= k) {}
        }
        else
