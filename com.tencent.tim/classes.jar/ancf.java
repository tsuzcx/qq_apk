    ...
    do
    {
      for (;;)
      {
        ...
        else
        {
          ...
          int i = ihk.bytesToInt(paramList, 4);
          int j = ihk.bytesToInt((byte[])localObject, 4);
          if ((i <= 0) || (j <= 0))
          {
            if (QLog.isColorLevel()) {
          ...
          else
          {
            ...
            catch (Exception localException)
            {
              ...
              boolean bool = imi.g(paramMsg);
              if (((!paramanax.isRead) && (!paramanax.cEG)) || ((paramanax.akr != paramanax.senderUin) || ((paramanax.akr == paramanax.senderUin) && (!bool))))
              {
                if (QLog.isColorLevel()) {
                  QLog.e("decodeC2CMsgPkg_QCall", 2, "<---decodeC2CMsgPkg_QCall return null:,isReaded:" + paramanax.isRead + "syncOther:" + paramanax.cEG + ",isSharpRequest" + bool);
                }
              }
              else
              {
                ...
                try
                {
                  new iuq(BaseApplicationImpl.getContext()).a(new ancg(this, paramMsg, l3 - l1, paramMessageHandler, l4, l2, l1, bool));
                  return;
                }
