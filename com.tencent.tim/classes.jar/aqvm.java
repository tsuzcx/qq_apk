  ...
  implements aqvc, INetInfoHandler
{
  ...
  public aqvm(AppRuntime paramAppRuntime, aqva.a parama)
  {
    ...
    try
    {
      if (!this.i.isEmpty())
      {
        ...
        while (localIterator.hasNext())
        {
          ...
          {
            if (QLog.isColorLevel()) {
              QLog.d("QueueDownloader", 2, "getTask | " + paramString + " task find =" + localaquz);
            }
            return localaquz;
          }
        }
  ...
  public void a(aquz paramaquz)
  {
    ...
    try
    {
      ...
      {
        if (QLog.isColorLevel()) {
          QLog.d("QueueDownloader", 2, "removeTask | task=" + paramaquz);
        }
        paramaquz.bme();
      ...
      synchronized (this.i)
      {
        ...
        if (paramBundle == null)
        {
          ...
          if (paramaquz.cWl)
          {
            ...
            if (QLog.isColorLevel()) {
              QLog.d("QueueDownloader", 2, "startDownload | task=" + paramaquz.key);
            }
            doTask();
            return;
          }
  ...
  public void doTask()
  {
    for (;;)
    {
      try
      {
        synchronized (this.i)
        {
          ...
          if ((!localaquz2.isLoading()) && (l > localaquz2.atV + localaquz2.atW))
          {
            ...
            QLog.d("QueueDownloader", 2, "remove task[" + localaquz2.key + "], isCancal=" + localaquz2.isCancel() + ", timeOut=" + bool);
          }
        }
        bool = false;
      }
      ...
      label185:
      if (QLog.isColorLevel()) {
        QLog.d("QueueDownloader", 2, "doTask | downloadLimitCount=" + this.y.get() + ",maxDownloadCount=" + 3 + ",downloadQueue size=" + this.i.size());
      }
      for (;;)
      {
        aquz localaquz1;
        ...
        return;
        label291:
        ThreadManager.postDownLoadTask(new QueueDownloader.1(this, localaquz1), 2, null, false);
        this.y.addAndGet(1);
      }
      label319:
      boolean bool = false;
