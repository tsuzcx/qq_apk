  ...
  extends LinearLayoutManager
{
  ...
  void layoutChunk(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, LinearLayoutManager.LayoutState paramLayoutState, LinearLayoutManager.LayoutChunkResult paramLayoutChunkResult)
  {
    ...
    for (;;)
    {
      float f2 = 1.0F * this.mOrientationHelper.getDecoratedMeasurementInOther(paramRecycler) / paramState.mSpanSize;
      if (f2 > f1) {
        f1 = f2;
      }
      for (;;)
      {
        ...
        for (;;)
        {
          ...
          for (;;)
          {
            ...
            label1328:
            i = getPaddingLeft();
            j = this.mCachedBorders[paramState.mSpanIndex] + i;
            i = this.mOrientationHelper.getDecoratedMeasurementInOther(paramRecycler) + j;
            continue;
            label1364:
            m = getPaddingTop();
            m = this.mCachedBorders[paramState.mSpanIndex] + m;
