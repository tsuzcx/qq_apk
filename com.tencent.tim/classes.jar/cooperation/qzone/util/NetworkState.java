...
public class NetworkState
{
  ...
  private static List<a> observers;
  ...
  public static void addListener(a parama)
  {
    if (parama == null) {
      return;
    }
    ...
    {
      if (!observers.contains(parama)) {
        observers.add(parama);
      }
      return;
    }
  }
  ...
  private static void notifyObservers(boolean paramBoolean)
  {
    synchronized (observers)
    {
      a[] arrayOfa = new a[observers.size()];
      observers.toArray(arrayOfa);
      if (arrayOfa != null)
      {
        ...
        if (i < j)
        {
          arrayOfa[i].onNetworkConnect(paramBoolean);
          i += 1;
        }
      }
    }
