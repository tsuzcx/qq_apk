  ...
  implements Serializable
{
  ...
  private static EOptionID[] ep = new EOptionID[17];
  private int eb;
  ...
  public static EOptionID convert(int paramInt)
  {
    ...
    for (;;)
    {
      EOptionID[] arrayOfEOptionID = ep;
      if (i >= arrayOfEOptionID.length) {
        break;
      }
      if (arrayOfEOptionID[i].value() == paramInt) {
        return ep[i];
      }
      i += 1;
    }
  ...
  public static EOptionID convert(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (arrayOfEOptionID[i].toString().equals(paramString)) {
        return ep[i];
      }
      i += 1;
    }
