...
public final class ETrafficTemplateType
{
  ...
  public static ETrafficTemplateType convert(int paramInt)
  {
    ...
    for (;;)
    {
      ETrafficTemplateType[] arrayOfETrafficTemplateType = ex;
      if (i >= arrayOfETrafficTemplateType.length) {
        break;
      }
      if (arrayOfETrafficTemplateType[i].value() == paramInt) {
        return ex[i];
      }
      i += 1;
    }
  ...
  public static ETrafficTemplateType convert(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (arrayOfETrafficTemplateType[i].toString().equals(paramString)) {
        return ex[i];
      }
      i += 1;
    }
