...
{
  public static final EMessageTime EMTI_END = new EMessageTime(7, 420103001, "EMTI_END");
  ...
  public static EMessageTime convert(int paramInt)
  {
    ...
    for (;;)
    {
      EMessageTime[] arrayOfEMessageTime = en;
      if (i >= arrayOfEMessageTime.length) {
        break;
      }
      if (arrayOfEMessageTime[i].value() == paramInt) {
        return en[i];
      }
      i += 1;
    }
  ...
  public static EMessageTime convert(String paramString)
  {
    ...
    for (;;)
    {
      ...
      if (arrayOfEMessageTime[i].toString().equals(paramString)) {
        return en[i];
      }
      i += 1;
    }
