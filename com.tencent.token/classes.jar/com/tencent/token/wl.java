package com.tencent.token;

import android.content.ContentValues;
import android.content.Context;
import android.os.Build;
import android.os.Build.VERSION;
import android.os.Handler;
import android.os.Message;
import android.text.TextUtils;
import com.tencent.token.core.bean.RealNameRegResult;
import com.tencent.token.global.RqdApplication;
import java.util.HashMap;
import oicq.wlogin_sdk.request.WtloginHelper;
import org.json.JSONObject;

public final class wl
  extends tr
{
  public static String i;
  public static String j;
  public long d;
  public long e;
  public int f;
  public String g;
  public String h;
  private int k;
  private byte[] l;
  private byte[] m;
  private byte[] n;
  private byte[] o;
  private byte[] p;
  private RealNameRegResult q;
  
  public final String a()
  {
    sh.a();
    this.a.a(104, null, null);
    return null;
  }
  
  public final void a(abc paramabc)
  {
    this.d = ((Long)paramabc.c.get("param.uinhash")).longValue();
    this.e = ((Long)paramabc.c.get("param.realuin")).longValue();
    this.g = ((String)paramabc.c.get("param.realname"));
    this.h = ((String)paramabc.c.get("param.idnumber"));
    this.k = ((Integer)paramabc.c.get("param.optype")).intValue();
    this.l = ((byte[])paramabc.c.get("param.facedata"));
    this.m = ((byte[])paramabc.c.get("param.frontdata"));
    this.n = ((byte[])paramabc.c.get("param.backdata"));
    this.o = ((byte[])paramabc.c.get("param.prontphotoinfo"));
    this.p = ((byte[])paramabc.c.get("param.backphotoinfo"));
    this.f = paramabc.j;
  }
  
  public final void a(JSONObject paramJSONObject)
  {
    int i1 = paramJSONObject.getInt("err");
    String str;
    if (i1 != 0)
    {
      str = paramJSONObject.getString("info");
      paramJSONObject = aao.d(paramJSONObject.getString("data"));
      if (paramJSONObject == null) {}
    }
    try
    {
      paramJSONObject = new JSONObject(new String(paramJSONObject));
      i = paramJSONObject.getString("hint_title");
      j = paramJSONObject.getString("hint_content");
      label66:
      a(i1, str);
      return;
      paramJSONObject = aao.d(paramJSONObject.getString("data"));
      if (paramJSONObject != null)
      {
        paramJSONObject = new JSONObject(new String(paramJSONObject));
        xj.a("ProtoRealNameReg json=".concat(String.valueOf(paramJSONObject)));
        i1 = paramJSONObject.getInt("seq_id");
        if (i1 != this.f)
        {
          this.a.a(10030, null, null);
          paramJSONObject = new StringBuilder("parseJSON error seq is wrong seq=");
          paramJSONObject.append(i1);
          paramJSONObject.append(",right = ");
          paramJSONObject.append(this.f);
          xj.c(paramJSONObject.toString());
          return;
        }
        this.q = new RealNameRegResult(paramJSONObject);
        this.a.a = 0;
        return;
      }
      xj.c("parseJSON error decodeData=".concat(String.valueOf(paramJSONObject)));
      a(10022, RqdApplication.p().getString(2131493068));
      return;
    }
    catch (Exception paramJSONObject)
    {
      break label66;
    }
  }
  
  public final abc b(abc paramabc)
  {
    int i1 = paramabc.j;
    paramabc.m = 1;
    paramabc.n = new ContentValues(3);
    Object localObject1 = paramabc.n;
    sh.a();
    ((ContentValues)localObject1).put("aq_base_sid", null);
    localObject1 = new JSONObject();
    try
    {
      ((JSONObject)localObject1).put("uin", this.d);
      ((JSONObject)localObject1).put("real_uin", this.e);
      ((JSONObject)localObject1).put("realname", this.g);
      ((JSONObject)localObject1).put("id_number", this.h);
      ((JSONObject)localObject1).put("op_type", this.k);
      Object localObject2 = new StringBuilder("doRealNameReg common_data=");
      ((StringBuilder)localObject2).append(((JSONObject)localObject1).toString());
      xj.a(((StringBuilder)localObject2).toString());
      StringBuilder localStringBuilder2;
      if (this.l != null)
      {
        localObject2 = aaf.a(this.l);
        ((JSONObject)localObject1).put("face_data", localObject2);
        localStringBuilder2 = new StringBuilder("doRealNameReg face_data=");
        localStringBuilder2.append(((String)localObject2).length());
        xj.a(localStringBuilder2.toString());
      }
      if (this.m != null)
      {
        localObject2 = aaf.a(this.m);
        ((JSONObject)localObject1).put("id_photo_front", localObject2);
        localStringBuilder2 = new StringBuilder("doRealNameReg front_data=");
        localStringBuilder2.append(((String)localObject2).length());
        xj.a(localStringBuilder2.toString());
      }
      if (this.n != null)
      {
        localObject2 = aaf.a(this.n);
        ((JSONObject)localObject1).put("id_photo_back", localObject2);
        localStringBuilder2 = new StringBuilder("doRealNameReg back_data=");
        localStringBuilder2.append(((String)localObject2).length());
        xj.a(localStringBuilder2.toString());
      }
      if (this.o != null) {
        ((JSONObject)localObject1).put("id_photo_info_front", aaf.a(this.o));
      }
      if (this.p != null) {
        ((JSONObject)localObject1).put("id_photo_info_back", aaf.a(this.p));
      }
      ((JSONObject)localObject1).put("token_seq", sj.b().a.d());
      ((JSONObject)localObject1).put("guid", aao.a(sg.a(RqdApplication.p()).a.GetGuid()));
      ((JSONObject)localObject1).put("seq_id", i1);
      ((JSONObject)localObject1).put("op_time", (int)(sj.b().i() / 1000L));
      ((JSONObject)localObject1).put("vendor_id", Build.MANUFACTURER);
      ((JSONObject)localObject1).put("android_id", aao.a(RqdApplication.p()));
      ((JSONObject)localObject1).put("imei", aao.b(RqdApplication.p()));
      ((JSONObject)localObject1).put("mac", aao.c(RqdApplication.p()));
      ((JSONObject)localObject1).put("device_model", xr.a);
      ((JSONObject)localObject1).put("sys_ver", Build.VERSION.RELEASE);
      ((JSONObject)localObject1).put("face_detect_time", uu.f);
      ((JSONObject)localObject1).put("face_detect_frame", uu.h);
      ((JSONObject)localObject1).put("face_model_init_time", uu.g);
      ((JSONObject)localObject1).put("vivo_model_init_time", uu.i);
      ((JSONObject)localObject1).put("vivo_reg_action1_time", uu.j);
      ((JSONObject)localObject1).put("vivo_reg_action2_time", uu.k);
      ((JSONObject)localObject1).put("vivo_reg_action1_frame", uu.l);
      ((JSONObject)localObject1).put("vivo_reg_action2_frame", uu.m);
      ((JSONObject)localObject1).put("vivo_ver_action_time", uu.n);
      ((JSONObject)localObject1).put("vivo_ver_action_frame", uu.o);
      ((JSONObject)localObject1).put("vivo_reg_action1_type", wa.d);
      ((JSONObject)localObject1).put("vivo_reg_action2_type", wa.e);
      ((JSONObject)localObject1).put("vivo_ver_action_type", wa.f);
      ((JSONObject)localObject1).put("id_photo_front_time", yg.f);
      ((JSONObject)localObject1).put("id_photo_front_frame", yg.e);
      ((JSONObject)localObject1).put("id_photo_back_time", yg.h);
      ((JSONObject)localObject1).put("id_photo_back_frame", yg.g);
      localObject2 = aao.e(RqdApplication.p());
      if (!TextUtils.isEmpty((CharSequence)localObject2)) {
        ((JSONObject)localObject1).put("route_name", localObject2);
      }
      localObject2 = aao.d(RqdApplication.p());
      if (!TextUtils.isEmpty((CharSequence)localObject2)) {
        ((JSONObject)localObject1).put("route_mac", localObject2);
      }
    }
    catch (Exception localException)
    {
      localException.printStackTrace();
    }
    localObject1 = aaf.a(aao.c(((JSONObject)localObject1).toString().getBytes())).replace('+', '-').replace('=', '_');
    StringBuilder localStringBuilder1 = new StringBuilder("doRealNameReg data = ");
    localStringBuilder1.append(((String)localObject1).length());
    xj.a(localStringBuilder1.toString());
    paramabc.n.put("data", (String)localObject1);
    return paramabc;
  }
  
  public final void b()
  {
    if ((!this.b.e) && (this.b.d != null))
    {
      Message localMessage = this.b.d.obtainMessage(this.b.f);
      localMessage.arg1 = 0;
      localMessage.obj = this.q;
      localMessage.sendToTarget();
      this.b.e = true;
    }
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.token\classes.jar
 * Qualified Name:     com.tencent.token.wl
 * JD-Core Version:    0.7.0.1
 */