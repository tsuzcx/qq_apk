package com.tencent.token;

import android.content.Context;
import android.graphics.Bitmap;
import android.os.Build;
import android.os.Build.VERSION;
import android.os.Handler;
import android.text.TextUtils;
import com.tencent.token.core.bean.QQUser;
import com.tencent.token.global.RqdApplication;
import com.tencent.token.utils.UserTask;
import java.net.URLEncoder;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.List;
import java.util.concurrent.CountDownLatch;
import java.util.concurrent.atomic.AtomicReference;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

public final class th
{
  public static final abe b = new abe("", 16);
  public static String c = null;
  public static String d = null;
  public static String e = null;
  public static String f = null;
  private static th q = null;
  public byte[] a = null;
  public long g = 0L;
  public long h = 0L;
  public boolean i = true;
  public boolean j = false;
  public ais k = null;
  public QQUser l = null;
  public long m = 0L;
  public long n = 0L;
  public long o = 0L;
  private abe p = new abe("-1");
  private HashMap<String, a> r = new HashMap();
  
  public static QQUser a(xh paramxh)
  {
    paramxh.a();
    th localth = a();
    if (localth.k.b() == null)
    {
      xj.c("there is no user bind");
      paramxh.a(110, "there is no user bind", null);
      return null;
    }
    Object localObject2 = localth.f();
    boolean bool = false;
    if (localObject2 != null)
    {
      paramxh.a = 0;
      return localObject2;
    }
    Object localObject1 = localObject2;
    if (localObject2 == null)
    {
      localObject1 = localth.k();
      if (!((xh)localObject1).b())
      {
        localObject2 = new StringBuilder("user login failed:");
        ((StringBuilder)localObject2).append(((xh)localObject1).a);
        ((StringBuilder)localObject2).append(((xh)localObject1).b);
        xj.c(((StringBuilder)localObject2).toString());
        paramxh.a((xh)localObject1);
        return null;
      }
      localObject1 = localth.f();
      if (localObject1 != null) {
        bool = true;
      }
      xj.a(bool);
    }
    return localObject1;
  }
  
  public static th a()
  {
    if (q == null) {
      q = new th();
    }
    return q;
  }
  
  public static String a(long paramLong, String paramString)
  {
    try
    {
      localObject = new JSONObject();
      ((JSONObject)localObject).put("tkn_seq", String.valueOf(paramLong));
      ((JSONObject)localObject).put("code", paramString);
      ((JSONObject)localObject).put("pm", URLEncoder.encode(xr.a));
      ((JSONObject)localObject).put("pb", URLEncoder.encode(Build.BRAND));
      ((JSONObject)localObject).put("ps", Build.VERSION.SDK);
      ((JSONObject)localObject).put("pr", URLEncoder.encode(Build.VERSION.RELEASE));
      ((JSONObject)localObject).put("net_type", aao.i());
      paramString = aao.d(RqdApplication.p());
      if (!TextUtils.isEmpty(paramString)) {
        ((JSONObject)localObject).put("router_id", paramString);
      }
      paramString = aao.c(RqdApplication.p());
      if (!TextUtils.isEmpty(paramString)) {
        ((JSONObject)localObject).put("mac", paramString);
      }
      paramString = aao.a(RqdApplication.p());
      if (!TextUtils.isEmpty(paramString)) {
        ((JSONObject)localObject).put("android_id", paramString);
      }
      paramString = aao.b(RqdApplication.p());
      if (!TextUtils.isEmpty(paramString)) {
        ((JSONObject)localObject).put("device_id", paramString);
      }
      int i1 = si.b + 1;
      si.b = i1;
      ((JSONObject)localObject).put("seq_id", i1);
      ((JSONObject)localObject).put("turingd_ticket_id", xq.b());
      paramString = ((JSONObject)localObject).toString();
      xj.a("plain:".concat(String.valueOf(paramString)));
      paramString = aao.b(paramString.getBytes());
      return paramString;
    }
    catch (JSONException paramString)
    {
      Object localObject = new StringBuilder("JSONException:");
      ((StringBuilder)localObject).append(paramString.getMessage());
      xj.c(((StringBuilder)localObject).toString());
    }
    return null;
  }
  
  public static void a(JSONObject paramJSONObject)
  {
    try
    {
      long l1 = paramJSONObject.getLong("server_time");
      sj.b();
      sj.a(l1);
      return;
    }
    catch (Exception paramJSONObject)
    {
      label17:
      break label17;
    }
    xj.c("correct time error");
  }
  
  public static Bitmap b(String paramString)
  {
    if (paramString == null) {
      return null;
    }
    paramString = new ajn().a(paramString);
    if (paramString == null) {
      return null;
    }
    return aap.a(paramString);
  }
  
  private xh b(xh paramxh)
  {
    Object localObject1 = new xh();
    paramxh.a();
    Object localObject3 = a();
    if (!((th)localObject3).i())
    {
      localObject2 = ((th)localObject3).m();
      localObject1 = localObject2;
      if (!((xh)localObject2).b()) {
        return localObject2;
      }
    }
    Object localObject2 = ((th)localObject3).k.b();
    if (localObject2 == null)
    {
      ((xh)localObject1).a(110, null, null);
      return localObject1;
    }
    try
    {
      final CountDownLatch localCountDownLatch = new CountDownLatch(1);
      localObject3 = new AtomicReference(null);
      rf.a().a(0, aao.k(), new rf.a()
      {
        public final void a(ri paramAnonymousri)
        {
          this.a.set(paramAnonymousri);
          localCountDownLatch.countDown();
        }
      });
      try
      {
        localCountDownLatch.await();
      }
      catch (InterruptedException localInterruptedException)
      {
        localInterruptedException.printStackTrace();
      }
      localObject3 = (ri)((AtomicReference)localObject3).get();
      if (localObject3 == null)
      {
        ((xh)localObject1).a(-798, null, null);
        return localObject1;
      }
      try
      {
        int i1 = ((ri)localObject3).b;
        if (i1 != 0)
        {
          localObject2 = ((ri)localObject3).d;
          ((xh)localObject1).a(i1, (String)localObject2, (String)localObject2);
          paramxh.a(i1, null, null);
          xj.c("server errcode=".concat(String.valueOf(i1)));
        }
        else
        {
          paramxh = ((ri)localObject3).c;
          if (paramxh != null)
          {
            paramxh = new JSONObject(paramxh);
            long l1 = paramxh.getLong("uin");
            if (l1 != ((QQUser)localObject2).mUin)
            {
              paramxh = new StringBuilder("uin not match=");
              paramxh.append(l1);
              paramxh.append(":");
              paramxh.append(((QQUser)localObject2).mUin);
              ((xh)localObject1).a(10000, paramxh.toString(), null);
              return localObject1;
            }
            a().g();
            if (!this.k.a(l1))
            {
              ((xh)localObject1).a(10000, "mUserStorage.setCurrentUserByUin failed", null);
              return localObject1;
            }
            this.l = ((QQUser)localObject2);
            this.m = (System.currentTimeMillis() / 1000L);
            this.n = paramxh.getInt("valid_time");
            ((xh)localObject1).a = 0;
          }
          else
          {
            xj.c("parseJSON error decodeData");
            ((xh)localObject1).a(10022, RqdApplication.p().getString(2131493068), RqdApplication.p().getString(2131493068));
          }
        }
      }
      catch (JSONException paramxh)
      {
        localObject2 = new StringBuilder("parse json failed: ");
        ((StringBuilder)localObject2).append(paramxh.toString());
        xj.c(((StringBuilder)localObject2).toString());
        localObject2 = new StringBuilder("JSONException:");
        ((StringBuilder)localObject2).append(paramxh.toString());
        ((xh)localObject1).a(10020, ((StringBuilder)localObject2).toString(), null);
      }
      return localObject1;
    }
    finally {}
  }
  
  public static void b()
  {
    q = null;
  }
  
  private void n()
  {
    if (RqdApplication.h) {
      switch (xf.a())
      {
      default: 
        d = null;
        break;
      case 3: 
        f = null;
        break;
      case 2: 
        e = null;
        break;
      case 1: 
        d = null;
        break;
      case 0: 
        c = null;
      }
    }
    this.j = false;
    this.g = 0L;
    this.h = 0L;
    this.l = null;
    this.m = 0L;
    this.n = 0L;
  }
  
  public final int a(final Handler paramHandler)
  {
    new UserTask() {}.a(new String[] { "" });
    return 0;
  }
  
  public final QQUser a(int paramInt)
  {
    return this.k.a(paramInt);
  }
  
  public final xh a(String paramString, long paramLong)
  {
    xh localxh = new xh();
    Object localObject1 = a();
    ((th)localObject1).h();
    localObject1 = ((th)localObject1).l();
    if (!((xh)localObject1).b()) {
      return localObject1;
    }
    boolean bool2 = true;
    final CountDownLatch localCountDownLatch = new CountDownLatch(1);
    localObject1 = new AtomicReference(null);
    rf.a().b(Long.parseLong(paramString), paramLong, new rf.a()
    {
      public final void a(ri paramAnonymousri)
      {
        this.a.set(paramAnonymousri);
        localCountDownLatch.countDown();
      }
    });
    try
    {
      localCountDownLatch.await();
    }
    catch (InterruptedException localInterruptedException)
    {
      localInterruptedException.printStackTrace();
    }
    localObject1 = (ri)((AtomicReference)localObject1).get();
    if (localObject1 == null)
    {
      localxh.a(-367, null, null);
      return localxh;
    }
    for (;;)
    {
      try
      {
        int i1 = ((ri)localObject1).b;
        if (i1 != 0)
        {
          if (102 == i1)
          {
            long l1 = System.currentTimeMillis() + rf.a().g();
            long l2 = 30 - (int)(l1 / 1000L % 30L) + 2L;
            localObject1 = new StringBuilder("sleep ");
            ((StringBuilder)localObject1).append(l2);
            ((StringBuilder)localObject1).append("s for dyncode refresh:");
            ((StringBuilder)localObject1).append(l1);
            xj.a(((StringBuilder)localObject1).toString());
            try
            {
              Thread.sleep(l2 * 1000L);
            }
            catch (Exception localException)
            {
              xj.c(localException.getMessage());
            }
            sj.b().d();
            return a(paramString, paramLong);
          }
          paramString = localException.d;
          localxh.a(i1, paramString, paramString);
          return localxh;
        }
        paramString = new JSONObject(localException.c);
        xj.a(true);
        xj.a("data hex: ".concat(String.valueOf(paramString)));
        localObject2 = new a();
        StringBuilder localStringBuilder = new StringBuilder();
        localStringBuilder.append(paramString.getLong("uin"));
        ((a)localObject2).a = localStringBuilder.toString();
        ((a)localObject2).b = paramString.getString("sig");
        if (paramString.getInt("is_login_prot_set") == 0)
        {
          bool1 = false;
          ((a)localObject2).c = bool1;
          bool1 = bool2;
          if (paramString.getInt("is_game_prot_set") == 0) {
            bool1 = false;
          }
          ((a)localObject2).d = bool1;
          ((a)localObject2).e = paramString.getInt("is_tkn_set");
          if (((a)localObject2).e == 2) {
            ((a)localObject2).f = paramString.getString("other_tkn_seq");
          }
          this.r.put(((a)localObject2).a, localObject2);
          localxh.a = 0;
          return localxh;
        }
      }
      catch (Exception paramString)
      {
        localObject2 = new StringBuilder("unknown err: ");
        ((StringBuilder)localObject2).append(paramString.toString());
        xj.c(((StringBuilder)localObject2).toString());
        localObject2 = new StringBuilder("JSONException:");
        ((StringBuilder)localObject2).append(paramString.toString());
        localxh.a(10021, ((StringBuilder)localObject2).toString(), null);
        return localxh;
      }
      catch (JSONException paramString)
      {
        Object localObject2 = new StringBuilder("parse json failed: ");
        ((StringBuilder)localObject2).append(paramString.toString());
        xj.c(((StringBuilder)localObject2).toString());
        localObject2 = new StringBuilder("JSONException:");
        ((StringBuilder)localObject2).append(paramString.toString());
        localxh.a(10020, ((StringBuilder)localObject2).toString(), null);
        return localxh;
      }
      boolean bool1 = true;
    }
  }
  
  public final xh a(JSONArray paramJSONArray)
  {
    xh localxh = new xh();
    Object localObject1 = new LinkedList();
    int i1 = 0;
    for (;;)
    {
      try
      {
        if (i1 < paramJSONArray.length())
        {
          localJSONObject = paramJSONArray.getJSONObject(i1);
          localObject2 = new StringBuilder("user json: ");
          ((StringBuilder)localObject2).append(localJSONObject.toString());
          xj.a(((StringBuilder)localObject2).toString());
          localObject2 = new QQUser();
          ((QQUser)localObject2).mUin = localJSONObject.getLong("uin");
          ((QQUser)localObject2).mUinMask = localJSONObject.getString("uin_mask");
          ((QQUser)localObject2).mNickName = localJSONObject.getString("nick");
          ((QQUser)localObject2).mRealUin = localJSONObject.getLong("real_uin");
          int i2 = localJSONObject.getInt("is_supper_qq");
          bool2 = true;
          if (i2 != 1) {
            break label315;
          }
          bool1 = true;
          ((QQUser)localObject2).mIsSupperQQ = bool1;
          ((QQUser)localObject2).mIsBinded = true;
        }
      }
      catch (JSONException paramJSONArray)
      {
        JSONObject localJSONObject;
        Object localObject2;
        boolean bool2;
        localObject1 = new StringBuilder("updateQueryUinList JSONException:");
        ((StringBuilder)localObject1).append(paramJSONArray.toString());
        localxh.a(10020, ((StringBuilder)localObject1).toString(), null);
        return localxh;
      }
      try
      {
        if (localJSONObject.getInt("have_face_auth") != 1) {
          break label321;
        }
        bool1 = true;
        ((QQUser)localObject2).mIsRegisterFacePwd = bool1;
        if (localJSONObject.getInt("is_zzb") != 1) {
          break label327;
        }
        bool1 = bool2;
        ((QQUser)localObject2).mIsZzb = bool1;
      }
      catch (Exception localException)
      {
        continue;
      }
      ((List)localObject1).add(localObject2);
      i1 += 1;
      continue;
      new StringBuilder("updateUinList, size:").append(((List)localObject1).size());
      this.k.a((List)localObject1);
      localxh.a = 0;
      return localxh;
      label315:
      boolean bool1 = false;
      continue;
      label321:
      bool1 = false;
      continue;
      label327:
      bool1 = false;
    }
  }
  
  public final String a(String paramString)
  {
    final CountDownLatch localCountDownLatch = new CountDownLatch(1);
    final AtomicReference localAtomicReference = new AtomicReference();
    rf.a().c(Long.parseLong(paramString), new rf.a()
    {
      public final void a(ri paramAnonymousri)
      {
        xh localxh = new xh();
        localxh.a = paramAnonymousri.b;
        localxh.c = paramAnonymousri.d;
        localxh.b = paramAnonymousri.d;
        if (paramAnonymousri.b == 0) {
          try
          {
            paramAnonymousri = new JSONObject(paramAnonymousri.c).getString("photo_url");
            localAtomicReference.set(paramAnonymousri);
          }
          catch (JSONException paramAnonymousri)
          {
            paramAnonymousri.printStackTrace();
            localxh.a = 10020;
          }
        }
        localCountDownLatch.countDown();
      }
    });
    try
    {
      localCountDownLatch.await();
    }
    catch (Throwable paramString)
    {
      paramString.printStackTrace();
    }
    return (String)localAtomicReference.get();
  }
  
  public final void a(long paramLong1, long paramLong2)
  {
    QQUser localQQUser = c(paramLong1);
    if (localQQUser != null)
    {
      this.l = localQQUser;
      this.m = (System.currentTimeMillis() / 1000L);
      this.n = paramLong2;
    }
  }
  
  public final boolean a(long paramLong)
  {
    return this.k.a(paramLong);
  }
  
  public final boolean a(QQUser paramQQUser)
  {
    if ((paramQQUser != null) && (paramQQUser.mRealUin != 0L)) {
      return this.k.b(paramQQUser);
    }
    return false;
  }
  
  public final QQUser b(int paramInt)
  {
    return this.k.b(paramInt);
  }
  
  public final boolean b(long paramLong)
  {
    return this.k.b(paramLong);
  }
  
  public final long c()
  {
    return this.k.d();
  }
  
  public final QQUser c(long paramLong)
  {
    return this.k.c(paramLong);
  }
  
  public final a c(String paramString)
  {
    return (a)this.r.get(paramString);
  }
  
  public final boolean c(int paramInt)
  {
    if (104 == paramInt)
    {
      if (!j().b()) {
        return false;
      }
      return k().b();
    }
    if (109 == paramInt) {
      return k().b();
    }
    return false;
  }
  
  public final QQUser d(long paramLong)
  {
    return this.k.d(paramLong);
  }
  
  public final boolean d()
  {
    ais localais = this.k;
    if (localais.a != null)
    {
      int i1 = 0;
      while (i1 < localais.a.size())
      {
        if (((QQUser)localais.a.get(i1)).mIsRegisterFacePwd) {
          return true;
        }
        i1 += 1;
      }
    }
    return false;
  }
  
  public final void e(long paramLong)
  {
    QQUser localQQUser = d(paramLong);
    if (localQQUser != null)
    {
      this.l = localQQUser;
      this.m = this.g;
      this.n = this.h;
    }
  }
  
  public final boolean e()
  {
    ais localais = this.k;
    QQUser localQQUser1 = localais.b();
    if (localQQUser1 != null)
    {
      if (localais.a == null) {
        return false;
      }
      int i1 = 0;
      while (i1 < localais.a.size())
      {
        QQUser localQQUser2 = (QQUser)localais.a.get(i1);
        if ((localQQUser2.mIsRegisterFacePwd) && (localQQUser2.mUin == localQQUser1.mUin)) {
          return true;
        }
        i1 += 1;
      }
      return false;
    }
    return false;
  }
  
  public final QQUser f()
  {
    if (!i())
    {
      xj.a("not prepare");
      return null;
    }
    QQUser localQQUser = this.k.b();
    if ((localQQUser != null) && (this.l == localQQUser))
    {
      long l1 = System.currentTimeMillis() / 1000L;
      localStringBuilder = new StringBuilder("mLoginTime =");
      localStringBuilder.append(this.m);
      localStringBuilder.append(",mLoginValidInterval= ");
      localStringBuilder.append(this.n);
      localStringBuilder.append(",nowTime=");
      localStringBuilder.append(l1);
      xj.a(localStringBuilder.toString());
      if (this.m + this.n > l1) {
        return this.l;
      }
    }
    StringBuilder localStringBuilder = new StringBuilder("user not login, login_user=");
    localStringBuilder.append(this.l);
    localStringBuilder.append(",current_user=");
    localStringBuilder.append(localQQUser);
    xj.a(localStringBuilder.toString());
    this.l = null;
    this.m = 0L;
    this.n = 0L;
    return null;
  }
  
  public final void f(long paramLong)
  {
    this.j = false;
    com.tencent.token.ui.AccountPageActivity.mNeedRefreshEval = true;
    this.o = paramLong;
  }
  
  public final void g()
  {
    this.g = (System.currentTimeMillis() / 1000L);
  }
  
  public final void h()
  {
    n();
    this.i = true;
  }
  
  public final boolean i()
  {
    if (!this.j)
    {
      xj.a("uinlist not queryed");
      return false;
    }
    return true;
  }
  
  public final xh j()
  {
    try
    {
      xj.a("start syncPrepare process");
      n();
      xh localxh = m();
      boolean bool = localxh.b();
      if (!bool) {
        return localxh;
      }
      xj.a(i());
      localxh = new xh((byte)0);
      return localxh;
    }
    finally {}
  }
  
  public final xh k()
  {
    try
    {
      xj.a("start syncLogin process");
      xh localxh2 = new xh();
      Object localObject1 = b(localxh2);
      boolean bool = ((xh)localObject1).b();
      if (bool) {
        return localObject1;
      }
      xh localxh1;
      if (localxh2.a == 102)
      {
        long l1 = System.currentTimeMillis() + rf.a().g();
        long l2 = 30 - (int)(l1 / 1000L % 30L) + 2L;
        localObject1 = new StringBuilder("sleep ");
        ((StringBuilder)localObject1).append(l2);
        ((StringBuilder)localObject1).append("s for dyncode refresh:");
        ((StringBuilder)localObject1).append(l1);
        xj.a(((StringBuilder)localObject1).toString());
        try
        {
          Thread.sleep(l2 * 1000L);
        }
        catch (Exception localException)
        {
          xj.c(localException.getMessage());
        }
        localxh1 = b(localxh2);
      }
      else if (localxh2.a == 104)
      {
        h();
        localxh1 = b(localxh2);
      }
      else if (localxh2.a == 112)
      {
        localxh1 = b(localxh2);
      }
      return localxh1;
    }
    finally {}
  }
  
  public final xh l()
  {
    try
    {
      sc.a.a();
      xh localxh = sc.a();
      return localxh;
    }
    finally
    {
      localObject = finally;
      throw localObject;
    }
  }
  
  public final xh m()
  {
    try
    {
      xj.a("start syncQueryUinList process");
      new xh();
      xh localxh = sc.a.a().c();
      return localxh;
    }
    finally
    {
      localObject = finally;
      throw localObject;
    }
  }
  
  public final class a
  {
    public String a;
    public String b;
    public boolean c;
    public boolean d;
    public int e;
    public String f;
    
    public a() {}
  }
}


/* Location:           L:\local\mybackup\temp\qq_apk\com.tencent.token\classes.jar
 * Qualified Name:     com.tencent.token.th
 * JD-Core Version:    0.7.0.1
 */