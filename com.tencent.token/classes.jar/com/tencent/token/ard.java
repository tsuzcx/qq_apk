...
public class ard
{
  ...
  public ard(byte paramByte)
  {
    this();
  }
  
  private boolean a(aqw paramaqw)
  {
    int m = 0;
    boolean bool = false;
    while (m < 3)
    {
      aqy localaqy = paramaqw.a();
      if (localaqy == null) {
        return false;
      }
      StringBuilder localStringBuilder = new StringBuilder("checkSocketWithRetry() before checkSocket(ipPoint) ipPoint ");
      localStringBuilder.append(localaqy.toString());
  ...
  private boolean a(aqy paramaqy)
  {
    if (paramaqy == null) {
      return false;
    }
    ...
    for (;;)
    {
      try
      {
        ...
        this.l = paramaqy;
        InetAddress localInetAddress = InetAddress.getByName(paramaqy.b);
        int m = paramaqy.a;
        paramaqy = new StringBuilder("acquireSocketWithTimeOut, addr: ");
        paramaqy.append(localInetAddress);
        paramaqy.append(", port: ");
        paramaqy.append(m);
        paramaqy = new Socket();
        paramaqy.setSoLinger(false, 0);
        paramaqy.connect(new InetSocketAddress(localInetAddress, m), 60000);
        this.i = paramaqy;
        if (this.i == null) {
          return false;
        }
      ...
      catch (Throwable paramaqy)
      {
        new StringBuilder("checkSocket() Throwable ").append(paramaqy.toString());
        return false;
      }
      catch (SocketTimeoutException paramaqy)
      {
        new StringBuilder("checkSocket() SocketTimeoutException ").append(paramaqy.toString());
        return false;
      }
      catch (UnknownHostException paramaqy)
      {
        new StringBuilder("checkSocket() UnknownHostException ").append(paramaqy.toString());
        return false;
      }
    }
  }
  ...
  public final boolean a(Context paramContext, aqw paramaqw)
  {
    try
    {
      if (b())
 ...
 * Qualified Name:     com.tencent.token.ard
