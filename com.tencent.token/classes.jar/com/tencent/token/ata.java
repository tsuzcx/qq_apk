...
public class ata
{
  ...
  public ata(byte paramByte)
  {
    this();
  }
  
  private boolean a(ast paramast)
  {
    int m = 0;
    boolean bool = false;
    while (m < 3)
    {
      asv localasv = paramast.a();
      if (localasv == null) {
        return false;
      }
      StringBuilder localStringBuilder = new StringBuilder("checkSocketWithRetry() before checkSocket(ipPoint) ipPoint ");
      localStringBuilder.append(localasv.toString());
  ...
  private boolean a(asv paramasv)
  {
    if (paramasv == null) {
      return false;
    }
    ...
    for (;;)
    {
      try
      {
        ...
        this.l = paramasv;
        InetAddress localInetAddress = InetAddress.getByName(paramasv.b);
        int m = paramasv.a;
        paramasv = new StringBuilder("acquireSocketWithTimeOut, addr: ");
        paramasv.append(localInetAddress);
        paramasv.append(", port: ");
        paramasv.append(m);
        paramasv = new Socket();
        paramasv.setSoLinger(false, 0);
        paramasv.connect(new InetSocketAddress(localInetAddress, m), 60000);
        this.i = paramasv;
        if (this.i == null) {
          return false;
        }
      ...
      catch (Throwable paramasv)
      {
        new StringBuilder("checkSocket() Throwable ").append(paramasv.toString());
        return false;
      }
      catch (SocketTimeoutException paramasv)
      {
        new StringBuilder("checkSocket() SocketTimeoutException ").append(paramasv.toString());
        return false;
      }
      catch (UnknownHostException paramasv)
      {
        new StringBuilder("checkSocket() UnknownHostException ").append(paramasv.toString());
        return false;
      }
    }
  }
  ...
  public final boolean a(Context paramContext, ast paramast)
  {
    try
    {
      if (b())
 ...
 * Qualified Name:     com.tencent.token.ata
