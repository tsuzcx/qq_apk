...
public class UpdateManager
{
  ...
  {
    Object localObject1 = MainConfig.getMainConfig(mContext);
    Common.savePointLog(true);
    Object localObject2 = ((MainConfig)localObject1).getPatchList();
  ...
  public static void clearPatchInstallFiles(PatchInfo paramPatchInfo)
  {
    String str = paramPatchInfo.mPatchName;
    paramPatchInfo = paramPatchInfo.mType;
    Common.Log.d("KingKongUpdateManager", "Clear update patch files " + str);
    paramPatchInfo = Patch.getFilesFileList(str, paramPatchInfo).iterator();
    for (;;)
    {
      if (!paramPatchInfo.hasNext()) {
        return;
      }
      str = Common.makeInstallName((String)paramPatchInfo.next());
      File localFile = new File(str);
      if ((localFile.exists()) && (!localFile.delete())) {
        Common.Log.d("KingKongUpdateManager", "Delete file failed " + str);
      }
  ...
  private static boolean installPatches(ArrayList<PatchInfo> paramArrayList)
  {
    ...
    do
    {
      ...
      str3 = Common.makeInstallName(str3);
  ...
  private static boolean removeObsoletedPatches(ArrayList<PatchInfo> paramArrayList)
  {
    ...
    for (;;)
    {
      ...
      String str = localPatchInfo.mPatchName;
      clearPatchInstallFiles(localPatchInfo);
      Common.removePatchSPs(str);
      localMainConfig.removePatch(str);
    }
  }
  
  public static boolean shouldDoUpdate(MainConfig paramMainConfig, PatchInfo paramPatchInfo, boolean paramBoolean)
  {
    PatchInfo localPatchInfo = paramMainConfig.findPatch(paramPatchInfo.mPatchName);
