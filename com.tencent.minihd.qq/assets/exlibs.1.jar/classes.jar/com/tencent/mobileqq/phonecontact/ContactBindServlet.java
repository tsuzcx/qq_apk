  ...
  extends MSFServlet
{
  ...
  private void j(Intent arg1, FromServiceMsg paramFromServiceMsg)
  {
    if (paramFromServiceMsg.isSuccess())
    {
      paramFromServiceMsg = a(paramFromServiceMsg);
      Object localObject = (RespondHeader)paramFromServiceMsg.getByClass("RespondHeader", new RespondHeader());
      paramFromServiceMsg = (ResponseGetRecommendedListV2)paramFromServiceMsg.getByClass("ResponseGetRecommendedListV2", new ResponseGetRecommendedListV2());
      if (((RespondHeader)localObject).result == 0)
      {
        ...
        if ((paramFromServiceMsg.alreadyBindContacts != null) && (paramFromServiceMsg.alreadyBindContacts.size() > 0))
        {
          ...
          notifyObserver(???, 29, true, (Bundle)localObject, ContactBindObserver.class);
        }
        for (;;)
        {
      ...
      notifyObserver(???, 29, false, null, ContactBindObserver.class);
      return;
    }
    notifyObserver(???, 29, false, null, ContactBindObserver.class);
  }
