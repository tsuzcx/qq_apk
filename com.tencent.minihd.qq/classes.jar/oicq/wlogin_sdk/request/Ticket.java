  ...
  implements Parcelable
{
  ...
  private String __getPskey(String paramString, Map<String, byte[]> paramMap, Map<String, Long> paramMap1)
  {
    ...
    if (paramMap1 != null)
    {
      ...
      if ((localLong != null) && (localLong.longValue() <= u.f()))
      {
  ...
  protected static int calPsBufLength(Map<String, byte[]> paramMap)
  {
    ...
    int j;
    for (int i = 2; paramMap.hasNext(); i = ((byte[])localEntry.getValue()).length + (j + (i + 2) + 2) + 2 + 8)
    {
      localEntry = (Map.Entry)paramMap.next();
      j = ((String)localEntry.getKey()).length();
    }
    return i;
  }
  ...
  public static void limitMapSize(int paramInt1, Map<String, byte[]> paramMap, Map<String, Long> paramMap1, int paramInt2)
  {
    ...
    {
      localb = new b("wtlogin_alarm", "pskey_net_to_much", "").a("size", String.valueOf(paramInt1));
      util.LOGI("limitMapSize net domainCnt=" + paramInt1, "");
      paramMap.clear();
    }
    for (;;)
    {
      if (localb != null)
      {
        localb.a(true).b(true);
        c.a(localb);
      }
      return;
      if (paramMap.size() + paramInt1 > paramInt2)
      {
        localb = new b("wtlogin_alarm", "pskey_mix_to_much", paramInt1 + "," + paramMap.size()).a("size", String.valueOf(paramMap.size() + paramInt1));
        ...
        do
        {
          ...
          localStringBuilder.append(",rm key=").append((String)localEntry.getKey()).append(",expire=").append(localEntry.getValue());
  ...
  public static byte[] packPsBuf(Map<String, byte[]> paramMap, long paramLong, Map<String, Long> paramMap1)
  {
    ...
    util.LOGI("packPsBuf mapSize=" + paramMap.size() + ",bufLen=" + i, "");
