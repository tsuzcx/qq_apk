  ...
  extends VasBasePlugin
{
  ...
  protected String decodeUrl(String paramString, boolean paramBoolean)
  {
    ...
    paramString = str1;
    if (paramBoolean) {
      paramString = str1.replace("[version]", "5.3.1.660");
    }
    str1 = paramString.replace("[version]", "3.5.0.660").replace("[sid]", "").replace("[platformId]", "2").replace("[device]", Build.DEVICE).replace("[system]", Build.VERSION.RELEASE);
