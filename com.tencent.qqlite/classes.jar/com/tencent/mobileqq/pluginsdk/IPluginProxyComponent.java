...
class IPluginProxyComponent
{
  ...
  static void registerAccountReceiverIfNeccessary()
  {
    ...
    if (sAccountReceiver == null)
    {
      ...
      localIntentFilter.addAction("mqql.intent.action.EXIT_" + MobileQQ.getMobileQQ().getPackageName());
    }
    try
    {
    ...
    public void onReceive(Context paramContext, Intent paramIntent)
    {
      ...
      do
      {
        do
        {
          ...
          if (!"mqql.intent.action.ACCOUNT_CHANGED".equals(str)) {
            break;
          }
        } while (paramIntent.getStringExtra("account") == null);
        return;
      } while ((!"mqql.intent.action.LOGOUT".equals(str)) && (!("mqql.intent.action.EXIT_" + MobileQQ.getMobileQQ().getPackageName()).equals(str)));
