  ...
  implements VerifyCodeManager
{
  ...
  {
    HttpVerifyCodeWrapper localHttpVerifyCodeWrapper = (HttpVerifyCodeWrapper)this.httpVerifyCodeWrapperMap.get(paramString1);
    if ((localHttpVerifyCodeWrapper != null) && (localHttpVerifyCodeWrapper.serverNotifyObserver != null))
    {
      Message localMessage = this.mHandler.obtainMessage();
      localMessage.obj = new Object[] { localHttpVerifyCodeWrapper.serverNotifyObserver, paramString1, Integer.valueOf(0), paramString2, paramArrayOfByte };
      localMessage.what = 0;
      localMessage.sendToTarget();
      return;
    }
    this.app.getApplication().startActivity(new Intent("mqq.i.intent.action.VERIFYCODE").setFlags(268435456).putExtra("key", paramString1).putExtra("note", paramString2).putExtra("image", paramArrayOfByte));
  }
  
  private void notifyVerifyCodeActivity(ToServiceMsg paramToServiceMsg, String paramString, byte[] paramArrayOfByte)
  {
    int i = paramToServiceMsg.getRequestSsoSeq();
    Object localObject = String.valueOf(paramToServiceMsg.getAttribute("process"));
    ...
    if (paramToServiceMsg.serverNotifyObserver != null)
    {
      ...
      ((Message)localObject).obj = new Object[] { paramToServiceMsg.serverNotifyObserver, "", Integer.valueOf(i), paramString, paramArrayOfByte };
      ((Message)localObject).what = 0;
      ((Message)localObject).sendToTarget();
      return;
    }
    paramToServiceMsg = "mqq.i.intent.action.VERIFYCODE";
    if ("com.tencent.mobileqq:openSdk".equals(localObject)) {
      paramToServiceMsg = "mqq.opensdk.intent.action.VERIFYCODE";
    }
    this.app.getApplication().startActivity(new Intent(paramToServiceMsg).setFlags(268435456).putExtra("seq", i).putExtra("note", paramString).putExtra("image", paramArrayOfByte));
  }
  ...
  public boolean checkVerifyCodeResp(ToServiceMsg paramToServiceMsg, FromServiceMsg paramFromServiceMsg)
  {
    ...
    if (paramFromServiceMsg.getBusinessFailCode() == 2002)
    {
      ...
      notifyVerifyCodeActivity(paramToServiceMsg, paramFromServiceMsg.verifyNote, paramFromServiceMsg.verifyimage);
      bool1 = false;
    }
